...
async function generateGeneratorIconLibrary(tree: Tree, options: IconLibraryGeneratorSchema) {
  const pathGenerator = `scripts/src/generators/${options.name}/${options.name}`;
...
export async function iconLibraryGenerator(
  ...
  const libraryRoot = `libs/${fileName}`;
  ...
  const ngPackageJSON = readJson(tree, path.join(libraryRoot, 'ng-package.json'));
