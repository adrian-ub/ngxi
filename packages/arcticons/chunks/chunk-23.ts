import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtorrentIcon],svg[arcticons-ttorrent-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.288 15.166h11.623M28.1 32.834V15.166m-11.709 2.319v13.141a2.2 2.2 0 0 0 2.193 2.208h.658m-5.153-11.705h4.605"></svg:path>`,
})
export class ArcticonsTtorrentIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtpCargoIcon],svg[arcticons-ttp-cargo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m22.118 34.29l13.83-.763c7.175-.396 6.34 1.236 6.368 4.368c1.908-2.214 1.36-6.114-.573-8.112C32.974 20.713 18.838 14.37 4.5 10.105c9.855 4.972 20.152 10.257 25.311 16.206c3.295 3.8-1.947 5.824-7.693 7.98"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.626 34.025c2.8-.857 6.245-1.69 7.43-4.772c1.482-3.859-12.538-12.718-27.268-18.615M38.342 31.4l-2.087-4.282c1.877.196 4.857 2.15 4.755 4.439c-2.075-.152-4.333-.275-5.272 1.857"></svg:path>`,
})
export class ArcticonsTtpCargoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtsEngineIcon],svg[arcticons-tts-engine-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.336 14.98l3.518 2.337s.833-1.169 1.169-2.642c-4.708-1.489-2.258-4.467 1.745-4.996L26.53 5.5h5.764c.048 0 .545 2.306.545 2.306c3.282 0 5.988 1.153 5.988 3.202s-2.722 2.546-4.932 2.498l-.032 2.354l-1.056.064c-.064 1.201-1.01 2.626-1.97 2.29s-1.585-.769-1.585-.769s1.89 3.651 3.01 4.788l-1.365 2.124"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.767 18.545c-1.537-1.025-2.2-5.594-2.2-5.594s-5.999.657-7.899.916s-2.711 2.458-2.412 4.55s2.07 1.836 2.07 1.836c-1.13 1.238-1.473 4.676-.277 6.725c-1.516 3.032.384 6.918.384 6.918l-2.796 3.608l1.6 1.473s-.49 1.43-.426 1.43s2.37 1.431 2.37 1.431m19.322-4.505c2.519 1.602 7.28 3.395 7.28 3.395l.512-.854c.77.406 4.564.537 4.164-2.407c-.223-1.646-1.172-2.373-1.84-2.323c-.976.072-1.918.823-1.918.823c-.726-.406-7.92-4.612-10.504-8.572"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.385 20.253c1.346 2.37 4.975 4.377 4.975 4.377s3.354 5.79 2.307 8.755m-7.41-10.165c2.2 2.67 2.605 6.406 2.605 6.406M34.58 39.84c.628 1.181.847 2.66.847 2.66H10.394v-3.608m28.727 1.246V42.5"></svg:path>`,
})
export class ArcticonsTtsEngineIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtsServerIcon],svg[arcticons-tts-server-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.704 11.107h4.876m-2.438 7.501v-7.501m7.494 6.658c.47.562 1.032.843 1.875.843h1.126c1.031 0 1.875-.843 1.875-1.875s-.844-1.875-1.875-1.875h-1.22c-1.03 0-1.875-.844-1.875-1.875s.844-1.875 1.876-1.875h1.125c.844 0 1.406.187 1.875.843m-9.791-.844h6.769m-4.707 7.501v-7.501M11.9 26.336c.147.007.992.09 1.429-.105c.796-.354 1.032-1.956 1.891-1.809c1.358.233.494 3.75 1.87 3.695c1.823-.073.258-5.117 2.082-5.073c2.206.053.016 6.27 2.223 6.251c2.609-.022-.015-7.433 2.605-7.433s-.004 7.411 2.605 7.433c2.207.02.017-6.198 2.223-6.251c1.824-.044.258 5 2.081 5.073c1.377.055.512-3.462 1.87-3.695c.86-.147 1.096 1.455 1.892 1.81c.436.193 1.282.112 1.428.104m1.741 7.841s-1.505.391-2.274.495c-3.815.518-6.886-4.248-11.532.637c1.335-.114 1.879 1.375 1.879 1.375s1.612-1.144 2.552-1.225c1.653-.143 3.146 1.141 4.792 1.354c1.734.224 5.243-.096 5.243-.096m-.66-4.148s-1.505.392-2.274.496c-3.815.518-6.886-4.248-11.566.636c-4.68-4.884-7.75-.118-11.566-.636c-.769-.104-2.275-.496-2.275-.496M9.5 36.717s3.51.32 5.243.096c1.646-.213 3.139-1.497 4.792-1.354c.94.08 2.552 1.225 2.552 1.225s.544-1.49 1.879-1.375c-4.646-4.885-7.717-.119-11.532-.637c-.769-.104-2.275-.496-2.275-.496"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsTtsServerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtslexxIcon],svg[arcticons-ttslexx-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 17.953h8.012m-4.006 12.094V17.953m6.424 10.769c.742.965 1.672 1.325 2.966 1.325h1.79a3.017 3.017 0 0 0 3.018-3.017v-.013A3.017 3.017 0 0 0 24.687 24H22.71a3.02 3.02 0 0 1-3.02-3.02h0a3.027 3.027 0 0 1 3.027-3.027h1.78c1.294 0 2.224.36 2.966 1.325m3.264 9.444c.742.965 1.672 1.325 2.965 1.325h1.79A3.017 3.017 0 0 0 38.5 27.03v-.013A3.017 3.017 0 0 0 35.483 24h-1.975a3.02 3.02 0 0 1-3.02-3.02h0a3.027 3.027 0 0 1 3.026-3.027h1.781c1.294 0 2.224.36 2.965 1.325"></svg:path><svg:circle cx="24" cy="23.069" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTtslexxIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTtsutilIcon],svg[arcticons-ttsutil-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.101 33.451v6.356A2.687 2.687 0 0 1 30.41 42.5H14.17a2.687 2.687 0 0 1-2.693-2.693l-.003-31.614A2.687 2.687 0 0 1 14.166 5.5h16.242a2.687 2.687 0 0 1 2.693 2.693v5.712"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.101 17.782c2.768-.005 4.158 3.34 2.203 5.298s-5.301.574-5.301-2.194a3.105 3.105 0 0 1 3.098-3.104m0 7.36c3.47 0 6.203.965 6.203 2.115v2.96H26.899v-2.989c0-1.15 2.734-2.087 6.203-2.087zm4.784-2.087c.481-1.033.638-2.212.338-3.344a5 5 0 0 0-.327-.985m1.934 5.208c.732-1.58.934-3.397.382-5.089a7 7 0 0 0-.358-1.004"></svg:path>`,
})
export class ArcticonsTtsutilIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuDortmundIcon],svg[arcticons-tu-dortmund-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.341 7.978v27.433a4.61 4.61 0 0 0 4.61 4.61h3.384"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 15.586h15.558a3 3 0 0 1 3 3V30.8a9.22 9.22 0 0 0 9.22 9.22h0A9.22 9.22 0 0 0 42.5 30.8V15.586m0 15.215v9.221"></svg:path>`,
})
export class ArcticonsTuDortmundIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuGreatsIcon],svg[arcticons-tu-greats-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.95 18.37v11.37c0 3.951 3.775 5.481 6.54 5.475V18.37m4.993-5.585v13.349a9.06 9.06 0 0 0 9.043 9.081h1.892a9.06 9.06 0 0 0 9.082-9.042V17.7c0-2.41-1.662-4.915-6.279-4.915v12.643c0 2.071-1.61 3.74-3.61 3.74h-.239c-2 0-3.61-1.669-3.61-3.74V17.7c0-2.41-1.737-4.915-6.324-4.915m.044 5.472H9.976c-3.952 0-5.482-2.706-5.476-5.472h18.983"></svg:path>`,
})
export class ArcticonsTuGreatsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTubelabIcon],svg[arcticons-tubelab-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="23.898" cy="23.898" r="17.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m32 24l-12 7V17z"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTubelabIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTubemateIcon],svg[arcticons-tubemate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.72 4.5c-1.35 5.23 3.21 15.32 9.41 15.91A4.12 4.12 0 0 0 21.86 17s10.08 3 9.94 16a15.1 15.1 0 0 0 7.2-3.25c.3.15-2.17 12-9.19 13.75c0 0-12.18-1.94-14.72-8c0 0 5 1.34 7.92 0c-.26-2.58-5.14-5.2-10-9a9.7 9.7 0 0 1-4-7.25l3.17 2.06S8 10.85 13.72 4.5"></svg:path>`,
})
export class ArcticonsTubemateIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTubiIcon],svg[arcticons-tubi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 42.5h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.365 20.384v4.962a3.007 3.007 0 0 0 3.007 3.007h0a3.007 3.007 0 0 0 3.008-3.007v-4.962"></svg:path><svg:circle cx="35.447" cy="16.699" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.447 20.384v7.969M11.5 17.902v7.444a3.007 3.007 0 0 0 3.008 3.007h.407M11.5 20.384h3.039m11.801 3.007a3.007 3.007 0 0 1 3.008-3.007h0a3.007 3.007 0 0 1 3.008 3.007v1.955a3.007 3.007 0 0 1-3.008 3.007h0a3.007 3.007 0 0 1-3.007-3.007m0 0v-9.023"></svg:path>`,
})
export class ArcticonsTubiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTubularIcon],svg[arcticons-tubular-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.189 10.49L42.5 23.95L19.013 37.51"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.51 10.49l21.123 13.46L5.5 37.51z"></svg:path>`,
})
export class ArcticonsTubularIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuesdaysIcon],svg[arcticons-tuesdays-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4m-28 8.551h5.592m-2.796 8.441v-8.441"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m30.343 25.508l-2.796 4.22l-2.796-4.22m2.796 8.441v-4.221m4.528-8.16c.517.673 1.167.924 2.07.924h1.25a2.106 2.106 0 0 0 2.105-2.105v-.01a2.106 2.106 0 0 0-2.106-2.105h-1.378a2.11 2.11 0 0 1-2.108-2.108h0c0-1.167.946-2.113 2.112-2.113h1.243c.903 0 1.552.251 2.07.925m-5.258 18.048c.517.674 1.167.925 2.07.925h1.25a2.106 2.106 0 0 0 2.105-2.106v-.01a2.106 2.106 0 0 0-2.106-2.105h-1.378a2.11 2.11 0 0 1-2.108-2.108h0c0-1.166.946-2.112 2.112-2.112h1.243c.903 0 1.552.25 2.07.925M18.255 14.051v5.645a2.796 2.796 0 1 0 5.592 0v-5.645m2.275 4.221h2.752m1.469 4.22h-4.221v-8.441h4.221M10.796 33.949v-8.441h1.9a3.693 3.693 0 0 1 3.692 3.693v1.055a3.693 3.693 0 0 1-3.693 3.693zm12.125-2.797h-3.74m-.926 2.797l2.796-8.441l2.797 8.441"></svg:path>`,
})
export class ArcticonsTuesdaysIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuiIcon],svg[arcticons-tui-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 42.5h-29c-2.2 0-4-1.8-4-4v-29c0-2.2 1.8-4 4-4h29c2.2 0 4 1.8 4 4v29c0 2.2-1.8 4-4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.24 20.413v4.426a2.683 2.683 0 0 0 2.683 2.683h0a2.683 2.683 0 0 0 2.682-2.683v-4.426m0 4.426v2.682"></svg:path><svg:circle cx="38.42" cy="17.127" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.42 20.413v7.108M20.646 18.2v7.98c0 .741.6 1.341 1.341 1.341h.403m-3.152-7.108h2.816m-8.331 6.237l4.024-2.683l-4.024-2.682M8.359 26.65l4.023-2.683l-4.023-2.682m16.379 2.682h2.682"></svg:path>`,
})
export class ArcticonsTuiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuiPolandIcon],svg[arcticons-tui-poland-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="m5.5 20.975l11.068.11m-5.326.392s1.966 15.276 11.294 15.782s13.402-12.301 13.402-12.301"></svg:path><svg:circle cx="39.06" cy="14.166" r="3.44"></svg:circle></svg:g>`,
})
export class ArcticonsTuiPolandIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTumblrIcon],svg[arcticons-tumblr-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.526 43.5c-5.865 0-10.236-3.017-10.236-10.237V21.702h-5.33V15.44c5.865-1.522 8.318-6.57 8.6-10.941h6.092v9.926h7.106v7.276h-7.106v10.067c0 3.017 1.523 4.06 3.947 4.06h3.441v7.67z"></svg:path>`,
})
export class ArcticonsTumblrIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTummocIcon],svg[arcticons-tummoc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.512 10.416a15.934 15.934 0 1 0-22.78 22.285l8.764 8.763a3.54 3.54 0 0 0 5.008 0l8.763-8.763a15.934 15.934 0 0 0 .245-22.285"></svg:path><svg:circle cx="24.003" cy="21.43" r="5.311" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.733 32.7a3.541 3.541 0 0 1 5.008-5.008l6.26 6.26l15.628-15.628"></svg:path>`,
})
export class ArcticonsTummocIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunableIcon],svg[arcticons-tunable-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m26.242 7.063l3.981 27.636m0 0a7.702 7.702 0 1 1-10.442-6.07M26.376 7l10.68-2.5m.04.054c.448 1.58.896 3.161-.239 4.288s-3.851 1.8-6.568 2.473m-18.03 2.739l3.244 8.744m.599-9.516l2.93 8.242m-8.605-4l8.674-2.618m-7.324 5.968l8.36-2.745"></svg:path>`,
})
export class ArcticonsTunableIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuneItIcon],svg[arcticons-tune-it-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:ellipse cx="37.914" cy="17.071" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4.655" ry="3.201" transform="rotate(-13.798 37.914 17.07)"></svg:ellipse><svg:ellipse cx="10.086" cy="22.519" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4.655" ry="3.201" transform="rotate(-13.798 10.086 22.52)"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.329 17.784v18.694L5.5 42.44V23.255"></svg:path><svg:circle cx="21.844" cy="37.458" r="1.448" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m21.844 36.01l.012-9.281m-2.321-21.17v18.784c0 3.198 4.482 3.209 4.482-.113V5.559"></svg:path>`,
})
export class ArcticonsTuneItIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuneinRadioIcon],svg[arcticons-tunein-radio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="24" height="14" x="4.5" y="19" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1"></svg:rect><svg:rect width="15" height="14" x="28.5" y="15" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.429 29.071H26.5m-3.071-6.142H26.5M23.429 26h2.002m-2.002-3.071v6.142M6.5 22.929h4.069m-2.035 6.142v-6.142m9.252 6.142v-6.142l4.069 6.142v-6.142m-9.712.001v4.106a2.034 2.034 0 0 0 4.069 0V22.93m16.865-4.001v6.142m1.777 0v-6.142l4.069 6.142v-6.142"></svg:path>`,
})
export class ArcticonsTuneinRadioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuneinRadioProIcon],svg[arcticons-tunein-radio-pro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="24" height="14" x="4.5" y="19" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1" ry="1"></svg:rect><svg:rect width="15" height="14" x="28.5" y="15" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1" ry="1"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.429 29.071H26.5m-3.071-6.142H26.5M23.429 26h2.002m-2.002-3.071v6.142M6.5 22.929h4.069m-2.035 6.142v-6.142m9.252 6.142v-6.142l4.069 6.142v-6.142m-9.712.001v4.106a2.034 2.034 0 0 0 4.069 0V22.93m16.865-4.001v6.142m1.777 0v-6.142l4.069 6.142v-6.142m-8.438 16.716v-4.513h1.477a1.515 1.515 0 0 1 0 3.032h-1.477m4.909 1.481v-4.513h1.477a1.515 1.515 0 0 1 0 3.032h-1.477m1.477 0l1.478 1.48"></svg:path><svg:rect width="2.99" height="4.513" x="40.51" y="31.132" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.495" ry="1.495"></svg:rect>`,
})
export class ArcticonsTuneinRadioProIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunerIcon],svg[arcticons-tuner-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.159 5.5v37m0-32.613c3.7 0 2.243 28.926 9.925 28.926H42.5M18.159 9.887c-3.701 0-2.243 28.926-9.925 28.926H5.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsTunerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuner2Icon],svg[arcticons-tuner-2-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 22.741v-3.169a.403.403 0 0 0-.406-.406h-13.87l-4.915-5.614l2.558-2.365l-10.943-3.883l2.17 11.277l2.632-2.064l2.307 2.649H4.906a.4.4 0 0 0-.406.406v3.169a.4.4 0 0 0 .406.406h21.597l.389.447a14.95 14.95 0 0 0-6.619 2.64c-3.526 2.62-4.363 5.535-4.046 7.88H4.907a.407.407 0 0 0-.407.407v3.169a.403.403 0 0 0 .406.406h13.35a12.87 12.87 0 0 0 7.929 2.6a10.2 10.2 0 0 0 5.964-1.746c.389-.284.762-.569 1.104-.853h9.84a.403.403 0 0 0 .407-.407v-3.168a.41.41 0 0 0-.406-.407H36.65a7.8 7.8 0 0 0 .918-3.436a4.66 4.66 0 0 0-.319-2.356a35 35 0 0 0-3.688-4.209l-.844-.967h10.376a.403.403 0 0 0 .406-.406m-20.466-3.575h6.191m3.494 3.981h-6.215m-8.247 14.949h14.997m3.398-3.981H16.227"></svg:path>`,
})
export class ArcticonsTuner2Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunerPitchedIcon],svg[arcticons-tuner-pitched-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.27 22.313c.1-3.295 1.388-6.341 3.63-8.584c1.83-1.828 4.203-3.032 6.827-3.461m17.006 17.036c-.434 2.613-1.635 4.977-3.457 6.799c-2.236 2.236-5.272 3.524-8.556 3.63M5.504 21.446c.207-4.316 1.934-8.292 4.879-11.237c2.53-2.531 5.834-4.172 9.479-4.709m22.637 22.671c-.541 3.634-2.18 6.927-4.704 9.451h0c-2.937 2.937-6.9 4.663-11.203 4.878m-15.409-4.096a1.42 1.42 0 0 0 2.009.008l.007-.008l5.668-5.668l.1.1c2.15.19 5.07 2.139 9.628-2.17l12.85-12.85a1.42 1.42 0 0 0 .006-2.008l-.007-.007l-1.712-1.712a1.42 1.42 0 0 0-2.009-.007l-.007.007L23.76 28.043l-3.8-3.8L33.917 10.29a1.42 1.42 0 0 0 .007-2.008l-.007-.007l-1.712-1.712a1.42 1.42 0 0 0-2.009-.007l-.007.007l-12.85 12.847c-4.31 4.558-2.36 7.478-2.17 9.626l.1.101l-5.667 5.67a1.42 1.42 0 0 0-.007 2.008l.007.007z"></svg:path>`,
})
export class ArcticonsTunerPitchedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunerlyIcon],svg[arcticons-tunerly-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 14.25V24A9.75 9.75 0 0 0 24 24a9.75 9.75 0 0 1 19.5 0v9.75"></svg:path>`,
})
export class ArcticonsTunerlyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTungngernIcon],svg[arcticons-tungngern-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m32.901 16.519l.282-.883m2.255-7.062l.282-.882m-.788 4.636l-2.858-.957m2.838.937c.944.319 1.463 1.374 1.153 2.345c-.31.97-1.336 1.504-2.28 1.186l-2.834-.956l2.254-7.062l2.834.956c.944.319 1.463 1.374 1.153 2.345c-.31.97-1.336 1.505-2.28 1.186m5.81-.203a6.605 6.605 0 0 1-6.605 6.605h-.001a6.605 6.605 0 1 1 6.606-6.606zM16.168 18.49c-.615-2.643-1.903-5.735-4.372-8.761l8.02-1.657c.66 1.265 1.419 2.82 1.612 3.764m4.512 9.62c.148-1.506.988-3.634 2.197-6.29m-.624-4.397l-5.1-1.102l-1.298 3.083m-3.024 6.69a3.005 3.005 0 0 1 5.544 2.318m-6.79-2.963v-5.898h8.26v8.733m-9.64 3.029l-3.783-5.035c1.732-2.574 4.17-1.33 6.556 0c2.335 1.3 4.49 3.04 7.59 2.016c2.846-.94 3.41-2.036 5.068-3.058l-4.012 5.765m-11.51.31h11.514v2.27H15.375zm11.514 2.27c1.292 1.097 2.074.908 4.38 2.88c6.178 5.282 4.235 12.41-10.137 12.694c-14.372-.283-16.86-7.453-10.682-12.736c2.306-1.972 3.512-1.745 4.926-2.839"></svg:path>`,
})
export class ArcticonsTungngernIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunnelbearIcon],svg[arcticons-tunnelbear-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:ellipse cx="24" cy="10.934" rx="13.143" ry="3.772"></svg:ellipse><svg:path d="M40.706 10.894c0 3.531-7.48 6.394-16.706 6.394S7.294 14.425 7.294 10.894m0 0C7.294 7.363 14.774 4.5 24 4.5s16.706 2.863 16.706 6.394v6.351c0 3.532-7.48 6.394-16.706 6.394S7.294 20.777 7.294 17.246z"></svg:path><svg:path d="M39.196 20.157v17.487c0 3.234-6.803 5.856-15.196 5.856S8.804 40.878 8.804 37.644V20.157"></svg:path></svg:g><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M14.665 39.329c1.231-1.362 4.722-.27 6.97-.27c1.014 0 2.128-.317 2.805-2.226c1.034-2.914 2.598-7.446 3.276-9.68"></svg:path><svg:path d="M33.813 27.154H20.97c-6.638 0-7.677 4.3-6.038 6.311c2.99 3.668 8.105.777 6.782-2.184c-.744-1.665-2.892-1.922-3.863-.751"></svg:path></svg:g>`,
})
export class ArcticonsTunnelbearIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTunnelbearAltIcon],svg[arcticons-tunnelbear-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 11.865H17.9c-13.232 0-15.304 8.572-12.035 12.581c5.96 7.312 16.155 1.55 13.518-4.354c-1.482-3.318-5.765-3.83-7.7-1.497M5.33 36.135c2.454-2.713 9.412-.536 13.894-.536c2.022 0 4.242-.634 5.592-4.44c2.06-5.809 5.18-14.842 6.53-19.294"></svg:path>`,
})
export class ArcticonsTunnelbearAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuoiTreIcon],svg[arcticons-tuoi-tre-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.07 25.06L19.61 5.5v10.031h3.01v9.53h-3.01v7.022c.501.867 2.507.87 3.01 0v6.52c0 5.02-11.034 6.022-11.034-2.006V25.06zm19.31 0L38.92 5.5v10.031h3.01v9.53h-3.01v7.022c.501.867 2.507.87 3.01 0v6.52c0 5.02-11.035 6.022-11.035-2.006V25.06z"></svg:path>`,
})
export class ArcticonsTuoiTreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurboVpnIcon],svg[arcticons-turbo-vpn-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.888 39.28c-1.916-1.095-4.548-1.944-8.108-2.401C.7 34.17 6.802 21.146 7.281 17.067c0 0-1.816-2.228-.754-4.902c0 0 2.331-.72 4.8 2.262c13.196-1.542 13.641 16.693 25.398 13.917c-2.4-8.707-13.47-20.806-20.155-22.589C32.166 3.116 36.142 21.66 36.725 28.344c1.432.536 3.84 1.234 5.176 5.758c0 0-4.97 1.256-11.963.228c0 0-6.684 8.033-17.584 8.17c9.7-1.2 12.296-6.828 12.296-6.828"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.78 36.879c-1.822-.271-4.25-2.888-4.096-5.776c3.668 0 5.022 1.114 6.965 4.569"></svg:path>`,
})
export class ArcticonsTurboVpnIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurbotaxIcon],svg[arcticons-turbotax-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.5 24c0 11.874-9.626 21.5-21.5 21.5S2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24m-10.395-7.964c-6.257 4.805-10.986 10.358-15.14 19.189c-2.643-1.657-5.403-3.646-7.809-5.764"></svg:path>`,
})
export class ArcticonsTurbotaxIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurffIcon],svg[arcticons-turff-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.785 8.4v25.45c0 5.426 6.295 9.65 11.046 9.65c5.47 0 8.416-.813 11.384-3.022l-5.298-5.428c-2.085 1.163-4.62 2.236-7.342-.65V18.8h8.158v-7.15h-8.158V4.5z"></svg:path>`,
})
export class ArcticonsTurffIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuringmachineIcon],svg[arcticons-turingmachine-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m4.5 33.75l9.71 7.93V25.83zm9.71 0h19.35a9.16 9.16 0 0 0 9.16-9.16h0a9.3 9.3 0 0 0-.23-2.06m1.01-8.28l-9.71-7.93v15.85z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.79 14.25H14.44a9.16 9.16 0 0 0-9.16 9.16h0a9.3 9.3 0 0 0 .23 2.06"></svg:path>`,
})
export class ArcticonsTuringmachineIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurkTelekomIcon],svg[arcticons-turk-telekom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 25.684L17.905 10.906v29.555zm-25.595-7.715L4.5 10.229v15.479z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m11.257 25.684l12.637 7.295V18.388zm31.488-14.173l-6.88-3.972v7.944z"></svg:path>`,
})
export class ArcticonsTurkTelekomIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurkcellIcon],svg[arcticons-turkcell-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.832 16.827c-8.887 7.388-11.2 19.685-7.098 21.3c3.94 1.293 3.584-9.025 21.174-15.502c.22-.081.664-.407.72-.933c.25-2.325 2.326-4.091 4.733-4.091a4.782 4.782 0 1 1 0 9.564c-1.23 0-2.176-.72-3.197-1.225c-.65-.323-1.515-.165-1.938.108c-6.301 4.054-10.167 9.1-12.134 15.113c-1.015 3.102 2.162 4.072 3.724 4.23q1.078.109 2.184.109c11.874 0 21.5-9.626 21.5-21.5S35.874 2.5 24 2.5C13.659 2.5 3.184 9.588 2.965 19.53c-.045 2.05.705 2.725 1.482 2.204c1.461-.981 3.21-4.492 10.757-7.889c.145-.065.424-.167.435-.43c.106-2.628 2.14-4.782 4.782-4.782a4.781 4.781 0 1 1 0 9.564c-1.3 0-2.467-.531-3.341-1.36c-.378-.36-1.008-.21-1.248-.01"></svg:path>`,
})
export class ArcticonsTurkcellIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurkcellTvplusIcon],svg[arcticons-turkcell-tvplus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4M14.091 33.286v-13.86"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m27.904 22.555l-4.024 10.73l-4.776-12.735a1.73 1.73 0 0 0-1.622-1.124H9.5m19.577 0H38.5m-4.711-4.712v9.423"></svg:path>`,
})
export class ArcticonsTurkcellTvplusIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurkishAirlinesIcon],svg[arcticons-turkish-airlines-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.136 6.733c2.626 2.246 3.814 4.562 4.787 7.306a17 17 0 0 1 .81 7.926c-.345 2.466-1.222 4.987-2.766 6.941c-2.203 2.789-4.39 4.15-8.723 6.13l32.43-9.081l-14.635.993c1.468-1.55 2.951-3.162 3.495-5.17c.392-1.449.176-3.068-.295-4.493c-.565-1.706-1.68-3.232-2.925-4.528c-1.585-1.651-3.56-2.942-5.611-3.958c-2.057-1.017-4.574-1.734-6.567-2.066"></svg:path>`,
})
export class ArcticonsTurkishAirlinesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurnMeOnIcon],svg[arcticons-turn-me-on-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="30.302" cy="24" r="7.505" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.419 18.774a5.226 5.226 0 1 0 0 10.452H24.6"></svg:path><svg:path fill="none" stroke="currentColor" d="M15.419 18.774h9.215"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTurnMeOnIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurnOffScreenIcon],svg[arcticons-turn-off-screen-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.611 11.744c6.257 8.936 4.83 21.141-3.318 28.393s-20.437 7.252-28.586 0S.132 20.68 6.39 11.744M24 5.083v22.394"></svg:path>`,
})
export class ArcticonsTurnOffScreenIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTurtlIcon],svg[arcticons-turtl-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.39 4.5v9.28L16 17.51l-8-4.65Zm3.22 0l14.47 8.35L32 17.5l-6.43-3.73ZM6.31 15.64l8 4.64v7.43l-8 4.65Zm35.38 0v16.71l-8-4.64v-7.43ZM24 16.57l6.43 3.71v7.43L24 31.43l-6.43-3.72v-7.43Zm-8 13.92l6.43 3.73v9.28L7.92 35.14Zm16 0l8 4.65l-14.39 8.36v-9.28Z"></svg:path>`,
})
export class ArcticonsTurtlIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuskyIcon],svg[arcticons-tusky-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M7.584 37.811c1.622-2.957 2.697-3.494 2.697-3.494m19.235.022c-.919 1.17-.78 3.915.337 5.829c.603 1.034 1.19 2.65 1.6 3.919m-7.157-17.461s-2.149-2.053-.82-3.412c1.593-1.63 3.066.905 3.44 1.586"></svg:path><svg:path d="M18.632 30.507s2.246 5.391.15 6.324s-2.755-2.132-2.755-2.132c.025.763-.385 2.176-1.484 2.098c-1.347-.096-1.563-2.12-1.558-3.156c-2.526 4.951-4.085-2.071-2.182-7.327c.781-2.99 2.211-5.065 2.211-5.065s-1.703-2.312-.755-3.11s1.201.4 1.201.4s.073-2.802 1.213-2.861s.857 1.345.857 1.345s.682-1.104 1.477-.804c.796.3.3 1.538.3 1.538s3.883-1.792 7.826.13c3.944 1.923 3.194 6.536 5.105 5.803c1.91-.733-.324-3.126-1.043-3.914c-.719-.787-1.826-3.293.146-5.18l.936 1.695l1.498-.307s-1.406 2.44 1.008 3.432s4.204 4.132 3.959 7.27c-.109 1.39-1.346 2.802-2.77 4.044"></svg:path><svg:path d="M33.776 27.533s-1.309 3.287-5.346 3.16c-1.596-.05-2.764.906-2.567 2.28c.214 1.496 1.827 1.712 3.574 1.36c4.122-.829 5.085-3.777 4.34-6.8ZM22.063 42.232s1.223 1.62 1.403 3.187m-4.869-.6c-.347-1.304-.76-2.567-1.88-2.89c-1.144-.33-2.194.182-3.032.896m15.092-30.55l-2.602-2.797l2.314-.993zm3.105.098l.107-3.819l2.347.916zm2.464 1.907l2.684-2.718l1.09 2.27z"></svg:path><svg:circle cx="24" cy="24" r="21.5"></svg:circle></svg:g>`,
})
export class ArcticonsTuskyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuskyAltIcon],svg[arcticons-tusky-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.16 11.73C8.7 13.28 4.5 17.42 4.5 24.46s5.75 11.81 13.91 11.81s16-5.38 18-19.41"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.65 11.73c4.08 1.48 7.61 4.85 9.4 11.19c2 7.11 5 11.06 8.84 11.06s6.61-2.47 6.61-10.32a18.5 18.5 0 0 0-1.06-6.38"></svg:path>`,
})
export class ArcticonsTuskyAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuskyAlt2Icon],svg[arcticons-tusky-alt-2-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.88 12.14a4.6 4.6 0 0 1 1 0c4.47.33 6.74 2.84 10 11.06c.57 1.44.83 2.06.95 2.32c0 .06.07.21.13.16a4 4 0 0 0 .3-.49c.06-.11.14-.26.22-.43l.28-.61a35.4 35.4 0 0 0 2.15-6.1c.17-.79.3-1 .68-1.21c1.23-.64 2.33-.05 2.33 1.25c0 2.35-1.53 7.07-3.3 10.13c-.11.2-.22.33-.28.44s-.22.33-.19.39l.08.09a2 2 0 0 0 .18.17a7.8 7.8 0 0 0 2.87 1.15A3.08 3.08 0 0 0 39 29.34c1.42-1.86 1.74-5.19 1-10.07c-.28-1.78-.28-1.81.06-2.26c.81-1.09 1.8-.89 2.48.52c1.46 2.87 1.21 9.91-.4 13.47c-.9 2-2.46 3.1-5 3.57a5.51 5.51 0 0 1-5.29-1.71l-1-1l-.49.52A16.3 16.3 0 0 1 23.89 36a24.3 24.3 0 0 1-4.54.59c-.28 0-1.08-.08-1.77-.14c-6.64-.65-11.1-3.93-12.58-9.32a11.39 11.39 0 0 1 1.74-9.81c1.93-3 4.51-4.88 6.55-4.88h.54l-.67.44a12.83 12.83 0 0 0-5 6.44a14.9 14.9 0 0 0-.27 6.19a9.28 9.28 0 0 0 5 6.55a11.8 11.8 0 0 0 5.64 1.2A13.08 13.08 0 0 0 28 29.61l.89-.84l-.45-1c-.39-.88-1.3-3.26-3-7.79a12.1 12.1 0 0 0-2.58-4.31a28.5 28.5 0 0 0-3.8-3.29c-.21-.15-.31-.23-.21-.26Z"></svg:path>`,
})
export class ArcticonsTuskyAlt2Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTutaIcon],svg[arcticons-tuta-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 42.27h28.377L42.5 13.893H14.123zm37-28.428H14.123L5.5 5.73h28.377zM5.5 42.27V5.73"></svg:path>`,
})
export class ArcticonsTutaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTutaAltIcon],svg[arcticons-tuta-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5c-2.2 0-4 1.8-4 4v31.1c0 .6.4 1 1 1h.3c8-2.3 20.4-5.8 23.3-10.1c1.6-3-8.5-5.1-8.1-8.2s6.5-4.3 9-4.8c1-.2 2-.2 2.9 0c0 0-8.3 2.6-8.1 4.4s9.8 2.3 14.9 4.7l1.1.6c.2.1.5 0 .7-.2V9.5c0-2.2-1.8-4-4-4z"></svg:path>`,
})
export class ArcticonsTutaAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTutaMailIcon],svg[arcticons-tuta-mail-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 42.27h28.377L42.5 13.893H14.123zm37-28.428H14.123L5.5 5.73h28.377zM5.5 42.27V5.73"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.673 37.788l6.01-19.662l2.273 19.91l14.447-19.91l-6.087 19.91"></svg:path>`,
})
export class ArcticonsTutaMailIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuttoandroidIcon],svg[arcticons-tuttoandroid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.342 42.05c-2.85 1.29-6.14 2.02-9.64 2.02H23.7c-3.5 0-6.79-.73-9.64-2.02M12.04 5.043l3.097 4.856"></svg:path><svg:circle cx="15.998" cy="17.031" r="2.545" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.27 27.387c2.334-1.55 10.55 3.773 11.366 10.645c.6 5.062-6.052 6.003-8.124 3.822c-2.473-2.601-5.443-13.006-3.242-14.467m38.69-2.361c0-9.449-8.488-17.108-18.96-17.108S5.04 15.578 5.04 25.026zM35.961 5.043l-3.098 4.856"></svg:path><svg:circle cx="32.002" cy="17.031" r="2.545" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.73 27.387c-2.334-1.55-10.55 3.773-11.366 10.645c-.6 5.062 6.052 6.003 8.124 3.822c2.473-2.601 5.443-13.006 3.242-14.467m-3.319.313H7.589"></svg:path>`,
})
export class ArcticonsTuttoandroidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuudoIcon],svg[arcticons-tuudo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:rect width="4.551" height="6.029" x="33.95" y="21.521" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.275" ry="2.275"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.852 21.521v3.754a2.275 2.275 0 0 0 2.275 2.276h0a2.275 2.275 0 0 0 2.276-2.276v-3.754m0 3.754v2.275m2.139-6.029v3.754a2.275 2.275 0 0 0 2.275 2.276h0a2.275 2.275 0 0 0 2.275-2.276v-3.754m0 3.754v2.275m-14.398-7.906v7.906M9.5 21.521h2.389m19.857 2.275a2.275 2.275 0 0 0-2.275-2.275h0a2.275 2.275 0 0 0-2.276 2.275v1.48a2.275 2.275 0 0 0 2.276 2.274h0a2.275 2.275 0 0 0 2.275-2.275m0 2.275v-9.1"></svg:path>`,
})
export class ArcticonsTuudoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuxguitarIcon],svg[arcticons-tuxguitar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.877 3.5c3.32.116 6.663 1.439 6.96 4.615l.279 6.324c1.653.107 1.583 3.45-.04 3.66l.24 3.222l6.801-3.38l-.04-1.59l3.302-1.95l3.023 4.375l-3.62 2.426l-1.432-.755l-6.246 3.739c3.999 5.576 4.198 10.946-1.552 15.992l3.263 1.59c3.1 1.685.745 2.003-1.591.876c1.309 1.838-1.77.838-3.86.478c-2.368 2.72-3.16.188-4.335-1.075l-2.824.08l-1.194 1.71c-1.31 1.275-2.397.438-3.5-.238c-1.407.076-2.959 1.458-4.177-.16c-.82-.19-4.405 1.17-1.354-1.192l4.06-1.751C4.661 39.112 3.749 29.877 8.113 27.01c1.703-2.412 2.842-3.873 5.608-4.455l.557-5.012c-1.506-.18-1.452-3.066.358-2.984l.239-7.4c.106-1.657 3.123-3.529 7.002-3.659"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.278 17.542l5.37 3.898h4.774l4.654-3.341m-15.355 4.455c3.394 2.342 7.22 4.48 5.172 9.388a5.18 5.18 0 0 1-2.268-3.46c-.164 1.706-1.037 3.486-2.745 2.467L8.113 27.01m5.927 13.484c3.074-.446 5.446-2.886 5.052-5.887l4.217-6.086l7.797-4.336"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m29.316 21.321l-8.355 4.615l-2.227.32m14.798 8.352c-3.517-1.313-6.94-2.754-8.274-7.08m8.591 2.289c-2.783-1.038-5.175-.587-6.23-4.011M14.636 14.559l3.779.04c.398-2.49 6.992-2.225 7.16-.08l3.54-.08"></svg:path>`,
})
export class ArcticonsTuxguitarIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuxriderIcon],svg[arcticons-tuxrider-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.756 40.511q-.357.022-.714.022a11 11 0 0 1-1.128-.058m-9.538-5.261a10.87 10.87 0 0 1-2.782-6.816c0-4.737 2.395-9.315 2.395-12.348S14.799 4.5 24.326 4.5c9.314 0 10.766 9.676 10.964 12.614c.32 4.737 2.182 7.079 2.182 10.964a11.44 11.44 0 0 1-2.762 7.257"></svg:path><svg:circle cx="24.033" cy="29.495" r="3.925" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.255 17.374a93.5 93.5 0 0 0-10.923 8.209m-11.465-8.209c3.502 1.705 9.303 5.655 11.166 8.196m.945-8.099a3.5 3.5 0 0 1-.446-1.774a3.29 3.29 0 0 1 3.426-3.632a3.964 3.964 0 0 1 3.68 3.852a3.67 3.67 0 0 1-3.234 3.413a3.7 3.7 0 0 1-.672-.072"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.308 19.063a3.485 3.485 0 0 1-3.48-3.865c0-3.094 2.122-4.59 3.852-4.59c2.089 0 3.852 2.395 3.852 5.089a3.3 3.3 0 0 1-.402 1.664m7.148 17.497c4.112 0 6.786 1.616 6.786 4.55c0 1.876-3.453 1.876-6.786 1.876s-6.626.059-6.626-1.915c0-2.495 3.033-4.511 6.626-4.511"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.848 40.122a1.27 1.27 0 0 1 .615 1.462c-.15.64-2.714 1.916-7.504 1.916c-4.504 0-6.256-.388-6.203-2.989q.005-.248.031-.523m-8.394-5.13c-4.112 0-6.786 1.616-6.786 4.55c0 1.876 3.453 1.876 6.786 1.876s6.626.059 6.626-1.915c0-2.495-3.033-4.511-6.626-4.511"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.823 40.122a1.26 1.26 0 0 0-.615 1.462c.17.682 2.714 1.916 7.504 1.916c4.525 0 6.271-.392 6.202-3.024q-.006-.232-.03-.488m13.06-18.991c.145.533 7.689 4.846 6.77 7.294c-.744 1.985-4.056-2.077-5.433-2.361m-25.322-4.933c-.171.897-7.59 4.846-6.672 7.294c.744 1.985 4.056-2.077 5.434-2.361m13.611-8.556c1.537 0 3.4 1.884 3.4 1.884l-3.4 4.163S20.14 19.81 20.18 19.33s1.915-1.956 4.15-1.956m-10.954 17.84c-.76-2.691-.56-13.124 5.787-16.15m14.692 16.044c.836-1.667 2.476-11.261-4.431-16.045"></svg:path><svg:circle cx="26.293" cy="15.585" r=".75" fill="currentColor"></svg:circle><svg:circle cx="22.981" cy="15.697" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.033 30.757v-2.523"></svg:path>`,
})
export class ArcticonsTuxriderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTuyaSmartIcon],svg[arcticons-tuya-smart-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.43 12.783v19.862a5.855 5.855 0 0 0 5.855 5.855h2.706M14.545 18.888h11.521m4.706 0a4.705 4.705 0 0 0-4.706-4.705m9.389 4.705A9.39 9.39 0 0 0 26.066 9.5"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTuyaSmartIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvIcon],svg[arcticons-tv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="39" height="25" x="4.5" y="9.75" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16 38.25h16"></svg:path>`,
})
export class ArcticonsTvIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvBrowserIcon],svg[arcticons-tv-browser-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path d="M39.207 24.21h-6.091l-5.167 12.706l-7.898-25.832l-5.167 12.706h-6.09"></svg:path></svg:g>`,
})
export class ArcticonsTvBrowserIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvBrowserUpdatepluginIcon],svg[arcticons-tv-browser-updateplugin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.207 24.21h-6.091l-5.167 12.705l-7.898-25.831l-5.167 12.705h-6.09"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.576 11.573L36.19 14.29l3.016 1.992l-3.607.234l.724 3.542l-2.717-2.385l-1.992 3.017l-.235-3.608l-3.542.724l2.385-2.716l-3.016-1.993l3.607-.234l-.724-3.542l2.717 2.385L34.8 8.69l.235 3.607z"></svg:path>`,
})
export class ArcticonsTvBrowserUpdatepluginIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvFritzIcon],svg[arcticons-tv-fritz-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37.008" height="25.233" x="5.496" y="9.444" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.682"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.1 38.556h17.8M23.46 18.1v7.92M10 18.1h3.96M10 22.06h2.57M10 18.1v7.92m6.08 0V18.1h2.59a2.66 2.66 0 0 1 0 5.32h-2.59m2.59 0l2.59 2.6m4.35-7.92h5.24m-2.62 7.92V18.1m4.53 0H38l-5.24 7.92H38"></svg:path>`,
})
export class ArcticonsTvFritzIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvShowTrackerTrakttvClientIcon],svg[arcticons-tv-show-tracker-trakttv-client-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.809 13.75L24 3.5L6.191 13.75v20.5L24 44.5l17.809-10.25z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.456 21.694a2.57 2.57 0 0 1-2.562-2.563c0-.897.512-1.665 1.153-2.178c-1.794-1.794-4.356-2.947-7.047-2.947c-5.51 0-9.994 4.485-9.994 9.994s4.485 9.994 9.994 9.994s9.994-4.485 9.994-9.994q0-1.346-.385-2.69c-.256.256-.769.384-1.153.384"></svg:path>`,
})
export class ArcticonsTvShowTrackerTrakttvClientIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTv8Icon],svg[arcticons-tv8-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4M12.01 20h5.2m-2.6 8v-8"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.21 20l-2.6 8l-2.6-8m12.58 4c-1.1 0-2 .9-2 2s.9 2 2 2h1.3c1.1 0 2-.9 2-2s-.9-2-2-2m.1 0c1.1 0 2-.9 2-2s-.9-2-2-2h-1.3c-1.1 0-2 .9-2 2s.9 2 2 2m-.1 0h1.4"></svg:path>`,
})
export class ArcticonsTv8Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTverIcon],svg[arcticons-tver-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.484 14.135l7.265 19.729l7.265-19.729H28.9l-4.151 11.279l-4.151-11.279H4.51v3.115h6.4v16.614h3.115V17.25h4.608m21.623 11.184c0-1.818 1.448-3.291 3.233-3.291m-3.233 0v8.722m-3.742-1.661a3.22 3.22 0 0 1-2.81 1.661c-1.785 0-3.232-1.474-3.232-3.291v-2.14c0-1.818 1.447-3.291 3.233-3.291s3.233 1.473 3.233 3.291v1.07h-6.466"></svg:path>`,
})
export class ArcticonsTverIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvingIcon],svg[arcticons-tving-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m37.322 13.704l-7.48 20.592H17.887V19.882h-7.209v-6.178z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m32.991 5.5l-8.346 22.975V5.5"></svg:path>`,
})
export class ArcticonsTvingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvisIcon],svg[arcticons-tvis-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m15.892 21.992l13.455-2.913l.703 3.314l-4.619 1.255l2.36 11.397l-4.117.902l-2.51-11.446l-4.619.904zm5.372 20.281l7.079-5.823m0 0l7.882-4.418m0 0L42.5 26.46m0 0L27.339 5.726M13.382 16.77L5.5 21.39l15.764 20.883m-1.105-32.129l-6.777 6.628l3.762 4.949m2.362 3.105l2.428 3.195m5.414 7.12l.995 1.31M27.338 5.726l-7.179 4.418l6.92 9.427m2.221 3.027l6.924 9.434"></svg:path>`,
})
export class ArcticonsTvisIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvkillIcon],svg[arcticons-tvkill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.48 34a1 1 0 0 0 1-1V11.39a1 1 0 0 0-1-1h-37a1 1 0 0 0-1 1V33a1 1 0 0 0 1 1Zm-19.59-3.48v-7.25H19l6.11-9.44v7.24H29ZM24 33.97v3.66m-7.43 0h14.86"></svg:path>`,
})
export class ArcticonsTvkillIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvsCreditSaathiIcon],svg[arcticons-tvs-credit-saathi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.45 11.425v25.15c-.85-6.6-6.49-11.7-13.33-11.7H24c0-7.43-6.02-13.45-13.45-13.45z"></svg:path>`,
})
export class ArcticonsTvsCreditSaathiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTvtimeIcon],svg[arcticons-tvtime-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 5.5h12.333v12.333H5.5zm12.334 0h12.333v12.333H17.834zm12.333 0H42.5v12.333H30.167zM17.834 17.834h12.333v12.333H17.834zm0 12.333h12.333V42.5H17.834z"></svg:path>`,
})
export class ArcticonsTvtimeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwentyIcon],svg[arcticons-twenty-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M15.164 20.398a3.922 3.922 0 0 1 5.291-2.97c1.564.596 2.609 2.224 2.48 3.892c-.094 1.24-.693 2.376-1.666 2.967c-1.802 1.095-7.479 3.9-7.479 3.9l7.717 1.36m11.748-8.523l-7.469 7.864"></svg:path><svg:rect width="7.836" height="11.828" x="25.607" y="19.043" rx="3.918" ry="3.918" transform="rotate(10 29.525 24.956)"></svg:rect></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.052 10.548L12.578 5.527a1.75 1.75 0 0 0-2.03 1.421L5.527 35.422a1.75 1.75 0 0 0 1.421 2.03l28.474 5.021a1.75 1.75 0 0 0 2.03-1.421l5.021-28.474a1.75 1.75 0 0 0-1.421-2.03"></svg:path>`,
})
export class ArcticonsTwentyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwidereIcon],svg[arcticons-twidere-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m23.57 14.64l-6.32 6.42l-6.33-6.42l6.33-6.37z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 8.27h12.76l-6.33 6.37zm6.42 6.37l6.33 6.42l-6.35 6.34z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.2 39.72l-9.24-9.36l9.29-9.3zm.05.01V21.06l18.59 18.6z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.4 34.22L17.25 21.06H43.5z"></svg:path>`,
})
export class ArcticonsTwidereIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwilightIcon],svg[arcticons-twilight-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M2.818 25.244c13.06-.475 27.717-.48 42.364 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.717 24.911a11.283 11.283 0 0 1 22.566 0"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTwilightIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwinEdgeIcon],svg[arcticons-twin-edge-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.449 4.239c.613.263 1.219.556 1.8.872M22.68 2.554a22 22 0 0 1 2-.03m4.935.73a21 21 0 0 0-1.953-.43M38.33 7.987a22 22 0 0 0-1.551-1.263M22.77 45.486a22 22 0 0 1-1.988-.205m4.986.168a22 22 0 0 0 1.983-.255m-9.896-.563a22 22 0 0 1-1.888-.659m14.693.495a21 21 0 0 0 1.871-.707M13.27 42.659a22 22 0 0 1-1.685-1.079m23.605.801a22 22 0 0 0 1.657-1.12m-27.584-1.58a22 22 0 0 1-1.391-1.436m31.248 1.059a22 22 0 0 0 1.355-1.47M6.049 35.865c-.365-.55-.71-1.128-1.025-1.717m37.215 1.26c.354-.564.684-1.151.98-1.742m-39.42-2.253a22 22 0 0 1-.6-1.907m41.177 1.392a21 21 0 0 0 .55-1.922M2.642 26.56a22 22 0 0 1-.142-1.995m42.909 1.451c.06-.658.091-1.331.091-1.998M2.63 21.57a22 22 0 0 1 .318-1.974m42.333 1.344a21 21 0 0 0-.376-1.965M3.767 16.709a21 21 0 0 1 .767-1.847m39.468 1.253a22 22 0 0 0-.822-1.824M6 12.244a22 22 0 0 1 1.172-1.62M41.64 11.72a22 22 0 0 0-1.22-1.584M9.2 8.415c.482-.458.991-.898 1.513-1.307m2.479-1.686c.573-.334 1.17-.646 1.773-.925m2.802-1.067c.637-.193 1.29-.357 1.94-.488"></svg:path>`,
})
export class ArcticonsTwinEdgeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwingateIcon],svg[arcticons-twingate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.713 43.5l11.092-7.623a10.86 10.86 0 0 0 4.709-8.95v-16.49L24.42 18.058a10.86 10.86 0 0 0-4.708 8.95z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.286 15.402V4.5l-11.087 7.623a10.86 10.86 0 0 0-4.713 8.948v16.494l7.232-4.973"></svg:path>`,
})
export class ArcticonsTwingateIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwinklyIcon],svg[arcticons-twinkly-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.694 13.29c1.175-6.599 3.403-7.663 3.403-7.663c5.557 3.932 5.648 10.666 5.648 10.666c5.694-4.79 11.825-2.378 12.564-.497c.994 2.53-2.034 7.683-5.695 9.988c2.488 3.747 2.643 9.205 2.53 12.012c-.043 1.035-.949 1.801-1.982 1.729c-7.745-.545-11.213-4.748-11.213-4.748c-1.949 4.203-9.762 8.316-11.457 7.49c-2.494-1.217-4.18-7.264-2.192-14.45c-4.926-3.977-7.23-7.23-6.734-9.4s3.209-4.971 9.265-4.971c5.378 0 9.942 5.923 9.942 5.923c-4.108-1.375-9.25-2.032-12.79-.56c4.204 2.455 8.045 4.583 8.497 6.613c.485 2.176-1.085 6.644-1.085 6.644c2.35-3.3 4.791-4.927 6.463-4.927s8.09 5.966 8.09 5.966c.316-3.073-1.898-7.909-1.898-7.909c5.468-5.107 6.824-8.09 6.824-8.09c-4.113-1.898-9.13-.813-9.13-.813"></svg:path>`,
})
export class ArcticonsTwinklyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwintIcon],svg[arcticons-twint-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 23.958L32.804 38.054L20.018 21.202h0a7.076 7.076 0 1 1 11.422 0h0L18.652 38.054L5.865 21.202h0a7.08 7.08 0 0 1 7.62-10.995"></svg:path>`,
})
export class ArcticonsTwintIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwireIcon],svg[arcticons-twire-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 45.5A21.5 21.5 0 1 1 45.5 24A21.51 21.51 0 0 1 24 45.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.61 31.39a10.45 10.45 0 1 1 14.78 0a10.45 10.45 0 0 1-14.78 0m0-14.78L3.32 29.9m28.07 1.49L44.7 18.07"></svg:path>`,
})
export class ArcticonsTwireIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwistIcon],svg[arcticons-twist-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.89 5.197L6.8 21.734a2 2 0 0 0-1.127 1.8v18.33a1 1 0 0 0 1.456.89l14.654-7.504l6.158 4.728l4.033-10.372l9.393-5.734a2 2 0 0 0 .958-1.707V6.097a1 1 0 0 0-1.437-.9"></svg:path>`,
})
export class ArcticonsTwistIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwistytimerIcon],svg[arcticons-twistytimer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="36.126" height="36.126" x="5.937" y="5.937" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="7.227" transform="rotate(-75 24 24)"></svg:rect><svg:rect width="6.638" height="6.638" x="20.691" y="20.871" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.161" transform="rotate(-75 24.01 24.19)"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m9.039 38.537l6.455-24.091a3.478 3.478 0 0 1 4.566-2.636l17.186 4.605l-3.488 13.014c-.118.442 7.024 1.72 8.852-5.11"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.472 32.914l17.266 4.626a3.758 3.758 0 0 0 5.096-2.856m-22.362-1.77s-2.95 7.245 5.641 9.547m14.133-26.046s.96-7.317-5.2-8.967"></svg:path>`,
})
export class ArcticonsTwistytimerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitcastingIcon],svg[arcticons-twitcasting-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.228 24.85c0 8.93-7.216 16.182-16.147 16.227a16.2 16.2 0 0 1-5.57-.952m-8.482-7.014a16.15 16.15 0 0 1-2.256-8.1c-.07-6.941 4.231-12.913 10.344-15.29m11.195-.21c6.24 2.161 10.773 8.03 10.913 15.013M18.2 9.623c3.18-3.744 6.953-6.25 7.066-4.612c-1.042 2.397-.733 2.37-.792 3.622c2.409-1.652 4.304-2.665 4.817-1.949c-.873.966-1.325 2.406.147 2.788m1.87 19.1c0 .87-.561 1.575-1.255 1.58s-1.26-.695-1.267-1.564s.548-1.582 1.242-1.595s1.265.679 1.28 1.548m-5.211 2.746l-2.015 1.189l-2.014-1.19l2.014-1.188z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.403 38.925a2.2 2.2 0 0 1-.277-1.075v-9.726c0-1.225.986-2.21 2.21-2.21h5.72c1.225 0 2.21.985 2.21 2.21v9.726c0 1.224-.985 2.21-2.21 2.21h-2.135M9.275 43.5H35.96m-1.499-5.816l1.498 5.815"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m9.276 43.5l1.48-6.006c.7-1.81 3.976-.4 3.159 1.339c1.99.18 2.076 1.146 2.118 2.13"></svg:path>`,
})
export class ArcticonsTwitcastingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitchIcon],svg[arcticons-twitch-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.11 4.52l-6.88 6.84v25.12h8.3v7l7.06-7h5.61l12.57-12.46V4.52z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.53 7.41H38v15.2l-5.45 5.41h-5.61l-5.12 5.13v-5.13h-6.29zm9.175 5.365v6.992m7.69.008v-7"></svg:path>`,
})
export class ArcticonsTwitchIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitlatteIcon],svg[arcticons-twitlatte-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.66 14.91h-1.38v-4a.61.61 0 0 0-.61-.6H10.28a.6.6 0 0 0-.6.6V29.6a5.48 5.48 0 0 0 1.88 4.12h-5a.7.7 0 0 0-.66.73a3.24 3.24 0 0 0 3.27 3.19h28.7a3.23 3.23 0 0 0 3.23-3.23a.69.69 0 0 0-.69-.69h-5a5.5 5.5 0 0 0 1.89-4.08v-2.32h1.11a6.39 6.39 0 0 0 6.44-5.6v-.63a6.18 6.18 0 0 0-6.19-6.18m0 8.57h-1.38v-4.75h1.38a2.43 2.43 0 0 1 0 4.75m-27.1 10.24h23.83"></svg:path>`,
})
export class ArcticonsTwitlatteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitterIcon],svg[arcticons-twitter-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.74 16.55v1c0 10.07-7.64 21.61-21.62 21.61c-4.13 0-8.17-1.19-11.62-3.45c.6.08 1.2.12 1.81.11c3.42 0 6.75-1.13 9.44-3.24a7.56 7.56 0 0 1-7.1-5.29c.47.1.96.15 1.44.15c.68 0 1.35-.09 2-.27A7.574 7.574 0 0 1 7 19.72v-.1c1.05.59 2.23.91 3.44.94A7.54 7.54 0 0 1 8.05 10.5c3.86 4.75 9.56 7.64 15.68 7.94a6.4 6.4 0 0 1-.21-1.74a7.546 7.546 0 0 1 7.27-7.82c2.24-.08 4.4.84 5.9 2.51c1.7-.34 3.33-.97 4.83-1.85a7.65 7.65 0 0 1-3.39 4.27c1.51-.21 2.98-.63 4.37-1.26c-1.01 1.55-2.28 2.9-3.76 4m-8.39-.08l2.5-2.51m-2.5 0l2.5 2.51"></svg:path>`,
})
export class ArcticonsTwitterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitterAlt1Icon],svg[arcticons-twitter-alt-1-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.74 16.55v1c0 10.07-7.64 21.61-21.62 21.61A21.14 21.14 0 0 1 5.5 35.71a12 12 0 0 0 1.81.11a15.25 15.25 0 0 0 9.44-3.24a7.56 7.56 0 0 1-7.1-5.29a7 7 0 0 0 1.44.15a7.5 7.5 0 0 0 2-.27A7.57 7.57 0 0 1 7 19.72v-.1a7.4 7.4 0 0 0 3.44.94A7.54 7.54 0 0 1 8.05 10.5a21.58 21.58 0 0 0 15.68 7.94a6.4 6.4 0 0 1-.21-1.74a7.55 7.55 0 0 1 13.17-5.31a15.6 15.6 0 0 0 4.83-1.85a7.65 7.65 0 0 1-3.39 4.27a15.9 15.9 0 0 0 4.37-1.26a15.6 15.6 0 0 1-3.76 4"></svg:path>`,
})
export class ArcticonsTwitterAlt1Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitterAlt2Icon],svg[arcticons-twitter-alt-2-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.5 41.5h-7.368L6.5 6.5h7.368l27.632 35zM21.941 26.059L6.5 41.5m35-35L26.059 21.941"></svg:path>`,
})
export class ArcticonsTwitterAlt2Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwitterDownloaderIcon],svg[arcticons-twitter-downloader-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.358 18.77v.703c0 7.076-5.369 15.185-15.193 15.185c-2.9.007-5.738-.836-8.165-2.424q.633.086 1.272.077c2.404.007 4.74-.795 6.633-2.277a5.31 5.31 0 0 1-4.989-3.717q.5.105 1.012.106c.475 0 .948-.064 1.405-.19a5.32 5.32 0 0 1-4.279-5.235v-.07c.74.413 1.57.64 2.417.66a5.3 5.3 0 0 1-1.68-7.07a15.16 15.16 0 0 0 11.02 5.58a4.5 4.5 0 0 1-.148-1.222a5.305 5.305 0 0 1 9.254-3.732a11 11 0 0 0 3.394-1.3a5.38 5.38 0 0 1-2.382 3A11.2 11.2 0 0 0 37 15.96a10.9 10.9 0 0 1-2.641 2.811Z"></svg:path><svg:circle cx="38.5" cy="38.499" r="7" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.432 43.326A21.4 21.4 0 0 1 24 45.5C12.126 45.5 2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24c0 3.383-.781 6.583-2.173 9.43M38.5 42.499v-8"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m35.203 39.203l3.297 3.296l3.297-3.296"></svg:path>`,
})
export class ArcticonsTwitterDownloaderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwodotsIcon],svg[arcticons-twodots-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="34.5" cy="13.5" r="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="13.5" cy="34.5" r="8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsTwodotsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTwrpIcon],svg[arcticons-twrp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34 5.59v4.9h5.77a.48.48 0 0 1 .48.49a.5.5 0 0 1-.09.29L28.77 26.49a.49.49 0 0 1-.69.09a.4.4 0 0 1-.1-.09L16.63 11.28a.48.48 0 0 1 .37-.78h5.77V2.93a.4.4 0 0 0-.41-.39h0a21.53 21.53 0 0 0-8.81 40.27a.41.41 0 0 0 .59-.35v-5.11h-5.8a.49.49 0 0 1-.49-.49a.54.54 0 0 1 .1-.3l11.36-15.24a.5.5 0 0 1 .7-.08l.09.08l11.35 15.24a.49.49 0 0 1-.1.69a.53.53 0 0 1-.29.1h-5.77v7.71a.4.4 0 0 0 .4.4h0a21.53 21.53 0 0 0 8.84-40.22a.41.41 0 0 0-.53.21a.4.4 0 0 0 0 .14"></svg:path>`,
})
export class ArcticonsTwrpIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTxdToolIcon],svg[arcticons-txd-tool-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.5 21.757l6.366 6.373l7.955-7.965l7.956 7.965l7.956-7.965l6.767 6.767M10.158 5.5h27.685a4.647 4.647 0 0 1 4.657 4.637v27.705a4.647 4.647 0 0 1-4.636 4.658H10.158A4.647 4.647 0 0 1 5.5 37.864V10.157A4.647 4.647 0 0 1 10.137 5.5z"></svg:path>`,
})
export class ArcticonsTxdToolIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTydomIcon],svg[arcticons-tydom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 42.5v-37M24 24H5.5m0 0L24 42.5m10.5-37h-25a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h25a8 8 0 0 0 8-8v-21a8 8 0 0 0-8-8"></svg:path>`,
})
export class ArcticonsTydomIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTypewiseIcon],svg[arcticons-typewise-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.171 13.794L6.768 33.599c-.261 1.396-.087 2.792.524 4.014c2.705 5.584 11.691 3.752 11.604-7.59"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.896 30.022c-.087 1.396 0 3.054.61 4.275c2.705 5.497 11.692 3.665 11.692-7.59"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.198 26.707c-.087 1.396 0 3.228.61 4.45C34.514 36.652 43.5 34.82 43.5 23.565m-31.497-2.268l2.443-.872M4.5 23.828l4.188-1.396m9.51-14.658h23.034v12.564H18.198zm5.235 9.282h12.825"></svg:path><svg:circle cx="22.037" cy="11.054" r=".75" fill="currentColor"></svg:circle><svg:circle cx="25.876" cy="11.054" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.715" cy="11.054" r=".75" fill="currentColor"></svg:circle><svg:circle cx="33.554" cy="11.054" r=".75" fill="currentColor"></svg:circle><svg:circle cx="37.393" cy="11.054" r=".75" fill="currentColor"></svg:circle><svg:circle cx="22.037" cy="13.671" r=".75" fill="currentColor"></svg:circle><svg:circle cx="25.876" cy="13.671" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.715" cy="13.671" r=".75" fill="currentColor"></svg:circle><svg:circle cx="33.554" cy="13.671" r=".75" fill="currentColor"></svg:circle><svg:circle cx="37.393" cy="13.671" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsTypewiseIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTypewiseOfflineIcon],svg[arcticons-typewise-offline-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.9 11.762L6.66 30.62c-.25 1.33-.083 2.659.498 3.822c2.575 5.316 11.132 3.572 11.049-7.228m-6.563-8.308l2.326-.83m-9.47 3.24l3.988-1.329M17.543 6.03h21.932v11.963H17.543zm4.984 8.838h12.211"></svg:path><svg:circle cx="21.198" cy="9.153" r=".75" fill="currentColor"></svg:circle><svg:circle cx="24.853" cy="9.153" r=".75" fill="currentColor"></svg:circle><svg:circle cx="28.508" cy="9.153" r=".75" fill="currentColor"></svg:circle><svg:circle cx="32.164" cy="9.153" r=".75" fill="currentColor"></svg:circle><svg:circle cx="35.819" cy="9.153" r=".75" fill="currentColor"></svg:circle><svg:circle cx="21.198" cy="11.646" r=".75" fill="currentColor"></svg:circle><svg:circle cx="24.853" cy="11.646" r=".75" fill="currentColor"></svg:circle><svg:circle cx="28.508" cy="11.646" r=".75" fill="currentColor"></svg:circle><svg:circle cx="32.164" cy="11.646" r=".75" fill="currentColor"></svg:circle><svg:circle cx="35.819" cy="11.646" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.445 25.86a8.055 8.055 0 1 0 8.055 8.055h0a8.046 8.046 0 0 0-8.036-8.055zm3.266 11.197l-5.932.086a2.856 2.856 0 0 1-2.857-2.857a2.856 2.856 0 0 1 2.952-2.97l-1.238-1.267l8.341 8.236zm-5.79-5.713a3.094 3.094 0 0 1 5.418.086q.259.452.41.952a2.8 2.8 0 0 1 2.409 2.666a1.8 1.8 0 0 1-1.714 2.009h-.705"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.634 21.067c-.074 4.715-1.439 6.368-1.439 6.368M29.92 24.057c-.164 2.653.366 3.662.366 3.662m-12.079-.505c-.083 1.329 0 2.907.582 4.07c1.706 3.467 6.036 3.872 8.766.634"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.248 28.76c.425-1.314.673-2.878.673-4.703"></svg:path>`,
})
export class ArcticonsTypewiseOfflineIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsTypingheroIcon],svg[arcticons-typinghero-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.52 7.457h.627a6.34 6.34 0 0 1 6.353 6.352v4.632c0 3.52-1.232 6.353-4.751 6.353m-29.467 0c-3.52 0-4.782-2.833-4.782-6.353V13.81a6.34 6.34 0 0 1 6.353-6.352h.611"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.56 23.946v12.282a6.04 6.04 0 0 1-6.053 6.053H12.493a6.04 6.04 0 0 1-6.053-6.053V23.985m5.115-18.267h24.89v8.961h-24.89zm24.89 5.704a1.629 1.629 0 0 1 0 3.257m-24.89.001a1.629 1.629 0 0 1 0-3.258"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.63 29.623h2.74v2.803h-2.74zm8.745 0h2.74v2.803h-2.74zm-17.491 0h2.74v2.803h-2.74zm4.373 0h2.74v2.803h-2.74zm8.746 0h2.74v2.803h-2.74zm-6.56-4.833h2.74v2.803h-2.74zm8.746 0h2.74v2.803h-2.74zm-17.491 0h2.74v2.803h-2.74zm4.373 0h2.74v2.803h-2.74zm8.745 0h2.74v2.803h-2.74zm8.746 0h2.74v2.803h-2.74zM17.896 34.906h12.207v2.035H17.896z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.529 36.836a1.954 1.954 0 0 1-2.185 1.828H13.656a1.954 1.954 0 0 1-2.185-1.828m3.476-28.657v4.08m2.703-4.08v4.08m-2.703-2.04h2.703m2.763 2.04h2.04m-2.04-4.08h2.04m-2.04 2.04h1.326m-1.326-2.04v4.08m4.803 0v-4.08h1.326a1.377 1.377 0 1 1 0 2.753h-1.326m1.377-.05l1.326 1.377m2.431-1.352a1.352 1.352 0 1 0 2.703 0V9.531a1.37 1.37 0 0 0-1.377-1.377A1.33 1.33 0 0 0 30.35 9.53Zm8.36 11.999v11.75a2.176 2.176 0 0 1-2.18 2.18m-25.059 0a2.176 2.176 0 0 1-2.18-2.18V22.908m-.001 0L24 18.967m14.71 3.941L24 18.967"></svg:path>`,
})
export class ArcticonsTypingheroIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUaePassIcon],svg[arcticons-uae-pass-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.677 23.255c.255.733.198.489.326 1.225m-2.09-5.282c.516.686.796 1.372 1 2.057m-2.47 12.5c-.402.836-.98 1.419-1.786 1.675m-7.543-5.419c.45.574.42.912 1.455 1.263M12.664 37.36c.327.635.62 1.28 1.249 1.848m10.552-20.462c.478-.012 1.213-.097 1.76.194m-15.261-9.5l-.709.749m3.38-2.878c6.757-4.382 15.61-3.035 20.602-.064M6.95 21.111c-.7 1.759-.977 3.57-.951 5.038m2.69-8.269c3.077-4.602 6.648-7.059 10.232-8.095m4.421-.609c6.994.028 13.264 4.492 15.049 7.682M27.037 24.1c.823 1.161.985 1.726 1.134 2.907m-4.265.163c-.2 3.261 2.332 9.766 9.906 9.15m-9.813-13.392c-6.365-1.037-7.483 13.224 5.435 17.672m-.552-20.895c1.03.67 3.944 3.363 4.178 7.965M19.19 40.703a9.6 9.6 0 0 0 3.526 2.797m-5.431-6.6c-4.655-8.541-1.731-16.039 3.672-17.522m-9.6 13.96c-4.72-15.183 8.642-21.38 16.72-19.204c8.15 2.196 12.971 15.746 5.479 17.945"></svg:path>`,
})
export class ArcticonsUaePassIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUae4all2Icon],svg[arcticons-uae4all2-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m4.5 19.75l4.57 6.66L21 8.58m-8.52 16.2l1.12 1.63L25.52 8.58M9 19.75l1.12 1.63m-.46 17.99l12.03-18.48m0 18.53V20.89m0 12.33h-8.03"></svg:path><svg:circle cx="33.97" cy="21.89" r="9.53" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.89 13.83a22 22 0 0 0 13.55 3.7M26 16.65a24 24 0 0 0 17.5 4.89m-18.94-1.18c4.08 3.23 12.34 7.11 18.37 4.79m-18.2-.89c4.72 3.87 10.61 5.65 16 4.38"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.28 12.45c-7.79 2.65-10.9 12.49-7.59 16.6m8.77-16.37a55.1 55.1 0 0 1-4.52 18.52m5.22-18.29c4.14 6.47.91 15.41-2.81 18.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.77 13.15c4.24 4.41 6.46 11.31-.14 17.54M36.1 12.6c-5.64 4.39-8.5 14.94-6 18"></svg:path>`,
})
export class ArcticonsUae4all2Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUae4armIcon],svg[arcticons-uae4arm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.25 28l8.16 11.89l21.26-31.8M18.68 35.82l2.81 4.09L42.75 8.09M13.33 28l2.74 4"></svg:path>`,
})
export class ArcticonsUae4armIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUae4droidIcon],svg[arcticons-uae4droid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.54 5.82a49.7 49.7 0 0 0 30.58 8.35M22 2.59C26.29 6 31.49 7 37 6.9M6 12.19c11.68 8.86 24.79 12.46 39.45 11M2.78 20.55C12 27.83 30.6 36.6 44.22 31.35m-41.05-2c10.65 8.73 23.93 12.75 36 9.87M7.31 37.55c7.54 5.26 15.86 8.05 24.94 6.31"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.83 2.91C11.34 6.75 4.8 13.07 2.5 23.69M27 2.71C9.39 8.7 2.39 30.88 9.85 40.15m19.77-36.9C28.82 18.16 25 31.45 19.42 45M31.21 3.74c9.32 14.61 2 34.77-6.35 41.74"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.57 4.29c9.56 10 14.58 25.51-.32 39.57M28.81 3.05C16.09 13 9.65 36.74 15.26 43.64"></svg:path>`,
})
export class ArcticonsUae4droidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUalaIcon],svg[arcticons-uala-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.125 19.775v8h4m-18.521-8v5.3c0 1.5 1.2 2.7 2.6 2.7s2.7-1.2 2.7-2.7v-5.3m6.103 5.3h-3.6m-.9 2.7l2.7-8l2.7 8m12.628-2.7h-3.6m-.9 2.7l2.7-8l2.7 8"></svg:path><svg:circle cx="37.646" cy="27.475" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsUalaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbSmartBusIcon],svg[arcticons-ub-smart-bus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 4.5a16.246 16.246 0 0 1 16.246 16.246C40.246 34.73 24 43.5 24 43.5S7.754 34.73 7.754 20.746A16.246 16.246 0 0 1 24 4.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 9.384a17 17 0 0 1 2.998.25a13.6 13.6 0 0 1 3.391 1.04a3.3 3.3 0 0 1 1.53 1.142a3.2 3.2 0 0 1 .36 1.205a54 54 0 0 1 .857 6.143v9.638H31.6v1.537a1.313 1.313 0 1 1-2.62 0v-1.536h-9.96v1.536a1.313 1.313 0 1 1-2.62 0v-1.536h-1.537v-9.639a55 55 0 0 1 .84-6.143a3.2 3.2 0 0 1 .36-1.205a3.3 3.3 0 0 1 1.53-1.142a13.6 13.6 0 0 1 3.391-1.04A17 17 0 0 1 24 9.385m-6.28 13.88a1.376 1.376 0 1 0 1.37 1.382v-.012a1.37 1.37 0 0 0-1.37-1.37m12.56 0a1.376 1.376 0 1 0 1.377 1.376v-.006a1.365 1.365 0 0 0-1.36-1.37ZM16.4 28.802h15.2m-16.736-9.638h18.272M20.972 25.64h6.056m-6.056-2h6.056"></svg:path>`,
})
export class ArcticonsUbSmartBusIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbaMobileIcon],svg[arcticons-uba-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.273 11.506l9.965 19.734c3.271 6.478 1.466 11.26-6.086 11.26H7.275zm19.454 24.988L23.762 16.76C20.49 10.282 22.296 5.5 29.848 5.5h10.877z"></svg:path>`,
})
export class ArcticonsUbaMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbankIcon],svg[arcticons-ubank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="12" r="7.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="36" r="7.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUbankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUberIcon],svg[arcticons-uber-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:rect width="14" height="14" x="17.53" y="17.15" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M2.5 24h15.03"></svg:path>`,
})
export class ArcticonsUberIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUberDriverIcon],svg[arcticons-uber-driver-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 42.5h-29a4 4 0 0 1-4-4v-29a4 4 0 0 1 4-4h29a4 4 0 0 1 4 4v29a4 4 0 0 1-4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.786 18.956A2.726 2.726 0 0 1 38.5 16.23m-2.714 0v7.204m-3.179-1.369a2.73 2.73 0 0 1-2.32 1.369a2.714 2.714 0 0 1-2.807-2.715v-1.763a2.726 2.726 0 0 1 2.714-2.726c1.5 0 2.715 1.215 2.715 2.715v.893H27.48M9.5 12.565v7.273a3.608 3.608 0 0 0 7.204 0v-7.273m2.796 6.391a2.726 2.726 0 0 1 2.714-2.726a2.726 2.726 0 0 1 2.714 2.726v1.763c0 1.5-1.215 2.715-2.714 2.715h0a2.714 2.714 0 0 1-2.715-2.715h0m0 2.715V12.565M5.5 33.009h17.44l-4.031-4.031m4.031 4.031l-4.031 4.031"></svg:path>`,
})
export class ArcticonsUberDriverIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbereatsIcon],svg[arcticons-ubereats-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 42.5h-29c-2.2 0-4-1.8-4-4v-29c0-2.2 1.8-4 4-4h29c2.2 0 4 1.8 4 4v29c0 2.2-1.8 4-4 4m-27-7.732h4.685m-4.685-9.37h4.685M11.5 30.083h3.054M11.5 25.398v9.37"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.158 18.737a2.34 2.34 0 0 1 2.342-2.343h0m-2.342 0v6.208m-3.521 11.642c.428.36.89.524 1.927.524h.525a1.55 1.55 0 0 0 1.549-1.552h0a1.55 1.55 0 0 0-1.549-1.552h-1.05a1.55 1.55 0 0 1-1.55-1.552h0a1.55 1.55 0 0 1 1.55-1.551h.525c1.037 0 1.499.164 1.926.523m-3.073-7.664a2.34 2.34 0 0 1-2.036 1.182h0a2.34 2.34 0 0 1-2.342-2.343v-1.522a2.34 2.34 0 0 1 2.342-2.343h0a2.34 2.34 0 0 1 2.343 2.343v.761h-4.685m-.401 7.13v6.969c0 .646.524 1.17 1.171 1.17h.351m-2.752-6.206h2.46m-4.923 3.865a2.34 2.34 0 0 1-2.343 2.342h0a2.34 2.34 0 0 1-2.342-2.343v-1.522a2.34 2.34 0 0 1 2.342-2.342h0a2.34 2.34 0 0 1 2.343 2.342m0 3.865v-6.207M11.5 13.232v6.266a3.104 3.104 0 0 0 6.207 0v-6.266m2.412 5.505a2.34 2.34 0 0 1 2.342-2.343h0a2.34 2.34 0 0 1 2.343 2.343v1.522a2.34 2.34 0 0 1-2.343 2.343h0a2.34 2.34 0 0 1-2.342-2.343m0 2.343v-9.37"></svg:path>`,
})
export class ArcticonsUbereatsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbikitouchIcon],svg[arcticons-ubikitouch-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.989 9.626a21.53 21.53 0 0 0-23.307-5.849m26.796 11.108a21.5 21.5 0 0 0-1.332-2.422M14.159 43.12a21.515 21.515 0 0 0 30.662-24.5m-37.325-8.4a21.5 21.5 0 0 0 2.322 29.94m20.505-15.34a6 6 0 0 0 .063-.838a6 6 0 0 0-.063-.826l1.801-1.376a.425.425 0 0 0 .1-.538l-1.7-2.952a.413.413 0 0 0-.514-.188l-2.126.851a7.2 7.2 0 0 0-1.439-.838l-.325-2.252a.425.425 0 0 0-.425-.362h-3.378a.44.44 0 0 0-.45.337l-.313 2.252a7 7 0 0 0-1.438.838l-2.126-.85a.4.4 0 0 0-.513.187l-1.714 2.952a.425.425 0 0 0 .113.538l1.789 1.389a4.6 4.6 0 0 0 0 .825v.838l-1.789 1.401a.425.425 0 0 0-.113.55l1.74 2.965a.39.39 0 0 0 .512.175l2.126-.85a6.4 6.4 0 0 0 1.439.837l.313 2.264a.425.425 0 0 0 .425.35h3.327a.44.44 0 0 0 .426-.35l.375-2.264a6.4 6.4 0 0 0 1.45-.838l2.115.85a.44.44 0 0 0 .525-.174l1.701-2.952a.44.44 0 0 0-.1-.55Zm-6.33 1.05a1.876 1.876 0 0 1 0-3.753h0a1.864 1.864 0 0 1 1.864 1.864h0a1.876 1.876 0 0 1-1.863 1.889"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.468 34.468a14.752 14.752 0 1 0-6.215 3.716"></svg:path>`,
})
export class ArcticonsUbikitouchIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbisoftConnectIcon],svg[arcticons-ubisoft-connect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.503 30.132a7.511 7.511 0 0 0-12.836-7.34a7.73 7.73 0 0 0-1.774 4.968a8.04 8.04 0 0 0 1.798 4.97a8.64 8.64 0 0 0 4.482 2.852a9.7 9.7 0 0 0 5.328-.156a11 11 0 0 0 4.54-2.826a12.8 12.8 0 0 0 2.824-4.557a14.3 14.3 0 0 0-.45-10.68a14.98 14.98 0 0 0-7.68-7.468a15.7 15.7 0 0 0-10.722-.53a16.52 16.52 0 0 0-8.643 6.404a17.6 17.6 0 0 0-2.974 8.407A19.1 19.1 0 0 0 4.776 33a20.2 20.2 0 0 0 7.686 9.234a20.63 20.63 0 0 0 11.571 3.26a21.06 21.06 0 0 0 11.437-3.726a21.5 21.5 0 0 0 7.557-9.368a21.61 21.61 0 0 0-9.39-27.156a21.61 21.61 0 0 0-28.062 6.18"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.503 30.132A10.487 10.487 0 0 0 12.64 19.736a10.63 10.63 0 0 0-3.84 7.473a13.4 13.4 0 0 0 .351 3.778a18.51 18.51 0 0 0 16.706 14.385"></svg:path>`,
})
export class ArcticonsUbisoftConnectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUblDigitalIcon],svg[arcticons-ubl-digital-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.533 18.277v11.446h5.723M13.198 18.277v7.654a3.791 3.791 0 0 0 7.583 0v-7.654M28.719 24a2.861 2.861 0 0 1 0 5.723h-4.722V18.278h4.722a2.861 2.861 0 0 1 0 5.722m0 0h-4.722"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.225 13.498c-3.355-1.121-7.29-1.767-11.498-1.767C14.451 11.731 4.5 17.224 4.5 24s9.951 12.269 22.227 12.269c6.695 0 12.698-1.634 16.773-4.218"></svg:path><svg:circle cx="40.162" cy="14.234" r="1.797" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUblDigitalIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUboiVyomIcon],svg[arcticons-uboi-vyom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.102 14.055c1.318-1.55 2.764-3.123 4.658-3.752c2.764-1.033 5.931.081 7.862 2.427c2.576 3.06 2.49 8.361-.187 11.318c-3.523 3.905-7.071 7.787-10.61 11.678c-2.699 3.014-7.573 3.18-10.37.251A2170 2170 0 0 1 10.131 24.61c2.993 2.74 7.712 2.192 10.267-.971q4.343-4.8 8.704-9.584"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.684 12.584c.628-1.833 2.518-2.889 4.241-2.628c1.87.328 2.964 2.188 4.254 3.514c1.91 2.004 3.67 4.183 5.666 6.083c-2.555 3.163-7.275 3.712-10.267.97c-1.074-1.127-2.062-2.353-3.156-3.459c-1.077-1.16-1.04-2.979-.738-4.48"></svg:path>`,
})
export class ArcticonsUboiVyomIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUboxIcon],svg[arcticons-ubox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.62 13.25c5.305 9.19 3.113 20.877-5.163 27.519s-20.16 6.253-27.983-.916S.227 20.879 6.123 12.056C12.018 3.234 23.469.031 33.086 4.516m6.658 16.955l-7.565 10.314m7.565 0l-7.565-10.314"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.229 21.47c2.089 0 3.783 1.743 3.783 3.892v2.53c0 2.15-1.694 3.892-3.783 3.892h0c-2.09 0-3.783-1.743-3.783-3.892v-2.53c0-2.15 1.694-3.891 3.783-3.891M14.495 24c2.09 0 3.783 1.743 3.783 3.892s-1.694 3.892-3.783 3.892H8.254V16.217h6.241c2.09 0 3.783 1.742 3.783 3.892S16.584 24 14.495 24m0 0H8.262"></svg:path>`,
})
export class ArcticonsUboxIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbsBankingIcon],svg[arcticons-ubs-banking-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.09 3.5v28.309m-1.791-1.472h3.583"></svg:path><svg:circle cx="24.09" cy="37.449" r="5.641" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.441 38.677a2.65 2.65 0 1 0 0-5.298c-1.463 0-2.281 1.134-2.81 2.649"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.74 38.677a2.65 2.65 0 1 1 0-5.298c1.463 0 2.281 1.134 2.81 2.649M24.09 44.5v-2.82M21.417 6.387V5.163m-2.461 1.224h5.134m-2.673 3.884l-2.461-.878l2.461-.878m-2.461 5.109V5.163m2.461 7.237v1.224M18.956 12.4h5.134M10.688 9.815l21.686 18.196m-2.279.426l2.303-2.744"></svg:path><svg:circle cx="36.695" cy="31.637" r="5.641" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.933 34.456a2.65 2.65 0 0 0-4.06-3.406c-.94 1.12-.597 2.477.224 3.856"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.339 30.397a2.65 2.65 0 0 1-4.06-3.406c.941-1.12 2.336-1.019 3.836-.45m2.981 9.628l-2.16-1.813M11.181 13.719l-.938-.787m-.644 2.672L12.9 11.67m1.257 4.546l-2.255 1.321l.909-2.45m2.332 5.169l-6.482-5.439m7.126 2.767l.938.786m-2.52 1.099l3.301-3.934m19.806-5.72L15.626 28.011m2.279.426l-2.303-2.744"></svg:path><svg:circle cx="11.305" cy="31.637" r="5.641" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.067 34.456a2.65 2.65 0 0 1 4.06-3.406c.94 1.12.597 2.477-.224 3.856"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.661 30.397a2.65 2.65 0 0 0 4.06-3.406c-.941-1.12-2.336-1.019-3.836-.45m-2.981 9.628l2.16-1.813m28.755-20.637l.938-.787m.644 2.672L35.1 11.67m-1.257 4.546l2.255 1.321l-.909-2.45m-2.332 5.169l6.482-5.439m-7.126 2.767l-.938.786m2.52 1.099l-3.301-3.934"></svg:path>`,
})
export class ArcticonsUbsBankingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbuntuIcon],svg[arcticons-ubuntu-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="8.609" cy="22.82" r="6.109" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="34.051" cy="9.424" r="6.109" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="32.675" cy="38.576" r="6.109" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.239 36.052A17.35 17.35 0 0 0 43.084 24a17.34 17.34 0 0 0-3.975-11.074M8.969 28.918c2.122 7.22 8.798 12.492 16.706 12.492q.784 0 1.552-.069m1.295-34.519a17.5 17.5 0 0 0-2.833-.232h-.015c-6.075 0-11.424 3.113-14.539 7.83a17.4 17.4 0 0 0-1.33 2.408"></svg:path>`,
})
export class ArcticonsUbuntuIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUbybbtIcon],svg[arcticons-ubybbt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23 43.4a19.2 19.2 0 0 1-7.63-2.67c-3-2-4.89-5.15-5.84-9.51c-.26-1.2-.27-1.59-.32-13.91V4.5h7.07c.11 7.91.14 23.75.14 23.75a11.66 11.66 0 0 0 6.8 8.86c3 1.16 7.36 1.23 11.74.2c.68-.19 2.09-.53 2.09-.53a15.7 15.7 0 0 1-2 2.4A14.51 14.51 0 0 1 23 43.4m6.22-9.4a7.5 7.5 0 0 0 1.7-3.93c.09-.46.13-5.07.13-13.13V4.5h7.71v10.3c0 10.63 0 12.08-.34 13.39A8.76 8.76 0 0 1 35.15 33a14.4 14.4 0 0 1-6.45 1.54l.52-.57Z"></svg:path>`,
})
export class ArcticonsUbybbtIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUcBrowserIcon],svg[arcticons-uc-browser-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="19.584" cy="35.252" r="7.248" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.384 38.58s5.985-1.098 5.985 3.92H16.524c-6.184 0-11.197-5.013-11.197-11.197c0-2.9 1.1-5.548 2.914-7.533c3.51-3.84 5.1-5.194 5.1-8.375s-3.953-5.124-8.014-2.65C8.806 6.678 11.574 5.5 17.228 5.5s8.305 4.712 8.305 8.835c0 8.187-13.197 8.894-13.197 20.917"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.384 38.58a9.131 9.131 0 0 0-15.58-9.521"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.141 25.52c5.272-3.705 12.575-2.233 17.228 2.185c5.36-.943 7.304 2.356 7.304 2.356c-2.28-.178-5.034.656-6.854 1.343a3.04 3.04 0 0 1-3.336-.79c-3.593-3.961-9.133-7.585-14.342-5.093"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.98 23.573s2.789-2.863 8.973-6.175c-.236-3.004-.088-4.13 1.414-5.066c2.65.67 3.18 3.829 3.18 3.829c4.889 1.649 6.832 8.304 5.007 9.247s-7.741.256-11.353-.98"></svg:path><svg:circle cx="19.584" cy="35.252" r="2.626" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUcBrowserIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUcBrowserMiniIcon],svg[arcticons-uc-browser-mini-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:circle cx="19.584" cy="35.252" r="7.248"></svg:circle><svg:path d="M29.384 38.58s5.985-1.098 5.985 3.92H16.524c-6.184 0-11.197-5.013-11.197-11.197c0-2.9 1.1-5.548 2.914-7.533c3.51-3.84 5.1-5.194 5.1-8.375s-3.953-5.124-8.014-2.65C8.806 6.678 11.574 5.5 17.228 5.5s8.305 4.712 8.305 8.835c0 8.187-13.197 8.894-13.197 20.917"></svg:path><svg:path d="M29.384 38.58a9.131 9.131 0 0 0-15.58-9.521"></svg:path><svg:path d="M18.141 25.52c5.272-3.705 12.575-2.233 17.228 2.185c5.36-.943 7.304 2.356 7.304 2.356c-2.28-.178-5.034.656-6.854 1.343a3.04 3.04 0 0 1-3.336-.79c-3.593-3.961-9.133-7.585-14.342-5.093"></svg:path><svg:path d="M22.98 23.573s2.789-2.863 8.973-6.175c-.236-3.004-.088-4.13 1.414-5.066c2.65.67 3.18 3.829 3.18 3.829c4.889 1.649 6.832 8.304 5.007 9.247s-7.741.256-11.353-.98"></svg:path></svg:g><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M15.586 34.602a2 2 0 0 1 2-2h0a2 2 0 0 1 1.998 2V37.9m-3.998-5.297V37.9"></svg:path><svg:path d="M19.584 34.602a2 2 0 0 1 2-2h0a2 2 0 0 1 1.999 2V37.9"></svg:path></svg:g>`,
})
export class ArcticonsUcBrowserMiniIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUcBrowserTurboIcon],svg[arcticons-uc-browser-turbo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:circle cx="19.584" cy="35.252" r="7.248"></svg:circle><svg:path d="M29.384 38.58s5.985-1.098 5.985 3.92H16.524c-6.184 0-11.197-5.013-11.197-11.197c0-2.9 1.1-5.548 2.914-7.533c3.51-3.84 5.1-5.194 5.1-8.375s-3.953-5.124-8.014-2.65C8.806 6.678 11.574 5.5 17.228 5.5s8.305 4.712 8.305 8.835c0 8.187-13.197 8.894-13.197 20.917"></svg:path><svg:path d="M29.384 38.58a9.131 9.131 0 0 0-15.58-9.521"></svg:path><svg:path d="M18.141 25.52c5.272-3.705 12.575-2.233 17.228 2.185c5.36-.943 7.304 2.356 7.304 2.356c-2.28-.178-5.034.656-6.854 1.343a3.04 3.04 0 0 1-3.336-.79c-3.593-3.961-9.133-7.585-14.342-5.093"></svg:path><svg:path d="M22.98 23.573s2.789-2.863 8.973-6.175c-.236-3.004-.088-4.13 1.414-5.066c2.65.67 3.18 3.829 3.18 3.829c4.889 1.649 6.832 8.304 5.007 9.247s-7.741.256-11.353-.98"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.44 31.252V38.1c0 .635.516 1.15 1.152 1.15h.345m-2.705-6.099h2.417"></svg:path>`,
})
export class ArcticonsUcBrowserTurboIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUcgoIcon],svg[arcticons-ucgo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-miterlimit="11.339" d="M10.622 9.501v8.484a4.203 4.203 0 0 0 8.405 0V9.501m12.233 8.431v.052a4.203 4.203 0 0 1-8.404 0v-4.281A4.203 4.203 0 0 1 27.058 9.5h0a4.203 4.203 0 0 1 4.203 4.203v.052m-6.117 16.26a4.203 4.203 0 0 0-8.405 0v4.282a4.203 4.203 0 0 0 8.405 0h-4.202M33.175 38.5a4.203 4.203 0 0 1-4.202-4.202v-4.282a4.203 4.203 0 0 1 8.405 0v4.282a4.203 4.203 0 0 1-4.203 4.202Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsUcgoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUchealthIcon],svg[arcticons-uchealth-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.5 21.637v2.73a1.66 1.66 0 0 0 1.655 1.655h0a1.66 1.66 0 0 0 1.655-1.655v-2.73m0 2.73v1.655m6.902-6.619v6.619m0-2.73a1.66 1.66 0 0 1 1.654-1.655h0a1.66 1.66 0 0 1 1.655 1.655v2.73m18.169-6.619v6.619m0-2.73a1.66 1.66 0 0 1 1.655-1.655h0a1.66 1.66 0 0 1 1.655 1.655v2.73m-6.041-5.792v5.792m-.911-4.385h1.738m-11.269 3.558a1.6 1.6 0 0 1-1.406.827h0a1.66 1.66 0 0 1-1.655-1.655v-1.075a1.66 1.66 0 0 1 1.655-1.655h0a1.66 1.66 0 0 1 1.655 1.655v.579h-3.31m10.501-4.468v5.792a.782.782 0 0 0 .828.827h.248m-18.785-.827a1.6 1.6 0 0 1-1.407.827h0a1.66 1.66 0 0 1-1.655-1.655v-1.075a1.66 1.66 0 0 1 1.655-1.655h0a1.6 1.6 0 0 1 1.407.827m15.748 1.903a1.66 1.66 0 0 1-1.655 1.655h0a1.66 1.66 0 0 1-1.655-1.655v-1.075a1.66 1.66 0 0 1 1.655-1.655h0a1.66 1.66 0 0 1 1.655 1.655m0 2.73v-4.385M4.5 29h39"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 35.5v-23a4 4 0 0 0-4-4h-31a4 4 0 0 0-4 4v23a4 4 0 0 0 4 4h31a4 4 0 0 0 4-4"></svg:path>`,
})
export class ArcticonsUchealthIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUcoMbankingPlusIcon],svg[arcticons-uco-mbanking-plus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="4" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.467 42.5V31.355l-6.392-7.738l7.654-8.113"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.533 42.5H37.5l5-5v-27l-5-5h-27l-5 5v27l5 5h9.967"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.533 42.5V31.355l6.392-7.738l-7.654-8.113"></svg:path>`,
})
export class ArcticonsUcoMbankingPlusIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUdbappProIcon],svg[arcticons-udbapp-pro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.75 44.118c-1.87-.519-1.961-.552-2.22-.822c-.204-.213-.226-.294-.25-.937c-.047-1.181.507-4.565.908-5.548c.197-.483.364-1.057 1.186-4.072c.432-1.582.43-1.57.538-3.845c.074-1.559.123-1.978.367-3.105l.281-1.303l-.01-3.423a747 747 0 0 1-.004-3.79c.005-.342.05-.42.649-1.11c.96-1.105.943-1.07.667-1.412c-.497-.616-.584-.789-.759-1.5c-.14-.578-.181-.983-.186-1.86l-.005-1.122l.78-.4l.781-.4l-.003-1.843c-.004-2.064-.044-2.393-.346-2.829c-.335-.483-.268-.83.218-1.13c.216-.133.402-.167.915-.167h.644l.086.54c.048.298.125 1.082.173 1.743c.102 1.431.244 2.177.57 2.997c.296.74.467 1.3.748 2.443c.334 1.357.394 1.433.801 1.009c.165-.172.305-.24.495-.24c.218 0 .297-.053.454-.302c.267-.423.367-.488.902-.592c.56-.108.673-.195.99-.756a1.73 1.73 0 0 1 1.291-.88c.307-.04.405-.013.735.208c.21.14.613.509.896.819c.475.52.522.605.611 1.103c.14.778.066 1.653-.173 2.082c-.318.57-.529.677-1.295.66c-.465-.01-.711.021-.839.105c-.185.122-.502.665-.502.86c0 .061.18.335.4.61c.498.62.512.823.1 1.526c-.693 1.185-1.052 1.353-3.08 1.437c-1.404.058-1.423.066-1.425.578c0 .167-.105.484-.247.75l-.246.462l-.042 2.934c-.047 3.284.002 3.954.363 4.978c.166.47.32 1.216.496 2.403c.25 1.676.274 1.773.925 3.605c.907 2.552 2.383 6.298 2.558 6.491c.245.271.966.218 1.73-.128c.796-.358 1.14-.356 1.175.01c.058.591-.223.813-2.613 2.059c-1.094.57-1.262.635-1.55.596c-.432-.058-.677-.303-.997-.998c-.22-.478-.3-.58-.454-.58c-.43 0-.358.158-3.33-7.41c-.225-.573-.47-1.128-.545-1.234l-.135-.193l-.185.232c-.275.345-1.108 3.095-1.866 6.162c-.217.88-.484 1.87-.592 2.198l-.196.596l.212.21c.116.114.616.458 1.11.763c1.11.684 1.184.81.616 1.047c-.42.176-.772.131-2.276-.285"></svg:path>`,
})
export class ArcticonsUdbappProIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUdeappIcon],svg[arcticons-udeapp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.655 35.892a3.864 3.864 0 0 1 3.712 3.886a3.71 3.71 0 0 1-3.712 3.712M7.44 8.213a3.712 3.712 0 1 1 7.425 0v0l.01 18.79h0a9.04 9.04 0 0 0 9.186 8.883a9.04 9.04 0 0 0 9.185-8.884h0l-.11-18.79a3.712 3.712 0 1 1 7.425 0V26.94a16.56 16.56 0 0 1-33.12 0z"></svg:path>`,
})
export class ArcticonsUdeappIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUdemyIcon],svg[arcticons-udemy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.858 15.824v18.335A9.305 9.305 0 0 0 24 43.5a9.305 9.305 0 0 0 9.142-9.34V15.823m-18.273-5.607L24 4.5l9.131 5.716"></svg:path>`,
})
export class ArcticonsUdemyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUdiscIcon],svg[arcticons-udisc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.855 9.668v5.128m0 8.812v5.703c-.167 5.218-3.998 8.616-9.022 9.022a9.02 9.02 0 0 1-9.022-9.022m0-6.51V9.667M5.5 29.744l24.56-9.47"></svg:path><svg:ellipse cx="37.535" cy="17.854" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="5.204" ry="1.861" transform="rotate(-18.752 37.535 17.854)"></svg:ellipse>`,
})
export class ArcticonsUdiscIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUdsIcon],svg[arcticons-uds-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 5.5C13.783 5.5 5.5 13.783 5.5 24S13.783 42.5 24 42.5S42.5 34.217 42.5 24v-6H34v6c0 5.523-4.477 10-10 10s-10-4.477-10-10s4.477-10 10-10h6V5.5z"></svg:path><svg:circle cx="38" cy="10.25" r="4.25" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUdsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUefaIcon],svg[arcticons-uefa-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.194 26.663c4.667 1.288 8.148 1.896 8.148 1.896a72 72 0 0 1-5.345-6.011c2.709-3.831 4.843-6.483 4.843-6.483a84 84 0 0 0-8.023 2.214c-2.312-3.32-3.581-5.656-3.581-5.656c-.675 2.492-1 4.987-1.11 7.332c-3.862 1.495-6.275 2.718-6.275 2.718a70 70 0 0 0 6.355 2.47c.34 4.507 1.252 7.635 1.252 7.635a103 103 0 0 1 3.736-6.115M45.5 25.481c-.635.994-1.606 1.703-2.742 2.201a56 56 0 0 0-.643-6.008c-.97 2.619-2 4.943-2.993 6.95c-3.422.439-6.78-.065-6.78-.065c1.841 2.21 3.273 3.52 4.43 4.404c-1.732 2.921-2.986 4.48-2.986 4.48c2.422-.554 4.346-1.474 5.878-2.564c.627.43 1.167.91 1.7 1.723c0 0 .988-1.401 1.326-4.646c2.447-3.2 2.81-6.475 2.81-6.475m-1.117-8.837a15.3 15.3 0 0 0-4.377-5.698c-1.46-3.07-3.143-4.487-3.143-4.487c.142.706.034 1.506-.228 2.331c-3.64-1.803-6.959-2.055-6.959-2.055c2.115 1.387 3.991 3.104 5.603 4.876c-1.485 2.416-3.438 4.454-3.438 4.454c2.589-.583 4.672-.829 6.339-.873c2.558 3.523 3.935 6.482 3.935 6.482c.05-2.382-.188-4.459-.586-6.251c2.166.413 2.853 1.221 2.853 1.221M29.676 6.735a26.7 26.7 0 0 0-5.026-1.931c2-1.692 3.86-2.12 3.86-2.12c-3.21-.128-5.972.452-8.284 1.341c-3.452-.275-5.651.324-5.651.324c.516.389.953 1.085 1.4 1.918c-2.918 1.998-4.4 4.079-4.4 4.079c1.759-.873 3.625-1.535 5.465-2.04c.2.369.41.737.642 1.096a21 21 0 0 0 2.555 3.221c.478-2.274 1.3-4.044 2.245-5.418c4.106-.57 7.194-.47 7.194-.47M7.512 13.738c1.1-1.063 2.44-2.21 4.063-3.392c0 0-1.496 0-3.078.403c1.046-2.534 2.207-3.939 2.207-3.939s-6.662 4.74-7.927 13.255c0 0 .56-1.54 2.458-3.862c.753 1.688 1.62 2.75 1.62 2.75c.012-1.943.275-3.69.657-5.214"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.852 22.673c-2.312.072-4.126.275-5.538.534c-.397-2.475-.458-4.255-.458-4.255c-1.142 1.744-1.881 3.424-2.339 4.99c-1.61.596-2.017 1.173-2.017 1.173c.251.858.754 1.693 1.388 2.476c-.199 3.707 1 6.156 1 6.156c.27-1.3.808-2.564 1.484-3.747a28 28 0 0 0 3.926 2.612c-.92-1.65-1.598-3.497-2.096-5.285c2.211-2.776 4.65-4.654 4.65-4.654m11.44 18.937s-1.94-.775-4.718-2.34c.1-1.669.357-3.795.885-6.491c0 0-2.207 2.184-5.066 3.877a48 48 0 0 1-5.096-4.043s.824 2.547 2.413 5.352c-1.507.564-3.06.825-4.481.478c0 0 1.678 2.095 6.854 2.985c1.436 1.703 3.185 3.11 5.225 3.6c0 0-.615-.676-.751-3.208a44 44 0 0 0 4.735-.21"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.675 41.395c.445-2.048.11-3.952.11-3.952c-1.31 1.883-2.596 3.317-3.788 4.408c-3.205.166-5.706-.242-5.706-.242s1.43 1.429 3.08 2.253c-2.178 1.347-3.637 1.467-3.637 1.467c9.534.202 15.256-6.022 15.256-6.022c-1.607 1.06-3.468 1.708-5.315 2.088"></svg:path>`,
})
export class ArcticonsUefaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUefaGamingIcon],svg[arcticons-uefa-gaming-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="m8.06 20.807l-1.144-9.344L24.08 5.968l17.01 5.637l-1.114 9.312M10.056 31.46l.406 2.213l13.613 8.36l13.573-8.337l.296-2.168"></svg:path><svg:path d="M14.375 13.69v4.538a2.248 2.248 0 0 0 4.495 0V13.69m1.609 3.395h2.212m1.181 3.393h-3.393v-6.785h3.393m1.612 3.392h2.205m-2.205 3.393v-6.785h3.393m5.283 4.515h-2.94m-.733 2.249l2.205-6.764l2.206 6.785m-6.273 2.199v7.088m-9.339-.008v-7.08l3.545 7.088l3.544-7.078v7.078m4.492 0v-7.088l4.696 7.088v-7.088m-19.289 4.717h-3.071m-.765 2.35l2.303-7.067l2.304 7.088m-6.849-4.74a2.348 2.348 0 1 0-4.696 0v2.392a2.348 2.348 0 1 0 4.696 0H7.848M42.5 25.025a2.35 2.35 0 0 0-2.348-2.348h0a2.35 2.35 0 0 0-2.348 2.348v2.392a2.35 2.35 0 0 0 2.348 2.348h0a2.35 2.35 0 0 0 2.348-2.348h-2.348"></svg:path></svg:g>`,
})
export class ArcticonsUefaGamingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUfficiopostaleIcon],svg[arcticons-ufficiopostale-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.346 32V16h5.342c3.082 0 5.547 2.4 5.547 5.4s-2.465 5.4-5.547 5.4h-5.342M24.765 16h10.889m-5.342 15.999V16"></svg:path>`,
})
export class ArcticonsUfficiopostaleIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUhcIcon],svg[arcticons-uhc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.878 14.446L35.122 4.5v27.878c0 6.143-4.98 11.122-11.122 11.122h0c-6.142 0-11.122-4.98-11.122-11.122z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.469 9.71v21.048a5.295 5.295 0 0 1-5.296 5.295h0a5.295 5.295 0 0 1-5.295-5.295"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.353 7.974V31.02a7.24 7.24 0 0 1-7.237 7.238h0a7.24 7.24 0 0 1-7.238-7.238"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.238 6.237v25.732a9.18 9.18 0 0 1-9.18 9.18h0a9.18 9.18 0 0 1-9.18-9.18"></svg:path>`,
})
export class ArcticonsUhcIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUjjivanIcon],svg[arcticons-ujjivan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.36 4.707c2.075-.005 5.913-.005 8.067-.005s2.627 0 3.76.109s2.927.325 4.512 1.424c1.586 1.1 2.963 3.082 3.663 4.518s.722 2.326.733 2.847c.011.522.011.675.033.751c.021.077.064.077.1.077s.064 0 .114-.545s.123-1.636.47-2.947c.346-1.312.965-2.844 1.886-3.953s2.144-1.797 3.075-2.14s1.569-.343 2.43-.343h2.545c.601 0 .72 0 .734.048s-.074.144-.204.275s-.302.297-.696.812s-1.01 1.38-1.302 2.428s-.264 2.28-.249 5.828s.015 9.412.015 12.555s0 3.56-.288 5.059c-.287 1.499-.862 4.078-2.866 6.414s-5.438 4.43-8.88 5.216c-3.44.788-6.89.268-9.913-1.256s-5.619-4.052-6.942-6.95C8.834 32.032 8.784 28.767 8.76 24.5s-.025-9.538-.025-12.448s0-3.461-.13-4.009C8.477 7.494 8.22 6.95 7.94 6.47s-.582-.894-.846-1.165c-.263-.272-.489-.4-.556-.472c-.066-.071.026-.086.229-.098s.517-.022 2.593-.027z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.624 41.629c1.012-1.5 2.025-2.999 3.846-4.586s4.45-3.262 7.604-4.059s6.831-.717 10.51-.636"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.201 40.004c-.031-1.033-.062-2.067.23-4.194c.292-2.128.908-5.349 3.276-8.019c2.369-2.67 6.49-4.79 8.803-5.85s2.82-1.062 3.156-1.062s.504 0 .588.054s.084.163.108.992s.074 2.38-.091 4.037s-.544 3.422-.946 4.61s-.827 1.8-1.251 2.412"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.764 25.2c.459-1.49.918-2.982 1.801-4.674s2.19-3.586 2.93-4.653s.911-1.306 1.047-1.373c.137-.067.237.04.754.7s1.448 1.876 2.275 3.25s1.546 2.905 1.987 4.102c.442 1.196.604 2.057.767 2.917"></svg:path>`,
})
export class ArcticonsUjjivanIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkgProClassicIcon],svg[arcticons-ukg-pro-classic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="4.3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="11.838" cy="36.162" r="4.3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="36.162" cy="36.162" r="4.3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="12.9" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUkgProClassicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkgWorkforceCentralIcon],svg[arcticons-ukg-workforce-central-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="3.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.812 5.505C6.919 5.604 5.5 7.303 5.5 9.198V40.5a2 2 0 0 0 2 2h31.302c1.895 0 3.594-1.42 3.693-3.312A3.5 3.5 0 0 0 39 35.5H12.5V9a3.5 3.5 0 0 0-3.688-3.495"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9 5.5a3.5 3.5 0 0 0-3.495 3.688c.099 1.893 1.798 3.312 3.693 3.312H35.5v26.302c0 1.895 1.42 3.594 3.312 3.693A3.5 3.5 0 0 0 42.5 39V7.5a2 2 0 0 0-2-2z"></svg:path>`,
})
export class ArcticonsUkgWorkforceCentralIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUklonIcon],svg[arcticons-uklon-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.696 14.778c0-5.735 32.5-12.427 35.369-7.647c3.823 5.735-1.912 35.369-7.648 35.369c-6.691 0-27.722-20.074-27.722-27.722z"></svg:path>`,
})
export class ArcticonsUklonIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkraineAlarmIcon],svg[arcticons-ukraine-alarm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.173 20.124h4.878v7.752h-4.878z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.051 27.876c7.697 0 12.533 3.364 17.748 8.075V12.05c-5.215 4.71-10.051 8.075-17.748 8.075M33.8 27.028a3.028 3.028 0 1 0 0-6.056m-22.627 6.904v13.379"></svg:path>`,
})
export class ArcticonsUkraineAlarmIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkraineTryzubIcon],svg[arcticons-ukraine-tryzub-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 35.03H12.955V9.132c4.982 2.98 5.173 10.99 5.713 15.197a2.86 2.86 0 0 0-2.455 2.833c0 1.58 1.03 2.672 2.86 3.02c2.599.492 4.927.746 4.927 4.846m-7.787-7.865h-3.258"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 43.5c0-1.764 4.418-3.29 4.418-9.25c0-4.624-4.418-8.605-4.418-13.778C24 12.915 26.174 7.79 24 4.5c-2.174 3.29 0 8.415 0 15.972c0 5.173-4.418 9.154-4.418 13.779c0 5.96 4.418 7.485 4.418 9.249"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 35.03h11.045V9.132c-4.982 2.98-5.173 10.99-5.713 15.197a2.86 2.86 0 0 1 2.455 2.833c0 1.58-1.03 2.672-2.86 3.02c-2.599.492-4.927.746-4.927 4.846m7.787-7.865h3.258M24 35.029V43.5"></svg:path>`,
})
export class ArcticonsUkraineTryzubIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkrposhtaIcon],svg[arcticons-ukrposhta-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.304 24.44c10.34-2.145 7.439-14.554-.177-14.554c-7.854 0-10.674 11.23-1.822 14.083l13.23 3.11L24.95 43.5s-9.611-13.065-12.436-17.67C6.804 16.52 13.748 4.545 24.961 4.5c12.567 0 17.54 14 8.383 21.83"></svg:path>`,
})
export class ArcticonsUkrposhtaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkvcasIdvIcon],svg[arcticons-ukvcas-idv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 16.2V8.5h7.7m23.6 0h7.7v7.7M12.2 39.5H4.5v-7.7m39 0v7.7h-7.7M12.2 28.8H15m-2.8-9.6H15m-1.4 0v9.6m6.3 0v-9.6h1.6c2.6 0 4.8 2.2 4.8 4.8h0c0 2.6-2.2 4.8-4.8 4.8zm15.9-9.6l-3.1 9.6l-3.2-9.6"></svg:path>`,
})
export class ArcticonsUkvcasIdvIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUkwRadioIcon],svg[arcticons-ukw-radio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 2.5A21.5 21.5 0 0 1 45.5 24v16.5a5 5 0 0 1-5 5H24A21.5 21.5 0 0 1 2.5 24v0A21.5 21.5 0 0 1 24 2.5"></svg:path><svg:circle cx="24" cy="24" r="15.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.703 21.492h30.606m-26.475-8.238h22.332m-3.82 8.238v-8.238"></svg:path><svg:circle cx="39.32" cy="39.32" r="3.456" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="36.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="25.714" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="22.286" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.141" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="32.569" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="18.859" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="15.431" cy="33.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="27.428" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="24" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="30.855" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="34.283" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="20.572" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="17.145" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="13.717" cy="30.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="18.859" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="27.428" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="24" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="30.855" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="34.283" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="20.572" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="17.145" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="13.717" cy="24.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="20.572" cy="36.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="25.714" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="22.286" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.141" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="32.569" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="35.996" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="15.431" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="12.004" cy="27.507" r=".75" fill="currentColor"></svg:circle><svg:circle cx="27.428" cy="36.507" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsUkwRadioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUlIcon],svg[arcticons-ul-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.088 41.057c-8.987 6.896-21.784 5.636-29.253-2.881s-7.049-21.369.962-29.38c8.01-8.01 20.862-8.43 29.379-.96c8.517 7.468 9.777 20.265 2.881 29.252"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.007 15.19v17.62h8.615M10.378 15.19v11.784c0 3.224 2.555 5.837 5.707 5.837s5.707-2.613 5.707-5.837V15.19"></svg:path><svg:circle cx="39.203" cy="39.203" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsUlIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUllasIcon],svg[arcticons-ullas-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.382 22.666c.61 1.545 1.212 3.082 2.041 5.286c.83 2.196 1.87 5.058 2.033 5.855s-.553-.463-1.04-1.065c-.489-.602-.757-.537-1.066-.52c-.31.007-.667-.025-.96.024c-.3.057-.544.203-.683.496s-.17.732-.195.927c-.024.203-.049.162.3.602c.359.447 1.09 1.366 1.7 2.049c.619.675 1.106 1.106 1.806 1.691c.707.594 1.61 1.35 2.301 1.7c.683.357 1.155.325 1.96.048c.805-.268 1.952-.772 2.7-1.39c.748-.61 1.106-1.334 1.48-2.285c.374-.96.764-2.147.634-3.383c-.138-1.228-.797-2.505-1.35-3.903c-.561-1.407-1.016-2.928-1.488-3.513s-.943-.22-1.22 0s-.341.3-.406.536c-.065.228-.13.61-.293.643c-.163.04-.423-.277-.716-.415c-.292-.138-.618-.098-.813.138s-.268.667-.423.756c-.162.082-.406-.178-.618-.341a1.05 1.05 0 0 0-.602-.244c-.22-.024-.487-.008-.56.35c-.074.366.048 1.073-.204.528s-.894-2.35-1.269-3.52c-.382-1.164-.504-1.684-.723-2.066a1.64 1.64 0 0 0-.87-.748c-.334-.139-.692-.155-.92-.066c-.227.09-.325.285-.39.61c-.073.326-.105.773-.146 1.22" opacity=".992"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.733 22.162c1.13-.56 2.39-.333 2.805.504c.423.846-.162 1.976-1.293 2.546c-1.13.56-2.39.333-2.805-.504c-.423-.846.162-1.976 1.293-2.546m-.147-4.098c.968-.643 2.155-.545 2.651.211s.114 1.879-.845 2.521c-.968.634-2.155.537-2.651-.22c-.496-.756-.114-1.878.845-2.512m-9.668-2.367c-.643-.292-1.293-.577-2.033.537c-.74 1.106-1.586 3.627-2.033 5.855s-.496 4.172-.22 5.896c.277 1.724.886 3.236 1.626 4.22c.732.992 1.602 1.464 2.375 1.675c.78.22 1.472.18 2.18-.049s1.422-.65 1.992-1.04c.569-.39.984-.749 1.398-1.115" opacity=".992"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.489 13.038c1.968-.528 2.44 2.66-.366 4.611l-.04-11.54H18.917v23.974h7.318M9.566 18.365L7.533 34.344l8.538-2.131m22.363-14.385l2.033 16.516l-4.066.528m0 3.196l2.44-1.065l3.659 2.13l-3.643 2.757l-2.61-.862l-30.406.586l-.341-5.14l19.517-.536" opacity=".992"></svg:path>`,
})
export class ArcticonsUllasIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUloggerIcon],svg[arcticons-ulogger-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 4.5a13.09 13.09 0 0 0-13.09 13.09c0 10.25 10 22.61 12.61 25.63a.8.8 0 0 0 1.21 0c2.55-3 12.36-15.38 12.36-25.63A13.09 13.09 0 0 0 24 4.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.52 11.65V19A4.48 4.48 0 0 0 24 23.52h0A4.48 4.48 0 0 0 28.48 19v-7.35m0 7.39v4.48m-8.96-4.48v10.22"></svg:path>`,
})
export class ArcticonsUloggerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUltimateSoftwareIcon],svg[arcticons-ultimate-software-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.1 16.2h7.8V24h-7.8zm0 15.6h7.8v7.8h-7.8zm7.8-23.4h7.8v7.8h-7.8zm7.8 7.8h7.8V24h-7.8zM12.3 8.4h7.8v7.8h-7.8zM27.9 24h7.8v7.8h-7.8zm-15.6 0h7.8v7.8h-7.8zm-7.8-7.8h7.8V24H4.5z"></svg:path>`,
})
export class ArcticonsUltimateSoftwareIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUltraAlarmIcon],svg[arcticons-ultra-alarm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:circle cx="24" cy="28.648" r="1.268"></svg:circle><svg:path d="m23.092 29.528l-3.388 3.492M24 21.361v6.02"></svg:path><svg:ellipse cx="24" cy="28.66" rx="13.813" ry="13.784"></svg:ellipse><svg:path d="m31.912 39.924l2.89 3.576m-18.779-3.623L13.095 43.5m7.346-35.97h7.118a.45.45 0 0 1 .451.45v1.633a.45.45 0 0 1-.451.45H20.44a.45.45 0 0 1-.451-.45V7.98c0-.25.201-.45.451-.45M24 10.064v4.815m4.845.876l2.296-3.341a.9.9 0 0 1 1.255-.234h0l2.281 1.56a.9.9 0 0 1 .234 1.253l-2.097 3.053m-13.64-2.263l-2.315-3.37a.9.9 0 0 0-1.255-.233l-2.281 1.56a.9.9 0 0 0-.234 1.253l2.108 3.069"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.105 4.584c4.42-.608 7.842 2.154 8.58 5.865m-7.859-2.714c2.369-.304 4.202 1.078 4.598 2.935M14.895 4.584c-4.42-.608-7.842 2.154-8.58 5.865m7.859-2.714c-2.369-.304-4.202 1.078-4.598 2.935"></svg:path>`,
})
export class ArcticonsUltraAlarmIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUltrasonicIcon],svg[arcticons-ultrasonic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.504 28.487a3.817 3.817 0 1 1 7.633 0m-.001 7.268a3.817 3.817 0 0 1-7.632 0m0 0v-7.268m7.632 0v7.268m-7.632-7.268v-5.213m-.005.005c0-9.376 8.282-16.977 18.5-16.977s18.5 7.6 18.5 16.977m-.001 5.213a3.817 3.817 0 1 0-7.633 0m.001 7.268a3.817 3.817 0 1 0 7.633 0m-.001 0v-7.268m-7.632 0v7.268m7.632-7.268V23.28"></svg:path>`,
})
export class ArcticonsUltrasonicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUlysIcon],svg[arcticons-ulys-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.67 20v8h4m5.384-8l-2.6 4l-2.6-4m2.6 8v-4m4.576 3.1c.5.6 1.1.9 2 .9h1.2c1.1 0 2-.9 2-2h0c0-1.1-.9-2-2-2h-1.3c-1.1 0-2-.9-2-2h0c0-1.1.9-2 2-2h1.2c.9 0 1.5.2 2 .9M11.77 20v5.3c0 1.5 1.2 2.7 2.6 2.7s2.7-1.2 2.7-2.7V20"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsUlysIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmaMusumePrettyDerbyIcon],svg[arcticons-uma-musume-pretty-derby-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.65 17.791C36.84 9.317 29.626 7.905 28.229 7.487s-4.872-.547-6.581.09c-5.243 2.012-8.277 7.947-8.277 7.947c-1.21.642-3.888 4.743-3.44 9.445c-.457-1.038-1.604-1.661-1.604-1.661"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.116 14.225c.99 2.402-.106 13.083-2.12 17.875c-2.016 4.792-4.892 6.994-4.892 6.994M3.865 16.461c2.444 1.78 2.115 5.484 4.462 6.848c.54 4.497-1.747 6.457-4.968 6.69"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.169 32.29s3.573-.484 4.768-1.908c1.996-2.377.994-5.413.994-5.413s1.036-.564 3.416.463c1.32 1.642 1.222 4.25 1.064 5.578c-.17 1.425-3.5 7.54-6.67 7.026m13.345-2.506c3.042-.045 8.506-2.54 8.506-2.54m-13.8-3.024s3.562 4.614 5.294 5.564m4.099-5.285c-.538-1.175-3.174-2.08-5.392-1.223c-.383 2.1-.068 4.438 1.825 4.739s3.416-1.416 3.567-3.516m-3.92-8.397c-2.559-3.661-4.891.289-5.418 1.34l.474.858m10.717-.857c3.914-1.861 4.577 1.543 4.31 3.388l-.929 1.032M21.648 7.577s.3-2.77-.368-3.227c-.668-.458-2.747 1.811-3.848 3.663c-.98.922-3.12-.267-4.501 1.22c.378 1.055 2.634 3.054 2.634 3.054m7.777 13.591l-.198.598"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.683 27.723c-.383 4.198 2.867 8.83 3.474 9.126c-1.764-.575-3.868-2.845-4.768-5.69M32.554 9.127c6.604 6.504 9.512 26.218-7.45 29.967"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.864 38.015c4.74-2.406 6.516-8.087 5.076-15.741m-4.658-11.257c.682-1.437 2.751-1.625 2.751-1.625l1.13-.582m2.303 2.677c-.98 2.215-2.555 3.438-3.147 3.283m-22.61 19.111c.273 4.656-1.78 9.09-1.78 9.09m5.228-6.122l.538 2.802l5.314-4.905"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.52 43.872c-.01-.809.6-2.797 3.175-4.222c-1.212 2.275-.827 4.291-.827 4.291m.827-4.29c2.927 8.783 10.207 1.736 0 0m6.235-5.197c1.044.766 1.08 2.668 1.08 2.668m.207 1.946c.34 1.713-.023 6.194-.023 6.194m.023-6.194c2.5.004 3.212 1.886 3.019 2.777c-.218 1.004.678 2.493.678 2.493m-.162-7.346c1.526.739-.406 3.044 3.29 6.02m2.914-26.16c3.582-.05 4.204 3.497 3.853 4.473c-.352.975-2.148 1.81-3.055.669M14.984 8.418c-1.498-6.482 3.612-4.255 4.408-3.01m9.007 12.042c.026 1.252-.055 2.507-.314 3.639c3.63-3.817 2.396-11.496.144-13.602c-4.701.653-9.888 7.91-11.388 12.582c1.16-1.988 6.825-6.142 8.568-8.343c.094 1.292-1.786 7.753-3.416 10.748c3.466-.568 5.022-2.084 6.337-4.851m-14.959-2.098c-1.753-1.36.55-4.792 2.194-3.237m-2.218 13.144c-.546-3.654.927-7.05.927-7.05"></svg:path>`,
})
export class ArcticonsUmaMusumePrettyDerbyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmangIcon],svg[arcticons-umang-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.44 30.23V8.923A4.436 4.436 0 0 1 17.863 4.5h12.274a4.436 4.436 0 0 1 4.423 4.423v16.323"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.206 31.881v-6.006c0-1.045.847-1.893 1.892-1.893h0c1.045 0 1.892.847 1.892 1.893v6.224m0 0v-4.46c0-1.045.848-1.892 1.893-1.892h0c1.045 0 1.892.847 1.892 1.892v4.622"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.775 32.26v-1.946c0-1.045.847-1.892 1.893-1.892h0c1.045 0 1.892.847 1.892 1.892v8.303c0 3.294-4.61 4.883-9.781 4.883c-7.071 0-11.339-3.208-11.339-10.124c0-.832.358-1.716 1.495-1.716c2.12 0 .796 5.61 3.132 5.61c1.665 0 1.354-2.52 1.354-4.422V21.714c0-1.045.847-1.892 1.892-1.892h0c1.045 0 1.893.847 1.893 1.892v10.168"></svg:path><svg:circle cx="21.313" cy="15.88" r="2.315" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.989 8.55h6.624"></svg:path>`,
})
export class ArcticonsUmangIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmassfiveIcon],svg[arcticons-umassfive-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m43.5 20.898l-12.664-2.826l-3.48-11.856l-6.963 9.485l-15.893.353l12.966 9.435L5.913 38.656l16.296-7.063l9.132 10.191l-1.917-15.791z"></svg:path>`,
})
export class ArcticonsUmassfiveIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmathIcon],svg[arcticons-umath-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.694 17.182a1.013 1.013 0 0 0-.37-1.506c.583-.198.815-2.248 0-3.014c.755-.748.411-3.738-.469-4.322a9 9 0 0 0-.42-2.717l-.89.345l-.79-1.235l-.84 1.532l-1.366-.025l-.51 2.297l-1.112-.05l-.643 2.002l-1.235-.717v1.186l-1.333-.716v1.16l-1.137.124v1.729l-.592.173l.963 2.272l-.963.346l.988 1.26h-.988l-.717.617"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.27 17.923a1.013 1.013 0 0 0-1.55-.074c-.023-.615-1.922-1.423-2.888-.86c-.502-.937-3.466-1.46-4.277-.784a9 9 0 0 0-2.724-.372l.078.95l-1.409.406l1.228 1.242l-.413 1.303l2.056 1.144l-.365 1.052l1.735 1.186l-1.04.98l1.137.338l-1.067 1.074l1.113.331l-.206 1.124l1.657.493l-.003.618l2.453-.275l.056 1.022l1.49-.588l-.282.947l.387.863m0 0a1.013 1.013 0 0 0-.442 1.487c-.602-.124-1.839 1.528-1.523 2.6c-1.028.264-2.243 3.018-1.78 3.967a9 9 0 0 0-1.01 2.557l.942.15l.058 1.466l1.499-.897l1.167.712l1.601-1.724l.934.604l1.566-1.402l.703 1.243l.6-1.023l.788 1.292l.587-1.002l1.043.468l.874-1.492l.599.15l.317-2.448l1.006.188l-.216-1.586l.852.5l.93-.172m.001.001a1.013 1.013 0 0 0 1.3.848c-.292.542.94 2.199 2.057 2.201c-.04 1.062 2.252 3.012 3.295 2.838c.226.532 2.162 1.698 2.162 1.698l.413-.86l1.421.362l-.432-1.692l1.015-.915l-1.196-2.027l.846-.723l-.897-1.9l1.391-.32l-.81-.866l1.464-.388l-.793-.848l.746-.866l-1.181-1.263l.315-.531l-2.256-1.002l.71-1.036m.001 0a1.013 1.013 0 0 0 1.253-.916c.404.464 2.389-.1 2.787-1.145c.979.413 3.614-1.041 3.82-2.078a9 9 0 0 0 2.353-1.421l-.658-.691l.842-1.201l-1.736-.195l-.497-1.273l-2.318.401l-.377-1.047l-2.095.167l.193-1.415l-1.096.451l.155-1.506l-1.074.441l-.546-1.004l-1.599.658l-.385-.483l-1.736 1.755l-.686-.76l-.79 1.393l-.376-.913l-.843-.428"></svg:path>`,
})
export class ArcticonsUmathIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmbrellaIcon],svg[arcticons-umbrella-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.806 40.838a3.097 3.097 0 0 0 6.194 0V23.565m-19.5 0a19.5 19.5 0 0 1 39 0a4.875 4.875 0 1 0-9.75 0a4.875 4.875 0 1 0-9.75 0a4.875 4.875 0 1 0-9.75 0a4.875 4.875 0 1 0-9.75 0"></svg:path>`,
})
export class ArcticonsUmbrellaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmlClassEditorIcon],svg[arcticons-uml-class-editor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 9.2h11.1m0 12.955V5.5H5.5v16.645zM5.5 15.678h11.1m-5.545 13.867h11.1m0 12.955V25.845h-11.1l-.01 16.655zm-11.1-6.478h11.1m8.323-16.644H42.5m0 12.944V15.678H30.478v16.644zm-12.022-6.477H42.5m-25.9-6.928q6.478.771 13.878 3.7m-8.323 10.167q3.7-2.617 8.323-3.7"></svg:path>`,
})
export class ArcticonsUmlClassEditorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmoneyIcon],svg[arcticons-umoney-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.795 22.571c.45-2.426 2.392-4.392 4.346-4.392h0c1.947 0 3.164 1.966 2.714 4.392l-1.346 7.25m-12.774-7.25c.453-2.426 2.396-4.392 4.346-4.392h0c1.947 0 3.164 1.966 2.714 4.392l-1.342 7.25M14.491 18.18l-1.346 7.249c-.45 2.422.767 4.391 2.718 4.391h0c1.95 0 3.893-1.965 4.346-4.391l1.342-7.25"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsUmoneyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUmoneyAgentIcon],svg[arcticons-umoney-agent-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.269 38.85h-3.541m-.881 2.649l2.649-8.001l2.654 8.001"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M44.496 37.5a7 7 0 0 1-7 7h0a7 7 0 0 1-7-7h0a7 7 0 0 1 7-7h0a7 7 0 0 1 7.004 6.997v.003zM27.797 22.575c.45-2.426 2.392-4.395 4.346-4.395h0c1.947 0 3.164 1.965 2.714 4.395l-1.35 7.246m-12.774-7.246c.453-2.426 2.396-4.395 4.346-4.395h0c1.947 0 3.164 1.965 2.714 4.395l-1.342 7.246"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.489 18.18l-1.346 7.249c-.45 2.426.767 4.391 2.718 4.391h0c1.95 0 3.893-1.965 4.346-4.391l1.342-7.25"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.61 42.5H9.5c-2.21 0-4-1.79-4-4v-29c0-2.21 1.79-4 4-4h29c2.21 0 4 1.79 4 4v23.11"></svg:path>`,
})
export class ArcticonsUmoneyAgentIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnabIcon],svg[arcticons-unab-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:ellipse cx="24" cy="23.178" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="16.871" ry="18.678"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.444 36.78l-4.02 2.97l-.018 3.75m27.15-6.72l4.02 2.97l.018 3.75M14.409 17.953h4.737v10.845h-4.737zm14.444 0h4.737v10.845h-4.737zm-2.483 0l-2.354-2.385l-2.385 2.385V28.8h4.738Z"></svg:path>`,
})
export class ArcticonsUnabIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnacademyIcon],svg[arcticons-unacademy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.916 7.534h38.168m-38.584 0a19.5 19.5 0 0 0 39 0Zm28.914 32.932a9.414 9.414 0 0 0-18.828 0Z"></svg:path>`,
})
export class ArcticonsUnacademyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnchainedIcon],svg[arcticons-unchained-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.058 28.713a12.439 12.439 0 1 1-15.69-12.006M20.01 18a12.439 12.439 0 1 1 16.605 12.982M14.553 11.011l20.943 27.483M14.14 27.543l8.074-6.478"></svg:path><svg:circle cx="12.117" cy="29.003" r="2.443" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUnchainedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUncivIcon],svg[arcticons-unciv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.13 5.45l-5.75 3.32v6.64l5.75 3.32l5.75-3.32V8.77Zm13.74 0l-5.75 3.32v6.64l5.75 3.32l5.76-3.32V8.77ZM10.25 17.36L4.5 20.68v6.64l5.75 3.32L16 27.32v-6.64Zm13.75 0l-5.75 3.32v6.64L24 30.64l5.75-3.32v-6.64Zm13.75 0L32 20.68v6.64l5.75 3.32l5.75-3.32v-6.64Zm-20.62 11.9l-5.75 3.33v6.64l5.75 3.32l5.75-3.32v-6.64Zm13.74 0l-5.75 3.33v6.64l5.75 3.32l5.76-3.32v-6.64Z"></svg:path>`,
})
export class ArcticonsUncivIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnderArmourIcon],svg[arcticons-under-armour-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.995 22.15c-2.676 0-5.225.278-7.546.78c-1.175-1.21-1.878-2.705-1.878-4.332v-6.456c-4.156.806-7.665 2.142-10.071 3.817c.09 3.326 3.33 6.25 8.243 8.04c-4.918 1.795-8.16 4.725-8.243 8.053c2.406 1.664 5.915 3 10.07 3.806v-6.456c0-1.627.705-3.123 1.88-4.333c2.323.502 4.876.78 7.555.78c2.676 0 5.225-.277 7.545-.779c1.175 1.21 1.88 2.705 1.88 4.332v6.456c4.155-.806 7.664-2.142 10.07-3.817c-.09-3.326-3.33-6.25-8.243-8.04c4.917-1.795 8.16-4.725 8.243-8.053c-2.406-1.664-5.915-3-10.07-3.805v6.455c0 1.627-.705 3.124-1.88 4.333c-2.323-.502-4.876-.78-7.555-.78Z"></svg:path>`,
})
export class ArcticonsUnderArmourIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnderhandIcon],svg[arcticons-underhand-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m16.36 24.17l-5.21.69c-1.12.15-1.27-.42-1.37-1L7.62 7.88a1 1 0 0 1 .63-1.2L18 4.57c.65-.21 1.12 0 1.34.88l1.89 12.61"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m21.24 18.06l1.17-1.33l.78 1l-.48 2.45l-2.8 4.47l-2.16 1L17 35.5l-.87-.6l-1.21-9Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.91 24.69l4.56-2.57l5.38-.71l-.05 1.78l-3.95 1.43L23 26.9l-1.35.17L20 34.44l1.67-7.37L23 26.9l4.2-1.52l5.8-.2l-1.45 2.53l-2.6.18l-4.06 2.18l-2.18 5.67l2.17-5.66l3.71-1.46l4.31.43l-1.7 2.12h-1.72l-1.89 1.63l-2 5.13l2-5.13l1.89-1.63h1.72l3.52 5.65l5.67 2.33v4.35l-8.76-.22l-13.53-5l-1.25-1.46l.15-1.32M11.16 16c1.28-4.14 4.64-5.06 6.92-1.37c-1.48 2.44-4 3.48-6.92 1.37"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.82 12.58a2.85 2.85 0 0 1 1.8 4.3"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.82 12.58c-1.84 2.45.32 4 1.8 4.3M11 13.08l-.92-.62m2.24-.64l-.88-1.16m2.48.44l-.32-1.35m2.12 1.32l.44-1.2m1.25 1.85l.69-.93m.03 6.21l1.14.84m-2.86.31l.66.94m-2.57-.52V20m-2.28-1.76l-.88 1.08"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.45 14.25a1.16 1.16 0 0 1 .33 1c-.29-.39-.48-.58-.33-1"></svg:path>`,
})
export class ArcticonsUnderhandIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnderlordsIcon],svg[arcticons-underlords-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 7.59s-4.51 3.93-4.84 4.31s-3.32 3.69-4 4.35s-7 6.68-7.24 7s-3.22 3.17-3.22 3.17s.4 1.47.4 1.71s.07 2 .07 2l2.74-3.22S4.72 30.55 4.7 30.8v5.21c0 .55.12 5.31.17 5.61s1.18-.74 1.61-1.11s8.77-9.13 8.89-9.23s1.14-.71 1.37-.9s4.14-4.8 4.33-4.91a28 28 0 0 0 3-2.46c.64-.74 6.1-6.29 6.51-6.6a18 18 0 0 0 1.58-1.49c0-.07-.48-.91-.6-.89s-.65.16-.65.16a9.2 9.2 0 0 0-1.14-1.87a4.8 4.8 0 0 0-1.66-.11l-.47 1.17a16.5 16.5 0 0 0 .81-2.24c0-.29-1.47-.6-1.59-.83s-.66-1.63-.66-1.63h-1l-.2-.97ZM13.58 15c-.07.12-1.56 1.54-1.56 1.54l-6.2 6.2c-.2.2-1.11-1-1.11-1l-.12-7.88l.29-1.32l-.38-1.14l.21-5.11Zm-2.62.28l1.15 1.17M5.64 14.6l-1.01-.97m11.55 19.56l.68.73l-.11.28l2 1.86v.55l2.31 2.22l2.59-2.41l-2.18 2l2.18 2.65s4.82-4.47 5.55-5.32s9.17-9.08 9.72-9.68s4.58-4.26 4.58-4.44s-.15-4.15-.15-4.15v-5.51s-.81.35-.87 0s1.08-1.63 1-1.9s0-3.64-.11-3.81a13.6 13.6 0 0 0-2.45 2.07c-.3.34-6.3 6.53-6.67 6.94s-6.1 5.92-6.88 6.77s-8.37 8.31-8.71 8.72s-2.48 2.43-2.48 2.43m26.06-7.81l1.26 1.23s-.17 9.38-.19 10s-.15 2.74-.12 3s0 2.08 0 2.18a4.3 4.3 0 0 1-1.3-.81c-.17-.19-5.82-5.74-6.08-6s-1.44-1.65-1.44-1.65s5.44-5.44 5.67-5.77s2.2-2.18 2.2-2.18"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42 29.75c.24.28 1.44 1.4 1.44 1.4M41 37.72c.16.24 2 1.66 2.2 1.82m-1.88-20.29c.24-.21 2-1.58 2-1.58m-.98-.81l1-.61"></svg:path>`,
})
export class ArcticonsUnderlordsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUndertaleIcon],svg[arcticons-undertale-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.437 9.853h-2.18V7.677h-2.18V5.5H30.54v2.177h-2.18v2.176h-2.18v4.353h-4.36V9.853h-2.18V7.677h-2.179V5.5h-6.539v2.177h-2.18v2.176H6.564v19.588h4.36v4.353h4.358v4.353h4.36V42.5h8.718v-4.353h4.36v-4.353h4.359v-4.353h4.359z"></svg:path>`,
})
export class ArcticonsUndertaleIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUniBonnAppIcon],svg[arcticons-uni-bonn-app-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.512 5.5c0 5.565-4.802 22.018-25.012 22.018"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4m-33 28.023h37"></svg:path>`,
})
export class ArcticonsUniBonnAppIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnicodepadIcon],svg[arcticons-unicodepad-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.485 33.181a2.058 2.058 0 0 1-2.075-2.513l1.39-20.61c.116-1.715 1.423-3.449 4.518-3.404a3.692 3.692 0 0 1 3.87 4.19l-.36 5.524"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.316 40.677H6.755l1.907-13.888c2.76-20.107 34.085-14.994 32.749 5.185c-8.383 4.177-28.296.523-32.75-5.185"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.316 38.66v2.017h10.083V38.66s.203-4.477-5.042-4.438s-5.041 4.438-5.041 4.438M20.762 14.121l.24-6.004m16.342 11.421l4.331-3.834"></svg:path><svg:circle cx="16.166" cy="21.85" r="1.467" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="29.61" cy="25.884" r="1.467" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m41.435 31.392l-.115 2.733"></svg:path>`,
})
export class ArcticonsUnicodepadIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnicreditIcon],svg[arcticons-unicredit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.869 41.287c.644.891 1.289 1.783 7.252-2.928c5.964-4.71 17.247-15.032 23.243-20.49c5.986-5.46 6.696-6.053 6.532-6.582c-.153-.539-1.17-1.001-2.174-1.463M8.32 38.69A21.38 21.38 0 0 1 2.5 24C2.5 12.13 12.13 2.5 24 2.5c6.13 0 11.66 2.57 15.57 6.69l.018.003c-7.57-.363-15.899-.833-18.203-.106c-2.294.737.666 2.564 3.637 4.39c-.787.859-1.573 1.706-4.358 5.855S13.094 31.26 8.31 38.7z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.54 13.12c1.88 3.19 2.96 6.91 2.96 10.88c0 11.87-9.63 21.5-21.5 21.5c-5.04 0-9.67-1.73-13.33-4.64"></svg:path>`,
})
export class ArcticonsUnicreditIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnieuroIcon],svg[arcticons-unieuro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.178 16.352a6.339 6.339 0 1 0-12.678 0c0 3.5 0 13.091 6.339 13.091s6.339-9.59 6.339-13.091M34.163 5.948a8.337 8.337 0 0 0-8.337 8.337c0 11.506-2.368 15.224-4.814 18.07c-2.613 3.04-5.315 3.186-5.315 6.554c0 2.409 2.251 3.143 7.304 3.143c8.337 0 19.499-7.165 19.499-27.767a8.337 8.337 0 0 0-8.337-8.337"></svg:path>`,
})
export class ArcticonsUnieuroIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnifiNetworkIcon],svg[arcticons-unifi-network-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="23.924" cy="24.094" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="23.992" cy="24.025" r="8.746" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.372 21.933v3.312a1.618 1.618 0 1 0 3.233 0v-3.312"></svg:path>`,
})
export class ArcticonsUnifiNetworkIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnifiProtectIcon],svg[arcticons-unifi-protect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="13.123" height="14.966" x="17.439" y="7.067" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="6.561"></svg:rect><svg:rect width="17.624" height="20.1" x="15.188" y="4.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="8.812"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.747 15.788v24.797a2.406 2.406 0 0 1-2.023 2.375h0a42.5 42.5 0 0 1-13.512 0h0a2.406 2.406 0 0 1-2.024-2.375V15.788"></svg:path>`,
})
export class ArcticonsUnifiProtectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnifiedRemoteIcon],svg[arcticons-unified-remote-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:defs><svg:path id="arcticonsUnifiedRemote0" fill="none" stroke="currentColor" d="M11 5.5A31.5 31.5 0 0 1 42.5 37"></svg:path></svg:defs><svg:use href="#arcticonsUnifiedRemote0" stroke-linecap="round" stroke-linejoin="round"></svg:use><svg:use href="#arcticonsUnifiedRemote0" stroke-linecap="round" stroke-linejoin="round"></svg:use><svg:circle cx="11" cy="37" r="7.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.392 5.835A31.527 31.527 0 0 1 42.5 37a32 32 0 0 1-.335 4.608"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.713 42.5A19.516 19.516 0 0 0 5.5 18.287"></svg:path>`,
})
export class ArcticonsUnifiedRemoteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnifiednlpIcon],svg[arcticons-unifiednlp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.9 3.5a1 1 0 0 0-1 .86l-.77 5.43a15.4 15.4 0 0 0-3.46 2L9.54 9.75a1 1 0 0 0-1.25.44l-4.1 7.11a1 1 0 0 0 .25 1.31L8.76 22a19 19 0 0 0-.14 2a17 17 0 0 0 .14 2l-4.32 3.39a1 1 0 0 0-.25 1.31l4.1 7.11a1 1 0 0 0 1.25.44l5.11-2.06a15.7 15.7 0 0 0 3.46 2l.77 5.43a1 1 0 0 0 1 .86h8.2a1 1 0 0 0 1-.86l.77-5.43a15.4 15.4 0 0 0 3.46-2l5.11 2.06a1 1 0 0 0 1.25-.44l4.1-7.11a1 1 0 0 0-.25-1.31L39.23 26a17 17 0 0 0 .14-2a17 17 0 0 0-.14-2l4.33-3.39a1 1 0 0 0 .25-1.31l-4.1-7.11a1 1 0 0 0-1.25-.44l-5.11 2.06a15.7 15.7 0 0 0-3.46-2l-.77-5.43a1 1 0 0 0-1-.86Zm4.1 9.92a8 8 0 0 1 8 8c0 4.68-3.8 8.33-8 13.12c-4.06-4.58-7.72-8.13-8-12.53v0a5 5 0 0 1 0-.55a8 8 0 0 1 8-8Zm0 4.18a3.87 3.87 0 1 0 3.87 3.86A3.86 3.86 0 0 0 24 17.6"></svg:path>`,
})
export class ArcticonsUnifiednlpIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnimainzIcon],svg[arcticons-unimainz-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.51 18.28h5.72m0 0v8.58a2.87 2.87 0 0 1-2.86 2.86h0a2.86 2.86 0 0 1-2.86-2.86v-.95m16.61-3.78a3.83 3.83 0 0 0-4-3.83a4 4 0 0 0-3.64 4.05v3.56a3.84 3.84 0 0 0 3.84 3.84h0a3.84 3.84 0 0 0 3.84-3.84h-3.88m10.53-7.61v7.61a3.84 3.84 0 0 0 3.84 3.84h0a3.84 3.84 0 0 0 3.84-3.84V18.3M28.4 36.36V11.64"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUnimainzIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnimoteIcon],svg[arcticons-unimote-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M5.5 26.217c0 2.87 2.31 5.18 5.18 5.18c2.44 0 3.309.529 4.352 1.571c1.042 1.043 1.572 1.911 1.572 4.353a5.17 5.17 0 0 0 5.179 5.179h4.434c2.87 0 5.18-2.31 5.18-5.18c0-2.44.529-3.309 1.572-4.351s1.91-1.573 4.352-1.573c2.87 0 5.18-2.31 5.18-5.179v-4.434c0-2.87-2.31-5.18-5.18-5.18c-2.442 0-3.31-.529-4.352-1.572c-1.043-1.042-1.573-1.91-1.573-4.352c0-2.87-2.31-5.18-5.179-5.18h-4.434c-2.87 0-5.18 2.31-5.18 5.18c0 2.442-.529 3.31-1.571 4.352c-1.043 1.043-1.911 1.573-4.353 1.573A5.17 5.17 0 0 0 5.5 21.783z"></svg:path><svg:path d="M23.1 8.351h1.632a3.445 3.445 0 0 1 3.453 3.453v1.633a3.445 3.445 0 0 1-3.453 3.453H23.1a3.445 3.445 0 0 1-3.453-3.453v-1.633A3.445 3.445 0 0 1 23.1 8.35"></svg:path><svg:path d="m22.998 13.184l.918-1.573l.917 1.573M23.1 31.279h1.632a3.445 3.445 0 0 1 3.453 3.453v1.633a3.445 3.445 0 0 1-3.453 3.452H23.1a3.445 3.445 0 0 1-3.453-3.452v-1.633a3.445 3.445 0 0 1 3.453-3.453"></svg:path><svg:path d="m22.998 34.985l.918 1.572l.917-1.572m14.816-11.717V24.9a3.445 3.445 0 0 1-3.453 3.453h-1.633a3.445 3.445 0 0 1-3.452-3.453v-1.633a3.445 3.445 0 0 1 3.452-3.453h1.633a3.445 3.445 0 0 1 3.453 3.453"></svg:path><svg:path d="m34.816 23.167l1.573.917l-1.573.918m-18.095-1.734V24.9a3.445 3.445 0 0 1-3.453 3.453h-1.633A3.445 3.445 0 0 1 8.183 24.9v-1.633a3.445 3.445 0 0 1 3.452-3.453h1.633a3.445 3.445 0 0 1 3.453 3.453"></svg:path><svg:path d="m13.015 23.167l-1.572.917l1.572.918m10.226-5.908h1.35a4.307 4.307 0 0 1 4.315 4.316v1.349a4.307 4.307 0 0 1-4.316 4.316h-1.349a4.307 4.307 0 0 1-4.316-4.316v-1.35a4.307 4.307 0 0 1 4.316-4.315"></svg:path><svg:path d="M22.318 25.692c-.54 0-.938-.482-.938-1.045v-1.085c0-.603.433-1.086.938-1.086h0c.541 0 .974.483.974 1.086v1.045c0 .603-.433 1.085-.974 1.085m2.641-3.216v3.216m0-1.125l1.515-2.09m0 3.215l-1.154-1.608"></svg:path></svg:g>`,
})
export class ArcticonsUnimoteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUninstallerIcon],svg[arcticons-uninstaller-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.37 21.48v18.377m-7.33-18.376v18.377M16.71 21.48v18.377m14.208-27.969V7.91c.001-2.812.08-3.41-2.732-3.41h-8.373c-2.812 0-2.732.598-2.732 3.41v3.98m20.295 5.844H7.354v-5.728h33.293v5.728zv23.25c0 1.394-.75 2.517-1.68 2.517H12.423c-.93 0-1.678-1.123-1.678-2.518v-23.25"></svg:path>`,
})
export class ArcticonsUninstallerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnionbankIcon],svg[arcticons-unionbank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 9.5v29a4 4 0 0 1-4 4h-29a4 4 0 0 1-4-4v-29a4 4 0 0 1 4-4h29a4 4 0 0 1 4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.5 12.994v15.299c0 4.185 3.341 7.148 7.133 6.661c3.47-.446 6.074-3.67 6.074-7.234V14.69m0 .001l6.41.824c1.91.246 3.383 2.334 3.383 4.676s-1.473 4.241-3.383 4.241h0c1.91 0 3.383 1.899 3.383 4.241s-1.473 4.43-3.383 4.676l-4.8.617m4.8-9.534h-6.41"></svg:path>`,
})
export class ArcticonsUnionbankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnionpayAppIcon],svg[arcticons-unionpay-app-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M9.12 20.687h7.813m-8.321 3h8.829m-1.289 2.082l.361.905c.307.77-.244 1.207-.81 1.207h-5.097c-.565 0-1.117-.437-.81-1.207l1.191-2.988m8.905-1.404v5.599m7.687-1.802v1.065c0 .415-.342.737-.767.737h-.881m-5.54-7.762l.643.568h5.779c.424 0 .766.335.766.75v2.616"></svg:path><svg:path d="m21.554 25.29l2.38-2.33l2.38 2.33l5.123-5.014m-.188 2.211v5.394m2.343-4.306l1.844 1.805m-2.196-3.74h6.148m-1.508-1.501v7.005c0 .415-.34.737-.765.737h-.882"></svg:path><svg:path d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path></svg:g>`,
})
export class ArcticonsUnionpayAppIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnipatcherIcon],svg[arcticons-unipatcher-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 31.525h5.924s5.248 5.474 10.555 5.053s2.95-11.169-1.887-15.83S27.79 16.524 24 16.524s-9.754-.435-14.592 4.226s-7.193 15.408-1.886 15.829s10.554-5.054 10.554-5.054zm-3.846-4.434h2.545m5.148 0H25.3"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.588 21.287a1.355 1.355 0 0 0-1.359 1.359v1.165h-1.165a1.36 1.36 0 0 0 0 2.72h1.165v1.164a1.358 1.358 0 0 0 2.717 0V26.53h1.168a1.36 1.36 0 0 0 0-2.72h-1.168v-1.164a1.355 1.355 0 0 0-1.358-1.359"></svg:path><svg:circle cx="36.59" cy="25.128" r="1.26" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="31.102" cy="25.128" r="1.26" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="33.846" cy="27.872" r="1.26" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="33.846" cy="22.384" r="1.26" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 16.523s-.79-2.597 1.647-2.545s1.999-2.579 1.999-2.579"></svg:path>`,
})
export class ArcticonsUnipatcherIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUniqloIcon],svg[arcticons-uniqlo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.346 25.155v8.4h4.2m4.909-19.11v8.4m-9.109 0v-8.4l5.565 8.4v-8.4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linejoin="round" d="M31.402 33.555a2.71 2.71 0 0 1-2.73-2.69V27.99c0-1.575 1.26-2.835 2.73-2.835a2.823 2.823 0 0 1 2.835 2.835v2.73a2.823 2.823 0 0 1-2.835 2.835Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.763 14.445v5.565c0 1.575 1.26 2.835 2.73 2.835s2.835-1.26 2.835-2.835v-5.565"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linejoin="round" d="M16.493 33.555a2.71 2.71 0 0 1-2.73-2.69V27.99c0-1.575 1.26-2.835 2.73-2.835c1.575 0 2.73 1.26 2.73 2.835v2.73c0 1.575-1.155 2.835-2.73 2.835Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.223 33.555l-2.73-2.835"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsUniqloIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnitconverterultimateIcon],svg[arcticons-unitconverterultimate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.66 5.69v5.24H31v8.66H13.69l-.18 4.95L4.5 15l9.16-9.34Zm20.41 17.76l9.43 9.43l-9.43 9.43v-5.1H16.75v-8.66h17.32z"></svg:path>`,
})
export class ArcticonsUnitconverterultimateIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnitedAirlinesIcon],svg[arcticons-united-airlines-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 12.627c1.814-1.307 3.935-2.424 6.246-3.094M5.514 13.94c3.4-2.18 7.848-3.447 12.645-3.603M5.505 15.165c5.3-2.77 12.396-3.389 19.185-2.01M5.518 16.236c2.497-.892 5.346-1.534 8.379-1.57c8.35-.1 11.85 1.233 17.018 3.859M5.502 17.427c3.556-.938 7.797-1.186 12.033-.483c6.651 1.102 12.91 4.207 17.31 8.396M5.523 26.7c2.731 3.245 5.803 7.129 8.781 11.104a272 272 0 0 1 3.431 4.68M5.52 21.957c2.864 1.682 6.282 4.499 9.79 7.692c4.362 3.972 8.565 8.566 11.746 12.839M5.51 19.858c4.693.869 9.873 3.194 14.71 6.42c6.74 4.493 12.162 10.487 14.663 16.21M5.51 18.643a35.7 35.7 0 0 1 15.462 2.576c7.29 2.99 13.094 7.936 15.569 13.268M5.54 12.209c2.357.538 3.579 2.524 3.153 5.124c-.309 1.886-1.456 3.895-3.169 5.55m30.807 7.26c.454 4.173-.526 8.457-2.824 12.343m1.547-16.934c1.305 5.632-.085 11.733-3.86 16.948m.786-22.653c3.524 6.9 2.015 15.666-3.898 22.638m.237-26.69a14.6 14.6 0 0 1 1.586 2.592c3.534 7.418.64 17.204-7.133 24.107m2.663-28.888a11.86 11.86 0 0 1 2.751 5.025c2.123 7.92-3.242 17.987-12.715 23.855M10.965 9.498c2.883.164 5.247 1.274 6.754 3.171c2.639 3.322 2.326 8.625-.813 13.787c-2.584 4.248-6.773 7.883-11.38 9.875m.005-25.805c5.466-1.005 9.227 1.744 9.007 6.583c-.207 4.542-3.88 9.797-9.012 12.89m-.011-19.724a25 25 0 0 1 6.171-.773c13.807 0 25 11.193 25 25h0c0 2.72-.444 5.42-1.314 7.998M6.82 41.47c.44-.16.87-.33 1.31-.52c2.1-.87 4.13-2.02 6-3.38c1.78-1.29 3.42-2.77 4.85-4.39c1.01-1.12 1.91-2.31 2.68-3.55c.35-.55.66-1.11.94-1.66c.91-1.73 1.52-3.46 1.83-5.12c.27-1.36.33-2.68.21-3.92c-.12-1.18-.41-2.29-.88-3.32a9.3 9.3 0 0 0-1.77-2.64c-.1-.1-.2-.2-.3-.29c-1.04-1.01-2.32-1.79-3.79-2.33c-.26-.09-.52-.17-.78-.25M5.52 35.88c.06.12.12.24.18.37a151 151 0 0 0 2.43 4.7c.26.5.54 1.01.81 1.51"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsUnitedAirlinesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnitsIcon],svg[arcticons-units-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.004 34.3a2.625 2.625 0 0 1 2.625-2.626h0a2.625 2.625 0 0 1 2.626 2.625V38.5m-5.251-6.826V38.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.255 34.3a2.625 2.625 0 0 1 2.625-2.626h0a2.625 2.625 0 0 1 2.626 2.625V38.5"></svg:path><svg:circle cx="9.514" cy="10.391" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.514 13.607v6.957m15.391 16.614a2.62 2.62 0 0 1-2.28 1.322h0A2.625 2.625 0 0 1 20 35.875v-1.707a2.625 2.625 0 0 1 2.625-2.625h0c.974 0 1.824.53 2.277 1.318m-7.038-12.297v-4.332a2.625 2.625 0 0 0-2.626-2.625h0a2.625 2.625 0 0 0-2.625 2.625v4.332m0-4.332v-2.625"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.023 35.037h-3.017a4.49 4.49 0 0 1-4.492-4.492h0a4.49 4.49 0 0 1 4.492-4.491h20.008a4.49 4.49 0 0 0 4.492-4.492h0a4.49 4.49 0 0 0-4.492-4.492H20.822"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m15.321 33.335l1.702 1.702l-1.702 1.702m7.204-17.967l-1.703-1.702l1.703-1.703"></svg:path>`,
})
export class ArcticonsUnitsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnittoIcon],svg[arcticons-unitto-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:circle cx="16.162" cy="16.162" r="6.662" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.162 16.162h6m-3-3v6"></svg:path><svg:circle cx="31.838" cy="16.162" r="6.662" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.874 16.162h6"></svg:path><svg:circle cx="31.838" cy="31.838" r="6.662" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="31.874" cy="34.581" r=".75" fill="currentColor"></svg:circle><svg:circle cx="31.874" cy="29.095" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.874 31.838h6"></svg:path><svg:circle cx="16.162" cy="31.838" r="6.662" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.041 29.717l4.242 4.242m0-4.242l-4.242 4.242"></svg:path>`,
})
export class ArcticonsUnittoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnitvIcon],svg[arcticons-unitv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.857 34.323L18.694 10.877V37.98l13.48-14.023"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.949 43.546c-9.593 4.392-20.955 1.174-26.82-7.595s-4.5-20.5 3.222-27.688C17.073 1.074 28.871.552 37.2 7.029s10.725 18.04 5.658 27.294"></svg:path>`,
})
export class ArcticonsUnitvIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnityRemoteIcon],svg[arcticons-unity-remote-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.515 11.478l6.2 3.563c.223.125.23.472 0 .597l-7.366 4.238a.69.69 0 0 1-.693 0l-7.366-4.238c-.227-.12-.231-.476 0-.597l6.195-3.564V4.5L6.667 13.596V31.79l6.065-3.487v-7.127c-.004-.254.296-.436.519-.299l7.366 4.238a.69.69 0 0 1 .348.597v8.47c.004.255-.295.437-.518.3l-6.2-3.564l-6.065 3.487L24 43.5l15.818-9.096l-6.065-3.487l-6.2 3.564c-.218.133-.526-.041-.518-.3v-8.47a.69.69 0 0 1 .348-.597l7.367-4.238c.218-.133.526.037.518.299v7.127l6.066 3.487V13.596L25.516 4.5z"></svg:path>`,
})
export class ArcticonsUnityRemoteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnivCoopIcon],svg[arcticons-univ-coop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="18.175" cy="25.6" r="5.542" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="25.853" cy="35.272" r="5.844" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="29.523" cy="23.164" r="5.844" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.207 12.682c-.356 3.531-3.183 6.323-6.754 6.67s-6.892-1.849-7.935-5.245s.484-7.048 3.647-8.724s7.082-.91 9.363 1.83m-6.519 28.261V43.5"></svg:path>`,
})
export class ArcticonsUnivCoopIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUniversalTunerIcon],svg[arcticons-universal-tuner-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.202 4.5v23.156A3.2 3.2 0 0 1 24 30.858h0a3.2 3.2 0 0 1-3.202-3.202V4.5M24 30.858v9.296"></svg:path><svg:circle cx="24" cy="41.827" r="1.673" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.117 11.115a9.93 9.93 0 0 0-.593 13.693m14.952 0a9.93 9.93 0 0 0-.593-13.693M12.51 6.721a16.293 16.293 0 0 0-.592 22.481m24.164 0a16.29 16.29 0 0 0-.562-22.45"></svg:path>`,
})
export class ArcticonsUniversalTunerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUniversalcopyIcon],svg[arcticons-universalcopy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.375 19.788h19.656m-19.656 5.989h11.832m2.608 3.189h2.67a2.67 2.67 0 0 1 2.67 2.67v0a2.67 2.67 0 0 1-2.67 2.67h0a2.67 2.67 0 0 1-2.67-2.67zm-16.951-6.294v2.67a2.67 2.67 0 0 1-2.67 2.67h0a2.67 2.67 0 0 1-2.67-2.67h0a2.67 2.67 0 0 1 2.67-2.67z"></svg:path>`,
})
export class ArcticonsUniversalcopyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnivlorraineIcon],svg[arcticons-univlorraine-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.84 31.343c-4.001 2.161-7.788.674-7.974-3.435V6.248c-3.455-.01-6.91-.005-10.366-.005h0l.031 22.218c-.047 7.163 8.776 18.242 23.815 10.658"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m23.518 6.443l.03 22.217c-.046 7.164 6.483 14.489 18.952 12.873c-.068-2.893-.15-5.477-.184-9.202c-5.91.004-8.127-.352-8.433-4.224V6.447c-3.455-.01-6.91-.005-10.365-.005Z"></svg:path>`,
})
export class ArcticonsUnivlorraineIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnlauncherIcon],svg[arcticons-unlauncher-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.086 16.289l2.633-1.52l.197-1.924l2.381-1.375l.341 2l-.445 5.784l6.526 11.304v4.71l-4.079-2.355l-6.526-11.303l-5.23-2.507l-1.563-1.295l2.38-1.375l1.765.79l2.633-1.52m6.541 17.21l4.08-2.355M20.114 21.61l-2.591-4.489m6.67 2.134l-2.468-4.275m.871 10.908l3.902-2.252"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.267 15.67v16.66a3.84 3.84 0 0 1-1.92 3.325l-14.427 8.33a3.84 3.84 0 0 1-3.84 0l-14.428-8.33a3.84 3.84 0 0 1-1.92-3.325V15.67a3.84 3.84 0 0 1 1.92-3.325l14.428-8.33a3.84 3.84 0 0 1 3.84 0l14.428 8.33a3.84 3.84 0 0 1 1.92 3.325"></svg:path>`,
})
export class ArcticonsUnlauncherIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnlimCloudIcon],svg[arcticons-unlim-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m41.697 32.508l-.01-.01c-.283-.171-4.766-3.063-4.766-8.805c0-.675-.07-1.33-.191-1.965c-.917-4.644-5.018-8.15-9.924-8.15c-3.496 0-6.569 1.773-8.393 4.473"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.488 34.423c-2.236 0-7.334-2.257-7.334-8.544c0-3.97-2.932-7.254-6.74-7.828a7.925 7.925 0 0 0-9.037 6.77"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.73 21.728c.14-.02.282-.02.423-.02c3.506 0 6.347 2.851 6.347 6.357a6.3 6.3 0 0 1-1.813 4.433a6.26 6.26 0 0 1-4.534 1.925H18.736m-9.36-9.602h-.07A4.803 4.803 0 0 0 4.5 29.617a4.805 4.805 0 0 0 4.806 4.806h9.43c-1.924 0-4.624-1.129-4.624-4.806a4.8 4.8 0 0 0-4.736-4.796"></svg:path>`,
})
export class ArcticonsUnlimCloudIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnlimitedEnergyIcon],svg[arcticons-unlimited-energy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="14.94" cy="29.388" r="2.729" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="33.408" cy="29.231" r="2.729" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24.024" cy="17.512" r="2.729" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24.024" cy="40.771" r="2.729" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.024 20.24v17.803M13.376 27.152C4.562 14.07 15.906 4.5 24.116 4.5s19.026 9.828 10.72 22.406"></svg:path>`,
})
export class ArcticonsUnlimitedEnergyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnlockIcon],svg[arcticons-unlock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.51 20.609h24.98c.947 0 1.715.768 1.715 1.715v18.46c0 .948-.768 1.716-1.715 1.716H11.51a1.715 1.715 0 0 1-1.715-1.715V22.324c0-.947.768-1.715 1.715-1.715m3.088-5.744a9.394 9.394 0 0 1 18.788 0v5.744"></svg:path><svg:circle cx="23.992" cy="31.554" r="3.643" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUnlockIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnlockmasterIcon],svg[arcticons-unlockmaster-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.132 14.889C31.069 2.688 13.825 2.46 13.877 13.728l-.052 4.232m14.542 23.073H8.834a2.34 2.34 0 0 1-2.345-2.342V20.955a2.34 2.34 0 0 1 2.345-2.343h27.289a2.34 2.34 0 0 1 2.344 2.343v7.283"></svg:path><svg:ellipse cx="34.477" cy="35.472" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="8.266" ry="8.259"></svg:ellipse><svg:circle cx="34.537" cy="35.335" r="1.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m34.561 27.213l-.024 2.33m8.206 5.929h-2.18m-6.026 6.172l.02 2.087m-6.106-8.259h-2.24m13.132-4.663l-1.784 3.125c-.12-.624-.735-1.183-1.173-1.417zm-16.245.778l.284 2.452h-2.066l.362-2.452s-1.328-.732-1.344-1.764c-.015-1.033 1.008-2.072 2.028-2.063c1.02.01 2.03 1.027 2.028 2.063c-.003 1.035-1.292 1.764-1.292 1.764"></svg:path>`,
})
export class ArcticonsUnlockmasterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnoIcon],svg[arcticons-uno-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="40.243" cy="23.182" r=".75" fill="currentColor" transform="rotate(-88.993 40.243 23.182)"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m35.032 11.801l3.876 8.464M22.004 31.136l-4.422-9.658l10.82 6.729l-4.422-9.658M8.39 25.687l2.958 6.459a3.519 3.519 0 1 0 6.398-2.93l-2.957-6.459"></svg:path><svg:rect width="7.037" height="10.622" x="28.582" y="15.361" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="3.519" ry="3.519" transform="rotate(-24.601 32.1 20.672)"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.692 15.468c1.228 2.643 1.229 5.63.22 8.586c-1.71 5.017-6.325 9.944-12.771 12.94c-10.244 4.762-21.234 2.844-24.547-4.283c-1.084-2.332-1.211-4.933-.533-7.544c1.393-5.368 6.192-10.778 13.084-13.982c6.692-3.11 13.702-3.37 18.682-1.19c2.644 1.157 4.717 3.002 5.865 5.473"></svg:path>`,
})
export class ArcticonsUnoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnoAltIcon],svg[arcticons-uno-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:ellipse cx="24" cy="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="13.915" ry="8.293" transform="rotate(-73.087 24 24)"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m20.198 37.381l7.604-26.762M15.659 24h16.699"></svg:path><svg:rect width="24" height="39" x="12" y="4.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2" ry="2"></svg:rect>`,
})
export class ArcticonsUnoAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnofficialTelegramStickersForWhatsappIcon],svg[arcticons-unofficial-telegram-stickers-for-whatsapp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.247 34.124l19.506-20.246m-13.54 0h13.539v13.636"></svg:path>`,
})
export class ArcticonsUnofficialTelegramStickersForWhatsappIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnpopularMusicPlayerIcon],svg[arcticons-unpopular-music-player-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.516 17.647C26.443 6.55 13.461 5.929 11.645 13.91c-.181.797.21.884 1.344.297c2.163-1.119 5.647.25 4.767 4.515c-.7 3.399-7.976 5.737-8.378-2.777c0-13.027 22.996-12.235 22.32 1.763c-.18 17.74-20.359 24.613-21.705 23.509c0 0 16.617-9.296 16.523-23.57"></svg:path><svg:circle cx="37.313" cy="12.959" r="2.054" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="37.313" cy="23.044" r="2.054" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 18.137H31.69m-5.218 0h-8.62m-8.29 0H5.5m37 10.273H28.779m-5.405 0H5.5m37 10.268H17.892m-4.119 0H5.5m37-31.386H24.143m-8.23 0H5.5"></svg:path>`,
})
export class ArcticonsUnpopularMusicPlayerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnpuzzlerIcon],svg[arcticons-unpuzzler-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.63 17.86a5.82 5.82 0 0 0-4.57 2.22V9.72H22.43a5.87 5.87 0 1 1-7.3 0H4.5v28.56h10.63a5.87 5.87 0 1 1 7.3 0h10.63v-10.9a5.86 5.86 0 1 0 4.57-9.52"></svg:path>`,
})
export class ArcticonsUnpuzzlerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnraidIcon],svg[arcticons-unraid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 19v10m4.626-2.5v5M24 19v10m-9.248 2v4m4.626-8.5v5M42.5 29V19m-9.248-2v-4m-4.626 8.5v-5m9.252 5v-5"></svg:path>`,
})
export class ArcticonsUnraidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnscriptedIcon],svg[arcticons-unscripted-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.728 34.418s-3.427-3.369-4.54-6.743m-1.05.758a8.5 8.5 0 0 0 1.88-5.55c-.172-3.206 1.475-7.655 1.475-7.655s-1.457-.806-3.05 2.897s-3.298 3.302-3.298 3.302"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.823 20.685a11.3 11.3 0 0 1-5.058-4.629m-.161 1.206l-.351-7.305c.212-.147 2.158-5.764-.317-6.457m0 0l-2.402 6.997s-.58 7.344-.438 7.739s.42 1.124.42 1.124"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.481 11.194s-1.475-.361-1.966 2.18c-1.013 5.24-.514 5.802.22 6.829"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.78 12.28s-1.84-1.02-2.515 2.954s-.324 2.97.385 4.215"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.966 17.167s-.803.07-1.305-1.788s-1.513-3.031-1.973-3.031s1.109 6.348 1.109 6.348s2.193 3.632 3.93 6.06m1.022 1.206c2.284 2.085 8.997 9.168 10.854 16.9m6.125-8.444s.69 1.517-1.836 5.027s-4.187 3.823-4.289 3.419m-6.289-31.643l2.318 1.047a2.16 2.16 0 0 1-2.236.661"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.776 14.025c-.05 3.223-1.52 8.564-3.753 9.29a9.03 9.03 0 0 0-5.311 6.047c-.894 3.003-3.526 9.703-3.526 9.703c3.55 5.29 6.808 5.435 6.808 5.435s.685-7.803 2.636-8.405s5.58-3.276 5.981-5.233s1.545-4.07 3.097-4.877s-.386-2.072-2.69-.532s-2.952 3.94-3.999 4.526"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.795 30.128a42 42 0 0 1 4.135-3.02m-13.032 3.974c1.095-.68 5.215-3.562 6.898-3.372m-6.092.36a12 12 0 0 0 1.23-2.833m5.139-.678a9.6 9.6 0 0 0 2.01-2.104m-3.566.66a6.25 6.25 0 0 0 2.405-2.502m-2.405 2.502a24.5 24.5 0 0 0 1.28-4.42"></svg:path>`,
})
export class ArcticonsUnscriptedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnseenGalleryIcon],svg[arcticons-unseen-gallery-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.48 5.5a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29.04a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4zm5.442 27.512l18.023-18.024"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.703 19.817a4.492 4.492 0 0 0-6.197 4.154c.003.584.12 1.16.344 1.7m2.805 2.584a4.5 4.5 0 0 0 1.344.21a4.492 4.492 0 0 0 4.284-5.84"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.49 17.03a18 18 0 0 0-4.501-.594A17.83 17.83 0 0 0 9.5 23.94a17.8 17.8 0 0 0 6.494 5.585m3.95 1.44a18 18 0 0 0 4.045.48A17.88 17.88 0 0 0 38.5 23.94a17.86 17.86 0 0 0-6.17-5.407"></svg:path>`,
})
export class ArcticonsUnseenGalleryIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnstoppableIcon],svg[arcticons-unstoppable-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.905 23.711a10.095 10.095 0 0 0 20.19 0v0l-.001-18.694a21.5 21.5 0 1 1-20.188 0h0z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.7 5.017v19.231h0a1.7 1.7 0 0 1-3.4 0h0V5.017"></svg:path>`,
})
export class ArcticonsUnstoppableIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUntappdIcon],svg[arcticons-untappd-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.802 6.411c-1.585-.918-2.48-.387-1.89.337c-1.01.12-.408.891-.408.891s-5.272 6.779-7.293 9.245c-2.1 2.564-2.847 1.057-4.403 3.744L4.5 36.705s.168 2.332 3.516 4.27s5.116.728 5.116.728l9.308-16.077c1.556-2.687-.123-2.584 1.055-5.682c1.133-2.98 4.387-10.928 4.387-10.928s.968.139.57-.798c.921.152.936-.888-.65-1.807"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.593 12.638c-.822-2.028-1.475-3.622-1.475-3.622s-.968.139-.57-.798c-.922.152-.937-.889.65-1.807s2.48-.387 1.89.337c1.01.12.407.891.407.891L24 9.569m2.407 3.069a328 328 0 0 0 3.382 4.246c2.1 2.564 2.847 1.057 4.403 3.744L43.5 36.705s-.168 2.332-3.516 4.27s-5.116.728-5.116.728L25.56 25.626c-1.556-2.687.123-2.584-1.055-5.682c-.14-.367-.311-.81-.505-1.302"></svg:path>`,
})
export class ArcticonsUntappdIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUntisMessengerIcon],svg[arcticons-untis-messenger-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.382 34.75a21.499 21.499 0 1 1 7.87 7.868m-7.87-7.868L3.6 44.21l9.651-1.592M23.86 2.64v7.327m-.048 3.579V24.02m-5.782 3.428l5.782-3.427M34.543 5.512l-3.664 6.345m11.479 1.483l-6.346 3.664m9.2 7.022h-7.327m4.455 10.682l-6.346-3.664m-1.482 11.478l-3.663-6.345m-7.024 9.199V38.05m-10.682 4.454l3.664-6.345M5.328 34.676l6.346-3.664M2.982 23.99h6.82M5.346 13.309l6.346 3.663m1.482-11.478l3.664 6.345"></svg:path>`,
})
export class ArcticonsUntisMessengerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUntisMobileIcon],svg[arcticons-untis-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.981 2.5A21.481 21.481 0 1 1 2.5 23.981h0A21.49 21.49 0 0 1 23.981 2.5m-.005 21.526h17.229M23.976 6.798v17.229m12.183-12.183L23.977 24.026"></svg:path>`,
})
export class ArcticonsUntisMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUntrackerIcon],svg[arcticons-untracker-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.359 26.433v2.638a5.24 5.24 0 0 1-5.235 5.245h-.229a5.24 5.24 0 0 1-5.254-5.226v-2.657m0-4.651v-2.638a5.24 5.24 0 0 1 5.235-5.245h.229c2.91 0 5.254 2.34 5.254 5.245v2.638M24 19.103v9.872M8.146 11.763C7.68 24.281 6.674 36.923 24 43.5c17.327-6.576 16.32-19.219 15.854-31.736L24 4.5z"></svg:path>`,
})
export class ArcticonsUntrackerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUntrackmeIcon],svg[arcticons-untrackme-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.89 15.13l6.59 6.43L25.89 28l-.05-3.66c-5.31.23-6.74 1.22-10.17 4.54c2-10.16 11-10.18 10.15-10Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.07 10.1L26.28 4.92a5.6 5.6 0 0 0-4.26 0L8.89 10.1A2 2 0 0 0 7.62 12v12.47c0 11.16 11.6 19 16.58 19s16.18-7.87 16.18-19V12a2 2 0 0 0-1.31-1.9"></svg:path>`,
})
export class ArcticonsUntrackmeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUnwantedExperimentIcon],svg[arcticons-unwanted-experiment-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24.303 16.228l.308 5.98l-.748.014l-.483-.41m1.979-6.609l4.807-.791m-8.413.966l-5.13-.689m5.439 9.663l.363-.238l3.15-.228M21.45 40.052l.038 5.301m9.869-5.343l.208 4.115"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.961 17.531c-.025-.401-.011-.795-.09-1.21s-.253-.85-.332-1.143s-.066-.445.041-.549s.307-.159.542-.57s.505-1.178.525-1.876c.021-.697-.207-1.326-.487-1.56s-.611-.077-.811-.374s-.27-1.05.076-1.769c.345-.718 1.105-1.402 1.716-1.671c.612-.27 1.075-.125 1.807.162s1.734.715 2.705.884c.97.17 1.91.08 2.573.114c.663.035 1.05.194 1.171.194s-.024-.16-.231-.363c-.208-.204-.477-.453-.515-.556s.155-.062.57.02c.414.083 1.05.208 1.52.232s.774-.052.85-.173c.076-.12-.076-.286-.28-.48c-.204-.193-.46-.414-.418-.48s.38.024.877.207c.498.183 1.154.46 1.675.532s.909-.058 1.34.114s.91.65 1.33 1.655c.422 1.005.788 2.539.726 3.875c-.062 1.337-.553 2.477-.615 3.368c-.062.892.304 1.534.26 3.106c-.046 1.572-.502 4.072-1.054 5.723c-.553 1.651-1.202 2.453-2.425 3.116s-3.02 1.188-4.622 1.23c-1.603.041-3.012-.4-4.31-1.14s-2.488-1.776-3.034-2.59c-.545-.816-.449-1.41-.56-1.888c-.11-.477-.429-.838-.747-1.05s-.635-.276-1.126-.623s-1.155-.977-1.493-1.761c-.337-.784-.346-1.722-.058-2.343c.288-.62.874-.923 1.35-.813s.843.633 1.11.98s.432.517.483.398c.05-.118-.014-.527-.039-.928"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.275 21.303c.52-.49 1.038-.98 1.332-1.6c.294-.622.362-1.374.318-1.945c-.044-.572-.2-.963-.234-1.398s.054-.913.205-1.094c.151-.18.366-.063.513-.063s.225-.118.354-1.262c.13-1.144.31-3.315.022-5.136s-1.044-3.294-1.8-4.767m12.568 15.268c-.928.606-1.856 1.212-2.516 2.145s-1.05 2.193-1.25 2.911s-.21.894-.267.975c-.056.08-.158.066-.249.075c-.09.01-.168.044-.247.435c-.078.39-.156 1.138-.244 1.585s-.185.594-.317.694s-.298.154-.45.037c-.151-.118-.288-.406-.44-1.283c-.15-.877-.317-2.342-.241-3.058s.393-.681.88-.591c.486.09 1.14.234 1.796.38"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.313 24.308c-.577-.046-1.154-.092-1.521-.004c-.367.09-.523.313-.572.787c-.05.474.008 1.198.221 1.995c.214.797.583 1.667.912 2.122c.328.456.614.498.924.49s.643-.065.77-.33c.126-.263.046-.734-.034-1.205"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.343 24.628c-.513-.044-1.027-.087-1.44-.01s-.725.275-.738 1.223s.273 2.646.721 3.508s1.021.887 1.46.732s.723-.514 1.009-.873"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.678 25.32c-.678-.206-1.355-.412-1.73 0c-.377.413-.45 1.444-.329 2.22s.439 1.3.796 1.44s.755-.102.968-.504c.214-.401.243-.96.35-1.3c.106-.338.288-.456.47-.573m-1.278-8.846c2.24.606 4.48 1.212 6.06 1.75c1.58.54 2.502 1.011 3.424 1.483m-29.587-3.854c-.422-.708-.845-1.415-.999-2.42s-.04-2.307-.034-2.953s-.099-.636-.276-.448c-.177.187-.427.552-.625.333s-.344-1.02-.11-1.776c.235-.755.85-1.463.97-1.713s-.256-.042-.522.036c-.265.078-.421.026-.386-.256c.036-.283.264-.796.491-1.31M2.576 25.814c1.498-1.08 2.995-2.16 4.65-2.977c1.654-.817 3.466-1.371 5.277-1.926m.35.424c-.63.93-1.262 1.858-1.668 2.643c-.407.785-.589 1.426-.581 1.743s.206.313.94.232s2.005-.237 2.598-.215s.508.222.248.536s-.695.74-1.144.98s-.913.291-1.112.402s-.133.28.383.976c.515.696 1.48 1.918 2.445 3.93c.965 2.01 1.93 4.81 2.398 6.894c.467 2.084.438 3.454.408 4.824"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.546 40.391c.089-1.856.178-3.712.075-5.443c-.103-1.732-.397-3.337-.43-4.614c-.034-1.278.193-2.226.42-3.175"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.19 30.334c.561 1.231 1.122 2.462 2.677 3.166s4.103.88 5.783.137s2.49-2.409 2.738-3.492s-.067-1.584-.381-2.086m3.119-.197a10 10 0 0 0-.355-.017c-.098-.001-.164.002-.256-.075s-.21-.236-.297-.315s-.142-.08-.243.168s-.249.74-.186 1.305s.335 1.203.562 1.505c.226.302.407.268.513.193s.14-.193.17-.28s.055-.142.697-.303s1.899-.427 3.156-.694"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.733 29.888c.06.508.12 1.016.023 1.314c-.096.298-.349.387-.96.226c-.613-.16-1.584-.569-2.078-.932c-.495-.363-.513-.68-.53-.998M22.013 40.025a23.3 23.3 0 0 0 4.585.467c1.652-.007 3.205-.244 4.1-.506c.895-.261 1.131-.547 1.172-.715c.04-.17-.114-.22-1.02-.456s-2.563-.656-4.567-.62c-2.003.038-4.353.531-5.216.917c-.863.385-.24.663.946.913m17.1-11.131c.714-.278 1.429-.556 1.863-.643c.435-.087.59.016.932.528s.873 1.432 1.286 1.931s.708.577 1.004.654"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.753 31.202c.839.114 1.677.228 2.486.006c.81-.221 1.589-.78 2.369-1.337m-29.414-10.45c-.037-.17-.074-.339.018-.416c.093-.077.313-.063.53.125s.432.549.531.913c.1.365.085.733.07 1.101m12.558-5.183c.56-.073 1.716-.088 2.354-.088s.758.015.854.125s.17.312-.03.599s-.675.656-1.199.828s-1.096.145-1.515-.076c-.42-.221-.685-.638-.836-.911s-.188-.404.372-.477m-6.359.119c-.689-.026-2.007.055-2.751.232s-.913.45-.854.707c.058.258.346.501.82.645a4 4 0 0 0 1.706.088c.567-.1 1.039-.342 1.337-.597s.423-.519.457-.718c.033-.199-.026-.331-.715-.357m6.949 2.744c.339.088.678.177.924.192c.247.014.402-.045.556-.104m-10.805.096c.405.059.81.118 1.222.044c.413-.074.833-.28 1.252-.486m4.57 6.815c.085.15.169.3.196.695s-.005 1.035-.13 1.418c-.127.382-.348.507-.552.525s-.39-.071-.577-.16m6.012 2.257c-.462.419-.924.837-1.14 1.388s-.184 1.233-.07 1.733s.313.817.332 1.648c.018.83-.144 2.174-.154 3.348c-.01 1.175.13 2.18.164 2.94c.033.759-.04 1.272-.115 1.785"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.513 43.262a81 81 0 0 1 2.183-.2c.853-.065 1.894-.128 2.749-.117c.854.01 1.52.093 2.328.045s1.757-.227 2.706-.406m7.604-11.251l.265 7.728m-30.2-2.517c-.042.167-.083.333-.1.818c-.016.484-.007 1.287.002 2.09"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.055 37.484c-.333-.272-.667-.543-.865-1.304s-.26-2.011-.323-3.261m1.625 2.125c-.208-1.02-.417-2.042-.354-2.781c.062-.74.396-1.198.729-1.657"></svg:path>`,
})
export class ArcticonsUnwantedExperimentIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUobTmrwIcon],svg[arcticons-uob-tmrw-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.271 23.608c-1.57 0-2.724-1.257-2.724-2.725v-2.828c0-1.572 1.258-2.829 2.724-2.829h0a2.817 2.817 0 0 1 2.83 2.805v2.748a2.817 2.817 0 0 1-2.806 2.829zm8.343-4.191h-3.563m3.353 0c1.153 0 2.096.943 2.096 2.096a2.1 2.1 0 0 1-2.096 2.095h-3.457v-8.381h3.457c1.153 0 2.096.943 2.096 2.095s-.943 2.096-2.096 2.096zm-18.151-4.191v5.553c0 1.572 1.257 2.829 2.724 2.829s2.724-1.257 2.724-2.829v-5.553M8.5 19.417h8.627m-1.507-4.191v8.382m-1.87-8.382v8.382m-1.872-8.382v8.382m-1.871-8.382v8.382m25.669 3.941l-1.307 5.225l-1.305-5.224l-1.306 5.224l-1.306-5.224m-16.43 5.224v-5.225m-1.697 0h3.395m11.371 3.559l1.601 1.6m-3.394.066V27.55h1.697c.98 0 1.763.784 1.763 1.764a1.755 1.755 0 0 1-1.748 1.763h-1.712m-7.497 1.631v-5.159l2.611 5.225l2.612-5.225v5.225"></svg:path>`,
})
export class ArcticonsUobTmrwIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpBankingIcon],svg[arcticons-up-banking-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.197 37.95H6.617a.884.884 0 0 1-.77-1.326l1.713-2.958m32.192-1.585l2.63 4.543a.883.883 0 0 1-.765 1.325H26.04m-9.525-19.747L23.35 6.401a.884.884 0 0 1 1.534 0h0l6.392 11.041"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.513 25.094a6 6 0 0 0-.502-2.304a6.91 6.91 0 0 0-7.719-4.178c-.58.09-1.152.232-1.707.424a1.02 1.02 0 0 0-.884-.353h-2.065a.55.55 0 0 0-.525.382c-.299.812-2.914 7.992-3.755 10.224a1.98 1.98 0 0 1-2.161 1.409a1.16 1.16 0 0 1-.77-.525a1.08 1.08 0 0 1-.138-.86l.036-.12c.084-.292.179-.626.292-.924l3.29-8.678a.55.55 0 0 0-.52-.752H20.95a1.08 1.08 0 0 0-1.087.764a182 182 0 0 1-1.492 4.088a74 74 0 0 0-.758 2.16c-.299.884-.597 1.791-.968 2.68a4.05 4.05 0 0 1-1.068 1.648a3.3 3.3 0 0 1-2.669.638c-1.247-.256-1.42-1.486-1.379-2.172a7.5 7.5 0 0 1 .526-2.077c.597-1.546 1.713-4.62 2.579-6.882a.54.54 0 0 0-.06-.513a.48.48 0 0 0-.454-.245h-2.238a1.37 1.37 0 0 0-.747.215q-.996.75-1.868 1.641a26.3 26.3 0 0 1-3.516 2.984a.6.6 0 0 0-.25.46a.54.54 0 0 0 .238.466a.6.6 0 0 0 .49.047h.053c.346-.065 1.116-.262 2.836-.704a20 20 0 0 0-1.26 4.005a5.6 5.6 0 0 0 1.236 4.655c1.79 2.137 4.734 2.22 6.716 1.397a5.2 5.2 0 0 0 1.79-1.325a4.54 4.54 0 0 0 3.97 1.623l-.37 1.003l-.143.394a224 224 0 0 1-1.79 4.775c-.144.335-.11.72.089 1.026c.218.297.569.467.937.454h2.012a.555.555 0 0 0 .525-.37c.322-.925.824-2.292 1.301-3.629l.49-1.355l1.194-3.348l.483-1.349q.32.373.693.692a6.94 6.94 0 0 0 5.802 1.588a8.96 8.96 0 0 0 4.734-2.28a8.78 8.78 0 0 0 2.985-6.9Zm-4.137.704a5.47 5.47 0 0 1-2.532 4.106c-.711.46-1.54.71-2.388.716a2.985 2.985 0 0 1-2.853-2.107a5 5 0 0 1-.173-.698v-.37a5.7 5.7 0 0 1 1.343-3.432a4.7 4.7 0 0 1 3.397-1.719h.215a2.92 2.92 0 0 1 2.68 1.737c.277.545.385 1.16.31 1.767"></svg:path>`,
})
export class ArcticonsUpBankingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpdateCentreIcon],svg[arcticons-update-centre-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M18.763 28.871a1.96 1.96 0 1 1 1.943-1.88a1.944 1.944 0 0 1-1.943 1.88m11.345 0a1.96 1.96 0 1 1 1.943-1.88a1.944 1.944 0 0 1-1.943 1.88"></svg:path><svg:path d="M24.403 16.873c7.674 0 13.895 6.22 13.895 13.894h0a1.976 1.976 0 0 1-1.976 1.976H12.42a1.976 1.976 0 0 1-1.976-1.976h0c0-7.674 6.22-13.894 13.894-13.894zm-11.982-2.072l3.951 4.637m20.045-4.637l-3.935 4.637"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" d="M44.129 26.458c-1.22 10.043-9.746 17.595-19.863 17.596S5.623 36.504 4.4 26.461"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m2.5 29.968l1.83-3.843l3.232 2.716"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" d="M4.1 22.521c.727-10.09 8.87-18.053 18.975-18.55c10.105-.499 18.992 6.624 20.706 16.595"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m45.5 16.829l-1.639 3.928l-3.361-2.553"></svg:path>`,
})
export class ArcticonsUpdateCentreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpdateMeIcon],svg[arcticons-update-me-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 12.633a11.368 11.368 0 1 1-10.485 6.977m0 0l-2.774 1.358m2.774-1.358l1.877 2.454"></svg:path>`,
})
export class ArcticonsUpdateMeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpdaterIcon],svg[arcticons-updater-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 31.186V16.814l5.456 5.884M24 16.814l-5.456 5.884"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.245 26.002q.13-.996.14-2.002a17 17 0 0 0-.14-2.002l4.334-3.393a1 1 0 0 0 .25-1.311l-4.104-7.117a1 1 0 0 0-1.251-.44l-5.115 2.061a15.7 15.7 0 0 0-3.463-2.002l-.771-5.435a1 1 0 0 0-1.001-.86h-8.228a1 1 0 0 0-1.001.86l-.771 5.435a15.4 15.4 0 0 0-3.463 2.002L9.526 9.736a1 1 0 0 0-1.252.44L4.17 17.294a1 1 0 0 0 .25 1.312l4.325 3.393A17 17 0 0 0 8.605 24q.01 1.006.14 2.002L4.42 29.395a1 1 0 0 0-.25 1.311l4.103 7.117a1 1 0 0 0 1.252.44l5.115-2.061a15.7 15.7 0 0 0 3.463 2.002l.77 5.435a1 1 0 0 0 1.002.86h8.208a1 1 0 0 0 1-.86l.772-5.435a15.4 15.4 0 0 0 3.463-2.002l5.115 2.062c.468.19 1.005.001 1.251-.44l4.104-7.118a1 1 0 0 0-.25-1.31z"></svg:path>`,
})
export class ArcticonsUpdaterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpdayIcon],svg[arcticons-upday-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 24c0 7.995-6.505 14.5-14.5 14.5S9.5 31.995 9.5 24"></svg:path>`,
})
export class ArcticonsUpdayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpdeckIcon],svg[arcticons-updeck-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.168 32.318a21.504 21.504 0 1 1 40.98-12.211m.336 4.724A21.5 21.5 0 0 1 9.007 39.41"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.617 25.402c2.187 0 2.775 1.85 2.775 3.981c0 3.505-4.906 12.28-10.261 12.28c-1.458 0-2.495-.28-2.495-2.187s4.261-14.074 9.98-14.074"></svg:path><svg:ellipse cx="40.734" cy="22.749" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.907" ry="3.651" transform="rotate(-7.549 40.734 22.749)"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.255 19.13c-2.62 1.282-11.298 3.558-14.591 4.057a1.948 1.948 0 0 0-1.879 2.44c.252 1.009 1.066 1.513 2.523 1.233a44.4 44.4 0 0 1 14.906-.492"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.526 25.98q.091.838.128 1.707c.337 7.865-2.497 14.945-4.255 17.13M13.874 42.97a9.18 9.18 0 0 0 4.514-8.26c0-3.911-4.514-13.248-4.514-16.15s1.107-4.92 8.341-4.92a10 10 0 0 1 7.644 3.163"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.794 14.578c-4.074-1.863-9-3.313-14.967-.223c-5.846 3.028-3.53 12.399-3.53 12.399"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.194 10.717c.63-1.261 4.498-2.338 5.172 1.849m15.316 17.476c4.745.39 13.534-4.784 13.052-7.906c-.4-2.59-4.237-2.843-5.441-2.62M11.26 20.2l.127-1.316a.857.857 0 0 1 .932-.77l5.732.53m1.269-1.05h4.473a1.01 1.01 0 0 1 1.009 1.009v1.65a2.26 2.26 0 0 1-2.26 2.261H20.65a2.26 2.26 0 0 1-2.258-2.154l-.08-1.71a1.01 1.01 0 0 1 1.008-1.056m14.112 1.577l-.12-1.574a1.347 1.347 0 0 0-1.645-1.21l-3.93.904a1.347 1.347 0 0 0-1.01 1.617l.182.79a3.16 3.16 0 0 0 3.833 2.352l.74-.182a2.574 2.574 0 0 0 1.95-2.697m-8.631-.568h1.89"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.859 16.803c1.419.79 2.76 2.164 1.333 5.136"></svg:path><svg:circle cx="21.771" cy="19.248" r="1.598" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="30.065" cy="18.603" r="1.335" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUpdeckIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpfcmDistributorIcon],svg[arcticons-upfcm-distributor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:ellipse cx="24" cy="38.527" rx="16.948" ry="4.973"></svg:ellipse><svg:path d="M24 4.5s-2.507-.056-2.757 3.39c0 0-7.285 1.661-7.266 12c.02 10.846-6.267 8.25-6.904 18.393M24 16.39l-4.898 6.556h2.335v4.195H24m5.976 6.732c0 3.345-2.676 6.056-5.976 6.056s-5.975-2.711-5.975-6.056M24 4.5s2.507-.056 2.758 3.39c0 0 7.284 1.661 7.265 12c-.02 10.846 6.267 8.25 6.904 18.393M24 16.39l4.898 6.556h-2.335v4.195H24"></svg:path></svg:g>`,
})
export class ArcticonsUpfcmDistributorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpgradIcon],svg[arcticons-upgrad-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.625 4.5v24.375C38.625 36.952 32.077 43.5 24 43.5S9.375 36.952 9.375 28.875V4.5z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22 35.187v-5.312h-5.312"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.375 28.875c0 6.413 10.245 9.882 12.625 1"></svg:path>`,
})
export class ArcticonsUpgradIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpgradeallIcon],svg[arcticons-upgradeall-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.187 41.878q.471-.325.923-.673m-6.478 3.409a22 22 0 0 0 2.754-1.106M22.19 45.592a21.3 21.3 0 0 0 5.254-.205m-15.949-3.739a21.6 21.6 0 0 0 7.438 3.413M41.88 12.285A21.479 21.479 0 1 0 9.146 39.704"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m42.33 6.914l-.451 5.37l-5.371-.451"></svg:path>`,
})
export class ArcticonsUpgradeallIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpgradoIcon],svg[arcticons-upgrado-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m10.313 21.744l29.325.023l-1.989 4.091l-29.385.093zM6.547 30.82l29.327.022l-1.99 4.091l-29.384.093zm7.628-17.846l29.325.023l-1.988 4.09l-29.387.093z"></svg:path>`,
})
export class ArcticonsUpgradoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpholdIcon],svg[arcticons-uphold-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.397 42.767c-.974.696-3.06.807-4.316.696c-1.256.11-3.342 0-4.316-.696m18-24.542c.154-.162-.37-9.874-8.745-7.588c-7.353 2.008-12.654 13.04-11.244 20.47c1.25 6.583 6.265 7.773 6.265 7.773M10.235 18.225c-.154-.162.37-9.874 8.745-7.588c7.353 2.008 12.654 13.04 11.244 20.47c-1.25 6.583-6.265 7.773-6.265 7.773M10.276 18.225C10.276 10.645 16.42 4.5 24 4.5s13.724 6.145 13.724 13.725S31.58 38.88 24 38.88m0 0c-7.58 0-13.724-13.076-13.724-20.655"></svg:path>`,
})
export class ArcticonsUpholdIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpliveIcon],svg[arcticons-uplive-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.982 16.054a5.41 5.41 0 0 0-3.873-3.052c-2.736-.391-3.182 2.233-3.238 3.46s.726 4.579 1.06 7.09s-.837 3.07-3.293 3.07s-3.182-1.06-3.406-2.4s2.513-6.085 2.903-8.709s-.837-3.405-2.4-3.405c-2.345 0-5.304 2.345-6.364 6.42s-1.564 9.1.949 12.45a10.67 10.67 0 0 0 11.165 3.46a11.2 11.2 0 0 0 4.194-2.298m9.567-17.938c1.252-.323 7.676 3.322 7.592 5.722s-6.42 4.634-7.592 4.773s-2.177-2.987-1.842-5.862s.977-4.41 1.842-4.633"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 20.092c0-2.35-1.842-9.686-9.853-9.91s-10.05 6.28-10.719 8.905s-4.625 13.112 2.289 18.255a3.78 3.78 0 0 0 3.805-.093c1.675-.977 1.565-3.737 1.162-4.844c-.306-.841-1.58-1.782-1.571-2.47c.028-2.14 3.582-2.447 6.736-2.419s8.151-2.428 8.151-7.424"></svg:path>`,
})
export class ArcticonsUpliveIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUploadgramIcon],svg[arcticons-uploadgram-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.075 19.36c.516-.02.907.447.698 1.287l-2.518 11.866c-.176.843-.73 1.117-1.391.655c-1.585-1.107-6.005-4.436-6.005-4.436a.182.182 0 0 1-.005-.292l6.942-6.265l-9.185 5.33a.2.2 0 0 1-.18.02l-3.696-1.153c-.82-.251-.825-.814.184-1.219l14.784-5.701a1 1 0 0 1 .372-.092"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.412 11c5.293-.29 10.8 2.76 12.63 9.75c10.375 1.408 9.484 15.304.06 16.27H13.275c-10.94-1.473-12.436-17.4 0-19.543A12.03 12.03 0 0 1 23.412 11m0 0"></svg:path>`,
})
export class ArcticonsUploadgramIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpnoteIcon],svg[arcticons-upnote-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.93 34.768c9.433-5.334 16.665-5.31 26.14 0L25.212 10.981c-.624-1.252-1.85-1.234-2.426-.075z"></svg:path>`,
})
export class ArcticonsUpnoteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpnpExplorerIcon],svg[arcticons-upnp-explorer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.598 28.687v-3.865a2.35 2.35 0 0 0-2.343-2.342h0a2.35 2.35 0 0 0-2.342 2.342m0 3.865v-6.324m6.988 6.295v-9.37h3.045c1.757 0 3.162 1.406 3.162 3.162s-1.405 3.163-3.162 3.163h-3.045m-15.499 3.074v-9.37h3.045c1.757 0 3.163 1.406 3.163 3.163s-1.406 3.162-3.163 3.162h-3.045m-8.51-6.324v6.207c0 1.757 1.405 3.162 3.045 3.162s3.162-1.406 3.162-3.162v-6.207"></svg:path>`,
})
export class ArcticonsUpnpExplorerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpsIcon],svg[arcticons-ups-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 43.5c1.96-2.113 16.32-3.605 16.32-16.515V8.145S34.489 4.5 24 4.5S7.68 8.144 7.68 8.144v18.841C7.68 39.895 22.04 41.387 24 43.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.32 9.055c-17.854 0-22.974.795-32.64 10.119m3.82 1.269v5.41a3.28 3.28 0 0 0 3.279 3.279h0a3.28 3.28 0 0 0 3.279-3.279v-5.41m0 5.41v3.279m12.842-.733a3.7 3.7 0 0 0 2.697.733h.735A2.17 2.17 0 0 0 36.5 26.96h0a2.17 2.17 0 0 0-2.167-2.172H32.86a2.17 2.17 0 0 1-2.167-2.172h0a2.17 2.17 0 0 1 2.167-2.173h.736a3.7 3.7 0 0 1 2.696.733m-15.009 4.677a3.28 3.28 0 0 0 3.279 3.279h0a3.28 3.28 0 0 0 3.278-3.279v-2.131a3.28 3.28 0 0 0-3.279-3.279h0a3.28 3.28 0 0 0-3.279 3.279m.001-3.279v13.115"></svg:path>`,
})
export class ArcticonsUpsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpscalerIcon],svg[arcticons-upscaler-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.5 24A21.5 21.5 0 1 1 24 2.5A21.5 21.5 0 0 1 45.5 24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.356 24H24v13.644H10.356ZM2.5 24H24m0-21.5v43"></svg:path>`,
})
export class ArcticonsUpscalerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpsideIcon],svg[arcticons-upside-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 42.641c-10.201 0-18.5-8.298-18.5-18.5v-9.92h8.816v9.92c0 5.34 4.344 9.684 9.684 9.684s9.684-4.343 9.684-9.684v-9.92H42.5v9.92c0 10.202-8.299 18.5-18.5 18.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.777 23.081V5.359H42.5z"></svg:path>`,
})
export class ArcticonsUpsideIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUpstoxIcon],svg[arcticons-upstox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.23 22.579v2.765a4.73 4.73 0 0 0 4.732 4.731h0a4.73 4.73 0 0 0 4.73-4.73v-7.807m0 7.806v4.731m5.346-4.731a4.73 4.73 0 0 0 4.731 4.731h0a4.73 4.73 0 0 0 4.731-4.73v-3.076a4.73 4.73 0 0 0-4.73-4.73h0a4.73 4.73 0 0 0-4.732 4.73m0-4.731v18.924M7.5 22.27a4.73 4.73 0 0 0 4.73-4.732"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsUpstoxIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUptimerobotIcon],svg[arcticons-uptimerobot-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="15.659" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="9.855" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUptimerobotIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUptionIcon],svg[arcticons-uption-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.608 25.117c-2.778.888-2.284.542-2.418-.457c-9.289 1.298-16.4 11.359-12.174 15.794l.996 1.046l-1.583-.408c-10.701-2.76-1.16-18.42 11.975-20.913c0 0 .414-.044.135-.556c-1.055-1.931 5.501 1.926 7.255 4.532c.41.608-1.813.203-4.186.961zm-9.194 6.141c-3.623 3.229-6.051 10.005 1.08 9.704c-.961-3.708.551-7.403 4.506-8.507c2.442-.681 3.633.943 3.402 2.463l-.196 1.284c-.065 1.009 1.986-1.893 2.438-3.265c1.861-5.645-4.996-7.233-11.23-1.679m6.602-14.42c-9.903 1.294-18.236 9.88-19.515 15.02m-2.849-1.974c1.72-7.687 11.342-15.204 18.09-16.224m-15.21 6.21c.634.567 1.055.943.355 1.277s-2.324.786-4.26 2.27c-1.553 1.19 1.316-6.773 1.916-7.16c.15-.097.396.837.77.422c1.657-1.844 13.626-12.466 21.368-5.21c-10.31-2.478-20.84 7.782-20.149 8.4zM8.158 17.3c.548-5.486 8.046-10.494 12.398-10.8"></svg:path>`,
})
export class ArcticonsUptionIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUptodateIcon],svg[arcticons-uptodate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.755 12.31A14.11 14.11 0 0 0 9.747 24m6.22 11.699A14.11 14.11 0 0 0 37.964 24m-26.543 0h3.955l-5.438 5.438L4.5 24h3.955zm25.158 0h-3.955l5.438-5.438L43.5 24h-3.955zm-18.248.176l4.329 4.329l9.009-9.01"></svg:path>`,
})
export class ArcticonsUptodateIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUptodownAppStoreIcon],svg[arcticons-uptodown-app-store-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.482 5.496c-3.9 0-6.84 3.1-7 7l-.98 23c-.2 3.9 3.1 7 7 7h23c3.9 0 7.16-3.1 7-7l-.98-23c-.2-3.9-3.1-7-7-7c0 1.1-.086 3.403-.87 4.903c-1 1.9-3 3.1-5.2 3.1h-9c-2.3 0-4.2-1.3-5.2-3.1c-.79-1.4-.77-3.803-.77-4.903m-.382 0h21.04"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m14.1 27.792l3.3-3.5l3.3 3.5l3.3-3.5l3.3 3.5l3.3-3.5l3.3 3.5l-10 9.5z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 18.31l2.99 2.99L24 24.293L21.007 21.3z"></svg:path>`,
})
export class ArcticonsUptodownAppStoreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUqMobileIcon],svg[arcticons-uq-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 42.5h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 27.64a66.6 66.6 0 0 1 29 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.036 12.702v9.091c0 2.487 1.987 4.504 4.437 4.504s4.437-2.016 4.437-4.504v-9.091m8.62 0a4.44 4.44 0 0 1 4.437 4.447v4.7a4.44 4.44 0 0 1-4.437 4.447h0a4.44 4.44 0 0 1-4.438-4.447v-4.7a4.44 4.44 0 0 1 4.438-4.447m0 11.141l2.985 4.56M11.5 32.68c0-.878.71-1.588 1.587-1.588h0c.877 0 1.588.71 1.588 1.587v2.62M11.5 31.092v4.206m3.175-2.618c0-.878.71-1.588 1.587-1.588h0c.877 0 1.587.71 1.587 1.587v2.62"></svg:path><svg:circle cx="29.307" cy="29.147" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.307 31.092v4.206m1.663-6.349v5.556c0 .438.355.793.793.793H32m4.292-.801a1.59 1.59 0 0 1-1.38.801h0c-.876 0-1.587-.71-1.587-1.587v-1.032c0-.876.711-1.587 1.588-1.587h0c.876 0 1.587.71 1.587 1.587v.516h-3.175m-8.833-.515c0-.878.711-1.588 1.588-1.588h0c.876 0 1.587.71 1.587 1.587v1.032c0 .876-.71 1.587-1.587 1.587h0c-.877 0-1.588-.71-1.588-1.587m0 1.587v-6.349m-4.876 3.731a1.587 1.587 0 0 1 3.175 0v1.03a1.587 1.587 0 0 1-3.175 0z"></svg:path>`,
})
export class ArcticonsUqMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrQuanMastersIcon],svg[arcticons-ur-quan-masters-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.151 8.848L36.88 6.577l-8.634 1.022l-4.901 3.87l.67 4.393l-7.529 7.529l-1.49-1.491l5.645-5.646l-4.04-4.04l-4.196 4.196l-.406-.405l-5.094 5.094v1.243s-3.17 4.313 0 7.484"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m6.906 29.826l5.828-5.828l1.52 1.52l-2.942 2.944l.687.687l-.95.95H7.606a4.3 4.3 0 0 1-.903 3.37c-1.249 1.249 1.852-.246 4.964 2.865"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.98 34.338s-1.277 2.004 1.2 4.482m27.676-27.676l-1.629-1.63l-5.999.425m4.759 4.074l-.682-.682h-4.759l1.294 4.829l-9.338 9.338"></svg:path><svg:circle cx="18.914" cy="29.086" r="2.061" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.521 27.567l-1.601-1.601M7.713 24.77l9.273-9.272m6.359-4.029l3.109 1.214m-2.439 3.179l3.456 1.185m9.849-10.03l1.513-1.513m-21.753 7.19a2.06 2.06 0 1 1 2.914 2.913m19.157-6.759l2.272 2.272l-1.022 8.634l-3.87 4.901l-4.394-.67l-7.529 7.53l1.49 1.49l5.646-5.646l4.04 4.04l-4.196 4.196l.406.406l-5.093 5.093h-1.244s-4.312 3.171-7.483 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.174 41.094l5.828-5.828l-1.52-1.521l-2.944 2.943l-.687-.687l-.95.95v3.443a4.3 4.3 0 0 0-3.37.903c-1.25 1.25.246-1.851-2.866-4.963"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.662 40.02s-2.004 1.277-4.482-1.2m27.676-27.676l1.629 1.628l-.424 6m-4.074-4.759l.682.682v4.759L29.84 18.16l-9.338 9.338"></svg:path><svg:circle cx="18.914" cy="29.086" r="2.061" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m20.433 30.479l1.601 1.601m1.196 8.207l9.273-9.272m4.027-6.36l-1.213-3.109m-3.179 2.439l-1.185-3.456m10.03-9.849l1.513-1.513m-7.19 21.753a2.06 2.06 0 0 0-2.913-2.914"></svg:path>`,
})
export class ArcticonsUrQuanMastersIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrbanCompanyIcon],svg[arcticons-urban-company-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.5 26.307v.058a4.64 4.64 0 0 1-4.642 4.642h0a4.64 4.64 0 0 1-4.641-4.642v-4.73a4.64 4.64 0 0 1 4.641-4.641h0a4.64 4.64 0 0 1 4.642 4.642v.057m-23-4.699v9.37a4.642 4.642 0 1 0 9.283 0v-9.37"></svg:path>`,
})
export class ArcticonsUrbanCompanyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrbanDictionaryIcon],svg[arcticons-urban-dictionary-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.513 23.575a4.5 4.5 0 0 0-4.5-4.5h0a4.5 4.5 0 0 0-4.5 4.5V26.5a4.5 4.5 0 0 0 4.5 4.5h0a4.5 4.5 0 0 0 4.5-4.5m1.8 4.5a1.8 1.8 0 0 1-1.8-1.8V13M22.041 26.5a4.5 4.5 0 0 1-4.5 4.5h0a4.5 4.5 0 0 1-4.5-4.5v-7.425M23.841 31a1.8 1.8 0 0 1-1.8-1.8V19.075m-9 0h-1.354m10.354 0h-1.354M34.513 13h-1.354"></svg:path>`,
})
export class ArcticonsUrbanDictionaryIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrbandroidIcon],svg[arcticons-urbandroid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.52 25a105.6 105.6 0 0 0-22.39 2.22c-2.65.57-4.75 1.16-6.31 1.63c-1.19.29-1.73.49-2.34.68v12a2 2 0 0 0 1.95 1.95l27.16.1a2 2 0 0 0 1.95-2v-16.6Zm-23.07 8.47l-.18 2.74l2.15 1.69l-2.63.68l-.95 2.57l-1.46-2.31l-2.73-.11l1.75-2.12l-.75-2.61l2.56 1Zm15.47-5.81A4.87 4.87 0 1 1 30.09 37a4.86 4.86 0 0 0 0-8.92a5 5 0 0 1 1.83-.41Zm-6.23 1.75l-.19 2.83L27.74 34l-2.74.67l-1 2.63l-1.5-2.37l-2.8-.11l1.79-2.15l-.72-2.67l2.6 1Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.52 25v-2a15.36 15.36 0 0 0-6.82-11.49l2.92-5.21a1.16 1.16 0 0 0-.48-1.64a1.24 1.24 0 0 0-1.66.46l-2.82 5.22a15.74 15.74 0 0 0-13.24 0L14.6 5.12a1.38 1.38 0 0 0-1.27-.61a.9.9 0 0 0-.39.15a1.17 1.17 0 0 0-.49 1.64l2.93 5.21a15.6 15.6 0 0 0-6.92 13v5m12.64-12.8a3.41 3.41 0 0 1-6.82 0m19.47 0a3.41 3.41 0 1 1-6.81 0"></svg:path>`,
})
export class ArcticonsUrbandroidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrbansportsIcon],svg[arcticons-urbansports-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25 4.5H10.8v25.8c0 7.3 5.9 13.2 13.2 13.2s13.2-5.9 13.2-13.2V17.7m0-5.3V4.5h-7.6"></svg:path>`,
})
export class ArcticonsUrbansportsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrecordIcon],svg[arcticons-urecord-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.89 13.41v14a7.11 7.11 0 1 0 14.22 0v-14"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" d="M31.11 27.69a5.43 5.43 0 0 1 5.54-5.28h0m-5.54 0v14"></svg:path>`,
})
export class ArcticonsUrecordIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrformsIcon],svg[arcticons-urforms-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.2 25.4c0-2.2 1.8-3.9 3.9-3.9h0m-3.9-.1v10.5M14.9 16.1v10.5c0 2.9 2.3 5.2 5.2 5.2s5.2-2.3 5.2-5.2V16.1"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29c-2.2 0-4 1.8-4 4v29c0 2.2 1.8 4 4 4h29c2.2 0 4-1.8 4-4v-29c0-2.2-1.8-4-4-4"></svg:path>`,
})
export class ArcticonsUrformsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrinoxIcon],svg[arcticons-urinox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.543 18.204a1.792 1.792 0 0 1 2.482-2.585l8.615 8.275a1.792 1.792 0 0 1-2.482 2.584L18.578 41.66a5.973 5.973 0 1 1-8.625-8.264l.01-.01Z"></svg:path><svg:circle cx="36.53" cy="14.297" r="3.178" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="31.422" cy="6.747" r="2.247" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m22.808 20.01l3.74 3.593m-7.736.567l3.74 3.592m-7.735.568l3.74 3.592m-7.736.568l3.74 3.592"></svg:path>`,
})
export class ArcticonsUrinoxIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrlCheckerIcon],svg[arcticons-url-checker-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.852 32.708a8.3 8.3 0 0 1-7.236-4.355a9.01 9.01 0 0 1 .009-8.704a8.3 8.3 0 0 1 7.245-4.338m0-.001l9.001-.035m-9.457 17.46l8.639.09m14.095-17.651a8.3 8.3 0 0 1 7.236 4.355a9.01 9.01 0 0 1-.009 8.704a8.3 8.3 0 0 1-7.245 4.338m.09-17.369l-9 .036m8.91 17.333l-8.64-.09m-10.984-8.749h16.733v.182H15.506z"></svg:path>`,
})
export class ArcticonsUrlCheckerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrlForwarderIcon],svg[arcticons-url-forwarder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.905 20.033H9.093L4.5 27.989h3.812ZM28.182 9.024v10.784h-3.811l-4.594 7.956h8.405v11.212L43.5 23.844Zm-7.75 11.009H16.62l-4.593 7.956h3.811Z"></svg:path>`,
})
export class ArcticonsUrlForwarderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrlRadioIcon],svg[arcticons-url-radio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.006 11.676H41.99a1.51 1.51 0 0 1 1.509 1.508V34.82a1.506 1.506 0 0 1-1.505 1.505H6.009A1.51 1.51 0 0 1 4.5 34.816V13.18a1.506 1.506 0 0 1 1.505-1.505"></svg:path><svg:ellipse cx="15.278" cy="23.787" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="7.355" ry="7.339"></svg:ellipse><svg:circle cx="27.985" cy="30.376" r="1.417" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="33.349" cy="30.376" r="1.417" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="38.723" cy="30.375" r="1.417" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.178 17.247h10.348a1.614 1.614 0 0 1 1.614 1.614v5.986a1.61 1.61 0 0 1-1.61 1.61H28.178a1.61 1.61 0 0 1-1.61-1.61v-5.99a1.61 1.61 0 0 1 1.61-1.61"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.54 20.187v3.331h1.669m-5.836 0v-3.331h1.085a1.124 1.124 0 1 1 0 2.248h-1.085m1.127-.041l1.085 1.124m-6.087-3.332v2.207a1.086 1.086 0 1 0 2.17 0v-2.207"></svg:path>`,
})
export class ArcticonsUrlRadioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUrlsanitizerIcon],svg[arcticons-urlsanitizer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m23.055 14.212l6.942-6.943c2.679-2.678 7.539-2.162 10.217.517s3.195 7.538.517 10.217L30.448 28.286c-2.679 2.678-7.538 2.162-10.217-.517"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24.945 33.789l-6.942 6.942c-2.679 2.678-7.539 2.162-10.217-.517s-3.195-7.538-.517-10.217l10.283-10.283c2.679-2.678 7.539-2.162 10.217.517"></svg:path>`,
})
export class ArcticonsUrlsanitizerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsBankIcon],svg[arcticons-us-bank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 29.175L24 40.646L5.5 29.175V7.354h37z"></svg:path>`,
})
export class ArcticonsUsBankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsMobileIcon],svg[arcticons-us-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.35 29.97c1.872.893 3.954 1.334 6.06 1.72l-5.25-9.094h13.624"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m10.645 26.576l1.81 1.426l8.638 1.966M7.401 23.25l1.326 1.482l8.84 1.844M5.095 19.717l.888 1.525l7.895 2.008m12.908-1.955l-.96-4.985h-4.265c-.709 0-.808.536-.84.946c.795-.734 1.887-.487 2.534 1.761l-1.44 2.694c-2.165-1.404-3.477-3.18-3.76-5.398H3.5l.801 1.59l4.054 1.814M31.65 29.97c-1.872.893-3.954 1.334-6.06 1.72l5.25-9.094"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m37.356 26.576l-1.812 1.426l-8.637 1.966m13.692-6.718l-1.326 1.482l-8.84 1.844m12.472-6.859l-.888 1.525l-7.895 2.008m-7.336-1.955c1.808-1.335 2.904-2.977 3.159-4.983H44.5l-.801 1.59l-4.054 1.815M19.53 26.702h8.873m-6.311.121v4.352m3.816-4.352v4.352m-5.146-6.556v-.927m0 .927l.881-.286m-.881.286l.544.751m-.544-.751l-.546.749m.546-.749l-.881-.288m4.085.288v-.927m0 .927l.882-.286m-.882.286l.544.751m-.544-.751l-.546.749m.546-.749l-.881-.288m4.086.288v-.927m0 .927l.881-.286m-.881.286l.544.751m-.544-.751l-.546.749m.546-.749l-.881-.288"></svg:path>`,
})
export class ArcticonsUsMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsaSwimmingIcon],svg[arcticons-usa-swimming-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m36.349 27.934l2.017.717m-26.354-.717l-2.017.717m6.886-2.168l-3.635 5.914m8.008-6.562l-1.752 10.593m4.676-8.813l.869 2.676h2.814l-2.276 1.654l.869 2.676l-2.276-1.654l-2.277 1.654l.87-2.676l-2.277-1.654h2.814z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.18 9.521c5.743 3.88 12.056 1.103 14.186-.57v10.877H43.5l-2.053 9.918l-3.081-1.095v2.51c0 2.453-9.318 8.861-14.186 9.888c-4.868-1.027-14.185-7.435-14.185-9.888v-2.51L4.5 30.603l1.35-10.775h4.145V8.95c2.13 1.673 8.443 4.45 14.185.57"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.012 27.934c1.584 1.44 3.574.595 4.869-1.451c1.239 1.69 3.697 1.126 4.373-.648c.6 1.614 1.952 1.78 2.924 1.78m7.297-1.132l3.634 5.914m-8.008-6.562l1.752 10.593m7.49-8.494c-1.584 1.44-3.573.595-4.868-1.451c-1.24 1.69-3.698 1.126-4.374-.648c-.6 1.614-1.952 1.78-2.923 1.78m-1.427-8.901c.36.468.811.643 1.44.643h.868c.809 0 1.464-.656 1.464-1.464v-.007c0-.808-.655-1.464-1.464-1.464h-.958c-.81 0-1.466-.656-1.466-1.466h0c0-.81.658-1.469 1.469-1.469h.864c.628 0 1.08.175 1.44.643m-9.658-.643v3.83c0 .954.643 1.838 1.581 2.007a1.946 1.946 0 0 0 2.307-1.911v-3.926m10.973 3.926H29.01m-.644 1.944l1.944-5.87l1.945 5.87m-14.039 1.929v3.227m12.045-3.227v3.227m-10.135-.004v-3.223l1.614 3.227l1.613-3.222v3.222m1.838-.004v-3.223l1.613 3.227l1.614-3.222v3.222m3.843.804v-4.031l2.67 4.031v-4.031m-18.54 0l-1.008 4.031l-1.007-4.031l-1.008 4.031l-1.008-4.031M7.515 26.12c.333.433.75.594 1.33.594h.804c.748 0 1.354-.606 1.354-1.354v-.006c0-.748-.606-1.354-1.354-1.354h-.886a1.356 1.356 0 0 1-1.356-1.355h0c0-.75.609-1.359 1.359-1.359h.8c.58 0 .997.162 1.33.595m29.384 1.203c0-.993-.805-1.798-1.798-1.798h0c-.993 0-1.798.805-1.798 1.798v1.832c0 .993.805 1.798 1.798 1.798h0c.993 0 1.798-.805 1.798-1.798h-1.798"></svg:path>`,
})
export class ArcticonsUsaSwimmingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsaaIcon],svg[arcticons-usaa-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.842 10.651c1.38-1.096 1.809-.57 2.796 0l9.868 5.698l2.546-10.782a9.6 9.6 0 0 1 2.112 0a2.7 2.7 0 0 1 1.016.622a4.2 4.2 0 0 1 1.284.28a14 14 0 0 1 1.44.7c-.524-.066-.875-.162-1.57-.196a2.47 2.47 0 0 0-1.37.63a1.44 1.44 0 0 0-.41 1.322c.212 1.312.397 1.854.927 2.256l10.957 6.627a2.6 2.6 0 0 0 2.72 0M7.842 14.893c1.38-1.097 1.99-.57 2.978 0c3.29 1.899 5.844 3.479 9.133 5.378M7.842 19.134c1.38-1.097 1.988-.57 2.975 0c3.29 1.899 5.294 3.16 8.583 5.059m-11.558-.818c1.38-1.097 2.065-.57 3.052 0c3.29 1.899 4.664 2.84 7.953 4.74"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m27.135 15.838l10.102 6.167a3.04 3.04 0 0 0 2.921 0m-12.37-1.511l9.562 5.708a2.79 2.79 0 0 0 2.809 0M28.442 25.15l8.888 5.25a2.83 2.83 0 0 0 2.827 0M8.548 32.676v6.508A3.28 3.28 0 0 0 11.74 42.5a3.28 3.28 0 0 0 3.192-3.316v-6.508m2.053 8.708a2.95 2.95 0 0 0 2.473 1.113h1.483a2.48 2.48 0 0 0 2.473-2.473h0a2.48 2.48 0 0 0-2.473-2.473h-1.607a2.48 2.48 0 0 1-2.473-2.473h0a2.48 2.48 0 0 1 2.473-2.473h1.484a2.66 2.66 0 0 1 2.472 1.113m2.732 5.46h4.142m3.974 0h4.141m-5.36 3.294l3.324-9.848l3.2 9.848m-14.66 0l3.323-9.848l3.201 9.848"></svg:path>`,
})
export class ArcticonsUsaaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsafootballIcon],svg[arcticons-usafootball-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M8.222 30.008c13.146.262 24.542-3.134 31.157-16.627"></svg:path><svg:path d="M8.222 32.974c16.973-.262 24.542-4.005 29.333-16.187"></svg:path><svg:path d="M8.877 36.97c14.53-.656 22.765-6.204 27.377-13.053c0 0-1.216 4.481-7.223 9.796S9.44 40.235 9.44 40.235"></svg:path></svg:g><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M37.115 4.511C17.363 3.987 2.393 21.746 10.402 43.5c24.804-.346 34.638-22.456 26.713-38.989"></svg:path><svg:path d="M10.14 27.66s1.001-1.694 1.525-2.087l1.694 2.526l-.084-4.004s1.955-1.825 3.218-2.311c0 0-2.264-.44-3.181-.085c0 0 .178-2.657.477-3.087c0 0-1.787 2.563-2.049 3.658c0 0-1.478.309-1.918.74l1.179 1.216c.01 0-1.17 2.825-.86 3.433m10.263-5.174c1.132-1.562 3.181-3.05 3.181-3.05s-1.955-2.564-2.694-2.788c0 0 3.35.084 4.921 1.085l3.434-2.049s-.44 2.349-.692 3.134c0 0 2.61.955 3.527 2.433l-4.397-.262s-.609 2.826-1.133 3.659c0 0-1.693-3.182-1.693-3.434c-.01.01-4.014.832-4.454 1.272m-4.481-7.317c.916-.655 2.788-1.74 3.695-1.955l1.432 1.656l.786-2.741s3.565-1.133 4.744-1.133c0 0-2.695-.739-3.434-.785c0 0 1.562-1.872 2.002-2.05c0 0-3.658 1.217-4.267 2.218c0 0-2.217.047-2.956.524l.786 1.31zm13.669-2.778c.74-.478 2.742-2.05 2.742-2.05s-1.132-2.133-1.694-2.48c0 0 2.349.44 3.219 1.348l2.05-1.478l-.44 2.872s1.655 2.396 1.824 3.266c0 0-1.61-1.263-2.264-1.348l-.393 3.743l-1.479-3.92z"></svg:path></svg:g>`,
})
export class ArcticonsUsafootballIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsagedirectIcon],svg[arcticons-usagedirect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12 36.5v-25m6 25V16.937M24 36.5V13.928M30 36.5V22.267M36 36.5V16.73M3.199 18.545s12.285-1.676 18.408 6.135c6.721 8.575 11.802 3.652 17.17 4.154c4.212.394 5.765 1.532 5.765 1.532"></svg:path>`,
})
export class ArcticonsUsagedirectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsbAudioPlayerProIcon],svg[arcticons-usb-audio-player-pro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.87 31.46v4.72c0 3.983 4.174 6.587 7.751 4.835l24.864-12.18c4.02-1.97 4.02-7.7 0-9.67L14.62 6.985c-3.576-1.752-7.752.852-7.752 4.835v4.72"></svg:path><svg:circle cx="7.471" cy="23.627" r="1.971" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.442 23.627h10.3m-5.054-3.895H12.51l-1.463 3.895l2.496 4.641h2.162m7.951-4.641l-3.914-2.26v4.519z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.705 27.012h2.511v2.511h-2.511z"></svg:path><svg:circle cx="16.102" cy="19.732" r="1.415" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUsbAudioPlayerProIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsbBackupIcon],svg[arcticons-usb-backup-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 22.298h5.997M4.5 13.95h8.832M4.5 30.644h7.155m14.33-13.814l3.497-3.453l3.497 3.453m2.117 6.2v-3.414h3.413v3.414zl-5.659 1.689V13.503m0 16.26a2.71 2.71 0 1 0-.004 5.42h.004a2.71 2.71 0 1 0 0-5.42v-3.231m0-1.813v1.813L23.21 24.02a2.05 2.05 0 0 1-3.8-1.074a2.051 2.051 0 1 1 3.8 1.074M43.5 24c0 8.044-6.521 14.566-14.566 14.566h0c-8.045 0-14.566-6.522-14.566-14.566h0c0-8.045 6.521-14.566 14.566-14.566h0C36.98 9.434 43.5 15.955 43.5 24"></svg:path>`,
})
export class ArcticonsUsbBackupIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsbkeyboardIcon],svg[arcticons-usbkeyboard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="12.13" height="12.13" x="4.5" y="11.87" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:rect width="12.13" height="12.13" x="16.63" y="11.87" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:rect width="12.13" height="12.13" x="28.77" y="11.87" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:rect width="12.13" height="12.13" x="7.1" y="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:rect width="12.13" height="12.13" x="19.23" y="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:rect width="12.13" height="12.13" x="31.37" y="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.73"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.65 21.43h2.1m-2.1-7h2.1m-1.05 0v7m-11.28 5.14h3.5v5.25a1.76 1.76 0 0 1-1.75 1.75h0a1.75 1.75 0 0 1-1.75-1.75v-.58m12-4.67v7m3.76 0l-2.88-3.5l2.88-3.48m-2.88 3.48h-.88m9.09-10.96a2.32 2.32 0 0 0 4.64 0v-2.36a2.32 2.32 0 0 0-4.64 0ZM8.25 14.43v4.68a2.32 2.32 0 0 0 4.64 0v-4.68m22.79 12.14v7h3.5"></svg:path>`,
})
export class ArcticonsUsbkeyboardIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUsbvendevdatabaseIcon],svg[arcticons-usbvendevdatabase-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.6 17.1L6 20.3l8.4 12.1l12.7-5.1Zm-4.2 15.3l-1.6 3.5l12.6-4.7l1.8-3.9M6 20.3l-1.5 3.8l8.3 11.8"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.9 31.8a2.22 2.22 0 0 0 2.5 1.3l4.1-2c2-2.1 1.4-3.8.9-5.5l-9.6-11.4l-4.5 1.2c-1.8.8-2 1.3-1.7 2.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m21.7 14.2l8.4-2.1a3.68 3.68 0 0 1 3 .5l7.6 7.1c.9 1.4 1.8 2.7-.4 5.7l-9.9 5.7"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m35.1 14.5l4.8-1.5l3.5 2.5l.1 2.9l-2.6 1.6m2.5-4.6l-4.7 2.4m2.5 2.9l-10.5 4.4l-4 1.9M5.6 22.3l7.9 11.3m13.6-6.2c1.7 1.7.4 4.4-1.2 5.8m-7.3-16.1c-.5-.8-1.1-1.5-1.9-1.5"></svg:path>`,
})
export class ArcticonsUsbvendevdatabaseIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUserManualIcon],svg[arcticons-user-manual-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.4 6.5v35a2 2 0 0 0 2 2h2.33v-39H10.4a2 2 0 0 0-2 2m4.33-2v39H37.6a2 2 0 0 0 2-2v-35a2 2 0 0 0-2-2Z"></svg:path><svg:ellipse cx="25.531" cy="33.668" fill="currentColor" rx=".823" ry=".75"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.301 20.049a6.43 6.43 0 0 1 1.959-4.763a7.27 7.27 0 0 1 5.224-1.786c3.918 0 7.183 2.977 7.183 6.549a6.43 6.43 0 0 1-1.959 4.762c-1.632 1.19-5.224 2.977-5.224 5.656"></svg:path>`,
})
export class ArcticonsUserManualIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUserlandIcon],svg[arcticons-userland-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 3.5l17.754 10.25v20.5L24 44.5L6.246 34.25v-20.5z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.209 34.565c-1.092-10.158-5.286-22.632-10.459-22.632c-3.7 0-4.878 1.794-6.75 5.103c-1.872-3.309-3.05-5.103-6.75-5.103c-5.173 0-9.366 12.474-10.458 22.632"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 30.325l5.733-4.483a.53.53 0 0 0 .008-.837A9.18 9.18 0 0 0 24 23.25a9.18 9.18 0 0 0-5.741 1.757a.53.53 0 0 0 .008.836Z"></svg:path>`,
})
export class ArcticonsUserlandIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUspsMobileIcon],svg[arcticons-usps-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.527 20.212q-1.2.516-3.418.586M5.5 11.114c8.857 2.162 26.571 4.414 27.459 7.284c-4.664-1.95-11.303-1.5-27.459-1.846m0 20.386c10.534-4.976 23.708-11.733 30.65-12.062l1.12.715c.963-1.624 1.478-2.88 1.731-3.918c.581-2.383-.2-3.275-1.572-3.275h-4.47a1.8 1.8 0 0 1-.157.713H11.7l3.343 10.172c5.853-3.522 15.462-6.97 20.595-6.156m-2.837-4.016q-.317.69-1.275 1.101h4.55c1.79 0 1.182 2.048 1.018 2.359M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsUspsMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUtdRoomsIcon],svg[arcticons-utd-rooms-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m13.249 4.518l18.133 2.57l.007 36.412l-18.144-2.57Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m13.245 4.5l21.51.073l-.073 35.781l-3.22.074"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.594 26.018c.058 1.54-.644 2.547-1.375 2.266c-.594-.23-1.014-1.223-1.042-2.276c-.027-1.039.282-2.225.903-2.298c.839-.099 1.457.796 1.514 2.308"></svg:path>`,
})
export class ArcticonsUtdRoomsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUtmIcon],svg[arcticons-utm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:rect width="26.429" height="26.429" x="10.786" y="10.786" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="3.548" ry="3.548"></svg:rect><svg:rect width="15.857" height="15.857" x="16.071" y="16.071" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="3.095" ry="3.095"></svg:rect><svg:circle cx="24" cy="24" r="2.643" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUtmIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUtorrentIcon],svg[arcticons-utorrent-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.759 15.732v10.24a6.24 6.24 0 0 0 6.185 6.296H24a6.24 6.24 0 0 0 6.242-6.24V15.73m-.001 10.297v6.242m-12.482-6.298V40.21"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsUtorrentIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUtsIcon],svg[arcticons-uts-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.325 12.919h19.349v7.854H14.325zm8.024-4.593h3.302"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.11 6.42h-1.686l-.318-.804a1.945 1.945 0 0 0-1.808-1.228h-6.596c-.797 0-1.514.487-1.808 1.228l-.318.803H16.89a6.763 6.763 0 0 0-6.764 6.763h0v16.19a5.936 5.936 0 0 0 5.936 5.936h15.874a5.936 5.936 0 0 0 5.936-5.936h0v-16.19a6.763 6.763 0 0 0-6.764-6.763M18.824 35.308l-7.177 8.08m22.028-2.934h-19.35m17.044-2.663H16.631m12.545-2.483l7.177 8.08M21.881 23.482h4.637m-2.318 7v-7"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.176 29.715c.43.56.967.767 1.716.767h1.036c.965 0 1.747-.781 1.747-1.746v-.007c0-.965-.782-1.747-1.746-1.747h-1.144a1.75 1.75 0 0 1-1.748-1.748h0c0-.967.784-1.752 1.752-1.752h1.03c.75 0 1.288.209 1.717.767m-19.211-.767v4.682a2.319 2.319 0 1 0 4.638 0v-4.682"></svg:path>`,
})
export class ArcticonsUtsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUuid0xfd6fscannerIcon],svg[arcticons-uuid0xfd6fscanner-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.68 21.092c.715 1.85.715 4.199 0 6.05m4.01 3.952c2.677-4.068 2.677-9.886 0-13.954M13.303 31.07l13.872-13.802l-6.779-6.768v27l6.779-6.768L13.303 16.93"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 42.5h-29c-2.2 0-4-1.8-4-4v-29c0-2.2 1.8-4 4-4h29c2.2 0 4 1.8 4 4v29c0 2.2-1.8 4-4 4"></svg:path>`,
})
export class ArcticonsUuid0xfd6fscannerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUvIndexIcon],svg[arcticons-uv-index-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.5 24c0 11.874-9.626 21.5-21.5 21.5S2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.567 24c0 9.15-7.417 16.567-16.567 16.567h0C14.85 40.567 7.433 33.15 7.433 24q0 0 0 0C7.433 14.85 14.85 7.433 24 7.433h0c9.15 0 16.567 7.417 16.567 16.567"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.22 24c0 6.197-5.023 11.22-11.22 11.22h0c-6.197 0-11.22-5.023-11.22-11.22h0c0-6.197 5.023-11.22 11.22-11.22h0c6.197 0 11.22 5.023 11.22 11.22"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m29.968 20.555l-2.3 6.89l-2.298-6.89m-7.338 0v4.608a2.29 2.29 0 0 0 2.298 2.282h.002a2.29 2.29 0 0 0 2.298-2.282h0v-4.608"></svg:path>`,
})
export class ArcticonsUvIndexIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUxLedIcon],svg[arcticons-ux-led-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.5 17.742v8.37c0 2.29 1.793 4.146 4.01 4.146c2.215 0 4.012-1.856 4.012-4.146v-8.37m10.978 0l-8.018 12.516m0-12.516L33.5 30.258"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsUxLedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUyapDokumanIcon],svg[arcticons-uyap-dokuman-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 42.5C13.783 42.5 5.5 34.218 5.5 24S13.783 5.5 24 5.5S42.5 13.783 42.5 24v18.5z"></svg:path><svg:circle cx="21.056" cy="22.094" r="11.967" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="35.368" cy="36.076" r="3.466" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.36 19.043v4.042c0 1.144.916 2.06 1.983 2.06s1.983-.916 1.983-2.06v-4.042m8.375 3.051h1.983m1.067 3.051h-3.05v-6.102h3.05m-9.175 6.101v-6.101h1.373c1.449 0 2.67 1.22 2.67 2.669v.763c0 1.449-1.221 2.669-2.67 2.669z"></svg:path>`,
})
export class ArcticonsUyapDokumanIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUyapMevzuatIcon],svg[arcticons-uyap-mevzuat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M3.5 26.66a7 7 0 0 0 2.506 2.506a7 7 0 0 0 3.416.946a7 7 0 0 0 3.516-.904c1.065-.604 1.791-1.49 2.407-2.549"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M3.5 26.66s4.005-.211 6.01-.21c1.946 0 5.835.21 5.835.21m8.567-2.356v7.542m0-21.296a36.7 36.7 0 0 0-16.395 3.882s10.899-1.755 16.395-1.755s16.396 1.755 16.396 1.755a36.7 36.7 0 0 0-16.396-3.883"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.422 26.45V14.14l-3.55 12.467m3.55-12.467l3.552 12.376m19.506.144a7 7 0 0 0 2.507 2.506a7 7 0 0 0 3.416.946a7 7 0 0 0 3.516-.904c1.065-.604 1.79-1.49 2.406-2.549"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.48 26.66s4.006-.211 6.01-.21c1.946 0 5.835.21 5.835.21"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.403 26.45V14.14l-3.551 12.31m3.551-12.31l3.551 12.31M21.731 14.354c-.435.045-.357.476-.298 1.273c.06.796.12 2.056-.251 3.685s-1.172 3.626-1.573 4.625c-.4 1 .337 1.305.572 1.554s.703.748 1.364 1.849c.66 1.1 1.514 2.803 2.367 4.506c.854-1.703 1.707-3.406 2.368-4.506s1.13-1.6 1.364-1.849s.973-.555.572-1.554c-.401-1-1.203-2.996-1.573-4.625a12 12 0 0 1-.251-3.685c.059-.797.115-1.13-.298-1.273c-1.451-.131-2.914-.15-4.363 0"></svg:path><svg:circle cx="23.912" cy="24.303" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.912 37.45c-1.792-1.225-3.585-2.45-5.536-2.691c-1.952-.24-4.062.504-6.172 1.248m9.176-2.177c-1.033-.492-2.065-.984-3.084-1.278c-1.019-.293-2.024-.387-3.029-.482m7.581 1.398c-.764-.899-1.528-1.798-2.301-2.463c-.774-.666-1.556-1.098-2.338-1.53m5.704 7.975c1.792-1.225 3.585-2.45 5.536-2.691s4.061.504 6.172 1.248m-9.175-2.177c1.032-.492 2.064-.984 3.083-1.278c1.019-.293 2.024-.387 3.029-.482m-7.581 1.398c.764-.899 1.528-1.798 2.301-2.463c.773-.666 1.556-1.098 2.338-1.53"></svg:path>`,
})
export class ArcticonsUyapMevzuatIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsUyapMobilIcon],svg[arcticons-uyap-mobil-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.83 21.11v11.862m11.259-16.266l-6.369 2.769v14.204m-7.44 2.271V19.474l-6.37-2.77"></svg:path><svg:circle cx="20.278" cy="37.567" r="1.136" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="27.721" cy="34.815" r="1.136" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.169 21.11v11.013"></svg:path><svg:circle cx="24" cy="15.303" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.169 13.074c.437.209.79.561 1 .998m1.579-.675a3.74 3.74 0 0 0-1.74-1.77m.412 4.073a2.454 2.454 0 1 1-2.864-2.806m15.863-3.597H8.579m0 0H39.42m-29.065 1.776a1.776 1.776 0 1 1-3.551 0a1.776 1.776 0 0 1 3.551 0m-5.301 8.683l2.848-7.004m1.337-.063l2.91 7.067m-7.649 0s2.311 3.465 4.101 3.465s4.101-3.465 4.101-3.465zm33.145-8.683a1.776 1.776 0 1 0 3.551 0a1.776 1.776 0 0 0-3.551 0m5.301 8.683l-2.849-7.004m-1.336-.063l-2.91 7.067m7.649 0s-2.311 3.465-4.101 3.465s-4.101-3.465-4.101-3.465z"></svg:path>`,
})
export class ArcticonsUyapMobilIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVAppstoreIcon],svg[arcticons-v-appstore-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.144 14.5v3.572c0 3.324 2.612 6 5.856 6h0c3.244 0 5.856-2.676 5.856-6v-3.573"></svg:path>`,
})
export class ArcticonsVAppstoreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVLauncherIcon],svg[arcticons-v-launcher-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m7.12 4.5l15.872 35.926M24 43.5l15.519-11.136L40.88 4.5L24 14.93L7.12 4.5l1.361 27.864z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.889 24.795L24 43.5L8.111 23.596m18.483 16.83h-5.048M40.88 4.5L25.008 40.426M24 14.93L8.111 23.596m31.778 1.199L24 14.93"></svg:path>`,
})
export class ArcticonsVLauncherIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVPointIcon],svg[arcticons-v-point-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 24L24 42.5L42.5 24M6.669 6.669L24 24L41.331 6.669"></svg:path>`,
})
export class ArcticonsVPointIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVPointPayIcon],svg[arcticons-v-point-pay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.299 41H8c-2.216 0-4-1.784-4-4V8c0-2.216 1.784-4 4-4h29c2.216 0 4 1.784 4 4v23.26"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4 22.5L22.5 41L41 22.5M5.169 5.169L22.5 22.5L39.831 5.169"></svg:path><svg:circle cx="37" cy="37" r="7" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.35 41v-8h2.619c1.48 0 2.681 1.203 2.681 2.687s-1.2 2.686-2.681 2.686H34.35"></svg:path>`,
})
export class ArcticonsVPointPayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVVnpayIcon],svg[arcticons-v-vnpay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linejoin="round" d="m28.622 37.722l14.445-14.444c.577-.578.577-1.733 0-2.311L34.4 12.3c-.578-.578-1.733-.578-2.311 0l-6.356 6.356L16.49 9.41c-.578-.578-1.734-.578-2.311 0l-9.245 9.245c-.578.577-.578 1.733 0 2.31L21.69 37.723c1.733 1.734 5.2 1.734 6.933 0Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.733 18.656l-8.089 8.089q-3.466 3.465-6.933 0"></svg:path><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M18.222 30.789q-1.732 1.734-3.467 0m22.534-15.6c-1.262-1.156-2.89-.578-4.045.578L18.222 30.789m0-15.022c-4.622-4.622-10.4 1.155-5.778 5.778l5.2 5.2l-5.2-5.2m10.978-.578l-4.044-4.045"></svg:path><svg:path d="m21.689 22.7l-4.622-4.622c-.578-.578-1.445-1.445-2.311-1.156m0 3.467c-.578-.578-1.445-1.444-1.156-2.311m5.778 6.933l-4.622-4.622"></svg:path></svg:g>`,
})
export class ArcticonsVVnpayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsV2rayngIcon],svg[arcticons-v2rayng-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.619 41.487V10.614H5.5V6.542h10.264v14.394c.696-.64 1.287-1.18 1.873-1.725L28.18 9.385c.942-.878 1.877-1.763 2.83-2.627c.144-.13.371-.238.56-.239c3.567-.013 10.93 0 10.93 0C31.545 18.156 20.628 29.793 9.619 41.488"></svg:path>`,
})
export class ArcticonsV2rayngIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsV380ProIcon],svg[arcticons-v380-pro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="19.052" r="7.91" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="19.297" r="14.797" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="16.369" r="2.282" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.496 33.895v5.276h10.809V43.5h-26.61v-4.329h10.809v-5.276"></svg:path>`,
})
export class ArcticonsV380ProIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVaaniIcon],svg[arcticons-vaani-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.721 23.772c10.546 4.502 15.06 11.367 18.305 18.728c3.563-8.286 6.99-16.7 16.253-19.583M35.49 5.614c.114 5.592-.015 11.65.314 16.305m3.793-10.632c.074 1.712-.29 4.378.228 5.13m-8.811-3.021c.304 1.218-.142 3.42 0 4.903m-3.821-9.72c.235 4.7-.493 8.41-.2 12.77"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.344 5.5c.143 7.012-.276 18.748.485 20.524m-4.02-13.027c.142 1.862-.4 4.019-.257 5.815M15.047 5.5c.228 7.012-.133 18.386.656 20.353m-5.018-13.17c.418 2.592-.29 5.152.342 7.469"></svg:path>`,
})
export class ArcticonsVaaniIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVacationDaysIcon],svg[arcticons-vacation-days-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.287 42.03a21.54 21.54 0 0 1 30.167-30.698m0 0L11.287 42.031"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.662 33.508C2.642 10.55 12.786 2.656 33.38 19.548m-3.728 9.834L42.5 42.2"></svg:path>`,
})
export class ArcticonsVacationDaysIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVakifbankIcon],svg[arcticons-vakifbank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.668 11.428c-9.666.022-11.772 3.417-13.406 12.555L4.5 36.573l28.103-.046c9.517.31 13.983-8.895 5.72-13.188c7.584-2.14 7.41-11.278-4.374-11.854l-13.28-.059z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.988 13.468q-6.024.14-7.483 8.236L7.559 34.536h4.213c1.242.015 2.652-.544 3.588-1.495l7.61-8.753c2.6-3.029 4.858-5.345 7.061-7.28a50 50 0 0 1 4.335-3.409c-3.276-.604-7.22 1.323-10.741 4.998L13.083 29.46l2.186-10.15q1.104-4.224 4.533-5.72a10.6 10.6 0 0 0-1.814-.122"></svg:path>`,
})
export class ArcticonsVakifbankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsValetudocompanionIcon],svg[arcticons-valetudocompanion-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.02 6.483a18.5 18.5 0 0 1-4.13 23.42C32.454 32.787 25.879 35.972 24 41.518c-2.064-5.51-8.451-8.73-11.891-11.616a18.5 18.5 0 0 1-4.13-23.42"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.409 17.974c0 4.644-4.526 7.01-8.409 9.53c-3.769-2.409-8.409-4.886-8.409-9.53a8.409 8.409 0 0 1 16.818 0"></svg:path><svg:circle cx="24" cy="17.974" r="2.803" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsValetudocompanionIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsValkyrieConnectIcon],svg[arcticons-valkyrie-connect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.264 31.996c-1.076 1.094-.568 2.223 1.146 2.672m-3.453-3.353c-.167 1.463-.08 2.926-1.171 4.39m-1.16-6.414c3.807 4.637 10.04 2.257 11.447.401m-.09-.165c0 1.584-.01 4.397-4.867 5.047c-10.37 1.391-8.669 5.231-8.419 6.624c2.266-3.275 4.87-3.22 7.381-4.095m10.098-12.12c.56 3.771 1.554 8.993-3.855 10.1c-7.117 1.458-10.045 3.964-10.598 8.32c3.596-2.507 7.909-2.681 12.003-2.987c5.364-.4 6.398-2.434 8.195-4.763c-.705 2.466-2.46 4.955-3.899 6.25c6.212.48 13.319-4.352 5.225-14.26c1.901 1.394 5.202 1.674 9.253-.076c-3.17-.693-6.5-.91-8.33-4.34c-.74-1.39-1.293-4.712-3.177-7.453m-19.304 4.82c.441-.13 1.331.037 1.776.666c.493.697 1.89.642 2.348.01c.245-.339.573-.421.903-.478m-3.924-2.356c1.248.546 2.27.484 3.088.284m-3.674-6.434c-1.703.371-5.257 2.849-4.613 6.532c.646 3.688-2.044 4.18-4.093 4.283c.217 2.254 2.634 2.509 4.131 2.534m5.566-11.958c-2.67-2.898-2.456-8.585-1.287-8.992c1.684-.588 1.97 1.925 2.305 3.347m-3.524 13.371c.026.992.21 2.22-.434 3.942c-.582 1.556.707 2.494 1.16 3.697c-3.812.887-4.842-2.418-2.354-6.584m5.64-8.685c-1.407-.498-4.334 1.09-4.735 4.287c-.25 1.999-.941 3.064-2.118 4.34c1.886-.122 2.543.037 4.53-3.667c1.67-3.116 2.013-2.285 2.644-2.904m-.184-4.618c2.707-2.277 7.007-2.933 11.751-.56m-11.374 3.2c-2.015.054-3.556-4.983-.042-6.22c-.436 1.6-.947 3.122.042 6.22m6.962 1.642C26.516 9.457 37.585 4.965 37.801 6.3c.292 1.81-2.585 4.469-6.245 6.795m-4.743.836c1.516.303.616 2.52-1.75 3.358m4.725-3.166c1.863-1.138 4.178-2.445 4.763-1.193c.252.538-.873 2.951-5.632 3.036c3.536.485.44 2.926-2.649 2.385m-3.609 6.825c.399 2.232-1.832 3.003-3.683 3.985c2.905 1.03 8.081.679 7.978-3.96m-8.289-6.167c4.328 5.17 10.287 3.963 13.29 3.707c-10.26 6.982-15.862-1.85-15.614-7.83c-.17-1.377.086-2.601.756-2.969c2.944-1.619 4.7 3.21 8.59 6.225c2.66 2 3.164.538 4.627.815c-.441 1.062-3.347 1.778-6.427.933c.227 1.372 1.713 2.043 2.606 3.045"></svg:path>`,
})
export class ArcticonsValkyrieConnectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsValkyrieprofileIcon],svg[arcticons-valkyrieprofile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.983 9.575c-.038 1.047-.059 1.618.238 1.675a4.7 4.7 0 0 1 3.095.968a1.74 1.74 0 0 0 2.084.156a10.9 10.9 0 0 1 2.593-1.974c1.298-.738 2.579-1.478 3.99-2.217a8 8 0 0 1 3.19-1.798c-.331 1.382-.45.733-.728 1.607a5.4 5.4 0 0 0 2.055-.737c-.216.661-.375.747-1.192 1.727c.386.23-.394-.612.322 1.064c-.152 1.563-.154 2.66-2.627 4.588a2.67 2.67 0 0 0-.466 1.464s.123.846-1.603 0c0 0 1.159 2.742-.67 1.315c0 0 .335 2.812-1.183.735c.956.86.231.365-.649-.75c0 0-.665-.402-.661.13a3.96 3.96 0 0 0 .51 2.116a10.1 10.1 0 0 1 2.217 2.75c.312 1.077.254 2.12.62 3.17c.343 1.86 1.587 3.191.865 4.987c-4.26 9.302-10.87 12.844-20.755 10.225a5 5 0 0 1-2.09-1.047c-1.305-1.28-1.662-1.276-2.349-1.02c-.914.402-1.271 1.44-2.572 1.297c-1.958-.08-2.069-.177-2.676-1.511c-.18-.551-.765-.36-1.344-1.376a4.1 4.1 0 0 1-.446-2.305a4.95 4.95 0 0 1-.775-2.084c-.067-.301.456-.612.47-.927c.012-.306-.485-.616-.526-.923a3.5 3.5 0 0 1 .205-1.039c-.041-.655-1.135-.805-1.613-2.46a5.03 5.03 0 0 1 1.13-3.414a11.7 11.7 0 0 0 1.042-3.237c1.128-.43.566-2.213 1.26-3.307c.74-.497 1.744-.753 2.17.616c.419 1.522.669 1.184 2.48 1.793c2.022.854 1.78 2.896-.262 4.482c-.65.566-.52 1.544.314 2.4c1.47 1.215 1.773 1.48 2.53 2.122a9.4 9.4 0 0 0 3.502 1.027a5.5 5.5 0 0 1 3.125.887c.656.341 1.587-.172 2.195-.864c.215-.295.192-.579-.11-1.02a2.35 2.35 0 0 1-1.996-.09a1.27 1.27 0 0 1-.447-.995c0-.223-.226-.399-.492-.599a2.3 2.3 0 0 1-.24-.809a6 6 0 0 1-.727-1.098c-.456-.464-.84-.25-1.06-.793c-.223-.634.479-1.233.616-2.112c.008-.16-.372-1.682-.128-.42c.026.144-.157-.213-.066-.562c.127.477.195.982-.76 1.182c-.355.136-1.185.122-1.369-.413c-.09-.776.678-1.697.42-2.735a8.14 8.14 0 0 1 3.687-7.76c1.25-.615 2.532-.843 3.094-1.156c.13-.072 1.317-.704 1.763-.917c.308-.147.84-.616.8-.014"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.746 26.475a3.88 3.88 0 0 1-.946-3.24a2.59 2.59 0 0 1 1.109-1.952c.708-.36 1.385-.225 1.399.132a1.615 1.615 0 0 1-.766 1.649c-.448.196-.553.13-1.741.172m6.56 14.212a5.34 5.34 0 0 0-2.93-.983c-.693-.056-2.351-.44-2.992.656c-.827 1.636 1.043 1.833 1.766.673a1.7 1.7 0 0 0 .192-1.084m.463-8.06a3.19 3.19 0 0 0 2.173 2.008m-4.17 1.352a3.79 3.79 0 0 0 3.676.149M7.61 30.555c1.096-.076 1.685.777.924 1.451c-.44.333-.896.75-.528 1.346a1.06 1.06 0 0 0 1.478-.422c.102-.383-.183-.688-.95-.897m-.65-4.055c.651-.258.874-.733.527-1.346c-.362-.675-.842-.768-1.583-.475a1.005 1.005 0 0 0-.528 1.451c.372.68.944.67 1.583.37Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.883 27.978c.587.402 2.668.222 2.034 1.036c-.467.642 1.833.901 2.08 1.231m-3.402 5.756c.729-3.023 7.596.237 7.218-2.652M8.996 19.133c.227.54.596 1.038.035 1.564a4.09 4.09 0 0 0-1.08 3.848"></svg:path>`,
})
export class ArcticonsValkyrieprofileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsValueResearchIcon],svg[arcticons-value-research-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 14.75L19.375 37.875L5.5 24"></svg:path>`,
})
export class ArcticonsValueResearchIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsValvIcon],svg[arcticons-valv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.721 10.485A21.5 21.5 0 0 1 45.5 24c0 11.874-9.626 21.5-21.5 21.5S2.5 35.874 2.5 24a21.5 21.5 0 0 1 4.773-13.507m3.667-3.571a21.5 21.5 0 0 1 26.122 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.5 6.925l18.5 37l18.5-37H28.167l-4.166 9.489l-4.167-9.49z"></svg:path>`,
})
export class ArcticonsValvIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVampiresurvivorsIcon],svg[arcticons-vampiresurvivors-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.264 14.71L23.847 38.92L13.43 14.71m13.023 18.162L18.347 14.71m13.486 0H36m-25 0h10.505"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVampiresurvivorsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVamprIcon],svg[arcticons-vampr-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m10.084 14.839l23.832-8.815c5.542-2.018 8.655 2.167 7.857 6.17l-4.277 25.051c-1.026 5.804-6.203 6.409-9.27 3.718L8.67 24.725c-4.516-3.786-2.452-8.575 1.414-9.886"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.256 12.718l18.986.08c4.402.034 5.485 3.774 3.889 6.374L30.563 35.58c-2.235 3.797-6.009 2.862-7.457.17l-9.43-16.477c-2.167-3.831.524-6.648 3.58-6.556"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24.28 13.151l11.483 6.545c2.657 1.54 2.03 4.174.171 5.2l-11.414 6.67c-2.657 1.529-4.63-.33-4.584-2.45l-.068-13.217c0-3.067 2.6-3.854 4.413-2.748"></svg:path>`,
})
export class ArcticonsVamprIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVancedmanagerIcon],svg[arcticons-vancedmanager-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M39.23 26a17 17 0 0 0 .14-2a17 17 0 0 0-.14-2l4.33-3.39a1 1 0 0 0 .25-1.31l-4.1-7.11a1 1 0 0 0-1.25-.44l-5.11 2.06a15.7 15.7 0 0 0-3.46-2l-.77-5.43a1.05 1.05 0 0 0-1-.86h-8.2a1 1 0 0 0-1 .86l-.77 5.43a15.4 15.4 0 0 0-3.46 2L9.54 9.75a1 1 0 0 0-1.25.44l-4.1 7.11a1 1 0 0 0 .25 1.31L8.76 22a17 17 0 0 0-.14 2a17 17 0 0 0 .14 2l-4.32 3.39a1 1 0 0 0-.25 1.31l4.1 7.11a1 1 0 0 0 1.25.44l5.11-2.06a15.7 15.7 0 0 0 3.46 2l.77 5.43a1 1 0 0 0 1 .86h8.2a1 1 0 0 0 1-.86l.77-5.43a15.4 15.4 0 0 0 3.46-2l5.11 2.06a1 1 0 0 0 1.25-.44l4.1-7.11a1 1 0 0 0-.25-1.31Zm-19.32-4.65a.52.52 0 0 1 .8-.44l4 2.6a.52.52 0 0 1 0 .87l-4 2.6a.52.52 0 0 1-.8-.44Zm.38-4.44l9.5 6.22a1 1 0 0 1 0 1.63L20.29 31"></svg:path>`,
})
export class ArcticonsVancedmanagerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVancedmicrogsettingsIcon],svg[arcticons-vancedmicrogsettings-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.78 2.5A21.54 21.54 0 0 1 36 6.13l-5.63 8.41a11.39 11.39 0 1 0 3.85 14.52H24V18.94h21.5V24A21.5 21.5 0 1 1 23.24 2.51Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.4 8.6a16.4 16.4 0 0 0-5.4-.91m-12.59 5.95a16.3 16.3 0 0 0 9.13 26.3m15.69-21c1.77 0 1.85 4.77 3.45 4.77s1.85-4.77 3.45-4.77"></svg:path><svg:circle cx="39.68" cy="26.33" r=".75" fill="currentColor"></svg:circle><svg:circle cx="22.55" cy="40.19" r=".75" fill="currentColor"></svg:circle><svg:circle cx="31.24" cy="9.35" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVancedmicrogsettingsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVancityIcon],svg[arcticons-vancity-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.336 43.5c10.988-9.99 13.32-25.855 13.32-39h-8.818c0 10.977-1.191 22.5-7.838 30.558C17.354 27 16.162 15.478 16.162 4.5H7.345c0 13.145 2.33 29.01 13.32 39z"></svg:path>`,
})
export class ArcticonsVancityIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVandleCardIcon],svg[arcticons-vandle-card-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 36.616c3.424 0 6.86.701 10.939.701c4.728 0 8.561-5.962 8.561-13.317s-3.833-13.317-8.561-13.317c-4.08 0-7.515.701-10.939.701s-6.86-.701-10.939-.701C8.333 10.683 4.5 16.645 4.5 24s3.833 13.317 8.561 13.317c4.08 0 7.515-.701 10.939-.701M13.5 21.25h21"></svg:path>`,
})
export class ArcticonsVandleCardIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVanillaIcon],svg[arcticons-vanilla-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.776 42.5c3.981-3.659 3.668-8.53 0-12.178c-4.318-4.294-11.103-1.883-17.44 3.949C12 40.103 10.654 42.5 10.654 42.5M5.5 39.206s.949-4.71 3.023-9.814s4.972-13.37 12.673-11.215c4.206 1.178 4.61 3.563 4.61 3.563"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 12.626c5.883 1.122 7.232 3.486 8.296 7.294m16.144 8.233l3.697-12.898a1.905 1.905 0 0 0-1.263-2.343l-1.764-.55a1.905 1.905 0 0 0-2.394 1.275l-3.15 10.597c-3.533-2.58-9.676-1.064-10.823 4.99c-1.01 5.327 2.16 6.867 2.16 6.867"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.654 14.582c1.57-3.47 9.645-9.75 18.785-5.6c9.14 4.149 11.96 12.272 10.71 18.784c-1.065 5.552-2.032 6.03-2.032 6.03"></svg:path>`,
})
export class ArcticonsVanillaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVanillamusicIcon],svg[arcticons-vanillamusic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.108 8.5a1.58 1.58 0 0 1 1.59 1.57v.038h0a1.58 1.58 0 0 1-1.57 1.589h-.02h0a1.58 1.58 0 0 1-1.607-1.55v-.039h0A1.58 1.58 0 0 1 10.108 8.5m27.784 0a1.58 1.58 0 0 1 1.608 1.608h0a1.58 1.58 0 0 1-1.57 1.59h-.019h0a1.58 1.58 0 0 1-1.608-1.55v-.04h0a1.58 1.58 0 0 1 1.55-1.608zM24 9.47c8.025 0 14.531 6.506 14.531 14.531S32.025 38.531 24 38.531S9.469 32.025 9.469 24S15.975 9.469 24 9.469M10.108 36.303c.878 0 1.59.711 1.59 1.589h0a1.58 1.58 0 0 1-1.55 1.608h-.04h0A1.58 1.58 0 0 1 8.5 37.892h0a1.58 1.58 0 0 1 1.608-1.59Zm27.784 0a1.58 1.58 0 0 1 1.607 1.55v.039h0a1.58 1.58 0 0 1-1.607 1.608h0a1.58 1.58 0 0 1-1.59-1.57v-.019h0a1.58 1.58 0 0 1 1.57-1.589h.02z"></svg:path><svg:circle cx="24" cy="24" r="5.144" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsVanillamusicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVanithaIcon],svg[arcticons-vanitha-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.157 22.45v5.748h-5.56"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.308 28.257c-.65-.397-1.3-.793-1.653-1.55c-.353-.755-.408-1.87.177-2.75s1.81-1.523 2.949-1.511s2.19.679 2.701 1.515s.479 1.842.204 2.535s-.793 1.074-1.182 1.305c-.388.232-.647.314-.907.397m20.496.059v-6.102c0-.192 0-.384-.104-.724s-.312-.826-.748-1.17s-1.1-.544-1.69-.512c-.591.031-1.11.294-1.392.52s-.33.414-.377.603m8.551 7.361c-.475-.18-.95-.361-1.317-.872s-.627-1.35-.546-2.21s.5-1.74 1.233-2.291c.732-.552 1.776-.775 2.727-.583s1.806.8 2.308 1.502s.652 1.494.572 2.247c-.08.754-.391 1.469-.868 1.864c-.477.394-1.121.469-1.64.137s-.91-1.07-1.016-1.887a3.54 3.54 0 0 1 .57-2.432c.494-.72 1.303-1.266 2.249-1.475c.946-.208 2.03-.078 2.78.478s1.166 1.537 1.229 2.444s-.228 1.74-.583 2.244c-.356.504-.776.681-1.196.858M22.577 23.46v4.797m-2.388-.122c-.805-.328-1.61-.655-2.09-1.35c-.48-.694-.636-1.754-.348-2.64s1.022-1.596 1.81-1.882c.79-.286 1.633-.147 2.145.123s.692.671.872 1.073m2.39 4.676c.805-.328 1.61-.655 2.09-1.35c.48-.694.636-1.754.347-2.64s-1.022-1.596-1.81-1.882s-1.632-.147-2.144.123s-.692.671-.872 1.073"></svg:path>`,
})
export class ArcticonsVanithaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVaroBankIcon],svg[arcticons-varo-bank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 42.5h-29c-2.2 0-4-1.8-4-4v-29c0-2.2 1.8-4 4-4h29c2.2 0 4 1.8 4 4v29c0 2.2-1.8 4-4 4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.711 19.23l-3.16 9.54l-3.161-9.54"></svg:path><svg:rect width="4.771" height="6.321" x="30.839" y="22.45" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.385" ry="2.385"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.726 24.835a2.385 2.385 0 0 1 2.385-2.385h0m-2.385 0v6.32m-2.436-2.385a2.385 2.385 0 0 1-2.386 2.386h0a2.385 2.385 0 0 1-2.385-2.386v-1.55a2.385 2.385 0 0 1 2.385-2.385h0a2.385 2.385 0 0 1 2.386 2.385m0 3.935v-6.32"></svg:path>`,
})
export class ArcticonsVaroBankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVarsityIcon],svg[arcticons-varsity-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.4 4.5a2 2 0 0 1 2 2v19.1c0 6.88-10.398 17.9-16.4 17.9S7.6 32.48 7.6 25.6V6.5a2 2 0 0 1 2-2z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.4 12.6L24 35.4l-7.4-22.8"></svg:path>`,
})
export class ArcticonsVarsityIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVaskehjelpIcon],svg[arcticons-vaskehjelp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.306 20.202h25.388m-23.254 0l1.037 21.12a2.29 2.29 0 0 0 2.287 2.178h14.472a2.29 2.29 0 0 0 2.287-2.177l1.038-21.12m-10.91 3.328l10.245 10.192"></svg:path><svg:circle cx="21.845" cy="30.144" r="1.92" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="20.394" cy="13.205" r="4.246" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="27.392" cy="6.164" r="1.664" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.982 20.202a3.414 3.414 0 1 0-6.827 0"></svg:path>`,
})
export class ArcticonsVaskehjelpIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVatanIcon],svg[arcticons-vatan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m15.318 17.5l-2.925 9l-2.925-9m14.606 1.125v6.75c0 .675.45 1.125 1.125 1.125h.337m-2.7-6.075h2.475m6.303 3.825a2.257 2.257 0 0 1-2.25 2.25h0a2.257 2.257 0 0 1-2.25-2.25v-1.462a2.257 2.257 0 0 1 2.25-2.25h0a2.257 2.257 0 0 1 2.25 2.25m0 3.712v-6.075m6.918 6.075v-3.712c0-1.238-1.013-2.25-2.25-2.25s-2.25 1.012-2.25 2.25m0 3.712v-6.075M20.573 24.25a2.257 2.257 0 0 1-2.25 2.25h0a2.257 2.257 0 0 1-2.25-2.25v-1.462a2.257 2.257 0 0 1 2.25-2.25h0a2.257 2.257 0 0 1 2.25 2.25m0 3.712v-6.075"></svg:path>`,
})
export class ArcticonsVatanIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVattenfallIcon],svg[arcticons-vattenfall-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M2.5 24h43"></svg:path>`,
})
export class ArcticonsVattenfallIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVaultyIcon],svg[arcticons-vaulty-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 8.076h-9.367v20.14l-14.05-20.14H5.5l22.481 31.848H42.5z"></svg:path>`,
})
export class ArcticonsVaultyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVauxEditorIcon],svg[arcticons-vaux-editor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.001 42.5a3.82 3.82 0 0 1-3.528-2.357L8.299 10.781a3.82 3.82 0 0 1 7.055-2.925l12.173 29.362a3.82 3.82 0 0 1-3.526 5.282"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 28.71l8.646-20.854a3.82 3.82 0 0 1 7.055 2.925L27.527 40.143"></svg:path>`,
})
export class ArcticonsVauxEditorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVaxicodeIcon],svg[arcticons-vaxicode-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="m15.098 34.279l2.2 2.198a.93.93 0 0 1 .003 1.313l-.004.004l-1.08 1.079a.93.93 0 0 1-1.313.003l-.003-.003l-2.199-2.2a.93.93 0 0 1-.003-1.312l.003-.002l1.08-1.08a.93.93 0 0 1 1.311-.005zm-1.295 3.494L9.076 42.5M34.262 9.728l7.587 7.587m-6.758 2.06l4.037-4.035m-6.926 1.145l4.035-4.035m-14.98 18.593l2.529 2.53m.36-5.42l2.53 2.529m.36-5.419l2.53 2.529m.361-5.42l2.53 2.53m-.929-8.875l4.022 4.022a1.75 1.75 0 0 1 .006 2.475l-.006.006l-14.86 14.859a1.75 1.75 0 0 1-2.475.006l-.006-.006l-4.022-4.022a1.75 1.75 0 0 1-.006-2.475l.006-.006l14.86-14.86a1.75 1.75 0 0 1 2.475-.005z"></svg:path><svg:path d="M15.73 17.098v2.554h2.555v2.555h-2.554v2.554h-2.555v-2.554h-2.554v-2.555h2.554v-2.554zm2.428-8.22v3.11s4.598.254 4.598 3.832v6.524M10.75 8.876v3.113s-4.599.28-4.599 3.831v12.861c0 .674.665 1.394 1.339 1.394h7.52M9.85 5.5h9.207c.493 0 .889.397.889.889v1.543a.887.887 0 0 1-.889.889H9.85a.89.89 0 0 1-.889-.887V6.389c0-.492.397-.889.89-.889Z"></svg:path></svg:g>`,
})
export class ArcticonsVaxicodeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVbanReceptorIcon],svg[arcticons-vban-receptor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:rect width="37" height="37" x="5.5" y="5.5" rx="4" ry="4"></svg:rect><svg:path d="M5.5 18.101h37M9.251 38.95h1.967m-1.967-1.967h1.967m-1.967-1.967h1.967m1.966 0h1.967m-1.967 1.967h1.967m-1.967 1.967h1.967m-1.967-5.9h1.967m-1.967-1.967h1.967m1.966 0h1.967m-1.967 1.967h1.967m-1.967 1.966h1.967m-1.967 1.967h1.967m-1.967 1.967h1.967m1.966 0h1.967m-1.967-1.967h1.967m-1.967-1.967h1.967M21.05 33.05h1.967m-1.967-1.967h1.967m-1.967-1.966h1.967m1.966 0h1.967m-1.967-1.967h1.967m-1.967-1.966h1.967m1.966 0h1.967m-1.967-1.967h1.967m1.966 1.967h1.967m1.966 5.899h1.967m-1.967 1.967h1.967m-1.967 1.966h1.967m-1.967 1.967h1.967m-1.967 1.967h1.967m-5.9 0h1.967m-1.967-1.967h1.967m-1.967-1.967h1.967m-1.967-1.966h1.967m-1.967-1.967h1.967m-1.967-1.966h1.967m-1.967-1.967h1.967m-5.9 0h1.967m-1.967 1.967h1.967m-1.967 1.966h1.967m-1.967 1.967h1.967m-1.967 1.966h1.967m-1.967 1.967h1.967m-1.967 1.967h1.967m-5.9 0h1.967m-1.967-1.967h1.967m-1.967-1.967h1.967m-1.967-1.966h1.967m-1.967-1.967h1.967"></svg:path></svg:g>`,
})
export class ArcticonsVbanReceptorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVcmiIcon],svg[arcticons-vcmi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.41 31.58c-2.87 6.65-6.74 11.88-11.79 11.92c-4.84 0-8.5-5.16-11.26-11.92M7.84 16.42C6.88 12.06 6.19 8.08 5.9 6c-.15-1.08.59-1.74 2.23-1.44h0c11.64 2.15 20.5 2.31 31.16.13c2.05-.42 3-.17 2.8 1.3c-.32 2.16-1 6.07-2 10.42m-1.66 12.68h3.05m-3.05-10.18h3.05m-1.53 0v10.18m-14.66-.02V18.91l5.09 10.18l5.09-10.16v10.16M13.26 18.91L9.89 29.09L6.52 18.91m15.61 6.76a3.37 3.37 0 0 1-3.37 3.37h0a3.37 3.37 0 0 1-3.37-3.37v-3.39a3.37 3.37 0 0 1 3.37-3.37h0a3.37 3.37 0 0 1 3.37 3.37v0"></svg:path>`,
})
export class ArcticonsVcmiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorIcon],svg[arcticons-vector-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m11.16 16.32l-2.095 1.672l-.599-.422s-.762.26-.775-.473s.394-2.948 1.488-2.957s1.002-.037 1.002-.037s-.306-.711.603-.997S16.5 10.89 16.5 10.89l8.263.319l.623-.502l4.16 1.85l.615.123l.407-.267s.331-3.255 2.815-3.193s3.649 1.803 4.46 1.931s-1.03 1.598-1.212 2.222c-.183.625-.694 1.797-2.794 3.404m0 .001a22 22 0 0 0 1.473 2.683c.975 1.537.598 1.952 1.542 3.487s1.65 3.514 2.166 3.85s.765 2.194 1.789 3.475"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.16 16.32s.198.44.992.127a20 20 0 0 1 2.814-1.434a4.8 4.8 0 0 0 2.131-1.032q1.912.176 3.795.553s-.448.859-4.933.67c0 0-1.314.674-.942 3.801c0 0-.335.601-2.554 1.495s-5.155 4.787-5.606 5.472c-.316.48-.726.224-1.288.834m.074 5.885a3.3 3.3 0 0 1 1.645-.995c1.11-.043 1.146-2.607 5.38-4.81s4.874-2.134 5.478-1.081"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.147 25.804s.531.471-1.734 1.093s-2.02 1.986-2.02 1.986s-1.21.596.404 2.07c0 0-.059.963 2.411 2.043s3.51.206 3.355-.492c-.114-.517-1.037-1.952-.507-2.132a4.8 4.8 0 0 0 2.205-1.309c.804-.866 7.124-2.095 8.131-3.783s.733-2.071.733-2.071l3.224 3.128l.018.67l.688.776l.016.482l3.483 5.02m3.936-.593l-1.683-2.42M5.506 38.78l36.988-5.775M29.41 16.22c.39-.107 1.353 1.253 1.277 1.582c-.109.468-1.796.412-1.796.225c0 0-.262 1.599-2.697 1.449s-2.567.361-3.813.492l-1.993-.602l.875-1.874a12.8 12.8 0 0 1 3.99.201c1.37.414 3.028-.038 3.281.213c.244-.585.028-1.381.876-1.686"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsVectorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorCameraIcon],svg[arcticons-vector-camera-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="26.372" r="10.014" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="26.372" r="6.324" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.5 12.142h-8.457L31.2 7.992a1 1 0 0 0-.914-.593h-12.57a1 1 0 0 0-.914.594l-1.844 4.149H6.5a2 2 0 0 0-2 2v24.46a2 2 0 0 0 2 2h35a2 2 0 0 0 2-2v-24.46a2 2 0 0 0-2-2"></svg:path>`,
})
export class ArcticonsVectorCameraIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorInkIcon],svg[arcticons-vector-ink-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.641 26.6l27.29 14.508m-27.29-20.1l27.29 14.835M26.458 5.5v37m4.808-37v37M37.55 20v21.618M42.359 20v21.618"></svg:path><svg:circle cx="39.954" cy="23.596" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="39.954" cy="15.196" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="28.862" cy="12.792" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="39.954" cy="38.081" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="28.862" cy="36.343" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="11.166" cy="26.803" r="2.404" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVectorInkIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorassetcreatorIcon],svg[arcticons-vectorassetcreator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.538 23.739a18.5 18.5 0 0 0-2.264-4.184l2.59-2.551a3.022 3.022 0 0 0-4.282-4.266l-.002.002l-2.55 2.6a18.41 18.41 0 0 0-21.515 0l-2.57-2.57a3.17 3.17 0 0 0-2.171-.916a2.98 2.98 0 0 0-2.122.876a3.02 3.02 0 0 0 0 4.264l2.599 2.55a18.5 18.5 0 0 0-3.476 10.807v1.797m13.63-5.613a2.92 2.92 0 1 1-2.921-2.92a2.92 2.92 0 0 1 2.92 2.92m10.547 2.02a2.92 2.92 0 1 1 4.904-1.171"></svg:path><svg:circle cx="5.775" cy="34.148" r="2" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="42.225" cy="25.614" r="2" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m7.751 33.87l32.531-7.791M7.775 34.148h1.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-dasharray="2.909 2.909" stroke-linecap="round" stroke-linejoin="round" d="M12.184 34.148h27.631"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.269 34.148h1.5v-1.5m-.046-3.578q-.053-.75-.166-1.489"></svg:path>`,
})
export class ArcticonsVectorassetcreatorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorifydahomeIcon],svg[arcticons-vectorifydahome-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linejoin="round"><svg:path d="m31.616 13.799l2.585 2.585a1.02 1.02 0 0 1-.012 1.455l-2.467 2.373l-3.934-3.934l2.373-2.467a1.02 1.02 0 0 1 1.455-.012Z"></svg:path><svg:path d="M13.5 34.5v-3.934l14.288-14.288l3.934 3.934L17.434 34.5z"></svg:path><svg:path stroke-linecap="round" d="M38.5 5.5h-29c-2.2 0-4 1.8-4 4v29c0 2.2 1.8 4 4 4h29c2.2 0 4-1.8 4-4v-29c0-2.2-1.8-4-4-4"></svg:path></svg:g>`,
})
export class ArcticonsVectorifydahomeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVectorpinballIcon],svg[arcticons-vectorpinball-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.5 25.24l11.674-4.689m3.268 15.058L42.5 26.752m-14.718-15.65a2.51 2.51 0 1 1 .002 5.019a2.51 2.51 0 0 1-.003-5.019z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsVectorpinballIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVeducationIcon],svg[arcticons-veducation-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.993 43.579c1.518-.907 4.553-2.72 6.394-4.068s2.487-2.235 3.021-3.106s.956-1.728 1.188-2.262s.274-.745.167-1.433s-.362-1.854-.482-2.534s-.105-.873.226-1.496s.98-1.672 1.388-2.352s.577-.99.749-1.513s.348-1.261.386-1.932s-.06-1.275-.102-1.697s-.028-.66.092-.956c.119-.295.344-.646.467-.962s.144-.597.225-.86c.08-.264.221-.51.32-.858c.098-.348.154-.798.105-1.405c-.05-.608-.204-1.374-.4-2.105a24 24 0 0 0-.7-2.132c-.263-.706-.551-1.423-.945-2.143a11 11 0 0 0-1.405-2.017a7.5 7.5 0 0 0-1.669-1.387a11.7 11.7 0 0 0-2.028-1.009a11 11 0 0 0-1.604-.45c-.329-.066-.382-.066-.412-.09c-.03-.022-.036-.068-.049-.175c-.012-.107-.03-.276-.057-.376s-.064-.131-.339-.247s-.788-.317-1.07-.418s-.332-.106-.382-.087c-.049.018-.096.058-.169.146s-.171.223-.253.3a.28.28 0 0 1-.232.09c-.085-.005-.19-.031-.493-.075c-.302-.044-.8-.106-2.115-.108s-3.446.053-4.624.141s-1.402.207-1.606.369s-.386.365-.559.513s-.335.24-.48.36c-.147.12-.276.269-.372.42c-.095.152-.157.306-.555 1.034a93 93 0 0 1-1.678 2.94c-.548.913-.91 1.434-1.193 1.81c-.284.374-.487.603-.641.802s-.259.367-.328.544s-.105.36-.127.511s-.032.271-.221.403s-.557.276-2.087.698s-4.223 1.123-5.636 1.505c-1.414.383-1.548.448-1.647.512s-.164.13-.194.244s-.025.278-.01.492s.04.477.104.765c.065.288.17.6.346.924c.176.323.424.656.705.942s.595.525.897.733c.301.209.591.386.994.471s.92.078 1.99-.15s2.693-.678 3.594-.93c.9-.25 1.08-.303 1.25-.335c.17-.031.332-.042.448.009c.116.05.187.163.243.415c.057.252.1.642.103.878c.003.235-.034.314-.488 1.096c-.453.78-1.322 2.264-1.747 3.072s-.405.943-.346 1.117s.16.388.301.596c.142.209.325.413.523.555s.409.225.637.265s.475.037.65.04c.175.002.28.01.356.02c.077.012.127.027.15.06s.022.081.018.132a.34.34 0 0 1-.039.154c-.03.051-.084.1-.113.178s-.03.181-.057.235s-.075.055-.159.054c-.083-.001-.2-.006-.263.023s-.073.09-.078.257c-.005.166-.005.437-.003.6c.003.165.008.222.043.287s.099.136.176.217s.166.17.282.248s.257.146.343.185s.115.053.124.069s-.004.036-.056.076s-.144.099-.255.218s-.24.298-.319.446a.9.9 0 0 0-.117.391a.8.8 0 0 0 .07.394a1.3 1.3 0 0 0 .324.391c.153.137.35.286.537.425s.366.268.45.453c.084.184.074.423.026.672s-.131.51-.156.787q-.037.419.102.879c.091.305.239.622.432.889s.432.484.738.625s.678.204 1.184.207s1.145-.053 1.795-.158s1.31-.26 1.704-.33s.52-.056.657.02c.137.078.285.219.369.602s.105 1.008.15 1.714c.047.706.117 1.493.377 2.365s.71 1.828.987 2.362c.276.534.378.646.455.695s.129.034.155.027q.026-.007 1.544-.913Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.959 36.774c.03-.465.06-.93.13-1.981s.178-2.69.205-3.79s-.028-1.661-.087-2.168c-.06-.507-.124-.959-.105-1.349s.125-.718.36-1.055a3.4 3.4 0 0 1 1.094-.97c.489-.282 1.1-.501 1.679-.78a10 10 0 0 0 1.553-.93c.427-.316.735-.61.909-.826s.213-.355.218-.509s-.025-.323-.129-.417c-.104-.095-.283-.114-.47-.102c-.186.012-.38.057-.588.114c-.209.057-.433.127-.621.124a.76.76 0 0 1-.45-.159a.5.5 0 0 1-.196-.313c-.03-.141-.03-.335-.015-.479s.045-.238.038-.317s-.048-.14-.116-.202c-.069-.062-.163-.124-.202-.199c-.038-.074-.02-.161.007-.222s.064-.096.04-.15c-.023-.053-.107-.125-.167-.182s-.094-.1-.093-.146c.001-.047.038-.1.09-.146s.115-.085.14-.134c.024-.048.01-.105-.135-.192c-.144-.087-.417-.204-.555-.308s-.14-.196-.121-.271c.018-.074.058-.132.15-.263s.236-.338.33-.509s.137-.304.145-.593a13 13 0 0 0-.048-1.103c-.028-.373-.057-.675.003-.998s.208-.668.492-1.117s.706-1.005 1.16-1.395s.937-.615 1.496-.734s1.191-.134 1.739-.035s1.012.309 1.374.534c.361.225.621.464.77.618c.15.155.189.225.268.297s.198.146.337.269s.297.295.415.43s.195.234.245.374s.076.324.094.612a14.6 14.6 0 0 1-.062 2.41c-.05.435-.128.857-.23 1.3s-.228.906-.339 1.263s-.206.606-.269.787s-.094.294-.055.4c.04.106.149.206.288.325s.308.258.514.343c.206.084.45.114.673.129s.428.015.609.077s.34.186.557.268s.489.122.732.209s.457.221.706.3s.531.105.805.115c.273.01.536.005.807.072c.27.067.55.206.86.312s.654.178.997.25"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.47 23.174c.17.264.34.528.611.737c.271.21.644.363 1.11.363c.468 0 1.03-.154 1.516-.462s.9-.77 1.215-1.167c.316-.396.534-.728.753-1.059"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.387 21.261c-.08.365-.161.73-.242 1.277s-.16 1.276-.24 2.006"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.675 21.586c-.351-.226-.703-.452-.956-.618s-.408-.271-.508-.44s-.146-.4-.06-.783s.304-.917.483-1.286c.18-.369.32-.572.465-.976a5.8 5.8 0 0 0 .32-1.718c.026-.71-.071-1.53-.168-2.349"></svg:path>`,
})
export class ArcticonsVeducationIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVeganradarIcon],svg[arcticons-veganradar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 8.766L24 39.234L4.5 8.766h13.406L24 18.516l6.094-9.75"></svg:path>`,
})
export class ArcticonsVeganradarIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVelobankIcon],svg[arcticons-velobank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 24h37M24 5.5v37M12.75 29.25v8h4m-4.65-26.5l2.65 8l2.65-8m13.85 4h2.608m1.384-4h-4v8h4m-1.992 10.5a2.65 2.65 0 0 1 2.65 2.65v2.7a2.65 2.65 0 0 1-2.65 2.65h0a2.65 2.65 0 0 1-2.65-2.65v-2.7a2.65 2.65 0 0 1 2.65-2.65"></svg:path>`,
})
export class ArcticonsVelobankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVelohOfficielIcon],svg[arcticons-veloh-officiel-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.3 26.297a2.13 2.13 0 0 1-1.853 1.076h0a2.13 2.13 0 0 1-2.132-2.133v-1.385c0-1.178.954-2.133 2.132-2.133h0c1.177 0 2.132.955 2.132 2.132v.693h-4.264m5.333-6.336l2.461 8.56c.194.675.9 1.065 1.575.87l.366-.105m2.736-7.507c1.178 0 2.132.954 2.132 2.132v1.386a2.13 2.13 0 0 1-2.132 2.132h0a2.13 2.13 0 0 1-2.132-2.132V22.16c0-1.178.955-2.132 2.132-2.132m-4.009-1.282l1.493-.533m6.959-.095v7.166m4.747-7.166v7.166m-4.747-3.597h4.747"></svg:path><svg:circle cx="38.194" cy="26.626" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m39 17.91l-.639 6.311m-25.259-2.979l-2.736 4.632L9 20.78m5.39 9.753c13.746 7.631 16.127 1.189 16.862-2.703"></svg:path>`,
})
export class ArcticonsVelohOfficielIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVendettaManagerIcon],svg[arcticons-vendetta-manager-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.015 14.527q1.232.05 2.455.216l.79-1.827a13 13 0 0 1 7.417 2.775a35.44 35.44 0 0 1 3.787 15.417s-2.214 3.787-8.049 3.953a38 38 0 0 1-2.26-3.012m-10.176-.013a39 39 0 0 1-2.3 3.025c-5.835-.166-8.049-3.953-8.049-3.953a35.44 35.44 0 0 1 3.803-15.432a13 13 0 0 1 7.416-2.752l.79 1.827q1.084-.15 2.173-.21m0 18.246c-3.595-.263-6.247-1.647-8.98-3.031m20.43 0c-2.8 1.418-5.516 2.836-9.247 3.048"></svg:path><svg:ellipse cx="19.013" cy="25.492" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.893" ry="1.977"></svg:ellipse><svg:ellipse cx="28.987" cy="25.492" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.893" ry="1.977"></svg:ellipse><svg:rect width="17.399" height="37" x="5.414" y="5.521" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2" ry="2"></svg:rect><svg:rect width="17.399" height="37" x="25.015" y="5.521" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2" ry="2"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.761 8.103h5"></svg:path><svg:circle cx="14.113" cy="8.103" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVendettaManagerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVendoraIcon],svg[arcticons-vendora-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m27.247 40.645l14.745-25.54a3.75 3.75 0 0 0-3.246-5.624H9.254a3.75 3.75 0 0 0-3.246 5.624l14.746 25.54a3.75 3.75 0 0 0 6.493 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.254 9.481c14.746 0 26.502 16.425 19.392 28.74"></svg:path>`,
})
export class ArcticonsVendoraIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVenlowIcon],svg[arcticons-venlow-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m23 34.45l-9.473-16.407a1.155 1.155 0 0 1 1-1.732h18.945a1.155 1.155 0 0 1 1 1.732L25 34.45a1.155 1.155 0 0 1-2 0"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVenlowIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVenmoIcon],svg[arcticons-venmo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.25 4.45a14.26 14.26 0 0 1 2.06 7.8c0 9.72-8.3 22.34-15 31.2h-15.4L5.74 6.58L19.21 5.3l3.27 26.24c3.05-5 6.81-12.76 6.81-18.08A14.5 14.5 0 0 0 28 6.94Z"></svg:path>`,
})
export class ArcticonsVenmoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVentraIcon],svg[arcticons-ventra-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.585 13.318L24 33.305l8.415-19.987"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.585 4.5L24 24.486L32.415 4.5"></svg:path><svg:circle cx="24" cy="40.002" r="3.498" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVentraIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVentuskyIcon],svg[arcticons-ventusky-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.286 33.278h17.85m-23.636 0h3.883M4.5 18.433h19.745m8.69 0h7.774M16.472 29.566h14.956m-20.842 0h3.882m11.78-3.71h7.774m-17.55 0h7.773m-13.66-3.712h3.883m14.774 0h7.773m-20.543 0h7.773m11.481-7.422H43.5m-27.028 0h7.773"></svg:path>`,
})
export class ArcticonsVentuskyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVeraIconPackIcon],svg[arcticons-vera-icon-pack-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.006 40.056a5.94 5.94 0 0 1-5.056-2.81L6.395 17.025a5.944 5.944 0 1 1 10.1-6.271l12.555 20.22a5.944 5.944 0 0 1-5.044 9.081Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 22.841l7.505-12.087a5.945 5.945 0 0 1 10.1 6.271L29.05 37.245a5.94 5.94 0 0 1-5.056 2.81"></svg:path><svg:circle cx="36.554" cy="13.89" r="5.944" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVeraIconPackIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerbsIcon],svg[arcticons-verbs-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.38 26.463L37.443 5.57m-2.797 36.863l-12.79-22.154l-16.22-4.346M38.538 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsVerbsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVergeIcon],svg[arcticons-verge-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.6 4.5l18.4 39l18.4-39"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 21.41L31.98 4.5H16.02z"></svg:path>`,
})
export class ArcticonsVergeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVergiftungsunfaelleBeiKindernIcon],svg[arcticons-vergiftungsunfaelle-bei-kindern-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.185 26.873v3.667c0 1.238 1.75 2.72 2.751 3.425c1.002.703 2.386 1.758 3.847 1.758s3.316-1.181 4.248-1.91c.858-.671 2.156-1.657 2.266-3.11V26.84m-16.456-7.954s-1.777-1.003-1.83-3.181s.75-2.058 3.935-5.258S21.564 6.903 23.83 6.9c0 0 3.58.022 7.097 3.326s4.617 3.131 4.58 5.188s-2.227 3.452-2.227 3.452"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.704 13.789c-1.141.978-1.733 1.628-1.689 3.03c.045 1.403.078 4.86.078 6.611s2.291 2.834 5.683 4.462c-.028 1.264 0 2.247 0 2.247s1.368 1.624 3.96 1.57c2.648-.054 3.832-1.521 3.832-1.521s.015-1.137 0-2.29c4.56-2.41 5.73-2.778 5.719-5.482s-.018-4.14.079-5.976s-1.124-2.307-1.124-2.307"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m22.179 27.907l1.527-2.756l1.527 2.756"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linejoin="round" d="m16.786 20.218l4.133.01s-.012 3.128-1.953 2.997s-2.18-3.007-2.18-3.007Zm13.857 0l-4.133.01s.012 3.128 1.953 2.997s2.18-3.007 2.18-3.007Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.84 24.694c-1.916-1.083-1.504-2.098-3.192-3.152c-1.565-.978-2.883-.506-2.581 2.482c-1.178.458-2.524 2.42-.6 3.125c2.4.88 2.227-1.769 4.934-.181s4.312 2.241 4.312 2.241m20.508-3.988c1.916-1.083 2.443-2.599 4.131-3.654c1.565-.977 2.883-.505 2.581 2.482c1.178.46 2.524 2.42.6 3.125c-2.4.88-2.227-1.768-4.933-.18s-5.122 2.691-5.122 2.691m-17.055 2.833s-2.47 1.54-3.559 2.072c-1.089.531-2.527.43-4.364-.391s-3.228.733-2.749 2.032c.48 1.3 2.239.983 2.239.983c-.66 2.49.391 3.867 1.737 3.842c1.302-.024 2.563-.875 2.655-1.918c.093-1.042.061-2.033 1.854-2.759c1.792-.726 3.95-1.806 3.95-1.806m15.303-2.012s2.47 1.54 3.558 2.072c1.09.532 2.528.43 4.365-.391s3.228.733 2.748 2.032c-.48 1.3-2.238.984-2.238.984c.66 2.49-.391 3.866-1.737 3.842c-1.302-.024-2.563-.876-2.656-1.918c-.092-1.043-.06-2.034-1.853-2.76s-3.95-1.806-3.95-1.806"></svg:path>`,
})
export class ArcticonsVergiftungsunfaelleBeiKindernIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerifitIcon],svg[arcticons-verifit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.885 22.084c4.706-3.003 3.737-11.272-1.573-13.123c-4.47-2.018-10.149.368-11.865 4.961c-1.918 3.318-.379 7.152.227 10.606c1.481 4.472 5.969 6.74 8.787 10.256c2.528 2.58 4.825 5.576 7.962 7.424c3.842 1.218 6.128-2.964 8.641-5.05c3.086-2.867 6.312-5.6 9.125-8.746c2.015-3.535 3.704-7.463 3.252-11.618c-.212-4.936-4.767-9.207-9.752-8.757c-9.613.584-10.454 14.258-1.069 16.09c-1.878.947-4.146 2.243-5.99 2.787c-1.09-3.867-3.451-8.125-7.745-4.83"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.828 14.232c2.159-2.948 7.14.823 4.631 3.677c-2.34 3.167-7.248-.703-4.631-3.677"></svg:path>`,
})
export class ArcticonsVerifitIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerityCuIcon],svg[arcticons-verity-cu-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.869 6.214h15.207m9.665 9.54h7.39"></svg:path><svg:circle cx="36.436" cy="8.98" r="3.481" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.743 6.214L25.23 42.5h2.344l8.862-26.746m-17.252-9.54l11.492 26.925"></svg:path>`,
})
export class ArcticonsVerityCuIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerizonCloudIcon],svg[arcticons-verizon-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m28.67 19.195l-7.543 13l-4.385-7.556"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.784 35.573h23.978c13.016 0 10.84-19.586-2.215-15.243c0-10.876-19.586-10.876-19.586 2.178C2.073 20.33 2.073 35.573 10.784 35.573"></svg:path>`,
})
export class ArcticonsVerizonCloudIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerizonMessagesIcon],svg[arcticons-verizon-messages-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38 8.81H19.266a4.5 4.5 0 0 0-4.5 4.5v13.244a4.5 4.5 0 0 0 4.5 4.5h6.287l9.553 4.325v-4.324H38a4.5 4.5 0 0 0 4.5-4.5V13.31a4.5 4.5 0 0 0-4.5-4.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.17 31.166v1.894c0 1.76-1.42 3.18-3.17 3.18h-4.08l-6.35 2.95v-2.95h-1.9c-1.75 0-3.17-1.42-3.17-3.18v-8.72c0-1.75 1.42-3.17 3.17-3.17h5.92"></svg:path>`,
})
export class ArcticonsVerizonMessagesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerkehrNrwIcon],svg[arcticons-verkehr-nrw-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.294 35.992v-5.606h1.83c1.055 0 1.9.841 1.9 1.892s-.845 1.892-1.9 1.892h-1.83m1.963-.004l1.766 1.826m-20.178-7.96h2.817m-2.817-5.564h2.817m-2.817 2.782h1.831m-1.831-2.782v5.564m4.362.001v-5.565h1.831c1.057 0 1.902.835 1.902 1.878s-.845 1.878-1.902 1.878h-1.831m1.965-.006l1.768 1.814m1.545-5.564v5.564m3.029.001l-2.325-2.783l2.325-2.782m-2.325 2.782h-.704m8.935-2.782v5.564m3.734-5.564v5.564M31.42 25.25h3.734M11.3 22.468l-1.83 5.565l-1.902-5.565m19.49 5.564h2.818m-2.818-5.564h2.818m-2.818 2.782h1.832m-1.832-2.782v5.564m9.64.001v-5.565h1.832c1.056 0 1.902.835 1.902 1.878s-.846 1.878-1.902 1.878h-1.832m1.966-.005l1.768 1.813m-16.782 7.96v-5.606q1.863 2.804 3.729 5.606v-5.606m13.054.001l-1.407 5.605l-1.407-5.605l-1.407 5.605l-1.408-5.605"></svg:path><svg:circle cx="21.633" cy="35.782" r=".753" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.5v23a4 4 0 0 0 4 4h31a4 4 0 0 0 4-4v-23a4 4 0 0 0-4-4h-31a4 4 0 0 0-4 4"></svg:path>`,
})
export class ArcticonsVerkehrNrwIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVernetIcon],svg[arcticons-vernet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="10.541" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="24" r="1.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.06 25.06l14.143 14.143"></svg:path>`,
})
export class ArcticonsVernetIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVeroIcon],svg[arcticons-vero-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 41.022L42.5 8.979h-37zm0-12.027L35.556 8.979"></svg:path>`,
})
export class ArcticonsVeroIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerstaIcon],svg[arcticons-versta-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="37.195" cy="12.557" r="5.305" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m35.658 25.597l-7.02 12.422h0A5.3 5.3 0 0 1 24 40.748a5.3 5.3 0 0 1-4.4-2.34l.019.026L5.978 14.752l-.061-.13a5.305 5.305 0 0 1 9.35-4.937l-.002.002l10.963 17.321"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.198 26.966a5.31 5.31 0 0 0 5.258 2.272c2.293-.36 4.135-2.284 4.408-4.59a5.307 5.307 0 0 0-7.722-5.352c-.784.41-1.41 1.068-1.899 1.805L24.324 24m10.321-12.863h5.1m-.797.007c-.264 2.238-1.423 2.996-3.506 3.568m0-3.568c.264 2.238 1.422 2.996 3.506 3.568m-1.753-3.576v-.733"></svg:path>`,
})
export class ArcticonsVerstaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVerticonsIconPackIcon],svg[arcticons-verticons-icon-pack-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:circle cx="32.04" cy="15.699" r="6.245" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m5.5 24.467l5.5-5.5l12.671 12.671l5.509-5.51l8.895 8.895l4.441-4.441"></svg:path>`,
})
export class ArcticonsVerticonsIconPackIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVeryIcon],svg[arcticons-very-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m6.133 5.5l14.182 37l11.692-.088l9.86-25.786l-11.083-5.234l-5.02 14.879L19.4 5.5ZM25.77 26.27L20.31 42.5"></svg:path>`,
})
export class ArcticonsVeryIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVesImageAndPhotoCompareIcon],svg[arcticons-ves-image-and-photo-compare-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M27.436 7.303h11.243a1.78 1.78 0 0 1 1.779 1.78v29.356c0 .983-.797 1.79-1.78 1.78H27.407V23.71l9.162 11.005v-23.97h-9.162V7.316m-6.873 16.342l-9.2 11.056h9.2z"></svg:path><svg:path d="M20.663 43.5v-3.28H9.322a1.78 1.78 0 0 1-1.78-1.78V9.081c0-.982.797-1.77 1.78-1.78h11.341V4.5l2.818.001v39z"></svg:path></svg:g>`,
})
export class ArcticonsVesImageAndPhotoCompareIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVespucciIcon],svg[arcticons-vespucci-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40 40c1-1 0-3 1-6s-1-5.8-1-8.62C40 23.38 17 39 14 41c3.31 1 4 1 6 0c3.22-1.61 6 0 8 1s9 0 12-2m-23-6l23-12c2-1 1-3 2-5s-.62-8-1.62-11c-3.85.55-2.38.33-8.38 2c-4.09 1.14-2-3.58-10-1.58c-4 1-4.12 1.28-6.37 1.58c.03 2 1.37 26 1.37 26M5.46 6c2.07 3 4.05 8.56 2.05 11.56S6.55 25 7.55 28C8.69 31.42 6 37 6 41c2.5.49 2.47 0 6 0l1.45-33.33S9.46 5 5.46 6"></svg:path>`,
})
export class ArcticonsVespucciIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVesselfinderIcon],svg[arcticons-vesselfinder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m13.743 33.088l30.93-17.858l-10.268 17.784z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.404 33.014L3.325 15.071l10.416 18.017m5.128-9.045l5.271-9.13l5.272 9.13m-5.273-9.131v12.173M3.327 15.071l17.45 5.66m6.749.048l17.149-5.549"></svg:path>`,
})
export class ArcticonsVesselfinderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVesyncIcon],svg[arcticons-vesync-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.065 34.279V22.158m26.772 6.336v7.887a5.405 5.405 0 0 1-5.405 5.405H24M42.5 21.35L24 6.137L5.5 21.35m5.565-9.701V7.87"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.837 25.522c0-1.18-.45-2.361-1.35-3.262c-1.802-1.802-4.952-2.005-6.525 0c-3.28 4.18-7.742 13.994-11.022 18.175c-1.573 2.004-4.722 1.802-6.524 0a4.6 4.6 0 0 1-1.35-3.262"></svg:path>`,
})
export class ArcticonsVesyncIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVetcIcon],svg[arcticons-vetc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.569 24.002h3.777m5.51-5.639h7.574m11.069 7.494v.046c0 2.062-1.697 3.734-3.791 3.734s-3.791-1.672-3.791-3.734V24.05m.005-2.137c.098-1.975 1.756-3.547 3.786-3.547h0c2.043 0 3.708 1.591 3.788 3.583M15.57 24.002h3.776M15.57 18.36h5.792M15.57 29.643h5.792M4.5 18.363L8.288 29.64l2.237-6.664m1.205-3.588l.345-1.025M28.643 29.64v-5.639M15.57 13.749H43.5m-39 20.502h24.143"></svg:path>`,
})
export class ArcticonsVetcIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVflatIcon],svg[arcticons-vflat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.903 7.763l23.074 6.527a.718.718 0 0 1 .36 1.146l-9.814 11.968a1.85 1.85 0 0 1-2.154.527L5.1 16.706a.99.99 0 0 1-.094-1.775L17.294 8.07a3.43 3.43 0 0 1 2.609-.306"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.42 19.406a64.5 64.5 0 0 0-6.593 5.537a.69.69 0 0 0 .167 1.08C8.53 27.947 24.073 36.378 31.452 40.12a2.27 2.27 0 0 0 3.15-1.205c2.062-5.355 7.04-11.564 8.71-13.335a.54.54 0 0 0-.158-.859a37 37 0 0 0-5.715-2.093"></svg:path>`,
})
export class ArcticonsVflatIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVguardSmart20Icon],svg[arcticons-vguard-smart-20-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.525 10.935c4.828 3.632 5.652 3.35 10.728 2.047s5.698-.173 11.488 2.366c4.536 1.99 6.009-1.5 5.67-2.66c-.385-1.326-1.652-1.74-1.652-1.74c-.454 2.225 1.614 3.9 2.812 4.528c1.199.627 1.904.663 1.904.663c.018 1.219-.878 1.707-.878 1.707c-2.112-.643-3.716-.094-4.625.814c-1.338 1.336-1.946 2.865-3.94 3.214"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.31 20.07c-4.839.424-2.916 5.732-.178 7.114c-5.173-.334-4.058-4.667-8.059-7.582c-4.179-3.045-7.04-2.857-9.647-3.637S8.78 13.061 8.78 13.061"></svg:path><svg:rect width="36" height="36" x="6" y="6" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="3.892" ry="3.892"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.428 32.359a2.653 2.653 0 0 1 3.176-2.6c1.112.214 1.999 1.175 2.11 2.301c.082.838-.183 1.665-.762 2.173c-1.072.94-4.524 3.476-4.524 3.476h5.3m8.701-7.058l-4.052 6.116"></svg:path><svg:rect width="5.3" height="8" x="19.756" y="29.709" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.65" ry="2.65"></svg:rect><svg:circle cx="18.145" cy="37.55" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVguardSmart20Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViIcon],svg[arcticons-vi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="9.128" height="24.342" x="33.872" y="8.251" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx=".507"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.018 8.635L18.55 26.507L14.082 8.635a.51.51 0 0 0-.492-.384H5.507a.507.507 0 0 0-.492.63l5.832 23.327a.51.51 0 0 0 .492.384h14.422a.51.51 0 0 0 .492-.384l5.832-23.327a.507.507 0 0 0-.492-.63H23.51a.51.51 0 0 0-.492.384"></svg:path><svg:circle cx="38.436" cy="39.185" r="4.564" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsViIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViMoviesAndTvIcon],svg[arcticons-vi-movies-and-tv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 13.89c-3.37 1.31-8.97-1.18-9.44-8.39m9.44 27.87c-11.63 0-28.25-6.26-29.13-27.87"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect>`,
})
export class ArcticonsViMoviesAndTvIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViaIcon],svg[arcticons-via-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 31.73L25.37 7.75l1.65 8.96l-14.79 18.2z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.97 22l.34-8.54l-6.14 1.08L4.5 31.73M25.37 7.75l6 32.5l12.13-2.67l-9.61-29.32z"></svg:path>`,
})
export class ArcticonsViaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViabusIcon],svg[arcticons-viabus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.68 16.333v5.517l24.457 9.02l-.465-5.918zm26.127 8.973l.408 5.47l8.228-3.142l-.335-5.46z"></svg:path><svg:ellipse cx="33.944" cy="33.481" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.211" ry=".953" transform="rotate(-73.52 33.944 33.481)"></svg:ellipse><svg:ellipse cx="41.068" cy="30.771" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.211" ry=".953" transform="rotate(-73.52 41.068 30.77)"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M8.41 28.142c-.038-.837.193-1.594.613-2.086c-1.11-1.17-2.355.375-2.307 1.481L4.5 26.724V14.978l12.264-4.521l23.747 8.621c3.512 1.276 2.57 3.637 2.989 13.705l-11.068 4.081l-2.645-.96"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.558 29.649c-.205.836-.714 1.465-1.412 1.614c-1.123.24-2.309-.855-2.649-2.445a4 4 0 0 1-.087-.676m15.542 5.644l-11.396-4.138m17.238 6.249c-.204.836-.714 1.465-1.412 1.614c-1.123.24-2.309-.855-2.648-2.445a4 4 0 0 1-.088-.676c-.038-.837.193-1.594.614-2.086c-1.11-1.17-2.356.375-2.308 1.481m5.842 2.112c-.204.836-.714 1.465-1.412 1.614c-1.123.24-2.309-.855-2.648-2.445a4 4 0 0 1-.088-.676c-.038-.837.193-1.594.614-2.086"></svg:path>`,
})
export class ArcticonsViabusIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVianavigoIcon],svg[arcticons-vianavigo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.1 37.5c.859-1.407 7.93-11.123 10.3-11.123c3.604 0 9.111 8.446 10.985 11.123M13.272 26.257c1.922-1.682 6.626-3.673 11.02-4.291s9.646-3.021 10.608-4.428M24.566 10.5c1.923 0 2.781 1.51 2.781 3.261s-1.51 4.395-3.605 4.395s-2.849-1.717-2.849-3.124s1.099-4.532 3.673-4.532"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsVianavigoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViaplayIcon],svg[arcticons-viaplay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 2.5a21.37 21.37 0 0 0-13.75 5L32 19.12v10L10.3 40.57A21.5 21.5 0 1 0 24 2.5m-19.28 12a21.48 21.48 0 0 0 0 19l18-9.51Z"></svg:path>`,
})
export class ArcticonsViaplayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibbIcon],svg[arcticons-vibb-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.868 8.558L24 25.28L17.132 8.558H5.5l12.684 30.884h11.632L42.5 8.558z"></svg:path>`,
})
export class ArcticonsVibbIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibboIcon],svg[arcticons-vibbo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 7.014a6.72 6.72 0 0 1-6.722 6.722V7.014zM23.909 17.732l-5.934 14.993l-5.935-14.993H5.5l9.204 23.254h6.541l9.204-23.254z"></svg:path>`,
})
export class ArcticonsVibboIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibeMusicIcon],svg[arcticons-vibe-music-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.33 28.254v-5.248h1.896c2.206 0 2.223 5.248-.031 5.248zM9.67 23.007v5.247H7.775c-2.206 0-2.223-5.248.032-5.248z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.33 23.007c0-23.358-28.369-23.327-28.66 0m3.267-2.747v4.93m3.034-7.523v9.895m2.909-8.004v6.334m10.243 7.139v4.932m2.94-7.429v10.021m2.845-8.225v6.17"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.866 19.558l-.158 17.203c0 1.903-2.898 6.033-6.828 5.722c-5.606-.443-7.623-6.749-2.909-9.768c2.063-1.32 3.802-1.327 5.66-1.087l.315-20.442c1.996 2.58 4.73 4.322 7.682 6.165c5.192 3.24 7.247 8.152 3.136 11.254c-.238-2.2-.133-4.518-2.472-5.99c-1.437-1.04-2.956-2.158-4.426-3.057"></svg:path>`,
})
export class ArcticonsVibeMusicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibeYouIcon],svg[arcticons-vibe-you-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.07 30.666s2.996-5.071 2.47-8.378C31.965 18.658 25 13.353 25 11.83c-.028-.848.03-1.691 0-2.54v22.523"></svg:path><svg:ellipse cx="20.184" cy="32.885" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="5.097" ry="3.598" transform="rotate(-27.452 20.184 32.885)"></svg:ellipse>`,
})
export class ArcticonsVibeYouIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViberIcon],svg[arcticons-viber-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9 4.5a1.88 1.88 0 0 0-1.92 1.88v30.07A1.88 1.88 0 0 0 9 38.33h1.88v5.17l10.34-5.17H39a1.88 1.88 0 0 0 1.88-1.88V6.38A1.88 1.88 0 0 0 39 4.5Zm4.7 6.58h5.6a1 1 0 0 1 .94.94a10.1 10.1 0 0 0 .63 3.76a1.85 1.85 0 0 1-.63 2.35l-2 2a18 18 0 0 0 3.38 4.6a19 19 0 0 0 4.61 3.39l2-2a1.85 1.85 0 0 1 2.35-.63a10.1 10.1 0 0 0 3.76.63a1 1 0 0 1 .94.94v5.64a1 1 0 0 1-.94.94A22.53 22.53 0 0 1 12.72 12a1 1 0 0 1 .94-.92Z"></svg:path>`,
})
export class ArcticonsViberIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibrafunIcon],svg[arcticons-vibrafun-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19 9h10v30H19zM6.5 9h10v30h-10zm25 0h10v30h-10zm10 25h1M29 34h2.5m-15 0H19M5.5 34h1m35-20h1M29 14h2.5m-15 0H19M5.5 14h1"></svg:path>`,
})
export class ArcticonsVibrafunIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibrationstestIcon],svg[arcticons-vibrationstest-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="6" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.17 38.17a21.5 21.5 0 0 0 0-28.37m-32.34.03a21.5 21.5 0 0 0 0 28.34M15.3 17.3a11 11 0 0 0 0 13.4m17.4 0a11 11 0 0 0 0-13.4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.63 13.63a16.15 16.15 0 0 0 0 20.74m24.74 0a16.15 16.15 0 0 0 0-20.74"></svg:path>`,
})
export class ArcticonsVibrationstestIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibratorIcon],svg[arcticons-vibrator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="m15.218 9.518l-8.651 8.177l8.473 8.234l-8.235 7.942l8.828 8.629m17.532-37l8.237 7.851l-8.205 8.004l8.236 8.236l-8.682 8.357"></svg:path><svg:circle cx="23.994" cy="23.707" r="3.364"></svg:circle></svg:g>`,
})
export class ArcticonsVibratorIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibronsIcon],svg[arcticons-vibrons-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 17.044v14.23m7.4 10.206V5.969m7.4 28.391V6.106m7.4 7.13v28.796m7.4-.397V6.292m7.4 5.564v21.29"></svg:path>`,
})
export class ArcticonsVibronsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVibulkijIcon],svg[arcticons-vibulkij-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.249 23.569v17.52H13.944V23.866m0-12.892V6.91H34.25v4.063m8.251 0H30.615L24 21.072l-6.615-10.097H5.5L24 39.22z"></svg:path>`,
})
export class ArcticonsVibulkijIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVicareIcon],svg[arcticons-vicare-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 42.5h37V20.359L24 5.5L5.5 20.317Z"></svg:path>`,
})
export class ArcticonsVicareIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVictronconnectIcon],svg[arcticons-victronconnect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.8 28.414A11.65 11.65 0 1 1 13.107 17.17m7.155 1.211a11.65 11.65 0 1 1 15.553 12.157"></svg:path><svg:path fill="none" stroke="currentColor" d="m15.153 11.836l19.614 25.739M14.765 27.319l7.24-5.809"></svg:path><svg:circle cx="12.871" cy="28.686" r="2.288" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVictronconnectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVictronconnectAltIcon],svg[arcticons-victronconnect-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.425 13.55c-3.05.022-5.16 4.16-5.25 10.7m22.25-1c-.045 7.66-1.87 11.2-5.36 11.2c-5.09-.086-5.5-6.42-5.55-10.2m-1.59-10.7c-3.05.022-5.16 4.16-5.25 10.7"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.825 32.45c-1.58-2.19-1.77-5.79-1.8-8.28m10.9-.92c-.045 7.66-1.87 11.2-5.36 11.2m-7.24-20.9c-3.05.022-5.16 4.16-5.25 10.7m11.25 0c-.048-4.03-.765-6.92-2.07-8.66m12.97 7.66c-.045 7.66-1.87 11.2-5.36 11.2"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.825 24.25c-.086-7.13-2.27-10.7-6.09-10.7c-3.05.022-5.16 4.16-5.25 10.7m22.34-1c-.045 7.66-1.87 11.2-5.36 11.2"></svg:path>`,
})
export class ArcticonsVictronconnectAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidaaIcon],svg[arcticons-vidaa-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.487 20v8M8.97 20l2.65 8l2.65-8m4.43 8v-8h1.8a3.5 3.5 0 0 1 3.5 3.5v1a3.5 3.5 0 0 1-3.5 3.5zm7.513 0l2.65-8l2.65 8m2.213 0l2.65-8l2.65 8"></svg:path>`,
})
export class ArcticonsVidaaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidelibriIcon],svg[arcticons-videlibri-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.726 4.5H10.4a2 2 0 0 0-2 2v35a2 2 0 0 0 2 2h2.326m0-39v39H37.6a2 2 0 0 0 2-2v-35a2 2 0 0 0-2-2z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m21.924 14.313l-2.65 8l-2.65-8"></svg:path><svg:circle cx="23.785" cy="14.563" r=".7" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.785 17.013v5.3m11.61-1.027a2 2 0 0 1-1.738 1.01h0a2 2 0 0 1-2-2v-1.3a2 2 0 0 1 2-2h0a2 2 0 0 1 2 2v.65h-4m-1.925-.633a2 2 0 0 0-2-2h0a2 2 0 0 0-2 2v1.3a2 2 0 0 0 2 2h0a2 2 0 0 0 2-2m0 2v-8M16.839 26.137v8h4m9.969-3.3a2 2 0 0 1 2-2h0m-2 0v5.3"></svg:path><svg:circle cx="22.742" cy="26.387" r=".7" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.742 28.837v5.3"></svg:path><svg:circle cx="34.742" cy="26.387" r=".7" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.742 28.837v5.3m-9.915-3.3a2 2 0 0 1 2-2h0a2 2 0 0 1 2 2v1.3a2 2 0 0 1-2 2h0a2 2 0 0 1-2-2m0 2v-8"></svg:path>`,
})
export class ArcticonsVidelibriIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoIcon],svg[arcticons-video-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 9v30m-5.74-26.96h5.74m-5.74 5.98h5.74M37.76 24h5.74m-5.74 5.98h5.74m-5.74 5.98h5.74M10.24 12.04h27.53v23.91H10.24zM4.5 39V9m5.74 30V9m27.52 30V9M10.24 35.96H4.5m5.74-5.98H4.5M10.24 24H4.5m5.74-5.98H4.5m5.74-5.98H4.5"></svg:path>`,
})
export class ArcticonsVideoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoDownloaderIcon],svg[arcticons-video-downloader-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="38.5" cy="38.499" r="7" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.432 43.326A21.4 21.4 0 0 1 24 45.5C12.126 45.5 2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24c0 3.383-.781 6.583-2.173 9.43"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m33.76 24l-14.722-8.5v17zm4.74 18.499v-8m-3.297 4.704l3.297 3.296l3.297-3.296"></svg:path>`,
})
export class ArcticonsVideoDownloaderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoMakerIcon],svg[arcticons-video-maker-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m30.683 32.82l5.714 3.411c1.334.796 2.142.471 2.142-.875V25.273c0-1.032-1.083-1.385-1.659-.936l-6.197 3.743"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.704 24.226v9.993a2.7 2.7 0 0 1-2.692 2.708H14.638a2.7 2.7 0 0 1-2.706-2.694V24.226m6.485-5.144a3.372 3.372 0 1 1-3.371-3.373h.001c1.86 0 3.37 1.51 3.37 3.373"></svg:path><svg:path fill="currentColor" d="M25.34 18.123a.75.75 0 0 1-1.5 0a.75.75 0 0 1 1.5 0m1.29 3.789a.75.75 0 1 1-1.5 0a.75.75 0 1 1 1.499 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.202 21.129a5.55 5.55 0 0 1-5.157 3.503a5.55 5.55 0 0 1-5.546-5.55a5.547 5.547 0 1 1 11.093 0h0c0 .723-.139 1.414-.39 2.047h2.504a5.547 5.547 0 0 1 5.156-7.597a5.547 5.547 0 1 1-5.156 7.598"></svg:path><svg:path fill="currentColor" d="M30.535 21.912a.75.75 0 1 1-1.5 0a.75.75 0 0 1 1.5 0m1.207-3.789a.75.75 0 0 1-1.5 0a.75.75 0 0 1 1.5 0m-3.16-2.347a.75.75 0 1 1-1.5 0a.75.75 0 0 1 1.5 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.079 10.943a1.862 1.862 0 1 1-3.724.003v-.003a1.862 1.862 0 0 1 3.724 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsVideoMakerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoMp3ConverterIcon],svg[arcticons-video-mp3-converter-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.396 9.589c-8.374-6.71-20.602-5.362-27.312 3.012s-5.362 20.603 3.012 27.313c7.729 6.193 18.879 5.582 25.884-1.418"></svg:path><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:circle cx="33.996" cy="29.488" r="4.512"></svg:circle><svg:path d="M38.508 29.488V14h4.008"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m33.676 3.841l1.72 5.748l-5.748 1.721"></svg:path>`,
})
export class ArcticonsVideoMp3ConverterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoProIcon],svg[arcticons-video-pro-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.429 33.535H20.604V22.267h18.62l1.205 1.779zm0-9.489H43.5v9.489h-3.071zm.042 9.489H21.367v2.372H39.37zM16.982 23.029h3.622v10.505h-3.622zm-8.811-.614h6.524v11.543H8.171zm6.524 11.543l2.287-.423V23.029l-2.287-.614zm-9.39.763l2.866-.763V22.415l-2.866-.875zm8.345-21.555h4.829v6.806H13.65z"></svg:path><svg:rect width="9.15" height="5.648" x="4.5" y="12.093" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.824" ry="2.824"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.479 16.442h16.069M21.53 18.247c-.21.8-.01 1.68.58 2.32l1.57 1.7m9.81-4.02l.01.03c.46.99.03 2.17-.98 2.62l-3.08 1.37m8.751 0l-3.643-5.825h4.716l2.287-1.666m0 6.354v-6.354l1.132 1.286a2.86 2.86 0 0 1 0 3.782z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m41.551 21.13l-2.089-1.497h-2.918m-15.94 11.946h6.983v-9.312M13.65 18.748h-3.157v-1.007m25.187.506h-17.2m21.949 5.799H27.587"></svg:path>`,
})
export class ArcticonsVideoProIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoToAudioIcon],svg[arcticons-video-to-audio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.583 27.506c0-.735-.602-1.338-1.338-1.338c-.735 0-1.338.603-1.338 1.338s.603 1.338 1.338 1.338s1.338-.602 1.338-1.338m2.97 2.97c0-.735-.604-1.338-1.339-1.338s-1.338.603-1.338 1.338s.603 1.338 1.338 1.338s1.338-.603 1.338-1.338zm2.97-2.97c0-.735-.604-1.338-1.339-1.338c-.736 0-1.338.603-1.338 1.338s.602 1.338 1.338 1.338c.735 0 1.338-.602 1.338-1.338m-2.97-2.969c0-.735-.604-1.338-1.339-1.338s-1.338.603-1.338 1.338s.603 1.338 1.338 1.338s1.338-.602 1.338-1.338m-1.339 9.872c3.706 0 6.71-3.09 6.71-6.902s-3.004-6.903-6.71-6.903c-2.962 0-5.476 1.975-6.366 4.715m1.737 7.184a6.6 6.6 0 0 0 4.63 1.906h6.71m-24.091-9.292a3.32 3.32 0 1 0-6.639 0a3.32 3.32 0 0 0 6.64 0l-.547-9.23l4.44-1.253M5.5 36.197h24.981L23.09 5.617H5.5zm16.52 0l2.12 6.185H42.5V11.803H24.585"></svg:path>`,
})
export class ArcticonsVideoToAudioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoToWallpaperIcon],svg[arcticons-video-to-wallpaper-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.29 22.24h-5.282m-7.854 0h-.677v14.895h24.917V22.24h-2.031m6.77 11.781l8.667 5.281V19.938l-8.667 5.281h-4.739v8.802z"></svg:path><svg:circle cx="23.894" cy="16.146" r="7.448" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="9.946" cy="18.719" r="5.146" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="23.894" cy="16.146" r="2.844" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="9.946" cy="18.719" r="1.76" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVideoToWallpaperIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideoWallpapersIcon],svg[arcticons-video-wallpapers-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.5 5.5h-18a2 2 0 0 0-2 2v33a2 2 0 0 0 2 2h18a2 2 0 0 0 2-2v-33a2 2 0 0 0-2-2m-20 34h-5c-1.1 0-2-.9-2-2v-27c0-1.1.9-2 2-2h5m22 0h6c1.1 0 2 .9 2 2v27c0 1.1-.9 2-2 2h-6"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.5 18.03l9 6l-9 5.97V18z"></svg:path>`,
})
export class ArcticonsVideoWallpapersIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideocropIcon],svg[arcticons-videocrop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 12.572v23.316l-8.627-9.127v8.601a1.935 1.935 0 0 1-1.93 1.94H6.439a1.935 1.935 0 0 1-1.939-1.93V12.637c0-1.074.865-1.94 1.939-1.94h26.496c1.074 0 1.939.866 1.939 1.94v9.06z"></svg:path>`,
})
export class ArcticonsVideocropIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideolandIcon],svg[arcticons-videoland-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 40.878L43.5 7.122H32.199L24 22.824L15.801 7.122H4.5z"></svg:path>`,
})
export class ArcticonsVideolandIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideosummarizerIcon],svg[arcticons-videosummarizer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m16.02 8.045l15.764-.076c6.546-.032 11.536 5.853 11.71 12.954c.278 11.424-7.968 16.485-14.287 19.108v-8.13H16.02c-6.79 0-11.52-4.634-11.52-10.978c0-7.722 3.898-12.841 11.52-12.878"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.34 14.997v11.51l9.966-5.755z"></svg:path>`,
})
export class ArcticonsVideosummarizerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideotranscoderIcon],svg[arcticons-videotranscoder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.78 32a9 9 0 0 0 .08-1.15a9 9 0 0 0-.08-1.15l2.5-2a.57.57 0 0 0 .14-.75l-2.36-4.1a.59.59 0 0 0-.72-.25l-2.94 1.17a9.2 9.2 0 0 0-2-1.16l-.4-3.13a.6.6 0 0 0-.59-.5h-4.78a.6.6 0 0 0-.58.5l-.44 3.13a9 9 0 0 0-2 1.16l-2.95-1.18a.58.58 0 0 0-.72.25l-2.36 4.1a.59.59 0 0 0 .14.75l2.5 2a9 9 0 0 0-.09 1.15A9 9 0 0 0 7.22 32l-2.5 2a.6.6 0 0 0-.14.76l2.36 4.09a.59.59 0 0 0 .72.26l2.95-1.19a8.6 8.6 0 0 0 2 1.16l.44 3.14a.6.6 0 0 0 .58.49h4.73a.6.6 0 0 0 .59-.49l.44-3.22a8.5 8.5 0 0 0 2-1.16L24.33 39a.59.59 0 0 0 .72-.26l2.37-4.09a.6.6 0 0 0-.14-.76ZM16 35a4.14 4.14 0 1 1 4.14-4.14A4.14 4.14 0 0 1 16 34.94Z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.69 33.45H40.2a3.3 3.3 0 0 0 3.3-3.3V5.38h-7.14l3.94 5.74h-4.91l-3.94-5.74H27.5l3.95 5.74h-4.91l-3.95-5.74h-3.94l3.94 5.74h-4.91l-3.94-5.74H11.5a3.3 3.3 0 0 0-3.3 3.3V22.8"></svg:path>`,
})
export class ArcticonsVideotranscoderIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVideotvsideviewIcon],svg[arcticons-videotvsideview-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.14 24.81v11.71h31.7V14.64H21.75m21.75 0v21.88m-4.66-19.66h4.66m-4.66 4.36h4.66m-4.66 4.36h4.66m-4.66 4.36h4.66m-4.66 4.36h4.66M7.14 21.83V11.48"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 14.64h7.13l4.08 7.19l4.63-10.35"></svg:path>`,
})
export class ArcticonsVideotvsideviewIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidhubIcon],svg[arcticons-vidhub-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.5 24c0 11.874-9.626 21.5-21.5 21.5S2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.802 28.605v3.676c0 1.226.996 2.228 2.213 2.228h0a2.227 2.227 0 0 0 2.214-2.228v-3.676m0 3.788v2.228m-12.537-9.023v9.023m0-3.788c0-1.225.996-2.228 2.214-2.228h0a2.227 2.227 0 0 1 2.214 2.228v3.676m11.79-3.676c0-1.225.997-2.228 2.214-2.228h0a2.227 2.227 0 0 1 2.214 2.228v1.448a2.227 2.227 0 0 1-2.214 2.228h0a2.227 2.227 0 0 1-2.213-2.228m-.001 2.339v-8.91"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.387 22.146h23.226c1.108 0 2 .892 2 2v12.519c0 1.108-.892 2-2 2H12.387c-1.108 0-2-.892-2-2V24.146c0-1.108.892-2 2-2M21.295 9.08l-3.137 9.71l-3.136-9.71m9.463 3.155v6.555m8.015-4.127a2.427 2.427 0 0 0-2.413-2.428h0a2.427 2.427 0 0 0-2.413 2.428v1.578a2.427 2.427 0 0 0 2.413 2.427h0a2.427 2.427 0 0 0 2.413-2.427m0 2.549V9.08"></svg:path><svg:circle cx="24.485" cy="9.328" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVidhubIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidioIcon],svg[arcticons-vidio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.581 32.259c2.907-1.324 8.181-9.236 9.635-16.772c-6.04-1.776-5.264-10.258.753-9.742c5.393.355 7.73 7.514 2.917 19.71c-7.352 15.082-24.263 26.084-20.916 5.587c3.854-17.33-.7-16.696-4.65-15.77c-3.617.883-2.885 6.308-1.27 6.965c5.716 3.53.775 10.98-5.878 2.82c-4.92-8.902 1.669-17.805 11.25-16.405c8.902 2.874 9.515 11.12 7.244 20.464q-1.077 4.036.915 3.143"></svg:path>`,
})
export class ArcticonsVidioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidiqIcon],svg[arcticons-vidiq-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.194 34.526L5.5 39.39V8.61h0l11.694 4.864m16.83 7L42.5 24l-8.644 3.596M12.038 14.234v19.01m19.114.001l-6.46-6.222m-.002-12.789c3.58 0 6.462 2.809 6.462 6.297v6.416c0 3.49-2.881 6.298-6.46 6.298h0c-3.58 0-6.462-2.809-6.462-6.298v-6.416c0-3.488 2.881-6.297 6.46-6.297"></svg:path>`,
})
export class ArcticonsVidiqIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidmateIcon],svg[arcticons-vidmate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="m4.5 13.247l3.207-1.862h8.89l13.51 25.265L43.5 11.35h-8.89l-8.938 16.885"></svg:path><svg:path d="M8.711 13.263L21.218 36.65h8.89M16.597 11.385l-3.207 1.862H4.5"></svg:path></svg:g>`,
})
export class ArcticonsVidmateIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidyaMusicIcon],svg[arcticons-vidya-music-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m39.648 21.47l-.967-6.575a5.25 5.25 0 0 0-5.192-4.487H12.373a5.25 5.25 0 0 0-5.192 4.487L4.527 32.958a2.54 2.54 0 0 0 2.513 2.908h.687a4.23 4.23 0 0 0 2.904-1.154l3.781-3.572a4.23 4.23 0 0 1 2.904-1.154l8.256.001m-9.778-13.951v8.207M11.69 20.14h8.208"></svg:path><svg:ellipse cx="29.187" cy="17.41" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.403" ry="1.374"></svg:ellipse><svg:ellipse cx="34.686" cy="28.845" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="8.814" ry="8.748"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.552 24.514h-2.93V31.8"></svg:path><svg:ellipse cx="33.219" cy="31.801" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.403" ry="1.374"></svg:ellipse>`,
})
export class ArcticonsVidyaMusicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVidyoconnectIcon],svg[arcticons-vidyoconnect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.093 12.093h23.815v23.815H12.093z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.063 17.063h13.875v13.875H17.063zm6.937 0v-4.97M30.938 24h4.969M24 30.938v4.969M17.063 24h-4.97"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.844 35.907V42.5H9.5a4 4 0 0 1-4-4V25.156h6.593m0-2.312H5.5V9.5a4 4 0 0 1 4-4h13.344v6.593m2.312 0V5.5H38.5a4 4 0 0 1 4 4v13.344h-6.593m0 2.312H42.5V38.5a4 4 0 0 1-4 4H25.156v-6.593M21.5 21.5h5v5h-5z"></svg:path>`,
})
export class ArcticonsVidyoconnectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietceteraIcon],svg[arcticons-vietcetera-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.153 11.777L24 39.249L13.847 11.777"></svg:path><svg:circle cx="42.514" cy="34.75" r="2.986" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVietceteraIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietcombankIcon],svg[arcticons-vietcombank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.696 6.073c5.44-2.773 23.353-1.376 25.543 2.703c1.294 5.695-3.128 12.285-4.688 14.626c-4.301 6.457-6.3 9.817-10.545 5.284c-7.27-7.762-15.444-20.835-10.31-22.614z"></svg:path><svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M20.584 4.987S3.049 7.143 4.861 16.31c1.96 9.918 11.197 20.114 15.251 23.112c3.528 2.608 8.073 5.624 12.94-.664c5.18-6.693 14.164-25.278 8.667-30.658"></svg:path><svg:path d="M18.664 17.223c3.41-.727 21.526-2.013 20.515 3.696"></svg:path></svg:g>`,
})
export class ArcticonsVietcombankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietcombankSmartOtpIcon],svg[arcticons-vietcombank-smart-otp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor"><svg:path stroke-linejoin="round" d="M17.585 5.753c4.723-2.407 20.272-1.195 22.173 2.346c1.123 4.944-2.716 10.665-4.07 12.697c-3.734 5.605-5.469 8.522-9.154 4.588c-6.311-6.739-13.407-18.087-8.95-19.632Z"></svg:path><svg:path d="M20.96 4.81S5.737 6.682 7.31 14.64c1.702 8.61 9.72 17.461 13.239 20.064c3.062 2.264 7.008 4.882 11.233-.577c4.497-5.81 12.295-21.943 7.524-26.614"></svg:path><svg:path d="M19.293 15.432c2.96-.631 18.687-1.748 17.809 3.208"></svg:path><svg:path stroke-linecap="round" stroke-linejoin="round" d="M6.953 43.025c.266.346.6.475 1.063.475h.642a1.08 1.08 0 0 0 1.08-1.081v-.005a1.08 1.08 0 0 0-1.08-1.08H7.95a1.08 1.08 0 0 1-1.083-1.083h0c0-.6.486-1.085 1.085-1.085h.638c.464 0 .797.129 1.063.475m24.112-.475h2.871M35.2 43.5v-4.334m-23.836 2.546a1.084 1.084 0 0 1 2.167 0V43.5m-2.167-2.871V43.5m2.167-1.788a1.084 1.084 0 0 1 2.167 0V43.5m3.792-1.084c0 .599-.485 1.084-1.083 1.084h0a1.084 1.084 0 0 1-1.084-1.084v-.704c0-.599.485-1.084 1.084-1.084h0c.598 0 1.083.485 1.083 1.084m0 1.788v-2.871m1.626 1.083c0-.598.485-1.083 1.083-1.083m-1.083 0V43.5"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.366 39.735V43.5m-.569-2.871h1.138m5.768 2.871a1.436 1.436 0 0 1-1.436-1.436v-1.463a1.436 1.436 0 1 1 2.872 0v1.463c0 .793-.643 1.436-1.436 1.436m7.558 0v-4.334h1.42c.801 0 1.451.651 1.451 1.455s-.65 1.456-1.452 1.456h-1.419"></svg:path>`,
})
export class ArcticonsVietcombankSmartOtpIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietinbankIpayIcon],svg[arcticons-vietinbank-ipay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.5 30.483V17.5h13v12.983"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.15 34.341a43 43 0 0 1 37.7 0"></svg:path>`,
})
export class ArcticonsVietinbankIpayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietnamAirlinesIcon],svg[arcticons-vietnam-airlines-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.156 30.01c-1.875-.502-2.72-2.266-.237-4.748c1.117-1.118-.765-10.692-4.105-12.89"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.814 12.371c.85 3.024-1.675 6.091-2.238 10.516c-.564 4.424 1.704 6.953 4.206 8.412"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.576 22.887c-3.105-2.098-6.33-3.557-9.973-2.782c3.22 2.714 3.903 7.227 5.699 9.023c1.564 1.564 4.981 3.46 6.716 3.46"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.302 29.128c-4.624.136-6.4 2.79-7.802 4.546c2.92-.783 7.697 1.045 10.38 1.763s6.335-.705 9.12-2.312m11.698-3.997c4.624.136 6.4 2.79 7.802 4.546c-2.92-.783-7.697 1.045-10.38 1.763s-6.335-.705-9.12-2.312m5.186-20.754c-.85 3.024 1.675 6.091 2.238 10.516c.564 4.424-1.704 6.953-4.206 8.412"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.424 22.887c3.105-2.098 6.33-3.557 9.973-2.782c-3.22 2.714-3.902 7.227-5.699 9.023c-1.564 1.564-4.981 3.46-6.716 3.46m-4.138-2.578c1.87-.5 2.72-2.266.237-4.748c-1.117-1.118.765-10.692 4.105-12.89"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.394 28.316c0-1.944 5.21-1.944 5.21 0"></svg:path>`,
})
export class ArcticonsVietnamAirlinesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietnamnetIcon],svg[arcticons-vietnamnet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 18.903c3.454 1.994 9.9 10.464 11.966 18.17L43.5 10.927L16.466 29.54V18.903z"></svg:path><svg:circle cx="20.047" cy="15.358" r="2.659" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVietnamnetIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVietnamworksIcon],svg[arcticons-vietnamworks-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M7.845 26.893c1.238-6.187 2.97-10.394 7.672-13.364c-8.91-1.276-11.136 6.187-7.672 13.364m16.829 10.395c12.374-10.89 11.879-18.066 9.899-24.006c15.344 0 6.435 24.006-9.9 24.006M6.113 27.14c1.485 1.98 7.672 11.137 17.076 11.137c-11.88 6.93-20.046-.742-17.076-11.136m7.672-15.096c7.92-5.445 13.859-7.92 19.798.494c-4.95 0-7.424 1.238-9.9 1.238s-5.939-1.733-9.898-1.733"></svg:path>`,
})
export class ArcticonsVietnamworksIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViettelMoneyIcon],svg[arcticons-viettel-money-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.071 4.5h-6.285v5.571h6.5V37.93h-6.5v5.57h6.285a3 3 0 0 0 3-3v-33a3 3 0 0 0-3-3m-18.142 39h6.285v-5.571h-6.5V10.07h6.5V4.5H14.93a3 3 0 0 0-3 3v33a3 3 0 0 0 3 3"></svg:path>`,
})
export class ArcticonsViettelMoneyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViettinbankIcon],svg[arcticons-viettinbank-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.166 17.211H29.78m-11.614 0v11.59m11.614-11.59v11.58"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.522 33.114c5.84-3 12.461-4.692 19.478-4.692s13.638 1.693 19.478 4.692"></svg:path>`,
})
export class ArcticonsViettinbankIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViggoIcon],svg[arcticons-viggo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.9 38.7c0-16.237-13.163-29.4-29.4-29.4v9.6c10.935 0 19.8 8.865 19.8 19.8zm0-29.4h9.6v29.4h-9.6z"></svg:path>`,
})
export class ArcticonsViggoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVigilanteIcon],svg[arcticons-vigilante-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M28.22 7.93V13h2.43a1.82 1.82 0 0 1 1.62 2.67l-6.15 11.8a2.39 2.39 0 0 1-4.24 0l-6.15-11.8A1.82 1.82 0 0 1 17.35 13h2.43V7.93a2 2 0 0 0-2-2h-8.9a3.37 3.37 0 0 0-3 4.93l15.37 29.52a3.1 3.1 0 0 0 5.5 0l15.36-29.49a3.37 3.37 0 0 0-3-4.93h-8.92a2 2 0 0 0-1.97 1.97"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.59 19.68v-2.99L24 15.19l-2.59 1.5v2.99l2.59 1.5z"></svg:path>`,
})
export class ArcticonsVigilanteIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVigiloIcon],svg[arcticons-vigilo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.327 36.693L6.213 19.58a5.85 5.85 0 0 1 8.272-8.272l12.978 12.978l6.052-6.052a5.85 5.85 0 0 1 8.272 8.272L31.599 36.693a5.85 5.85 0 0 1-8.272 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m27.463 24.285l-3.653 3.653c2.747 2.747 5.786-1.506 8.468 1.176c1.771 1.772 1.947 4.954.328 6.572m-1.99-14.554l8.272 8.272m-14.577-8.272l-8.273 8.273"></svg:path>`,
})
export class ArcticonsVigiloIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVikiIcon],svg[arcticons-viki-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.294 21.174l-3.397 9.003l-3.397-9.003m13.826-4.586v13.589m.002-2.887l6.151-6.121m-4.194 4.173l4.836 4.814"></svg:path><svg:circle cx="21.692" cy="17.012" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.692 21.174v9.003"></svg:path><svg:circle cx="35.311" cy="17.012" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.311 21.174v9.003"></svg:path>`,
})
export class ArcticonsVikiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVikunjaIcon],svg[arcticons-vikunja-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M32.135 23.398c0 7.002-5.073 12.627-8.109 12.627c-3.077 0-9.066-5.625-9.066-12.627c0-.83.01-1.589.133-3.145c-.707-4.917 3.734-7.275 8.903-7.327c5.048.051 8.546 2.979 8.003 6.738c.142 1.456.136 2.76.136 3.734"></svg:path><svg:path d="M24 24.363c-2.525.338-3.689 2.56-3.097 5.437s4.507 4.274 5.988.804c1.48-3.47.058-6.354-2.891-6.241m-5.03-.698s.128-1.883 1.122-1.841s.889 1.904.889 1.904m5.786-.063s.127-1.883 1.121-1.841c.995.042.889 1.904.889 1.904m-8.909-10.203c.05-2.64.72-7.896-.23-7.544c0 0-4.511.513-4.635 6.987c-.048 2.552-.017 4.284.04 5.445m13.437-4.642c.111-2.706.015-8.583.843-7.85c0 0 5.273 2.18 2.528 11.403"></svg:path><svg:path d="M23.174 27.452s.838 1.09 2.004-.224M24 27.961v2.67m-2.635-.231c2.635.688 5.68-.182 5.68-.182m-7.222-12.535c-.16 1.193.927 1.107 1.361.988c-.461 1.366 1.151 2.19 1.837.8c.221 1.599 1.834 1.132 1.696.125m-5.554 17.74c.085 2.052 2.031 1.206 2.031 1.206s.297 2.243 2.243.952"></svg:path><svg:path d="M29.5 32c-.268 2.712.542 8.617 1.563 12.291A21.5 21.5 0 0 0 45.5 24c0-11.874-9.626-21.5-21.5-21.5S2.5 12.126 2.5 24a21.5 21.5 0 0 0 14.453 20.297c.542-4.596.438-13.186.438-13.186"></svg:path><svg:path d="M26.097 37.453c1.069.338 1-.808 1-.808s1.206.266.91-.962m-2.469 8.453c.893.678 1.213-.425 1.213-.425s1.047.656 1.18-.6"></svg:path></svg:g>`,
})
export class ArcticonsVikunjaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVimeoIcon],svg[arcticons-vimeo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.481 14.938q-.26 5.694-7.954 15.591q-7.953 10.334-13.457 10.336q-3.411 0-5.77-6.295l-3.147-11.542q-1.75-6.292-3.758-6.295a12 12 0 0 0-3.061 1.837L4.5 16.207q2.887-2.536 5.69-5.075q3.852-3.325 5.78-3.501q4.552-.438 5.604 6.222q1.137 7.187 1.577 8.937q1.314 5.963 2.89 5.96q1.223 0 3.678-3.868q2.448-3.867 2.625-5.888q.35-3.337-2.625-3.34a7.3 7.3 0 0 0-2.887.64q2.875-9.416 10.983-9.152q6.011.176 5.666 7.796"></svg:path>`,
})
export class ArcticonsVimeoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVimusicIcon],svg[arcticons-vimusic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.225 24.903a99 99 0 0 0-12.78-12.65q-2.701-2.038-.181-3.765l2.931-1.717a4.2 4.2 0 0 1 5.713.623a80.3 80.3 0 0 1 10.5 16.746l-.33-17.62a2.71 2.71 0 0 1 2.85-.923a22.97 22.97 0 0 0 11.385 3.584q1.707 0 1.637 1.837l-.13 6.676A1.334 1.334 0 0 1 40.313 19a25.3 25.3 0 0 1-10.802-2.801l.04 9.637v7.67a8.697 8.697 0 0 1-11.967 8.374c-8.373-3.293-6.626-15.893 2.64-16.977"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.145 33.617q1.957-.602 5.01 2.992c3.001 3.192-.954 5.27-3.103 2.7q-3.082-3.262-1.907-5.692"></svg:path>`,
})
export class ArcticonsVimusicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVinmonopoletIcon],svg[arcticons-vinmonopolet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 45.5c-1.093 0-2.082-.893-2.082-2.649c0-3.829 2.872-8.552 12.636-8.552c2.856 0 4.723 2.354 4.723 4.03c0 1.216-.91 1.818-2.01 1.818c-1.325 0-3.542-1.412-3.542-6.558c0-4.906 4.738-10.076 8.663-10.076c2.027 0 3.064.766 3.064 2.25c0 1.42-1.101 2.058-3.446 2.058c-5.728 0-25.705-11.057-25.705-22.353c0-1.07.16-2.968 1.963-2.968s2.33 1.644 2.33 3.462c0 1.82-2.107 10.746-10.747 10.746c-2.098 0-4.778-.885-4.778-3.278c0-1.947 1.69-2.394 3.031-2.394c2.074 0 3.81 2.074 5.67 4.953c3.526 5.459 7.85 12.565 10.23 16.779c2.38-4.214 6.704-11.32 10.23-16.78c1.859-2.878 3.595-4.952 5.67-4.952c1.34 0 3.031.447 3.031 2.394c0 2.393-2.68 3.278-4.779 3.278c-8.64 0-10.746-8.927-10.746-10.746s.527-3.462 2.33-3.462s1.962 1.899 1.962 2.968c0 11.296-19.976 22.353-25.704 22.353c-2.345 0-3.446-.638-3.446-2.058c0-1.484 1.037-2.25 3.063-2.25c3.925 0 8.664 5.17 8.664 10.076c0 5.146-2.218 6.558-3.542 6.558c-1.101 0-2.01-.602-2.01-1.819c0-1.675 1.866-4.029 4.722-4.029c9.765 0 12.637 4.723 12.637 8.552c0 1.755-.99 2.649-2.082 2.649"></svg:path>`,
})
export class ArcticonsVinmonopoletIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVintedIcon],svg[arcticons-vinted-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.23 5.82c4.69-.69 5.11 2.25 5 5.36c0 15 .24 24.84.82 25.37c4.07-9.75 7.45-25.5 9.45-28.34A10.2 10.2 0 0 1 34 5.72c1.2-.76 2.57-1.48 2.91-1.12c-.14 4.52-8 30.84-8.37 31.8a12.1 12.1 0 0 1-6.89 6.6c-6.39 1.52-6.82-.87-7.63-2.21c-1.89-4.1-3.25-28.36-2.78-31c.1-1.02.32-3.68 4.99-3.97"></svg:path>`,
})
export class ArcticonsVintedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVinylIcon],svg[arcticons-vinyl-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 2.5A21.5 21.5 0 1 0 45.5 24A21.51 21.51 0 0 0 24 2.5M24 8A16.06 16.06 0 0 0 8 24h0m16-10.38A10.38 10.38 0 0 0 13.62 24h0M24 17.86A6.14 6.14 0 1 1 17.86 24A6.14 6.14 0 0 1 24 17.86m0 16.52A10.38 10.38 0 0 0 34.38 24h0M24 40.05a16.06 16.06 0 0 0 16-16h0"></svg:path>`,
})
export class ArcticonsVinylIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViofoIcon],svg[arcticons-viofo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:circle cx="24" cy="25.529" r="6.742" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.742 17.71v-2.744a2.705 2.705 0 0 0-2.697-2.697h-8.09a2.705 2.705 0 0 0-2.697 2.697v2.743a10.3 10.3 0 0 0-3.591 7.82c0 5.707 4.626 10.333 10.333 10.333s10.333-4.626 10.333-10.333a10.3 10.3 0 0 0-3.591-7.82"></svg:path><svg:circle cx="24" cy="25.529" r="2.606" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.495 29.987l1.551-1.68M9.101 33.66l1.453-1.573M5.5 37.558l1.567-1.696m28.438-5.875l-1.551-1.68m4.945 5.353l-1.453-1.573m5.054 5.471l-1.567-1.696"></svg:path>`,
})
export class ArcticonsViofoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVipAccessIcon],svg[arcticons-vip-access-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="23.453" cy="24" r="20" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.139 19.695a25.8 25.8 0 0 1 9.968 11.283c2.243-4.282 12.507-19.643 19.44-25.42"></svg:path>`,
})
export class ArcticonsVipAccessIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViperIcon],svg[arcticons-viper-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.22 42.26L5.5 5.74h13.9l9.15 17.85l8.38-17.75h5.57Zm4.33-18.67l2.55 4.96"></svg:path>`,
})
export class ArcticonsViperIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVippsIcon],svg[arcticons-vipps-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="28.304" cy="16.948" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.083 23.803c5.977 8.485 14.174 11.149 21.577 1.15"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4z"></svg:path>`,
})
export class ArcticonsVippsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVipshopIcon],svg[arcticons-vipshop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.373 20.08v6.062c0 .703-.571 1.269-1.281 1.269H8.12v-7.33zm2.317-.844l-2.317 2.389m1.753-1.806v8.89m.27-7.832h4.548m-4.548 7.455h4.548m-4.548-4.97h4.548m-4.548 2.485h4.548m-1.895-6.453v8.938m8.023-3.895v3.058c0 .703-.572 1.27-1.282 1.27h-2.128v-4.329zm5.271 0v3.058c0 .703-.572 1.27-1.282 1.27h-2.128v-4.329zm-7.827-1.593h6.12a.85.85 0 0 0 .854-.846v-2.622h-6.974zm10.098 2.271h8.939m-6.603 0l-1.428 3.318h6.151c1.256 0 1.402-.602.76-1.636m-8.145-3.392l4.796-3.95l4.795 3.95m-8.742-.699h7.892"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsVipshopIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViralIcon],svg[arcticons-viral-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m36.843 42.5l-5.912-5.912m0 0l-3.605-3.605m.004-.003l-3.89-3.88m17.888 12.228L34.16 34.16m0 0l-3.41-3.41m0 0l-3.12-3.12m5.35-.3l-3.88-3.88m3.882 3.876l3.606 3.605m0 0l5.912 5.912M20.37 27.63l-3.13 3.13M5.5 24h8.918M24 18.87V5.5m17.328 1.171L25.91 22.09m-5.54-1.72l-3.14-3.14L6.67 6.67m19.28 15.46a2.701 2.701 0 1 1-3.9 3.74c-1.03-1.08-1-2.79.08-3.82c1.07-1.02 2.75-1 3.78.04z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.582 24A9.58 9.58 0 0 1 24 33.582h0A9.58 9.58 0 0 1 14.418 24h0a9.582 9.582 0 1 1 19.164 0"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.407 24c0 7.957-6.45 14.407-14.407 14.407h0c-7.957 0-14.407-6.45-14.407-14.407h0c0-7.957 6.45-14.407 14.407-14.407h0c7.957 0 14.407 6.45 14.407 14.407"></svg:path><svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.1 23.45a5.1 5.1 0 0 1-1.47 4.18a5.1 5.1 0 0 1-4.19 1.47a5.1 5.1 0 0 1-3.07-1.47c-.93-.93-1.5-2.21-1.5-3.63s.57-2.7 1.5-3.63s2.21-1.5 3.63-1.5s2.7.57 3.63 1.5c.81.81 1.35 1.88 1.47 3.08"></svg:path>`,
})
export class ArcticonsViralIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirginRadioSwitzerlandIcon],svg[arcticons-virgin-radio-switzerland-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m3.5 19.383l15.596.002L23.888 4.47l4.828 14.917l15.784.002l-12.778 9.284l4.81 14.856l-12.59-9.204l-12.583 9.143l4.772-14.853z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m16.596 27.26l1.175-3.152l1.448 7.73c.527-5.387 1.765-10.054 3.846-13.866m-1.158 8.163l-.226 2.842m1.85-.956l.07-2.612l1.03-.493m1.908-1.083c-.654.103-.868 1.09-.998 1.983c-.27 1.887 1.374 1.366 2.012.41m.063-2.589l-.2 9.968m1.876-10.823l-.18 2.444m1.633-.666l.088-1.942c.605-.753 1.494-1.804 1.956-1.2c.608.794-.504 2.84.766 2.71"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.745 23.786c-2.734.431-13.83 8.438-13.642 9.344"></svg:path><svg:circle cx="22.342" cy="24.188" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.721" cy="20.98" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVirginRadioSwitzerlandIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirginmoneyIcon],svg[arcticons-virginmoney-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 35.5v-23a4 4 0 0 0-4-4h-31a4 4 0 0 0-4 4v23a4 4 0 0 0 4 4h31a4 4 0 0 0 4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.348 20.4H43.5v7.2h-7.152a2.85 2.85 0 0 1-2.848-2.848v-1.504a2.85 2.85 0 0 1 2.848-2.848"></svg:path><svg:circle cx="36.472" cy="24" r=".795" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.5 27.545s2.11-3.932 2.617-4.468c0 0 2.363 7.506 2.617 10.484c.59-3.336 4.052-17.037 7.766-19.122"></svg:path>`,
})
export class ArcticonsVirginmoneyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtualAndroidIcon],svg[arcticons-virtual-android-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 3.5l17.754 10.25v20.5L24 44.5L6.246 34.25v-20.5z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M41.754 13.75L24 24L6.246 13.75M24 24v20.5m0-35.875v3.417"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.082 15.459L24 12.042l5.918 3.417"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 8.625l-8.877 5.125L24 18.875l8.877-5.125zM10.685 31.687l2.958-1.708m5.919 3.417l-5.919-3.417v-6.835"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.562 26.563v10.25l-8.877-5.125v-10.25zm8.877 0v10.25l8.876-5.125v-10.25zm8.877 5.124l-2.96-1.708"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m28.439 33.396l5.918-3.417v-6.835"></svg:path>`,
})
export class ArcticonsVirtualAndroidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtualMasterIcon],svg[arcticons-virtual-master-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.585 22.557v13.837L23.999 42.5l-10.583-6.106V22.557"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M40.62 32.7V11.278L24 21.343L7.379 11.28V32.7"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m36.593 5.5l-12.594 7.423L11.407 5.5"></svg:path>`,
})
export class ArcticonsVirtualMasterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtualMusicPlayerIcon],svg[arcticons-virtual-music-player-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M29.434 44.83c-1.252.326-2.53.539-3.82.636m-3.24-.074a21.5 21.5 0 0 1-3.77-.63m17.285-2.826a21.5 21.5 0 0 1-3.404 1.844m-16.957-.064a21.5 21.5 0 0 1-3.367-1.824m28.9-4.784a21.5 21.5 0 0 1-2.621 2.85m-28.854-.047a21.5 21.5 0 0 1-2.596-2.826m37.389-6.225a21.5 21.5 0 0 1-1.555 3.543m-37.629-.017a21.5 21.5 0 0 1-1.54-3.53m-.833-3.123a21.5 21.5 0 0 1-.322-3.699l.014-.16m42.973 0l.013.16a21.5 21.5 0 0 1-.324 3.698M2.795 20.655c.2-1.274.515-2.528.94-3.746m40.53 0c.425 1.218.74 2.472.94 3.746m-40.14-6.697a21.5 21.5 0 0 1 2.099-3.22m33.672 0a21.5 21.5 0 0 1 2.097 3.22M9.376 8.366A21.5 21.5 0 0 1 12.4 6.011m23.199 0a21.5 21.5 0 0 1 3.023 2.355m-23.359-3.89a21.5 21.5 0 0 1 3.617-1.244m10.238 0c1.24.306 2.451.722 3.618 1.244M22.088 2.7q.952-.126 1.912-.166q.96.04 1.912.166M24 8c8.837 0 16 7.164 16 16M24 40A16 16 0 0 1 8 24"></svg:path><svg:circle cx="24" cy="24" r="10.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.581 19.517a.88.88 0 0 0-.439.762v7.38a.878.878 0 0 0 1.317.763l3.192-1.846l3.19-1.845a.88.88 0 0 0 0-1.523l-3.19-1.846l-3.192-1.845a.88.88 0 0 0-.878 0"></svg:path>`,
})
export class ArcticonsVirtualMusicPlayerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtualPagIbigIcon],svg[arcticons-virtual-pag-ibig-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.139 24.072V36.37a4.11 4.11 0 0 1-4.1 4.116h-8.208c-.794-1.985.393-5.144 2.536-5.955c1.57-.594 3.965-2.329 4.905-3.864c.598.492.47 2.556-1.838 3.87c-.588.336-.312 1.363.565 1.03c1.951-.743 3.904-2.507 3.987-6.312c.038-1.742-.07-3.553.458-4.58c.314-.61.908-.912 1.695-.602zm-26.278 0V36.37a4.11 4.11 0 0 0 4.1 4.116h8.208c.794-1.985-.393-5.144-2.536-5.955c-1.57-.594-3.965-2.329-4.905-3.864c-.598.492-.47 2.556 1.838 3.87c.588.336.312 1.363-.565 1.03c-1.951-.743-3.904-2.507-3.987-6.312c-.037-1.742.07-3.553-.458-4.58c-.314-.61-.908-.912-1.695-.602zm27.975-8.317c.71.309 1.752.176 2.198-.753s-.088-2.2-.92-2.563L24 5.5L7.887 12.44c-.833.361-1.367 1.633-.921 2.562c.446.93 1.487 1.062 2.198.753L24 9.401zm.492.227v21.805a4.703 4.703 0 0 1-4.693 4.713H13.386a4.703 4.703 0 0 1-4.714-4.692V15.982m22.451 4.277a8.27 8.27 0 0 1 1.15 4.206h0a8.27 8.27 0 0 1-8.272 8.271H24a8.27 8.27 0 0 1-8.272-8.271h0a8.27 8.27 0 0 1 8.271-8.272H24a8.27 8.27 0 0 1 4.081 1.077m-1.426 1.618l2.984 2.752m2.569-3.28a2.21 2.21 0 1 1-4.42 0a2.21 2.21 0 0 1 4.42 0"></svg:path>`,
})
export class ArcticonsVirtualPagIbigIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtualSoftKeysIcon],svg[arcticons-virtual-soft-keys-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12.53 19.197v9.607L4.5 24zM28.145 24c0 2.475-1.934 4.481-4.32 4.481h0c-2.385 0-4.32-2.006-4.32-4.481h0c0-2.475 1.935-4.481 4.32-4.481h0c2.386 0 4.32 2.006 4.32 4.481m7.553-4.047H43.5v8.093h-7.802z"></svg:path>`,
})
export class ArcticonsVirtualSoftKeysIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirtuosityIcon],svg[arcticons-virtuosity-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.38 4.5s4.36 7.5 4.36 10.05s-5.08 5.36-5.08 8.07c0 3 8.34 12.87 8.34 12.87s-10-6.63-10 .61a8.78 8.78 0 0 0 3.76 7.4"></svg:path>`,
})
export class ArcticonsVirtuosityIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirustotalIcon],svg[arcticons-virustotal-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 7.5h-37l17 16.5l-17 16.5h37z"></svg:path>`,
})
export class ArcticonsVirustotalIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVirustotalMobileIcon],svg[arcticons-virustotal-mobile-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m38.232 11.573l-3.5-6.073l-28.51 8.093l21.41 9.413L15.14 42.5l26.638-7.162v-1.982"></svg:path>`,
})
export class ArcticonsVirustotalMobileIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViruswarIcon],svg[arcticons-viruswar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="5.23" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 10.96V7.95"></svg:path><svg:circle cx="24" cy="24.05" r="13.09" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="36.1" cy="9.63" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m32.41 14.02l1.94-2.3"></svg:path><svg:circle cx="42.54" cy="20.78" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m36.89 21.77l2.96-.52"></svg:path><svg:circle cx="40.3" cy="33.46" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m35.34 30.59l2.6 1.5"></svg:path><svg:circle cx="30.44" cy="41.73" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m28.48 36.35l1.02 2.82"></svg:path><svg:circle cx="17.56" cy="41.73" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m19.52 36.35l-1.02 2.82"></svg:path><svg:circle cx="7.7" cy="33.46" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.66 30.59l-2.6 1.5"></svg:path><svg:circle cx="5.46" cy="20.78" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m11.11 21.77l-2.96-.52"></svg:path><svg:circle cx="11.9" cy="9.63" r="2.73" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m15.59 14.02l-1.94-2.3"></svg:path>`,
})
export class ArcticonsViruswarIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisecaoneIcon],svg[arcticons-visecaone-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.343 36.47a3.36 3.36 0 0 1-2.387.99h-.002a3.375 3.375 0 0 1-3.375-3.376v-2.226a3.375 3.375 0 0 1 3.376-3.375h.001a3.375 3.375 0 0 1 3.375 3.375v1.012m0 .101h-6.752"></svg:path><svg:rect width="6.748" height="8.976" x="10.669" y="28.483" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="3.374" ry="3.374"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.625 31.858a3.374 3.374 0 0 1 3.374-3.375h0a3.374 3.374 0 0 1 3.374 3.375v5.601m-6.748-8.976v8.976"></svg:path><svg:circle cx="32.54" cy="15.46" r="4.791" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path>`,
})
export class ArcticonsVisecaoneIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisibleIcon],svg[arcticons-visible-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m13.03 22.75l-2 5.3l-2-5.3m8.925 4.853c.365.307.76.447 1.645.447h.449c.73 0 1.322-.593 1.322-1.325s-.592-1.325-1.322-1.325h-.898c-.73 0-1.322-.593-1.322-1.325s.592-1.325 1.322-1.325h.449c.885 0 1.28.14 1.645.447m4.926 1.453a2 2 0 1 1 4 0v1.3a2 2 0 1 1-4 0m0 2v-8m5.75 0v7a1 1 0 0 0 1 1h.3m5.488-1.009a2 2 0 0 1-3.738-.99v-1.3a2 2 0 1 1 4 0v.65h-4"></svg:path><svg:circle cx="15.429" cy="21.75" r=".75" fill="currentColor"></svg:circle><svg:circle cx="23.771" cy="21.75" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVisibleIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisitKoreaKrIcon],svg[arcticons-visit-korea-kr-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.313 17.131c3.767 0 6.824 3.08 6.824 6.869s-3.057 6.869-6.824 6.869C6.557 30.869 3.5 27.789 3.5 24s3.057-6.869 6.813-6.869m19.344 13.817h7.85c3.868-.006 7-3.122 6.993-6.96s-3.147-6.942-7.016-6.936H29.68c-3.868-.006-7.01 3.1-7.016 6.937s3.125 6.953 6.993 6.959M22.212 24h-5.076"></svg:path>`,
})
export class ArcticonsVisitKoreaKrIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisitluxembourgIcon],svg[arcticons-visitluxembourg-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 5.5h7.276l7.367 7.36v7.284h-7.24L5.5 12.85zm36.998.001l-7.274.001l-7.367 7.358v7.284h7.24L42.5 12.85zM5.5 42.5h7.276l7.367-7.359l-.002-7.283h-7.238L5.5 35.15zm37 0h-7.276l-7.367-7.359v-7.284h7.241l7.401 7.294z"></svg:path>`,
})
export class ArcticonsVisitluxembourgIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisualStudioCodeIcon],svg[arcticons-visual-studio-code-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m16.362 28.43l-6.35 4.81c-.59.45-1.42.41-1.97-.09l-2.04-1.85c-.67-.61-.67-1.67 0-2.28l5.51-5.02m4.84-4.42l.01-.01L30.93 6.282c.01-.01.03-.02.04-.03c.04-.04.07-.07.11-.09c.01-.02.03-.03.04-.04c.06-.04.12-.09.18-.13c.01-.01.02-.02.03-.02c.06-.04.11-.08.17-.11h.01c.05-.02.09-.05.14-.06c.04-.03.08-.04.13-.05a.3.3 0 0 1 .1-.04c.07-.02.14-.04.21-.05c.01-.01.03-.01.04-.01l.23-.03c.21-.02.43-.02.64.03c.02 0 .04 0 .07.01s.05.01.07.02q.21.045.42.15h.01l7.62 3.67c.09.05.18.1.27.16s.17.13.25.2h.01s.01 0 .01.01c.01 0 .02.01.03.02c.03.02.06.05.08.08c.03.03.06.07.1.12c0 0 .01 0 .02.01c.03.03.05.05.06.08c.05.05.08.11.12.17c.05.06.08.13.11.2c.01.02.03.05.03.08c.03.05.05.1.07.15q.01.01.01.02l.06.18c.02.06.03.13.04.19l.03.18c.01.08.01.15.01.23v.09c0-.11-.01-.22-.03-.32v-.01M22.191 24.01V24l11.049-8.379l.02-.01"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m33.26 32.389l-.02-.01L22.19 24v-.01l-5.83-4.42l-6.348-4.809c-.59-.45-1.42-.41-1.97.09l-2.04 1.85c-.67.61-.67 1.67 0 2.28L11.513 24l4.839 4.42l.01.01L30.93 41.718h.01c.06.07.12.12.18.16c.12.09.24.18.38.25c0 0 0 .01.01.01c.13.06.27.11.41.15q.105.045.21.06c.14.02.27.04.41.04c.35.01.7-.07 1.02-.22h.01l7.619-3.67c.09-.05.18-.1.27-.16c.09-.07.19-.14.27-.21s.16-.15.23-.22c.34-.41.54-.93.54-1.49v-.09c0 .11-.01.22-.03.32v.01"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.573 42.165a2.3 2.3 0 0 1-2.628-.448c.853.853 2.311.25 2.311-.957V7.24c0-1.206-1.458-1.81-2.31-.958a2.305 2.305 0 0 1 2.627-.447l7.618 3.663a2.31 2.31 0 0 1 1.309 2.083V36.42c0 .888-.509 1.698-1.31 2.083z"></svg:path>`,
})
export class ArcticonsVisualStudioCodeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVisualTimerIcon],svg[arcticons-visual-timer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.352 39.202A21.5 21.5 0 1 1 27.15 2.5V24z"></svg:path>`,
})
export class ArcticonsVisualTimerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVitaIcon],svg[arcticons-vita-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="36.465" cy="15.816" r="6.035" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.115 38.388c-1.832 0-3.64-.83-4.827-2.406L6.713 19.277a6.036 6.036 0 0 1 9.644-7.259l12.575 16.706a6.036 6.036 0 0 1-4.817 9.663Z"></svg:path>`,
})
export class ArcticonsVitaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVita3kIcon],svg[arcticons-vita3k-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="39.77" height="18.081" x="4.115" y="14.96" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="7.708" ry="7.708" transform="rotate(-21.909 24 24)"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m10.327 21.647l21.91-8.811l5.13 12.756l-21.91 8.811z"></svg:path><svg:circle cx="39.355" cy="14.847" r=".75" fill="currentColor"></svg:circle><svg:circle cx="36.239" cy="16.1" r=".75" fill="currentColor"></svg:circle><svg:circle cx="38.423" cy="17.031" r=".75" fill="currentColor"></svg:circle><svg:circle cx="37.171" cy="13.916" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m6.479 28.086l.614-.246m2.022-.814l.614-.247m-2.279-.971l.247.614m.813 2.022l.247.614"></svg:path><svg:circle cx="10.813" cy="31.526" r="1.243" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="38.616" cy="20.344" r="1.243" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVita3kIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVituneIcon],svg[arcticons-vitune-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.632 17.538v-4.42c-2.788.428-4.125-.437-5.942-1.321v4.419c1.813.884 3.154 1.749 5.941 1.322M24.69 25.22a4.181 4.181 0 1 0-8.362-.004a4.181 4.181 0 1 0 8.362.008v-9.008m11.596 8.993l6.957 12.819c.725 1.337-.235 1.94-1.005 1.94H28.252c-.77 0-1.73-.604-1.005-1.94l6.961-12.82c.197-.366.567-.698 1.039-.698s.838.332 1.035.699zm-13.02 12.705l14.05-25.947c1.466-2.712-.471-3.936-2.024-3.936H7.045c-1.553 0-3.49 1.224-2.025 3.932L19.07 37.91c.404.744 1.149 1.417 2.1 1.417c.948 0 1.692-.673 2.097-1.413Z"></svg:path>`,
})
export class ArcticonsVituneIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViuIcon],svg[arcticons-viu-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.139 21.968v10.161m-4.804-16.258v16.258L4.5 15.871m24.165 0v10.873a5.385 5.385 0 0 0 5.385 5.385h4.065a5.385 5.385 0 0 0 5.385-5.385V15.87m-19.361 2.033v-2.032"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m39.971 21.968l-6.687-3.861v7.722z"></svg:path>`,
})
export class ArcticonsViuIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsViu1Icon],svg[arcticons-viu-1-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.241 12.414L32.75 24.033L15.241 35.652"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsViu1Icon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivaWalletIcon],svg[arcticons-viva-wallet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m33.674 10.081l9.826 4.704l-9.244 18.673c-3.93 7.939-8.082 3.754-10.643-1.77l-9.963-21.49L4.5 14.69l8.358 18.278c5.243 11.466 10.378-.47 10.755-1.28z"></svg:path>`,
})
export class ArcticonsVivaWalletIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivacutIcon],svg[arcticons-vivacut-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.507 21.778L37.92 5.879c.575-.735 1.754-.33 1.754.604v19.63c0 .841-.334 1.649-.93 2.244L25.027 42.075a1.45 1.45 0 0 1-2.052 0L9.256 28.357a3.17 3.17 0 0 1-.93-2.245V6.482c0-.932 1.18-1.338 1.755-.603l12.412 15.899c.765.98 2.249.98 3.014 0m3.838 15.977L8.327 16.737"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m22.493 21.778l.336.43c3.622 4.64 10.288 5.537 15.008 2.02l1.836-1.37"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m23.948 22.509l1.475.248a12.8 12.8 0 0 0 11.756-4.198l2.494-2.853"></svg:path>`,
})
export class ArcticonsVivacutIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivaldiIcon],svg[arcticons-vivaldi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.68 15.1q-7.22 12.58-14.45 25.15a4.83 4.83 0 0 1-4 2.63a4.62 4.62 0 0 1-4.57-2.43c-3.05-5.26-6.06-10.53-9.09-15.8c-1.84-3.2-3.69-6.4-5.52-9.61A4.86 4.86 0 0 1 9 7.68a4.66 4.66 0 0 1 4.46 2.51c1.36 2.33 2.69 4.68 4 7c1 1.69 1.92 3.38 2.91 5.05a7.64 7.64 0 0 0 6.4 3.95A7.79 7.79 0 0 0 35 19.28c0-.32.06-.63.07-.8a8.2 8.2 0 0 0-.82-3.63a4.86 4.86 0 1 1 9.07-3a5.06 5.06 0 0 1-.67 3.28"></svg:path>`,
})
export class ArcticonsVivaldiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivaldiSnapshotIcon],svg[arcticons-vivaldi-snapshot-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m29.028 38.863l-.793 1.384a4.82 4.82 0 0 1-3.975 2.63a4.61 4.61 0 0 1-4.571-2.432c-3.048-5.253-6.062-10.527-9.089-15.792l-5.52-9.611a4.856 4.856 0 0 1 3.945-7.36a4.65 4.65 0 0 1 4.465 2.508c1.359 2.331 2.693 4.676 4.038 7.014c.97 1.684 1.92 3.38 2.917 5.047a7.63 7.63 0 0 0 6.391 3.953a7.784 7.784 0 0 0 8.19-6.92c.036-.32.06-.638.074-.8a8.16 8.16 0 0 0-.825-3.636a4.855 4.855 0 1 1 9.073-3.025a5.08 5.08 0 0 1-.665 3.281m0 0l-7.17 12.473"></svg:path><svg:circle cx="34.5" cy="34.5" r="7" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.008 37.624a2.24 2.24 0 0 0 1.962.876h1.184a2 2 0 0 0 1.996-2h0a2 2 0 0 0-1.996-2h-1.308a2 2 0 0 1-1.996-2h0a2 2 0 0 1 1.996-2h1.184a2.24 2.24 0 0 1 1.962.877"></svg:path>`,
})
export class ArcticonsVivaldiSnapshotIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivaldiSopranosIcon],svg[arcticons-vivaldi-sopranos-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m42.68 15.1l-.996 1.735"></svg:path><svg:path fill="none" stroke="currentColor" stroke-dasharray="0 0 3.967 3.967" stroke-linecap="round" stroke-linejoin="round" d="M39.709 20.276q-5.736 9.99-11.479 19.974a4.83 4.83 0 0 1-4 2.63a4.62 4.62 0 0 1-4.57-2.43c-3.05-5.26-6.06-10.53-9.09-15.8c-1.84-3.2-3.69-6.4-5.52-9.61A4.86 4.86 0 0 1 9 7.68a4.66 4.66 0 0 1 4.46 2.51c1.36 2.33 2.69 4.68 4 7c1 1.69 1.92 3.38 2.91 5.05a7.64 7.64 0 0 0 6.4 3.95A7.79 7.79 0 0 0 35 19.28c0-.32.06-.63.07-.8a8.2 8.2 0 0 0-.82-3.63a4.86 4.86 0 1 1 8.96-3.563"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.315 13.256a5.1 5.1 0 0 1-.665 1.874"></svg:path>`,
})
export class ArcticonsVivaldiSopranosIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivavideoIcon],svg[arcticons-vivavideo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.253 12.854c1.728-3.557 5.368-6.022 9.578-6.022h13c5.855 0 10.669 4.814 10.669 10.668v13c0 5.871-4.814 10.669-10.668 10.669h-13c-4.429 0-8.254-2.735-9.864-6.61"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m15.63 22.772l-8.646-4.991a.99.99 0 0 0-1.484.856v9.983a.99.99 0 0 0 1.484.857l8.645-4.991a.99.99 0 0 0 0-1.714m14.078-7.503l-.09 5.45l5.156 1.768c.867.297.847 1.531-.03 1.8l-5.21 1.6l-.089 5.448c-.015.917-1.194 1.28-1.721.529l-2.736-3.898a.95.95 0 0 0-1.052-.361l-4.553 1.397c-.877.269-1.585-.741-1.034-1.474l3.275-4.356l-3.131-4.461c-.527-.75.215-1.737 1.082-1.44l5.155 1.77l3.275-4.356c.551-.733 1.718-.333 1.703.584"></svg:path>`,
})
export class ArcticonsVivavideoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivedIcon],svg[arcticons-vived-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m21.657 41.852l6.723-5.24L17.715 28.5v11.813c-.007 1.636 1.882 3.061 3.942 1.539"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m28.375 36.618l7.871-6.01c.885-.673 1.026-2.221-.035-3.103l-21.68-16.511c-1.147-.761-3.382.03-3.436 1.18L11.04 22.55c.038.684.199.786 1.031 1.471zm2.034-13.708l5.182-4.011c.83-.6.676-.795.715-1.369V7.593c-.128-1.48-2.046-2.681-3.35-1.785l-12.398 9.508"></svg:path>`,
})
export class ArcticonsVivedIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivinoIcon],svg[arcticons-vivino-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="27.322" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="13.981" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="40.663" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="31.675" cy="27.322" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="16.325" cy="27.322" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="27.837" cy="34.011" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="20.163" cy="34.011" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="27.837" cy="20.674" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="27.837" cy="7.337" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="20.163" cy="20.674" r="3.837" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVivinoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivintIcon],svg[arcticons-vivint-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.55 8.83l-12 10.92a3.24 3.24 0 0 0-1.06 2.4v16.22a1.08 1.08 0 0 0 1.08 1.08H31a1.08 1.08 0 0 0 1.08-1.08V22.15a3.24 3.24 0 0 0-1.08-2.4L19 8.83a1.09 1.09 0 0 0-1.45 0"></svg:path><svg:circle cx="39.18" cy="35.12" r="4.32" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVivintIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoIcon],svg[arcticons-vivo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m38.65 21.658l-.943-2.466a1.657 1.657 0 0 0-2.094-.972l-7.512 2.62a1.058 1.058 0 0 1-1.407-1v-.324c0-.418.254-.78.628-.967a7.42 7.42 0 0 0 3.935-8.206c-.579-2.811-2.846-5.088-5.656-5.675c-4.797-1.003-9.021 2.631-9.021 7.252a7.41 7.41 0 0 0 4.1 6.63c.373.187.626.546.626.963v.328c0 .728-.719 1.239-1.406.999l-7.513-2.62a1.657 1.657 0 0 0-2.093.973l-.944 2.465a1.657 1.657 0 0 0 .986 2.152l7.017 2.527a3.17 3.17 0 0 1 2.098 2.984h0a3.17 3.17 0 0 1-.684 1.967l-6.141 7.769a1.87 1.87 0 0 0 .265 2.595l1.68 1.41a1.87 1.87 0 0 0 2.672-.273L24 34.242l6.757 8.547a1.87 1.87 0 0 0 2.672.273l1.68-1.41a1.87 1.87 0 0 0 .265-2.595l-6.141-7.769a3.17 3.17 0 0 1-.684-1.967h0c0-1.338.84-2.531 2.098-2.985l7.017-2.526a1.657 1.657 0 0 0 .986-2.152"></svg:path>`,
})
export class ArcticonsVivoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoAppstoreIcon],svg[arcticons-vivo-appstore-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:rect width="37" height="37" x="5.5" y="5.5" rx="5.34" ry="5.34"></svg:rect><svg:path d="M32.343 11.785a8.637 8.637 0 0 1-16.686 0m-9.85 21.852s8.218-4.807 17.182-.747c12.475 5.652 19.498-1.172 19.498-1.172"></svg:path></svg:g>`,
})
export class ArcticonsVivoAppstoreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoCloudIcon],svg[arcticons-vivo-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.783 35.573h23.98c13.015 0 10.838-19.586-2.216-15.243c0-10.876-19.586-10.876-19.586 2.178C2.073 20.33 2.073 35.573 10.783 35.573"></svg:path>`,
})
export class ArcticonsVivoCloudIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoConsoleIcon],svg[arcticons-vivo-console-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M40.04 14.75v18.5L24 42.5L7.96 33.25v-18.5L24 5.5zm-32.08.086L24 23.901m0 .017l16.04-9.082M24 42.244V23.918"></svg:path><svg:path d="m24 39.59l-13.486-7.777V16.277"></svg:path><svg:path d="m23.998 34.263l-8.934-5.153V18.85"></svg:path></svg:g>`,
})
export class ArcticonsVivoConsoleIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoEasyIcon],svg[arcticons-vivo-easy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.618 26.247c-2.573.596-2.525 2.43-1.174 4.07l5.428 6.59c.975 1.183.785 2.527.138 3.092l-1.95 1.704c-1.038.907-2.147.734-2.992-.205l-5.025-5.585c-.658-.731-1.793-.619-2.298 0l-4.646 5.694c-.608.745-1.876 1.375-2.983.387l-2.445-2.18c-.548-.489-.548-1.68-.049-2.278l6.504-7.8c.57-.683 1.19-2.506-1.027-3.062l-5.575-1.396c-1.47-.369-2.036-.827-1.858-1.938l.489-3.053c.176-1.097 1.045-1.758 2.677-1.369l5.74 1.369c1.38.33 2.45-1.271 1.036-2.184c-1.821-1.175-3.487-3.29-3.438-5.714c.053-2.632 1.852-6.566 6.724-6.88c3.308-.214 7.035 2.781 7.018 7.22c-.01 2.302-1.594 3.86-3.343 5.374c-1.152.998-.21 2.518 1.186 2.184l5.295-1.265c1.685-.403 2.538.01 2.73.974l.538 2.714c.266 1.342-.62 2.13-1.467 2.325z"></svg:path>`,
})
export class ArcticonsVivoEasyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoGamecenterIcon],svg[arcticons-vivo-gamecenter-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m41.846 15.218l-.047-.127c-.962-2.58-4.246-3.34-6.243-1.446l-25 23.713c-2.392 2.27-6.322.379-6.042-2.907l1.64-19.233a6.087 6.087 0 0 1 6.065-5.57H24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.175 15.026c.978-2.56 4.244-3.302 6.232-1.416l25.036 23.748c2.393 2.27 6.323.379 6.043-2.907l-1.64-19.233a6.087 6.087 0 0 0-6.065-5.57H24"></svg:path><svg:circle cx="13.788" cy="24.657" r="2.758" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.534 27.415v-5.516m-2.758 2.758h5.516"></svg:path>`,
})
export class ArcticonsVivoGamecenterIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoIManagerIcon],svg[arcticons-vivo-i-manager-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 43.5c1.693 0 15.274-7.782 15.274-16.966V6.504C35.267 6.503 29.133 4.5 24 4.5S12.723 6.503 8.727 6.503v20.03C8.727 35.719 22.307 43.5 24 43.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m16.135 21.997l5.243 5.243l10.487-10.486"></svg:path>`,
})
export class ArcticonsVivoIManagerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoThemesIcon],svg[arcticons-vivo-themes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.073 4.5h21.854A2.567 2.567 0 0 1 37.5 7.073l-.464 14.917H10.783L10.5 7.073C10.476 5.648 11.648 4.5 13.073 4.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.036 21.99v4.632c.172 1.885-2.133 1.862-3.628 1.87c-5.493.028-6.634 2.567-6.469 5.291c.156 2.572.197 2.898.232 4.349c.085 3.487-1.595 5.368-3.262 5.368s-3.346-1.88-3.261-5.368c.035-1.451.076-1.777.232-4.349c.164-2.724-.976-5.263-6.47-5.291c-1.495-.008-3.8.015-3.627-1.87V21.99"></svg:path>`,
})
export class ArcticonsVivoThemesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoTipsIcon],svg[arcticons-vivo-tips-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.005 43.5h12.188m-19.5-28.667C12.429 8.353 17.39 4.5 24.099 4.5s11.67 3.852 13.406 10.333s-1.502 13.125-7.312 16.48v7.312H18.005v-7.312c-7.65-3.654-9.049-10-7.312-16.48"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.28 18.296s-.786-8.599 9.888-9.982"></svg:path>`,
})
export class ArcticonsVivoTipsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivoVivoIcon],svg[arcticons-vivo-vivo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.877 12.069c-8.894 0-10.872 12.99 0 23.862c10.858-10.858 8.894-23.862 0-23.862"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.684 19.056c6.289-6.29 16.873 1.499 16.873 16.873C8.201 35.93.394 25.345 6.684 19.056m34.633 0c-6.29-6.29-16.874 1.499-16.874 16.873c15.356 0 23.163-10.584 16.873-16.873"></svg:path>`,
})
export class ArcticonsVivoVivoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVivocomIcon],svg[arcticons-vivocom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.517 28.615a3.493 3.493 0 0 1-3.482-3.483v-2.264c0-1.915 1.567-3.482 3.482-3.482S39 20.953 39 22.868v2.264a3.493 3.493 0 0 1-3.483 3.482M8 19.386l3.483 9.229l3.482-9.23m7.307.001l3.483 9.229l3.482-9.23m-10.618.001v9.229"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 35.5v-23a4 4 0 0 0-4-4h-31a4 4 0 0 0-4 4v23a4 4 0 0 0 4 4h31a4 4 0 0 0 4-4"></svg:path>`,
})
export class ArcticonsVivocomIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVixIcon],svg[arcticons-vix-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4m-9.603 12.179L37.5 30.665m0-12.986l-8.603 12.986"></svg:path><svg:circle cx="23.389" cy="18.085" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.389 22.062v8.603m-4.286-12.986l-4.301 12.986L10.5 17.679"></svg:path>`,
})
export class ArcticonsVixIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVizMangaIcon],svg[arcticons-viz-manga-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="39" height="31" x="4.5" y="8.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 18.497c0 6.078 4.927 11.006 11.006 11.006V18.497m6.989 0H38.5c-6.078 0-11.005 4.928-11.005 11.006H38.5M24 18.497v10.765"></svg:path>`,
})
export class ArcticonsVizMangaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVizioIcon],svg[arcticons-vizio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M18.797 19.75v8.5m10.248-8.5v8.5m-12.456-8.5l-2.816 8.5l-2.815-8.5"></svg:path><svg:rect width="5.631" height="8.5" x="31.411" y="19.75" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.816" ry="2.816"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M21.1 19.75h5.632l-5.632 8.5h5.632"></svg:path>`,
})
export class ArcticonsVizioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkIcon],svg[arcticons-vk-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.55 35.19v-6.64c4.46.68 5.87 4.19 8.71 6.64h7.24a29.36 29.36 0 0 0-7.9-10.47c2.6-3.58 5.36-6.95 6.71-12.06h-6.58c-2.58 3.91-3.94 8.49-8.18 11.51V12.66H18l2.28 2.82v10.05c-3.7-.43-6.2-7.2-8.91-12.87H4.5c2.5 7.66 7.76 24.47 23.05 22.53"></svg:path>`,
})
export class ArcticonsVkIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkMessengerIcon],svg[arcticons-vk-messenger-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.78 5.5c-.45 1.62-.69 3.34-.69 5.11c0 10.61 8.6 19.21 19.21 19.21v6.13c5.11 0 9.76-2 13.2-5.27"></svg:path><svg:circle cx="17.969" cy="14.493" r="2.903" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="26.953" cy="14.493" r="2.903" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="35.938" cy="14.493" r="2.903" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVkMessengerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkMusicIcon],svg[arcticons-vk-music-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.493 18.937c-1.622-1.277-3.206-2.607-4.983-3.672c-1.792-1.073-3.678-1.9-5.752-2.27c-2.244-.403-4.36-.017-6.384.97c-1.655.806-3.16 1.855-4.668 2.9c-2.713 1.882-5.489 3.646-8.681 4.632c-2.025.625-4.1.908-6.211.966c-.102.003-.205.012-.307.018"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4M12.175 29.67v-4.032m5.913 4.032v-4.032M24 32.021v-8.734m5.912-4.308V36.33m5.913-13.043v8.734"></svg:path>`,
})
export class ArcticonsVkMusicIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkPlayIcon],svg[arcticons-vk-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 14.75h37m0 14.5s-6.27 4-18.5 4s-18.5-4-18.5-4"></svg:path><svg:circle cx="30.929" cy="24" r="4.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m20.253 20.818l-6.355 6.356m0-6.348l6.348 6.348"></svg:path>`,
})
export class ArcticonsVkPlayIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkVideoIcon],svg[arcticons-vk-video-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.24 31.819c-2 1.18-3.749-.426-3.582-2.652V19.032c-.167-2.226 1.582-3.831 3.582-2.652l8.523 5.027c1.245.734 2.41 1.275 2.505 2.693c.095 1.417-1.361 2.018-2.505 2.692zM20.186 42.5a8.48 8.48 0 0 1-8.5-8.462V14a8.48 8.48 0 0 1 8.46-8.5h.04M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4"></svg:path>`,
})
export class ArcticonsVkVideoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkXIcon],svg[arcticons-vk-x-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 15.005h21.629M5.5 22.713h21.629M5.5 30.421h15.961m12.333-12.333l7.777-1.377c.537-.096.929-.563.929-1.108v-4.134c0-.707-.643-1.238-1.337-1.105L34.707 11.6c-.53.102-.913.566-.913 1.105V30.42c0 1.95.295 6.02-3.276 6.904c-4.125 1.02-5.68-.409-5.781-2.993c-.102-2.585 2.04-3.605 4.455-4.149l2.14-.483a3.156 3.156 0 0 0 2.462-3.087v-.68"></svg:path>`,
})
export class ArcticonsVkXIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVklassIcon],svg[arcticons-vklass-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 9.375h9.75l9.75 19.5h-9.75Zm39 0h-9.75L24 28.875l4.875 9.75Z"></svg:path>`,
})
export class ArcticonsVklassIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVkusvillIcon],svg[arcticons-vkusvill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.008 24c1.819 0 3.294 1.506 3.294 3.365s-1.475 3.365-3.294 3.365h-5.434V17.27h5.434c1.819 0 3.293 1.507 3.293 3.365S18.827 24 17.008 24m0 0h-5.434m21.559 0c1.819 0 3.294 1.506 3.294 3.365s-1.475 3.365-3.294 3.365h-8.728V17.27h8.728c1.819 0 3.294 1.507 3.294 3.365S34.952 24 33.133 24m0 0h-8.728"></svg:path>`,
})
export class ArcticonsVkusvillIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVlcIcon],svg[arcticons-vlc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.913 18.8a10.64 10.64 0 0 0 5.768-1.97l2.82 8.44a14.93 14.93 0 0 1-8.647 2.99a14.93 14.93 0 0 1-8.646-2.99l2.82-8.44a10.64 10.64 0 0 0 5.769 1.97Zm-.058-14.3c-1.413 0-2.084 1.377-2.397 2.332l-.983 2.866a5.77 5.77 0 0 0 3.38.961a5.77 5.77 0 0 0 3.38-.96l-.983-2.867c-.312-.955-.984-2.332-2.397-2.332m0 32.063a16.55 16.55 0 0 1-11.208-4.033l1.122-3.272h-2.818c-1.093 0-1.538.363-1.814 1.22l-3.24 10.949C5.574 42.69 5.707 43.5 7.16 43.5h33.392c1.453 0 1.586-.81 1.261-2.073l-3.24-10.948c-.275-.858-.72-1.221-1.813-1.221h-2.818l1.122 3.272a16.55 16.55 0 0 1-11.208 4.033"></svg:path>`,
})
export class ArcticonsVlcIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVlibIcon],svg[arcticons-vlib-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24.01 34.798l-.913 4.953l-2.588-4.321m9.144.821a1.78 1.78 0 0 1-1.084 1.42h0a1.78 1.78 0 0 1-2.323-.962l-.442-1.068a1.78 1.78 0 0 1 .963-2.323h0a1.78 1.78 0 0 1 2.323.963l.22.533l-3.285 1.36m2.62-6.274l3.606 5.073a.89.89 0 0 0 1.24.21q0 0 0 0l.217-.155"></svg:path><svg:circle cx="31.325" cy="28.073" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m32.428 29.51l3.034 3.605m.395-4.326a1.78 1.78 0 0 1-.325-2.494h0a1.78 1.78 0 0 1 2.494-.325l.916.705c.778.599.924 1.715.325 2.494h0a1.78 1.78 0 0 1-2.494.325m1.41 1.084l-5.637-4.337m3.33-2.66l-.236-1.301m-7.872 19.384l11.839-8.726"></svg:path>`,
})
export class ArcticonsVlibIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVlnIcon],svg[arcticons-vln-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.08 4.5c1.37 0 7.77 7.7 10.69 11.92s3.72 6.9 4.84 8.39S43 29 43 30.27s-2.23 2.55-2.73 3.48A36.5 36.5 0 0 1 35 39.84c-1.12.74-9.39 3.66-10.94 3.66s-8.63-3-9.74-3.54S10.07 36.3 9 35.32s-4-4.38-4-5.17s1.07-1.63 1.81-2.8s4-6.38 5.08-7.77s6.43-8.81 7.59-9.83s3.01-5.25 4.6-5.25"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M31.63 41.27a3.87 3.87 0 0 0 2.1-2.13c.23-.79-.24-3.17-1-5.08s-7.08-7.5-8.75-7.5c-1.41 0-10.11 7.17-10.11 11c0 1.77 3.84 3.82 3.84 3.82m5.85-36.74c.39 1.33.8 2 .71 2.64s-1.4 5.68-1.35 8.34s1 4.89 1.16 6.14a38 38 0 0 1-.08 4.8"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.1 35.34a29.4 29.4 0 0 0 6.74.93c1.55 0 6-.58 6.39-.67c0 0-2.85 4.89-6.39 4.89s-6.74-5.15-6.74-5.15m6.74-.46v1.39m-5.16-1.95v1.39m9.53-.95v1.12"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.53 38.56a13 13 0 0 1 2.42 0"></svg:path>`,
})
export class ArcticonsVlnIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVmallIcon],svg[arcticons-vmall-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.509 24.015v6.599h3.3m2.144-6.599v6.599h3.3m-10.881 0l-2.145-6.599L24 30.614m.742-2.227h2.888m-12.37 2.227v-6.599l3.299 6.599l3.3-6.599v6.599m-8.745-6.599l-2.144 6.599l-2.228-6.599"></svg:path><svg:rect width="39" height="19.452" x="4.5" y="17.588" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.669"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.123 17.588V13.22a2.255 2.255 0 0 1 2.26-2.26h12.81a2.256 2.256 0 0 1 2.26 2.26v4.368"></svg:path>`,
})
export class ArcticonsVmallIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVmosIcon],svg[arcticons-vmos-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 7.978L24 40.022m-2.314-4.007L37.874 7.978m-4.624 0L19.375 32.011L5.5 7.978h9.25L24 24"></svg:path>`,
})
export class ArcticonsVmosIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVnIcon],svg[arcticons-vn-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M25.988 31.179V16.821L35.5 31.179V16.821m-13.488 0l-4.756 14.358L12.5 16.821"></svg:path>`,
})
export class ArcticonsVnIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVncviewerIcon],svg[arcticons-vncviewer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.43 7.31L14 38.5a1.53 1.53 0 0 0 3.05 0l9.52-31.19m3.99 13.93v-7.41c0-2.85 3-5.6 6.31-5.6s6.43 3 6.43 5.6v7.41m.13 18.57H36c-2.85 0-5.6-3-5.6-6.31s3-6.43 5.6-6.43h7.41"></svg:path>`,
})
export class ArcticonsVncviewerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVndirectIcon],svg[arcticons-vndirect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.41 35.625v-23.5h5.5c6 0 10.5 4.5 10.5 10.25v3c0 5.5-4.5 10.25-10.5 10.25z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.56 6.875h8.5c8 0 12.63 7.5 12.63 15l-.03 4.25c-.049 7-4.6 15-12.6 15h-8.75"></svg:path>`,
})
export class ArcticonsVndirectIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVndirectDstockIcon],svg[arcticons-vndirect-dstock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.858 27.123c.49.639 1.106.877 1.962.877h1.184A1.996 1.996 0 0 0 15 26.004v-.009A1.996 1.996 0 0 0 13.004 24h-1.307A2 2 0 0 1 9.7 22.003h0c0-1.106.896-2.002 2.002-2.002h1.178c.856 0 1.47.237 1.961.876M16.45 20h5.3m-2.65 8v-8m6.75 8a2.65 2.65 0 0 1-2.65-2.65v-2.7a2.65 2.65 0 1 1 5.3 0v2.7A2.65 2.65 0 0 1 25.85 28m9.4-2.683v.033a2.65 2.65 0 1 1-5.3 0v-2.7A2.65 2.65 0 0 1 32.6 20h0a2.65 2.65 0 0 1 2.65 2.65v.033M36.7 20v8m4.3 0l-3.29-4L41 20.027M37.706 24H36.7"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M20.613 29.402c-1.547 3.692-5.203 6.223-9.703 6.223h-5.5v-23.5h5.5c4.5 0 8.156 2.531 9.703 6.223m5.657 11.296c-1.26 5.911-5.535 11.481-12.21 11.481H5.31m.25-34.25h8.5c6.738 0 11.085 5.32 12.29 11.48"></svg:path>`,
})
export class ArcticonsVndirectDstockIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVneidIcon],svg[arcticons-vneid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M10.824 4.5h26.352c1.844 6.851 2.108 11.068 1.792 16.338c-1.265 10.54-4.164 13.176-7.59 17.128C29.534 40.338 25.318 43.5 24 43.5s-5.534-3.162-7.378-5.534c-3.426-3.953-6.325-6.588-7.59-17.128c-.316-5.27-.052-9.487 1.792-16.338"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.713 16.095v7.378l-4.847-7.378v7.378v-7.378l4.848 7.326zm5.29 6.463a1.81 1.81 0 0 1-1.577.923a1.82 1.82 0 0 1-1.815-1.83v-1.188c0-1.01.813-1.83 1.815-1.83s1.815.82 1.815 1.83v.594h-3.63m5.524-4.962v7.378m1.897 0v-7.378h1.647c1.768 0 3.202 1.445 3.202 3.228v.922c0 1.783-1.434 3.228-3.202 3.228zM11.12 16.095l2.424 7.378l2.424-7.378"></svg:path>`,
})
export class ArcticonsVneidIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVocabularyIcon],svg[arcticons-vocabulary-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.811 6.922H42.5m-2.844 0L28.278 41.078h-8.545L7.64 6.922m9.249 0L28.233 40.72M5.5 6.922h14.233"></svg:path>`,
})
export class ArcticonsVocabularyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVocadbIcon],svg[arcticons-vocadb-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29c-2.2 0-4 1.8-4 4v29c0 2.2 1.8 4 4 4h29c2.2 0 4-1.8 4-4v-29c0-2.2-1.8-4-4-4M32 24h10.5m-26.9 0h11.7M5.5 24h2.4"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.4 17.3h-3.1v13.4h4l.7-1V18.8zM15.6 24l-1.9 6.7l-3.8-13.4l-2 6.7m8.9 1.5h1.5v4.2h-1.5zm20.8-2.9l-3.8-5.3v13.4h4.4l1.9-2.3l-3.2-4.4zm-16.1 2.9h-1l-.8 2.1l.9 2.1h.9m3.9 0l-1.3-4.2l-1.3 3.7h1.1"></svg:path>`,
})
export class ArcticonsVocadbIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVocaloidCollectionIcon],svg[arcticons-vocaloid-collection-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m7.632 17.988l21.824 12V17.963l10.912 6.034l-10.912 5.99V42.01L7.632 29.987v12.024l10.912-5.958l-10.912-6.065v-24l10.912 6.035z"></svg:path>`,
})
export class ArcticonsVocaloidCollectionIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoiIcon],svg[arcticons-voi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 11.755h7.453l7.01 16.77l7.276-16.947h6.92L23.601 36.422H15.97z"></svg:path><svg:circle cx="38.33" cy="32.252" r="4.17" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVoiIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoiceIcon],svg[arcticons-voice-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 45.5A21.5 21.5 0 1 1 45.5 24A21.51 21.51 0 0 1 24 45.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.75 14.4A7.1 7.1 0 0 1 9 13.09c8.44 0 7.91 22 18.09 22c8.82 0 8.31-18.09 16.63-19.77"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.23 13.51c6.52 3 6.84 21.58 16.18 21.58S29.66 15.2 39.33 15.2a6.87 6.87 0 0 1 5.17 2.3"></svg:path>`,
})
export class ArcticonsVoiceIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoiceAccessIcon],svg[arcticons-voice-access-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="14.355" cy="19.496" r="4.934" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.734 33.276v1.69a1.64 1.64 0 0 1-1.645 1.645H6.145A1.64 1.64 0 0 1 4.5 34.966v-1.69c2.159-7.02 18.973-5.817 20.234 0m.524-12.399v5.032m4.561-9.697v14.363M34.38 11.39v24.007m9.12-14.52v5.032m-4.56-9.697v14.363"></svg:path>`,
})
export class ArcticonsVoiceAccessIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoiceToolsIcon],svg[arcticons-voice-tools-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m36 24.12l1.62-1.62l5.88 5.88M33 21.12l4.62-4.62l5.88 5.88m-39 3.144l5.318 5.318L13.16 27.5M4.5 19.524l5.318 5.318L15.16 19.4m-1.164 4.422a10.2 10.2 0 0 0 20.008 0M24 7.425a6.47 6.47 0 0 1 6.469 6.469V21.9A6.47 6.47 0 0 1 24 28.37h0a6.47 6.47 0 0 1-6.468-6.468v-8.007A6.47 6.47 0 0 1 24 7.425m2.499 24.353v8.253c0 .297-.238.54-.535.544h-3.927a.544.544 0 0 1-.536-.544v-8.253"></svg:path>`,
})
export class ArcticonsVoiceToolsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoicegptIcon],svg[arcticons-voicegpt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 13.5v21m-4.668-16.295v11.59m9.336-11.59v11.59m-14.003-6.986v2.28m18.67-2.28v2.28"></svg:path>`,
})
export class ArcticonsVoicegptIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoicemailIcon],svg[arcticons-voicemail-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.5 19.6v18.9a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4h-29a4 4 0 0 0-4 4z"></svg:path><svg:circle cx="15.561" cy="24" r="6.561" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="32.439" cy="24" r="6.561" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.561 30.561h16.878"></svg:path>`,
})
export class ArcticonsVoicemailIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoicemailAltIcon],svg[arcticons-voicemail-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="12.495" cy="25.74" r="8.34" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="32.195" cy="22.26" r="8.34" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m13.945 33.95l19.7-3.47m-26.8-10.875V9.5c0-2.216 1.784-4 4-4h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-6.625"></svg:path>`,
})
export class ArcticonsVoicemailAltIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoicemodControllerIcon],svg[arcticons-voicemod-controller-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="37" height="37" x="5.5" y="5.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="4" ry="4"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m8.54 17.638l9.116 11.22V17.286m4.027 9.467v-9.501l11.089 11.473l-1.666-11.497l7.626 8.298"></svg:path>`,
})
export class ArcticonsVoicemodControllerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoidpetGardenIcon],svg[arcticons-voidpet-garden-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.077 10.134c-5.284 4.104-3.343 11.151-4.634 13.24c-1.3 2.098-8.05 8.162-12.943 2.303c.724 11.355 12.6 17.224 21.885 12.655c9.285-4.55 5.162-12.72 8.078-14.243c2.925-1.513 4.865-2.256 6.778-4.318c1.903-2.042 3.955-10.315-.01-10.315s-13.871-3.436-19.154.678"></svg:path><svg:circle cx="29.505" cy="18.017" r="3.668" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="28.755" cy="18.767" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVoidpetGardenIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolgaIcon],svg[arcticons-volga-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.257 36.091C12.134 34.694 19.972 30.977 24 24c4.028 6.977 11.866 10.694 17.743 12.091"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M2.745 27.184C8.526 26.662 19.092 23.001 24 14.5c4.908 8.501 15.473 12.162 21.255 12.684"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M3.51 17.6C9.419 16.841 19.3 13.139 24 5c4.71 8.159 14.632 11.86 20.534 12.606"></svg:path>`,
})
export class ArcticonsVolgaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolkskrantIcon],svg[arcticons-volkskrant-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m39.431 5.5l-15.417 37l-15.417-37m19.594 26.976L16.951 5.5m18.882 0H42m-37 0h15.548"></svg:path>`,
})
export class ArcticonsVolkskrantIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolkswagenIcon],svg[arcticons-volkswagen-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 2.5c11.876 0 21.5 9.624 21.5 21.5S35.876 45.5 24 45.5S2.5 35.876 2.5 24S12.124 2.5 24 2.5"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M44.297 16.894L31.51 40.74L24 25.386L16.49 40.74L3.703 16.894"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.825 4.39L24 22.47L15.174 4.39"></svg:path>`,
})
export class ArcticonsVolkswagenIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVollaMessagesIcon],svg[arcticons-volla-messages-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.5 42.5c-2.216 0-4-1.784-4-4V24C5.5 13.783 13.783 5.5 24 5.5S42.5 13.783 42.5 24S34.217 42.5 24 42.5z"></svg:path><svg:circle cx="24" cy="19" r="3.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="24" cy="29" r="3.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVollaMessagesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolleyballIcon],svg[arcticons-volleyball-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.5 24c0 11.874-9.626 21.5-21.5 21.5S2.5 35.874 2.5 24S12.126 2.5 24 2.5S45.5 12.126 45.5 24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M33.252 21.505c-1.478 10.562-5.063 19.433-10.389 23.92m18.694-24.327c-.952 9.1-3.266 16.54-7.582 21.9m-12.268-9.526C13.228 26.815 7.324 19.166 6.21 12.248M17.618 40.47C10.006 34.567 4.752 28.35 2.671 21.445M24.245 24.2c-6.587-4.976-11.297-11.745-11.991-18.204"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.828 43.433c5.027-3.925 8.396-11.249 9.417-19.234"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M45.406 21.949c-5.972-2.128-13.88-1.117-21.18 2.284m-6.843-6.728c9.808-4.087 19.23-5.514 25.788-3.231M13.339 10.44c8.282-3.7 15.824-5.399 22.585-4.337"></svg:path>`,
})
export class ArcticonsVolleyballIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolumeIcon],svg[arcticons-volume-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.017 16.708c1.768 1.326 3.241 4.714 2.946 8.397c-.294 2.799-1.767 5.156-2.946 6.04M5.5 17.74v12.816h8.544l11.049 8.839V8.605L14.044 17.74zm31.673-6.777c3.094 2.357 5.598 8.397 5.303 15.026c-.442 5.009-2.946 9.133-5.303 10.901"></svg:path>`,
})
export class ArcticonsVolumeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolumeNotificationIcon],svg[arcticons-volume-notification-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.243 31.431a5.742 5.742 0 1 0-11.484 0a5.742 5.742 0 0 0 11.484 0V10.828H42.5m-37 16.5h15.706M5.5 19.078h23.8M5.5 10.827h23.8"></svg:path>`,
})
export class ArcticonsVolumeNotificationIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolumeStylesIcon],svg[arcticons-volume-styles-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.5 5.5h-12a2 2 0 0 0-2 2v33a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-33a2 2 0 0 0-2-2m21 0h-12a2 2 0 0 0-2 2v33a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-33a2 2 0 0 0-2-2"></svg:path><svg:circle cx="13.5" cy="15" r="3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="34.5" cy="28" r="3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.5 33V18m0-6V9m21 24v-2m0-6V9m.315 30.212v-3.919h1.532"></svg:path><svg:circle cx="33.734" cy="39.212" r="1.081" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.652 37.886c.21-.215.997-1.02 1.07-1.091c.163-.157.078-.409.078-.409s-.118-.51-.15-.853c-.025-.27-.31-.24-.31-.24h-1.15a.205.205 0 0 0-.183.19c-.078 1.038.506 2.15.623 2.363h0l.005.01za4.8 4.8 0 0 0 1.758 1.754l-.001.002l.06.034h0c.262.142 1.337.686 2.341.61c0 0 .163-.005.188-.183v-1.15s.03-.285-.24-.31c-.34-.033-.851-.15-.851-.15s-.251-.085-.408.078c-.07.073-.875.86-1.09 1.07"></svg:path>`,
})
export class ArcticonsVolumeStylesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolumeeIcon],svg[arcticons-volumee-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m37.876 29.287l-4.627-2.644L37.875 24l4.625 2.643v5.286L24 42.5L5.5 31.929v-5.286L24 16.073l9.25 5.285l-4.625 2.643L24 21.358L5.5 31.929"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M42.5 16.071L24 26.642l-4.624-2.643l-4.626 2.643L24 31.928l18.5-10.571zL24 5.5L5.5 16.071v5.286L10.125 24l4.626-2.643l-4.627-2.644"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M37.876 18.715L24 10.785L5.5 21.358m37 5.285L24 37.214l-13.876-7.93M24 37.214V42.5m0-15.858v5.286m0-15.855v5.285M24 5.5v5.286"></svg:path>`,
})
export class ArcticonsVolumeeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolumioIcon],svg[arcticons-volumio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M14.79 8.74c2.53 4.17 5.06 8.34 7.59 12.5"></svg:path><svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle>`,
})
export class ArcticonsVolumioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolunteerExchangeIcon],svg[arcticons-volunteer-exchange-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24.334 21.235c-13.02-4.786-11.879-24.472.103-14.42c1.31-2.378 5.55-5.17 8.328-1.1c-5.454-.647-8.792 6.037-8.431 15.52"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.552 23.569c.462-13.864 19.513-18.952 13.724-4.423c2.67.499 6.65 3.65 3.657 7.563c-1.097-5.38-8.49-6.455-17.381-3.14m-.897 2.765c13.496-3.205 23.422 13.833 7.88 12.075c.223 2.707-1.769 7.377-6.332 5.52c4.902-2.475 3.991-9.89-1.548-17.595m-3.79-.224c7.895 11.405-4.365 26.849-8.38 11.733c-2.44 1.192-7.516 1.036-7.446-3.89c4.089 3.665 10.665.12 15.826-7.843m-.503-3.252C12.62 33.63-5.637 26.18 7.752 18.095c-1.828-2.01-3.098-6.925 1.652-8.236C7.028 14.81 12.273 20.13 21.36 22.858Z"></svg:path>`,
})
export class ArcticonsVolunteerExchangeIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVolvoCarsIcon],svg[arcticons-volvo-cars-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M35.88 6.103A21.35 21.35 0 0 0 24 2.5C12.126 2.5 2.5 12.126 2.5 24S12.126 45.5 24 45.5S45.5 35.874 45.5 24c0-4.397-1.335-8.475-3.602-11.879M45.5 2.5h-9.627l4.058 3.189l-1.979 1.979l2.381 2.38l1.978-1.979l3.189 4.058z"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.754 20.743l-2.877 6.514L7 20.743"></svg:path><svg:rect width="5.429" height="6.514" x="14.839" y="20.743" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.714" ry="2.714"></svg:rect><svg:rect width="5.429" height="6.514" x="35.571" y="20.743" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="2.714" ry="2.714"></svg:rect><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m33.486 20.743l-2.878 6.514l-2.877-6.514m-4.579 0v6.514h4.343"></svg:path>`,
})
export class ArcticonsVolvoCarsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoteinfoIcon],svg[arcticons-voteinfo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.89 31.343H8.409L4.5 37.774h38.142l-3.909-6.431h-1.538"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m28.091 20.141l-11.833-3.512l-5.474 18.441h25.305l3.425-11.538l-7.866-2.335"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.522 15.213c-.749 1.498-3.69 3.983-3.946 4.366s-2.41 2.265-.968 3.471s3.709-1.699 4.44-2.192s4.31-1.461 5.078-2.1s1.826-1.316 2.1-1.992l-1.498-4.75s-6.448-.11-7.307 0s-4.463 4.934-3.927 6.375c.64 1.718 2.856.407 2.856.407"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m37.728 12.017l3.38-1.791l2.392 6.403l-4.274.137"></svg:path>`,
})
export class ArcticonsVoteinfoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoterHelplineIcon],svg[arcticons-voter-helpline-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M6.33 15.477q17.332-19.955 35.34 0M24.005 39.731c7.079 0 12.824.625 12.824 1.384c0 .77-5.746 1.385-12.824 1.385s-12.824-.614-12.824-1.385c0-.76 5.746-1.384 12.824-1.384"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.662 38.347q6.235-5.55 9.347-13.772c7.276-21.308-25.981-21.568-18.362-.083q2.78 8.307 9.015 13.855"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.662 10.887c4.59 0 8.317 3.612 8.317 8.057S28.252 27 23.662 27s-8.317-3.612-8.317-8.056s3.726-8.057 8.317-8.057"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.327 14.738c2.228 1.522 2.797 4.595 1.27 6.863c-1.525 2.269-4.57 2.874-6.798 1.352s-2.798-4.594-1.271-6.863a5 5 0 0 1 1.365-1.362"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M23.703 12.958c1.51 0 2.727 1.187 2.727 2.644c0 1.468-1.217 2.644-2.727 2.644s-2.727-1.176-2.727-2.644c0-1.457 1.218-2.644 2.727-2.644"></svg:path>`,
})
export class ArcticonsVoterHelplineIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVouchervaultIcon],svg[arcticons-vouchervault-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="7.323" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.458 16.831v-4.008A1.7 1.7 0 0 1 24 11.28a1.735 1.735 0 0 1 1.542 1.542v4.008m5.704 5.705h4.008a1.7 1.7 0 0 1 1.542 1.542a1.734 1.734 0 0 1-1.542 1.542h-4.008m-14.492-3.084h-4.008a1.7 1.7 0 0 0-1.542 1.542a1.734 1.734 0 0 0 1.542 1.542h4.008m5.704 5.704v4.008A1.7 1.7 0 0 0 24 36.873a1.734 1.734 0 0 0 1.542-1.542v-4.008m2.491-13.46l2.835-2.834a1.7 1.7 0 0 1 2.18 0a1.734 1.734 0 0 1 0 2.18l-2.834 2.834m0 8.067l2.834 2.835a1.7 1.7 0 0 1 0 2.18a1.734 1.734 0 0 1-2.18 0l-2.834-2.834m-8.067-12.428l-2.835-2.834a1.7 1.7 0 0 0-2.18 0a1.734 1.734 0 0 0 0 2.18l2.835 2.834m-.001 8.067l-2.834 2.835a1.7 1.7 0 0 0 0 2.18a1.734 1.734 0 0 0 2.18 0l2.834-2.834M15.135 14.75L8.738 5.5m24.127 9.25l6.398-9.25M42.5 18.219l-9.046 13.9m-.862 1.324L26.698 42.5m-5.396 0l-5.93-9.113m-.815-1.251L5.5 18.219"></svg:path>`,
})
export class ArcticonsVouchervaultIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoyagerForLemmyIcon],svg[arcticons-voyager-for-lemmy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.336 22.812c-5.495-1.03-7.542-6.41-4.738-9.317c2.3-2.383 6.365-1.906 9.224 3.309c2.075-1.038 4-1.374 6.178-1.374m-6.465 17.855c-1.095 2.553-1.31 4.463-1.31 4.463c-1.487.645-2.78 2.775-3.115 4.794m14.287-1.682c1.276-.14 2.538-.673 2.538-.673"></svg:path><svg:circle cx="24" cy="32.224" r="2.257" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M22.208 26.373a1.817 1.817 0 1 0-3.618.352m.242 3.242l-2.098-.827m1.761 2.58l-3.504.224"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.533 15.374c-2.327-2.72-5.44-1.402-5.44 1.682s1.683 3.589 1.683 7.29c0 4.906-3.733 10.157-9.55 11.775a4.6 4.6 0 0 1-2.452 0c-5.818-1.618-9.55-6.869-9.55-11.775c0-3.701 1.682-4.206 1.682-7.29s-3.112-4.402-5.439-1.682"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.664 22.812c5.495-1.03 7.542-6.41 4.738-9.317c-2.3-2.383-6.365-1.906-9.224 3.309c-2.075-1.038-4-1.374-6.178-1.374m6.465 17.855c1.095 2.553 1.31 4.463 1.31 4.463c1.487.645 2.78 2.775 3.115 4.794"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.927 34.498C30.457 36.645 28.551 38.7 24 38.7s-6.457-2.056-6.927-4.203m8.72-8.124a1.817 1.817 0 1 1 3.617.352m-.242 3.242l2.098-.827m-1.761 2.58l3.504.224"></svg:path>`,
})
export class ArcticonsVoyagerForLemmyIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoyagerForLemmyPsychIcon],svg[arcticons-voyager-for-lemmy-psych-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="24" cy="24" r="21.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.336 22.812c-5.495-1.03-7.542-6.41-4.738-9.317c2.3-2.383 6.365-1.906 9.224 3.309c2.075-1.038 4-1.374 6.178-1.374m-6.465 17.855c-1.095 2.553-1.31 4.463-1.31 4.463c-1.487.645-2.78 2.775-3.115 4.794m14.287-1.682c1.276-.14 2.538-.673 2.538-.673"></svg:path><svg:circle cx="24" cy="32.224" r="2.257" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:circle cx="28.606" cy="24.24" r=".75" fill="currentColor"></svg:circle><svg:circle cx="19.394" cy="24.24" r=".75" fill="currentColor"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.832 29.967l-2.098-.827m1.761 2.58l-3.504.224m23.542-16.57c-2.327-2.72-5.44-1.402-5.44 1.682s1.683 3.589 1.683 7.29c0 4.906-3.733 10.157-9.55 11.775a4.6 4.6 0 0 1-2.452 0c-5.818-1.618-9.55-6.869-9.55-11.775c0-3.701 1.682-4.206 1.682-7.29s-3.112-4.402-5.439-1.682"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.664 22.812c5.495-1.03 7.542-6.41 4.738-9.317c-2.3-2.383-6.365-1.906-9.224 3.309c-2.075-1.038-4-1.374-6.178-1.374m6.465 17.855c1.095 2.553 1.31 4.463 1.31 4.463c1.487.645 2.78 2.775 3.115 4.794"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.927 34.498C30.457 36.645 28.551 38.7 24 38.7s-6.457-2.056-6.927-4.203m12.095-4.53l2.098-.827m-1.761 2.58l3.504.224"></svg:path><svg:rect width="3.076" height="5.021" x="17.856" y="22.043" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.538" ry="1.538"></svg:rect><svg:rect width="3.076" height="5.021" x="27.068" y="22.043" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.538" ry="1.538"></svg:rect>`,
})
export class ArcticonsVoyagerForLemmyPsychIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoyagerForLemmySpaceIcon],svg[arcticons-voyager-for-lemmy-space-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:ellipse cx="24" cy="24.694" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="14.336" ry="8.405"></svg:ellipse><svg:circle cx="24" cy="36.629" r="2.038" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12.248 32.945l-2.584 1.173m6.181 2.171l-2.069 2.123M27.01 30.84c5.351-.743 8.813-3.396 8.977-5.956M20.99 18.549c-5.35.743-8.813 3.396-8.977 5.956"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M43.5 14.608c0-2.913-2.704-5.274-6.039-5.274c-3.049 0-5.563 1.976-5.973 4.54c-2.182-1.252-4.74-1.987-7.488-1.987s-5.306.735-7.488 1.987c-.41-2.564-2.924-4.54-5.973-4.54c-3.335 0-6.039 2.361-6.039 5.274s2.704 5.274 6.039 5.274c.12 0 .236-.01.354-.016a12.6 12.6 0 0 0-1.229 5.41c0 7.396 6.419 13.39 14.336 13.39s14.336-5.994 14.336-13.39a12.6 12.6 0 0 0-1.23-5.41c.119.007.236.016.355.016c3.335 0 6.039-2.361 6.039-5.274m-7.748 18.337l2.584 1.173m-6.181 2.171l2.069 2.123"></svg:path>`,
})
export class ArcticonsVoyagerForLemmySpaceIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVoyagespiratesIcon],svg[arcticons-voyagespirates-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.873 30.903q-2.19-5.193-5.78-7.47q3.233-3.561 7.839-4.715"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M36.375 16.68q-4.358 1.62-7.281 6.753c-2.407 2.278-6.455-.606-4.944-3.601q2.835-5.37 8.226-7.618m-13.81 1.09Q13.374 9.065 13.075 4.5c4.685 7.123 12.93 3.292 15.915 6.425"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.602 10.617q-2.358-.726-5.292.976q4.386.03 7.232 2.834"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M16.557 13.72q-2.208.32-3.98 2.328q2.617-1.183 3.92.13m10.448 3.874a1.268 1.268 0 1 1 0 2.536a1.267 1.267 0 0 1-1.263-1.272v-.002c0-.696.567-1.262 1.263-1.262"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M26.646 10.6c-6.1 0-11.044 4.945-11.044 11.044c0 8.647 8.437 19.075 10.64 21.623a.675.675 0 0 0 1.02 0c2.151-2.53 10.428-12.976 10.428-21.623c0-6.099-4.945-11.044-11.044-11.044"></svg:path>`,
})
export class ArcticonsVoyagespiratesIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVozfappNextIcon],svg[arcticons-vozfapp-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m9.634 4.5l13.744 39l8.63-8.608l-7.458-22.636zm22.375 30.392l6.357-6.483l-1.99-5.526l-10.077-5.32"></svg:path>`,
})
export class ArcticonsVozfappNextIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVpbankNeoIcon],svg[arcticons-vpbank-neo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M24 16.417V40.25m0-23.833l8.667-8.667M24 16.417L15.333 7.75M4.5 19.667h10.833L24 28.333l8.667-8.666H43.5S27.25 40.25 25.083 40.25h-2.166C20.75 40.25 4.5 19.667 4.5 19.667"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M15.333 19.667V7.75L24 16.417l8.667-8.667v11.917M24 16.417V40.25"></svg:path>`,
})
export class ArcticonsVpbankNeoIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVpbankSmartOtpIcon],svg[arcticons-vpbank-smart-otp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M34.119 40.338V27.689h4.14c2.342 0 4.24 1.902 4.24 4.248s-1.898 4.248-4.24 4.248h-4.14m-12.174-8.496h8.379m-4.19 12.649V27.689m-15.099 6.483a1.581 1.581 0 1 1 1.581 0m-1.581 0l-.527 2.635h2.635l-.527-2.635"></svg:path><svg:circle cx="11.826" cy="34.014" r="6.324" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.46 11.597V22.42m0-10.823l3.981-3.935m-3.981 3.935L9.477 7.662M4.5 13.073h4.978l3.982 3.935l3.982-3.935h4.977s-7.466 9.346-8.462 9.346h-.995c-.996 0-8.462-9.346-8.462-9.346"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M9.477 13.073V7.662l3.982 3.935l3.982-3.935v5.41m-3.981-1.475V22.42m8.463-1.517c.258.337.582.462 1.033.462h.624c.581 0 1.052-.47 1.052-1.051v-.005c0-.58-.47-1.052-1.052-1.052h-.688a1.053 1.053 0 0 1-1.053-1.053h0c0-.582.472-1.055 1.055-1.055h.621c.451 0 .775.126 1.034.462m1.664 3.749v-4.211l2.109 4.216l2.108-4.21v4.21m3.863-1.41h-1.827m-.455 1.397l1.37-4.203l1.37 4.216m1.581 0V17.15h1.38c.781 0 1.414.634 1.414 1.416s-.633 1.416-1.413 1.416h-1.38m1.38-.001l1.38 1.383m1.614-4.215H43.5m-1.397 4.216v-4.216"></svg:path>`,
})
export class ArcticonsVpbankSmartOtpIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVphonegagaIcon],svg[arcticons-vphonegaga-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m17.5 6.5l11.5 23l-6 12l-17.5-35z"></svg:path><svg:ellipse cx="24" cy="31.9" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" rx="1.127" ry="1.623"></svg:ellipse><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m24 39.5l1 2l17.5-35h-12l-6.5 13m-1 1.998L19 29.5l4 8"></svg:path>`,
})
export class ArcticonsVphonegagaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVpnhIcon],svg[arcticons-vpnh-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17.354 28.12v-8.24h2.698c1.525 0 2.762 1.239 2.762 2.767s-1.237 2.768-2.762 2.768h-2.698m7.773 2.705v-8.24l5.459 8.24v-8.24m2.454 0v8.24m5.46-8.24v8.24m-5.46-4.135h5.46M14.96 19.88l-2.73 8.24l-2.73-8.24"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M38.5 5.5h-29c-2.2 0-4 1.8-4 4v29c0 2.2 1.8 4 4 4h29c2.2 0 4-1.8 4-4v-29c0-2.2-1.8-4-4-4"></svg:path>`,
})
export class ArcticonsVpnhIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVpnhotspotIcon],svg[arcticons-vpnhotspot-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M32.94 27.21a14.07 14.07 0 0 1-6.49 9.06m-14.45 0a14.06 14.06 0 1 1 20.75-15.81c.07.25.14.51.19.77"></svg:path><svg:circle cx="19.21" cy="24.22" r="3.95" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></svg:circle><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M27.42 27.21a8.75 8.75 0 1 1 0-6h16.73v6H41v6.9h-6.11v-6.9Z"></svg:path>`,
})
export class ArcticonsVpnhotspotIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVrFpsIcon],svg[arcticons-vr-fps-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m9.528 30.545l1.009-1.502m26.822-3.829l8.141-4.03l-8.164-.358a12.2 12.2 0 0 0-1.629-4.182l6.493-.316l-7.665-1.276a12.3 12.3 0 0 0-1.561-1.51l2.95-5.074l-5.272 3.593l-.007.012a12.2 12.2 0 0 0-5.132-1.23l-1.205-2.375l-.447 2.45c-1.095.128-2.144.4-3.131.796l-5.354-6.411l3.634 7.269a12.4 12.4 0 0 0-2.334 1.811l-6.643-2.535l4.343 5.662a12.16 12.16 0 0 0-1.353 5.584c0 .71.067 1.402.183 2.078L2.5 31.538l11.536-3.581a12.32 12.32 0 0 0 5.937 6.179l2.136 8.56l2.254-7.382c.307.022.613.046.926.046c2.536 0 4.892-.77 6.848-2.088l2.874 2.96l-.98-4.54a12.3 12.3 0 0 0 2.24-3.159h5.983l-5.429-1.306q.351-.973.534-2.013m-24.21-3.87l-3.245-.348m7.103-6.926l-.639-.907M17.4 32.468l-1.514 1.725m15.316-9.449l2.254.537m-2.02-3.503l1.175-.159m-2.519-2.886l.406-.272m-1.509-.805l.845-.986m-3.135-.134l.178-.57m-3.773.67l-.19-.61M20.6 18.138l-1.006-.975m-.958 4.939l-.792-.04m2.374 4.929l-.362.262m7.881 1.376l.669 1.165m1.408-2.672l.322.291"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m25.899 16.387l-.652-1.682l-.77 1.653a6.437 6.437 0 0 0 .597 12.846a6.435 6.435 0 0 0 .825-12.817"></svg:path>`,
})
export class ArcticonsVrFpsIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVrMatkallaIcon],svg[arcticons-vr-matkalla-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M13.453 29.824s1.878 2.779 3.71 2.779c1.907.004 2.652-1.061 3.74-2.38l5.28-8.45a3.47 3.47 0 0 1 2.634-1.526h8.073c1.565 0 1.426 2.896-.14 2.896h-6.724a2.79 2.79 0 0 0-2.443 1.591l-1.98 3.248h5.839c1.755 0 2.901 4.629 5.323 4.629h5.063l-2.894-4.699a6.05 6.05 0 0 0 4.566-5.867c0-4.627-3.273-6.633-4.965-6.633H25.368a3.72 3.72 0 0 0-2.966 1.932l-5.082 8.409l-5.046-8.412s-1.282-1.95-2.994-1.95l-4.78-.002Z"></svg:path>`,
})
export class ArcticonsVrMatkallaIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVrMediaPlayerIcon],svg[arcticons-vr-media-player-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M19.124 17.13c1.552-.11 3.214-.222 4.876-.222s3.324.111 4.876.222c1.108.11 2.216.222 3.213.333c7.314.997 12.411 3.434 12.411 6.315c0 2.77-5.097 5.209-12.41 6.317c-.998.11-2.106.221-3.214.332c-1.552.11-3.214.222-4.876.222s-3.324-.111-4.986-.222m-3.103-.332C8.597 28.987 3.5 26.66 3.5 23.778s5.097-5.208 12.41-6.316"></svg:path><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.316 31.868C29.32 39.292 26.881 44.5 24 44.5s-5.319-5.208-6.316-12.632c-.111-.998-.222-2.106-.333-3.214c-.11-1.551-.221-3.103-.221-4.654c0-1.773.11-3.546.222-5.097c.11-1.108.221-2.216.332-3.214C18.792 8.486 21.12 3.5 23.89 3.5s5.208 4.987 6.206 12.19m.554 3.213c.111 1.662.221 3.324.221 5.097c0 1.662-.11 3.214-.222 4.654"></svg:path>`,
})
export class ArcticonsVrMediaPlayerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVrOffshoreIcon],svg[arcticons-vr-offshore-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M30.834 4.62c-2.569 1.209-8.487 7.101-10.325 10.652S17.16 21.92 17.16 21.92a9.5 9.5 0 0 0 2.795 2.493a13 13 0 0 0 3.4 1.133s4.004-5.44 4.532-7.026s3.098-8.536 2.947-13.9"></svg:path><svg:circle cx="18.973" cy="25.903" r=".75" fill="currentColor"></svg:circle><svg:circle cx="18.299" cy="29.479" r=".75" fill="currentColor"></svg:circle><svg:circle cx="21.416" cy="31.575" r=".75" fill="currentColor"></svg:circle><svg:circle cx="24.488" cy="33.59" r=".75" fill="currentColor"></svg:circle><svg:circle cx="27.459" cy="35.655" r=".75" fill="currentColor"></svg:circle><svg:circle cx="28.517" cy="39.142" r=".75" fill="currentColor"></svg:circle><svg:circle cx="29.625" cy="42.63" r=".75" fill="currentColor"></svg:circle>`,
})
export class ArcticonsVrOffshoreIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVrXRacerIcon],svg[arcticons-vr-x-racer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><svg:path d="M31.777 5.5H16.223a.97.97 0 0 0-.969.969v9.676a.97.97 0 0 0 .969.969h3.396c2.736 0 2.396-4.426 4.38-4.426s1.513 4.426 4.382 4.426h3.396a.97.97 0 0 0 .969-.97V6.47a.97.97 0 0 0-.97-.969"></svg:path><svg:circle cx="19.028" cy="11.075" r="2.119"></svg:circle><svg:circle cx="28.972" cy="11.075" r="2.119"></svg:circle><svg:path d="m14.965 23.645l-4.089 2.283l-.595 6.681l-3.329 6.396V42.5h2.901l.737-2.449l2.045-2.639l.095-3.471l3.352-3.305l1.103-5.891zm4.178 5.109l-2.704-.021m-6.158 3.876l2.449 1.332M24 22.037l3.555 2.04l.891 6.297l-.737 2.663l5.183 5.469L24 39.599l-8.892-1.093l5.183-5.469l-.737-2.663l.675-6.27zm9.035 1.608l4.089 2.283l.595 6.681l3.329 6.396V42.5h-2.901l-.737-2.449l-2.045-2.639l-.095-3.471l-3.352-3.305l-1.103-5.891zm-4.178 5.109l2.704-.021m6.158 3.876l-2.449 1.332m-13.014 3.534h3.488"></svg:path></svg:g><svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m18.31 35.834l5.69-.881l5.69.881"></svg:path>`,
})
export class ArcticonsVrXRacerIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[arcticonsVradioIcon],svg[arcticons-vradio-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M4.5 11.804h3.786l8.738 24.392m7.315-20.422l-7.315 20.422m18.45-20.422l-7.315 20.422m8.597-24.392H25.762m13.907 0c3.313 0 4.439 4.247 3.526 7.135c-.905 2.864-2.846 5.874-7.41 6.117l4.223 11.14"></svg:path>`,
})
export class ArcticonsVradioIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}
