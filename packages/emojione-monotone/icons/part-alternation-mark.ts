import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[emojioneMonotonePartAlternationMarkIcon],svg[emojione-monotone-part-alternation-mark-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M60.146 54.634c-14.061-8.682-16.969-24.087-18.701-39.226c-.379-3.312-2.344-10.36-7.361-7.018c-2.038 1.355-3.084 3.558-4.266 5.622c-1.354 2.367-2.869 4.558-4.625 6.536c-.313-5.075-.831-10.212-2.314-14.857c-1.144-3.582-5.334-5.153-7.992-1.972c-2.086 2.496-3.907 10.283-8.033 9.081c-4.873-1.419-6.938 6.131-2.088 7.544c4.295 1.251 8.079-.104 11.037-3.358c.41-.452.785-.935 1.142-1.432c.522 4.406.579 8.974.851 13.188c.178 2.742 3.275 5.158 5.909 3.378c4.475-3.028 7.69-6.679 10.558-10.788c.1.868.195 1.736.316 2.602c2.08 15.102 8.166 29.159 21.605 37.456c4.315 2.666 8.256-4.104 3.962-6.756"></svg:path>`,
})
export class EmojioneMonotonePartAlternationMarkIcon {
  readonly viewBox = input("0 0 64 64")
  readonly width = input("1em")
  readonly height = input("1em")
}
