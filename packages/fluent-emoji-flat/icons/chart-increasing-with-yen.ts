import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiFlatChartIncreasingWithYenIcon],svg[fluent-emoji-flat-chart-increasing-with-yen-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:path fill="#00D26A" d="M2 6a4 4 0 0 1 4-4h20a4 4 0 0 1 4 4v20a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4z"></svg:path><svg:path fill="#F4F4F4" d="M5.461 5.119a.75.75 0 0 1 1.014.31l2.068 3.9l2.206-3.916a.75.75 0 1 1 1.307.737l-2.402 4.26h1.749a.75.75 0 1 1 0 1.5h-2.13v.95h2.13a.75.75 0 0 1 0 1.5h-2.13v1.4a.75.75 0 0 1-1.5 0v-1.4h-1.82a.75.75 0 0 1 0-1.5h1.82v-.95h-1.82a.75.75 0 0 1 0-1.5h1.466l-2.27-4.278a.75.75 0 0 1 .312-1.014m21.328 9.326a.75.75 0 0 0-1.06-1.06L15.304 23.809a.25.25 0 0 1-.355-.001l-2.617-2.637a1.75 1.75 0 0 0-2.48-.005l-4.421 4.422a.75.75 0 0 0 1.06 1.06l4.422-4.422a.25.25 0 0 1 .354.001l2.618 2.637a1.75 1.75 0 0 0 2.48.005z"></svg:path></svg:g>`,
})
export class FluentEmojiFlatChartIncreasingWithYenIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
