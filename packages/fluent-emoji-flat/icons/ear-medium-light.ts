import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiFlatEarMediumLightIcon],svg[fluent-emoji-flat-ear-medium-light-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:path fill="#EEBFAA" d="M7.125 23.156V11.094c0-4.844 3.75-9.063 9.344-9.063c7.14 0 9.531 6.156 9.531 9.063c0 3.328-1.564 5.434-3.094 8.015l-2.686 4.854q-.314.576-.626 1.13c-.922 1.532-2.61 4.938-6.532 4.938c-3.124 0-5.937-2.89-5.937-6.875"></svg:path><svg:path fill="#D6A089" d="M10.992 9.844q0-.02.002-.039c.06-1.908 1.932-4.774 5.99-4.774c4.125 0 6 5.032 6 6.969c0 1.344-1.984 1.328-1.984 0c0-1.687-1.547-4.984-4.344-4.984c-2.139 0-4.558 1.36-4.664 2.81v2.308a5149 5149 0 0 0 7.79 7.272c.327.297.109.64-.22.5c-.295-.126-6.355-4.121-7.57-4.923v3.013l.04-.002c.423-.011 1.015-.027 1.643.102c.815.169 1.709.576 2.58 1.498a.5.5 0 1 1-.728.687c-.723-.766-1.43-1.077-2.054-1.205c-.503-.104-.942-.093-1.357-.082q-.063 0-.124.003v6.206c0 .077.019.126.049.168a.5.5 0 0 0 .131.116c1.843.182 2.593-1.278 3.217-2.492c.426-.828.793-1.542 1.408-1.542c1.187 0 1.384 1.001 1.187 2.375C17.641 26.234 15.125 27 13.97 27c-1.069 0-2.704-.357-2.946-1.508a1.4 1.4 0 0 1-.03-.289z"></svg:path></svg:g>`,
})
export class FluentEmojiFlatEarMediumLightIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
