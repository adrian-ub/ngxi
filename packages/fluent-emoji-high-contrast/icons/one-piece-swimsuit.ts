import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiHighContrastOnePieceSwimsuitIcon],svg[fluent-emoji-high-contrast-one-piece-swimsuit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#fluentEmojiHighContrastOnePieceSwimsuit0)"><svg:path fill="currentColor" d="M5.205 10.696a5.4 5.4 0 0 1-.197-1.828V2.88a2.9 2.9 0 0 1 2.9-2.9h.19a2.9 2.9 0 0 1 2.9 2.9v1.21h9.022V2.9a2.9 2.9 0 0 1 2.9-2.9h.19a2.9 2.9 0 0 1 2.9 2.9v6.025a5.2 5.2 0 0 1-.2 1.804l-1.716 6.002l2.512 5.433a3.05 3.05 0 0 1-.61 3.427l-.137.138l-7.768 5.243a4.88 4.88 0 0 1-5.69-.019l-7.308-5.232l-.136-.145a3.05 3.05 0 0 1-.5-3.445l2.449-5.398zm18.685-.526c.112-.38.153-.776.12-1.17V2.9a.9.9 0 0 0-.9-.9h-.19a.9.9 0 0 0-.9.9v2.25a.94.94 0 0 1-.276.665a.94.94 0 0 1-.665.275H9.94A.94.94 0 0 1 9 5.15V2.88a.9.9 0 0 0-.9-.9h-.19a.9.9 0 0 0-.9.9v6.06a3.4 3.4 0 0 0 .12 1.21L7.933 13H23.08l-.001.003zm-1.35 4.718a1 1 0 0 0-.035.112H8.495l.282 1h13.446zM22.477 18H8.527l-2.269 5a1.05 1.05 0 0 0 .16 1.21l7.15 5.12a2.88 2.88 0 0 0 3.38 0l7.631-5.15a1.05 1.05 0 0 0 .21-1.18z"></svg:path></svg:g><svg:defs><svg:clippath id="fluentEmojiHighContrastOnePieceSwimsuit0"><svg:path fill="#fff" d="M0 0h32v32H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class FluentEmojiHighContrastOnePieceSwimsuitIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
