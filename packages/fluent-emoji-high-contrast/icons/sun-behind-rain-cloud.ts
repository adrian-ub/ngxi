import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiHighContrastSunBehindRainCloudIcon],svg[fluent-emoji-high-contrast-sun-behind-rain-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M10.977 2.913c-.62-1.24-2.4-1.21-2.98.05c-.22.47-.79.67-1.25.43l-.29-.15c-1.12-.59-2.41.38-2.16 1.62l.06.32c.1.51-.24 1-.76 1.08l-.32.05c-1.24.18-1.72 1.73-.79 2.58l.24.22c.38.35.39.96.02 1.32l-.23.23c-.646.631-.556 1.603.009 2.153a6.47 6.47 0 0 0 .884 6.589a6.48 6.48 0 0 0 5.17 2.56h12.69c2.82 0 5.31-1.39 6.83-3.52c2.72-3.56 2.43-8.68-.86-11.89c-3.308-3.225-8.465-3.392-11.952-.528a.9.9 0 0 1-.611-1.014l.05-.32c.21-1.24-1.11-2.17-2.21-1.55l-.28.16a.91.91 0 0 1-1.26-.39m3.689 3.675a9 9 0 0 0-.453.48a4.04 4.04 0 0 0-5.625 1.848l-.128-.002c-1.846 0-3.51.773-4.687 2.013a5.94 5.94 0 0 1 10.892-4.339m7.912 19.535l-2.31 3.16c-.57.78-1.66.95-2.44.38s-.95-1.66-.38-2.44l2.31-3.16c.57-.78 1.66-.95 2.44-.38c.77.57.94 1.66.38 2.44m-8.88 3.16l2.31-3.16c.56-.78.39-1.87-.38-2.44c-.78-.57-1.87-.4-2.44.38l-2.31 3.16c-.57.78-.4 1.87.38 2.44s1.87.4 2.44-.38m-4.27-3.16l-2.31 3.16c-.57.78-1.66.95-2.44.38s-.95-1.66-.38-2.44l2.31-3.16c.57-.78 1.66-.95 2.44-.38s.95 1.66.38 2.44"></svg:path>`,
})
export class FluentEmojiHighContrastSunBehindRainCloudIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
