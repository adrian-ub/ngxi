import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiHighContrastMechanicalArmIcon],svg[fluent-emoji-high-contrast-mechanical-arm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor"><svg:path d="M7.94 19.996a3.502 3.502 0 1 0 0 7.004a3.502 3.502 0 0 0 0-7.004m-2.503 3.502a2.502 2.502 0 1 1 5.004 0a2.502 2.502 0 0 1-5.004 0M25 21.5a3.5 3.5 0 1 1-7 0a3.5 3.5 0 0 1 7 0"></svg:path><svg:path d="M28.067 14.741a9.97 9.97 0 0 0-6.661-2.747a9.07 9.07 0 0 0-8.162 4.851c-.556-1.107-1.321-2.639-1.938-3.876h2.118A2.8 2.8 0 0 0 16 10.661a3.36 3.36 0 0 0 1.2-2.692c0-3.662-4.1-5.5-6.374-5.5c-3.113-.05-4.027 2.234-4.842 4.268l-.01.026c-.5 1.294-4.912 12.744-4.912 16.221a8.1 8.1 0 0 0 2.3 5.687a7.84 7.84 0 0 0 5.734 2.3h11.417c7.2 0 10.487-5.287 10.487-10.2a8.48 8.48 0 0 0-2.933-6.03M7.835 7.5c.873-2.185 1.357-3.029 2.907-3.029h.07c1.491 0 4.391 1.235 4.391 3.5c.004.228-.037.454-.121.666l-.025-.04q-.014-.025-.032-.046a2 2 0 0 0-.384-.346a.96.96 0 0 0-.511-.172h-.537c-.515-.652-2.1-1.813-3.343-1.593l1.3 1.648h1.85l.268.639c.332.791.217 1.183.109 1.355a.55.55 0 0 1-.415.26H7.079l.618.8c.277.34.616.624 1 .837a1 1 0 0 0 .1.433l.354.709c.336.674.864 1.733 1.39 2.784a9 9 0 0 0-2.474-.377a7.1 7.1 0 0 0-3.389.869A169 169 0 0 1 7.835 7.5m9.779 21.469h-8.52a5.9 5.9 0 0 1-4.309-1.7A6.07 6.07 0 0 1 3.062 23a14.2 14.2 0 0 1 .561-3.061a5.55 5.55 0 0 1 4.439-2.408a7.4 7.4 0 0 1 2.388.443c.53.185 1.035.44 1.5.756c.128.254.223.439.26.509a1.28 1.28 0 0 0 1.1.746a8.5 8.5 0 0 0 4.305 8.984m3.304-.007A7.5 7.5 0 0 1 21.614 14c1.903.1 3.71.881 5.089 2.204A6.58 6.58 0 0 1 29 20.769c0 3.987-2.504 8.008-8.082 8.193"></svg:path></svg:g>`,
})
export class FluentEmojiHighContrastMechanicalArmIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
