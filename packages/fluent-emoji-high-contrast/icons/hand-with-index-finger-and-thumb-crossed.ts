import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiHighContrastHandWithIndexFingerAndThumbCrossedIcon],svg[fluent-emoji-high-contrast-hand-with-index-finger-and-thumb-crossed-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#fluentEmojiHighContrastHandWithIndexFingerAndThumbCrossed0)"><svg:path fill="currentColor" d="M4.742 15.418a2 2 0 0 0-.182.358a11.25 11.25 0 0 0-1.135 4.937C3.425 26.947 8.48 32 14.713 32c3.927 0 7.384-2.007 9.403-5.042l4.248-5.506a4.63 4.63 0 0 0-.93-6.56a4.36 4.36 0 0 0-2.045-3.445a4.35 4.35 0 0 0-2.08-.658a4.66 4.66 0 0 0-2.65-3.348l.45-.636a4.323 4.323 0 0 0-6.405-5.725a6 6 0 0 0-.339-.246C13.316.128 11.701-.42 9.995.432C9.147.857 8.43 1.435 7.962 2.216a3.85 3.85 0 0 0-.52 2.373c.12 1.247.8 2.37 1.128 2.913l.051.084a17 17 0 0 1 .22.376l.016.032c.158.342.278.63.37.868l-3.075 4.494a11.3 11.3 0 0 0-1.41 2.062m2.788-.593q.102-.124.208-.244l3.358-4.908a.5.5 0 0 0 .098-.106l3.927-5.723a.5.5 0 0 0 .073-.16l.471-.689a2.323 2.323 0 1 1 3.813 2.654l-2.215 3.127l-2.658 3.417a.5.5 0 0 0 .79.614l2.92-3.755a2.678 2.678 0 0 1 3.04 3.142L19.1 15.2a.5.5 0 0 0 .8.6l2.051-2.735a2.38 2.38 0 0 1 2.378.078a2.36 2.36 0 0 1 1.112 1.938l-2.226 2.969a.5.5 0 0 0 .8.6l1.802-2.403q.155.072.3.166l.02.013l.02.013a2.627 2.627 0 0 1 .623 3.79l-4.292 5.566a9.287 9.287 0 0 1-16.765-7.422a9.2 9.2 0 0 1 .704-1.86l.008-.015l-.01.002l.037-.055q.451-.869 1.068-1.62m6.692-11.437l-3.178 4.63a17 17 0 0 0-.371-.863c-.077-.165-.199-.368-.337-.599c-.717-1.192-1.875-3.12.554-4.335c.926-.463 1.845-.14 2.583.436c.279.217.531.47.749.73"></svg:path></svg:g><svg:defs><svg:clippath id="fluentEmojiHighContrastHandWithIndexFingerAndThumbCrossed0"><svg:path fill="#fff" d="M0 0h32v32H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class FluentEmojiHighContrastHandWithIndexFingerAndThumbCrossedIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
