import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fluentEmojiHighContrastRevolvingHeartsIcon],svg[fluent-emoji-high-contrast-revolving-hearts-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M25.825 2.076C24.505 2.318 23.5 3.93 23.5 3.93s-1.015-1.612-2.324-1.854c-3.258-.59-4.598 2.37-4.06 4.572c.697 2.834 3.725 5.414 5.348 6.626a1.72 1.72 0 0 0 2.072 0c1.623-1.212 4.651-3.792 5.349-6.626c.538-2.202-.802-5.162-4.06-4.572M2.96 11.697C4.35 8.451 9.105 5.51 15.462 5.999a.5.5 0 0 0 .077-.997C8.895 4.49 3.649 7.549 2.04 11.303a.5.5 0 1 0 .919.394m25.921-.495a.5.5 0 0 0-.784.621c.72.908.98 1.82.94 2.682c-.041.873-.39 1.734-.95 2.518c-1.133 1.585-3.054 2.753-4.657 2.982a.5.5 0 1 0 .142.99c1.897-.271 4.051-1.603 5.328-3.39c.644-.902 1.084-1.944 1.135-3.053c.052-1.118-.293-2.264-1.154-3.35m-16.874 3.692s1.558-2.415 3.586-2.779c5.014-.889 7.071 3.547 6.222 6.851c-1.122 4.444-6.139 8.747-8.535 10.593a2.09 2.09 0 0 1-2.565-.001c-2.396-1.848-7.413-6.155-8.536-10.592c-.829-3.304 1.238-7.74 6.242-6.85c2.028.363 3.586 2.778 3.586 2.778"></svg:path>`,
})
export class FluentEmojiHighContrastRevolvingHeartsIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
