import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[cbiBulbGroupClassicHung3AltIcon],svg[cbi-bulb-group-classic-hung-3-alt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M7.09 7.95L6.8 6.12a.57.57 0 0 0-.18-.34l-.27-.25c-.02-.02-.04-.03-.05-.05l-.29-.34a.57.57 0 0 0-.46-.21h-.43c-.18 0-.35.08-.46.21l-.29.34c-.02.02-.03.04-.05.05l-.27.25c-.1.09-.16.21-.18.34l-.29 1.83a19.3 19.3 0 0 1 3.51 0M5.32 19.07c.43 0 .83-.09 1.22-.21c-.5-.83-.8-1.77-.8-2.74c0-.21.03-.39.05-.54a1 1 0 0 0 .02-.19c.03-.23.07-.42.11-.62c.01-.02.01-.05.02-.07l.27-.9v-.01c-.29-.01-.58-.02-.9-.02c-2 0-3.36.31-4 .58c-.03.14-.06.26-.08.41c-.02.16-.05.31-.05.48c.01 1.87 1.87 3.83 4.14 3.83m1.14-5.85c.25-.52.53-.94.78-1.33c.2-.29.37-.55.5-.82c-.02-.08-.05-.16-.06-.25c-.08-.48-.15-1.16-.1-1.65a.58.58 0 0 0-.53-.62c-.51-.05-1.09-.09-1.73-.09c-.65 0-1.23.03-1.73.09c-.31.03-.55.31-.53.62c.05.49-.02 1.17-.1 1.65c-.19 1.11-.97 1.64-1.42 2.82c1.11-.35 2.74-.47 3.79-.47c.32.01.72.03 1.13.05m10.36.87c-1.41-.45-3.48-.59-4.82-.59s-3.41.14-4.82.59c.57-1.51 1.57-2.18 1.81-3.59c.11-.61.19-1.47.13-2.1c-.04-.4.27-.75.67-.79c.65-.07 1.38-.11 2.21-.11s1.56.04 2.21.11c.4.04.7.39.66.79c-.06.63.03 1.49.13 2.1c.24 1.41 1.24 2.08 1.82 3.59m-2.57-7.24c-.78-.07-1.64-.1-2.25-.1s-1.47.03-2.25.1l.38-2.33c.02-.16.1-.32.23-.43l.35-.32c.02-.02.05-.04.07-.07l.37-.44a.72.72 0 0 1 .57-.26h.55c.23 0 .45.1.59.27l.37.44c.02.02.04.05.07.07l.35.32c.12.11.21.27.23.43zM12 21c-2.89 0-5.25-2.5-5.25-4.88c0-.21.04-.41.06-.61c.02-.19.06-.35.1-.53c.82-.35 2.55-.74 5.09-.74s4.27.4 5.09.75c.04.18.07.34.1.53c.02.2.06.39.06.61C17.25 18.5 14.89 21 12 21m6.65-13.13c.48 0 1.16.02 1.77.08l-.29-1.83a.57.57 0 0 0-.18-.34l-.27-.25c-.02-.01-.03-.03-.05-.05l-.29-.34a.6.6 0 0 0-.46-.21h-.43c-.18 0-.35.07-.46.21l-.29.34c-.01.02-.03.03-.05.05l-.27.25c-.1.09-.16.21-.18.34l-.29 1.83c.61-.06 1.26-.08 1.74-.08m4.17 7.37c0-.17-.03-.32-.05-.48c-.02-.15-.05-.27-.08-.41c-.64-.27-2-.58-4-.58c-.32 0-.61.01-.9.02v.01l.27.9c.01.02.01.05.02.07c.04.2.08.39.11.62c0 .08.01.14.02.19c.02.15.05.33.05.54c0 .97-.3 1.91-.8 2.74c.39.12.79.21 1.22.21c2.27 0 4.13-1.96 4.14-3.83m-4.15-2.07c1.05 0 2.68.12 3.79.47c-.45-1.18-1.23-1.71-1.42-2.82c-.08-.48-.15-1.16-.1-1.65a.594.594 0 0 0-.53-.62c-.5-.06-1.08-.09-1.73-.09c-.64 0-1.22.04-1.73.09c-.32.03-.56.31-.53.62c.05.49-.02 1.17-.1 1.65c-.01.09-.04.17-.06.25c.13.27.3.53.5.82c.25.39.53.81.78 1.33c.41-.02.81-.04 1.13-.05"></svg:path>`,
})
export class CbiBulbGroupClassicHung3AltIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}
