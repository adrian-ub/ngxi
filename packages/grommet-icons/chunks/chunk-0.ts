import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAccessibilityIcon],svg[grommet-icons-accessibility-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h7v6l-4 7M20 8h-7v6l4 7M12 5a1 1 0 1 0 0-2a1 1 0 0 0 0 2m-1 3h2v5h-2z"></svg:path>`,
})
export class GrommetIconsAccessibilityIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAchievementIcon],svg[grommet-icons-achievement-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10.325 14.763A6 6 0 0 1 6 9V1h12v9M6 3H1v4c0 2.509 1.791 4 4 4h1m14.034-.115C21.742 10.49 23 9.103 23 7V3h-5m-8 16H5v4h11.5m0-13a6.5 6.5 0 1 0 0 13a6.5 6.5 0 0 0 0-13Zm3.5 4l-4.5 4.5L13 16m-2.794-.576A4 4 0 0 0 8 19"></svg:path>`,
})
export class GrommetIconsAchievementIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsActionIcon],svg[grommet-icons-action-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m1 23l3-3zM20 4l3-3zM9 11l3-3zm4 4l3-3zM10 5l9 9l1-1c2-2 4.053-5 0-9s-7-2-9 0zm-6 6l1-1l9 9l-1 1c-2 2-5 4.087-9 0s-2-7 0-9Z"></svg:path>`,
})
export class GrommetIconsActionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsActionsIcon],svg[grommet-icons-actions-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 17.5a5.5 5.5 0 1 0 0-11a5.5 5.5 0 0 0 0 11Zm0-11V1m0 22v-5.5M1 12h5.5m11 0H23M4.438 4.438l4.125 4.125m6.874 6.874l4.126 4.126m0-15.125l-4.125 4.125m-6.876 6.874l-4.124 4.126"></svg:path>`,
})
export class GrommetIconsActionsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAddIcon],svg[grommet-icons-add-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22V2M2 12h20"></svg:path>`,
})
export class GrommetIconsAddIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAdIcon],svg[grommet-icons-ad-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 10h4v4h-4zm4 8h-4a4 4 0 0 1-4-4v-4a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v4m-4 8h-4a8 8 0 0 1-8-8v-4a8 8 0 0 1 8-8h4a8 8 0 0 1 8 8v4"></svg:path>`,
})
export class GrommetIconsAdIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAdobeCreativeCloudIcon],svg[grommet-icons-adobe-creative-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M24 11.456v.79q-.013.064-.022.128c-.04.343-.067.69-.124 1.03a8.4 8.4 0 0 1-1.176 3.093a8.8 8.8 0 0 1-3.645 3.303a8.5 8.5 0 0 1-2.812.83l-.652.071H7.138c-.043-.008-.085-.02-.128-.023a7.2 7.2 0 0 1-2.448-.588C2.865 19.347 1.6 18.143.771 16.486a7.1 7.1 0 0 1-.7-2.33L0 13.563v-.767c.008-.043.02-.085.023-.128a7 7 0 0 1 .461-2.139C1.2 8.68 2.45 7.31 4.222 6.42a7.3 7.3 0 0 1 2.928-.756a8 8 0 0 1 1.503.069a.2.2 0 0 0 .142-.046a8.9 8.9 0 0 1 2.457-1.781a8.6 8.6 0 0 1 2.826-.833q.35-.039.7-.073h.72q.07.012.14.021c.357.042.715.068 1.068.127a8.5 8.5 0 0 1 3.073 1.166a8.8 8.8 0 0 1 3.319 3.652c.438.883.719 1.835.83 2.814zM10.58 6.343c.015.02.019.028.025.031q.741.366 1.41.853c.028.02.098.012.132-.01c1.222-.774 2.547-1.041 3.97-.789c1.395.247 2.53.944 3.397 2.057c.982 1.26 1.357 2.684 1.085 4.27c-.181 1.061-.607 2.014-1.33 2.812c-1.32 1.456-2.972 2.057-4.917 1.803a5.2 5.2 0 0 1-3.086-1.538c-1.098-1.11-2.204-2.211-3.305-3.318c-.147-.147-.31-.265-.521-.291a.83.83 0 0 0-.864.47c-.142.296-.124.63.185.934c1.227 1.205 2.444 2.421 3.67 3.627q.315.313.674.573c.896.65 1.906 1.01 3 1.155a7.2 7.2 0 0 0 2.555-.1a7.08 7.08 0 0 0 4-2.455a7.03 7.03 0 0 0 1.576-3.609a7 7 0 0 0-.065-2.32c-.244-1.251-.773-2.37-1.62-3.322c-1.618-1.816-3.653-2.627-6.074-2.446a6.75 6.75 0 0 0-2.641.777a7 7 0 0 0-1.255.836ZM9.989 19.02l-.1-.093c-.501-.474-1.006-.945-1.502-1.425a.4.4 0 0 0-.32-.135c-.502.012-1.005.015-1.5-.097c-2.461-.557-3.89-3.233-2.983-5.587c.715-1.857 2.696-2.988 4.649-2.639c.875.156 1.644.527 2.275 1.177c.769.794 1.56 1.566 2.338 2.352c.177.179.38.26.626.225c.316-.045.569-.196.683-.507a.75.75 0 0 0-.173-.83c-.816-.826-1.613-1.673-2.462-2.465c-1.647-1.536-3.588-1.992-5.736-1.34c-2.888.878-4.579 3.86-3.919 6.804a5.816 5.816 0 0 0 5.691 4.566h2.299c.038 0 .076-.003.134-.006"></svg:path>`,
})
export class GrommetIconsAdobeCreativeCloudIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAddCircleIcon],svg[grommet-icons-add-circle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm0-4V6m-6 6h12"></svg:path>`,
})
export class GrommetIconsAddCircleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAedIcon],svg[grommet-icons-aed-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 8.4C2 4 5 3 7 3s4 2 5 3.5C13 5 15 3 17 3s5 1 5 5.4C22 15 12 21 12 21S2 15 2 8.4ZM12 6c-.5-.5-2 4-2 4h2v2.5L14 9h-2s1-6 5-6c-4 0-5 3-5 3Z"></svg:path>`,
})
export class GrommetIconsAedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAggregateIcon],svg[grommet-icons-aggregate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 15h7V8a7 7 0 1 0-7 7Zm8-6H9v7a7 7 0 1 0 7-7Z"></svg:path>`,
})
export class GrommetIconsAggregateIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAidIcon],svg[grommet-icons-aid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 22h22V6H1zM8 6h8V2H8zm0 8h8m-4-4v8"></svg:path>`,
})
export class GrommetIconsAidIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAidOptionIcon],svg[grommet-icons-aid-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 9v10zm5 5H7zM1 6.995C1 5.893 1.89 5 2.991 5H21.01C22.109 5 23 5.893 23 6.995v14.01C23 22.107 22.11 23 21.009 23H2.99A1.99 1.99 0 0 1 1 21.005zM7 5V2.01C7 1.451 7.456 1 7.995 1h8.01c.55 0 .995.443.995 1.01V5"></svg:path>`,
})
export class GrommetIconsAidOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAlarmIcon],svg[grommet-icons-alarm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M21 13a9 9 0 0 1-9 9a9 9 0 0 1-9-9a9 9 0 0 1 9-9a9 9 0 0 1 9 9ZM5.5 19.5L2 23zm13 0L22 23zM9 4c-.71-1.092-2.118-2-4-2c-2.1 0-4 1.9-4 4c0 1.882.908 3.29 2 4m18 0c1.092-.71 2-2.118 2-4c0-2.1-1.9-4-4-4c-1.882 0-3.29.908-4 2m-3 4v5l3 3"></svg:path>`,
})
export class GrommetIconsAlarmIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAlertIcon],svg[grommet-icons-alert-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 17v2m0-9v6m0-13L2 22h20z"></svg:path>`,
})
export class GrommetIconsAlertIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAmazonIcon],svg[grommet-icons-amazon-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M11.992 23.94c-3.155-.021-5.956-1.061-8.46-2.955a16.7 16.7 0 0 1-2.446-2.282c-.027-.03-.072-.058-.075-.09c-.011-.117-.008-.234-.011-.35c.1.016.218.003.298.053c.67.417 1.314.878 2 1.264a18.6 18.6 0 0 0 6.378 2.192c1.18.18 2.365.24 3.55.17a17.8 17.8 0 0 0 6.486-1.648c.325-.151.644-.316.964-.479c.154-.079.307-.111.438.028c.139.146.133.322.036.485a1.1 1.1 0 0 1-.222.248c-1.841 1.633-3.995 2.636-6.396 3.103c-.832.161-1.684.217-2.527.321zm8.89-5.095c-.456.044-.961.094-1.467.14c-.11.01-.224.017-.334.01c-.128-.008-.148-.086-.078-.18a.7.7 0 0 1 .162-.137c.536-.359 1.141-.516 1.769-.6a4.5 4.5 0 0 1 1.773.096c.394.106.48.153.45.61c-.078 1.206-.466 2.303-1.34 3.183c-.045.046-.087.104-.143.126c-.074.029-.16.028-.24.04c.002-.073-.013-.152.009-.219c.208-.634.426-1.265.633-1.9a2 2 0 0 0 .073-.397c.051-.428-.12-.643-.574-.71c-.212-.03-.427-.039-.693-.062M13.997 9.42c-.86-.005-1.68.032-2.466.296c-.388.13-.781.295-1.115.527c-.923.639-1.163 1.59-1.087 2.65c.04.562.209 1.082.623 1.489c.588.578 1.579.654 2.374.184c.693-.41 1.107-1.042 1.375-1.779c.395-1.085.27-2.216.296-3.367m.632 6.353c-.309.28-.594.545-.884.804c-1.118.994-2.438 1.438-3.92 1.467c-.805.017-1.596-.044-2.349-.342c-1.342-.53-2.197-1.51-2.544-2.902c-.431-1.728-.286-3.392.737-4.902c.756-1.116 1.849-1.78 3.11-2.186c1.084-.35 2.205-.502 3.33-.628c.618-.07 1.236-.13 1.884-.199c-.02-.773.09-1.55-.181-2.297c-.241-.662-.75-1.003-1.414-1.155c-1.007-.23-2.147.145-2.688.938c-.184.27-.296.597-.395.913c-.139.445-.343.59-.805.535c-.902-.107-1.807-.196-2.71-.302c-.447-.052-.624-.307-.537-.742c.397-1.98 1.543-3.363 3.41-4.09c2.201-.86 4.452-.927 6.684-.106c1.85.68 2.865 2.035 3 4.013c.046.668.052 1.34.055 2.009c.007 1.732.016 3.464-.003 5.196c-.01.965.258 1.82.844 2.582c.141.183.284.367.403.564c.198.327.145.575-.141.822q-1.105.954-2.208 1.911c-.404.352-.685.347-1.092.004a7 7 0 0 1-1.357-1.569c-.069-.107-.142-.21-.229-.338"></svg:path>`,
})
export class GrommetIconsAmazonIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAmexIcon],svg[grommet-icons-amex-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m4.314 11.965l-.82-1.997l-.815 1.997zm7.859 2.161l-.005-3.922l-1.736 3.922h-1.05L7.64 10.2v3.926H5.206l-.46-1.117H2.253l-.465 1.117h-1.3l2.144-5.008H4.41l2.036 4.742V9.118H8.4l1.567 3.397l1.439-3.397H13.4v5.008zm3.133-1.024v-.997h2.628v-1.022h-2.628v-.911h3.001l1.31 1.46l-1.368 1.47zm8.111 1.044h-1.556l-1.474-1.659l-1.532 1.659h-4.742v-5.01h4.815l1.473 1.642l1.523-1.642h1.564l-2.327 2.505z"></svg:path>`,
})
export class GrommetIconsAmexIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAnalyticsIcon],svg[grommet-icons-analytics-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 7c1.25 1.67 2 3.75 2 6c0 5.52-4.48 10-10 10S2 18.52 2 13S6.48 3 12 3m0-2v12l9.6-7.2C19.41 2.89 15.92 1 12 1Z"></svg:path>`,
})
export class GrommetIconsAnalyticsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAnchorIcon],svg[grommet-icons-anchor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 6a2 2 0 1 0 0-4a2 2 0 0 0 0 4Zm8 11a1 1 0 1 0 0-2a1 1 0 0 0 0 2ZM4 17a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm8-11v15m-8-5.027Q7.29 21 12 21t8-5.027M16 10H8"></svg:path>`,
})
export class GrommetIconsAnchorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAndroidIcon],svg[grommet-icons-android-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M5.685 7.914h12.718v10.364c0 .615-.499 1.115-1.114 1.115h-1.274v3.156A1.44 1.44 0 0 1 14.587 24a1.44 1.44 0 0 1-1.43-1.451v-3.156h-2.225v3.156A1.44 1.44 0 0 1 9.502 24a1.44 1.44 0 0 1-1.429-1.451v-3.156H6.8c-.615 0-1.115-.5-1.115-1.115zm-2.492-.085c-.797 0-1.443.656-1.443 1.466v5.727c0 .808.646 1.465 1.443 1.465s1.443-.657 1.443-1.465V9.295c0-.81-.646-1.466-1.443-1.466m15.21-.96H5.685C5.842 5.059 7.018 3.5 8.71 2.597L7.5.82a.525.525 0 1 1 .868-.59l1.318 1.936a7.2 7.2 0 0 1 4.717 0L15.721.23a.524.524 0 1 1 .867.59L15.38 2.596c1.692.902 2.866 2.461 3.023 4.274m-8.338-2.461a.703.703 0 1 0-1.406-.001a.703.703 0 0 0 1.406 0m5.454 0a.704.704 0 1 0-1.408 0a.704.704 0 0 0 1.408 0m5.378 3.42c-.797 0-1.444.656-1.444 1.466v5.729c0 .81.647 1.466 1.444 1.466s1.441-.657 1.441-1.466v-5.73c0-.809-.644-1.466-1.441-1.466"></svg:path>`,
})
export class GrommetIconsAndroidIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAnnounceIcon],svg[grommet-icons-announce-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M11 15c3 0 8 4 8 4V3s-5 4-8 4zm-6 0l3 8h4l-3-8m10-1a3 3 0 1 0 0-6m-8 11c1 0 3-1 3-3M2 11c0-3.111 1.791-4 4-4h5v8H6c-2.209 0-4-.889-4-4Z"></svg:path>`,
})
export class GrommetIconsAnnounceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAppleIcon],svg[grommet-icons-apple-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M15.3 3.832c.84-1.014 1.404-2.427 1.25-3.832c-1.208.049-2.67.805-3.535 1.819c-.777.898-1.457 2.335-1.273 3.712c1.346.105 2.722-.684 3.56-1.699m3.02 8.918c.034 3.632 3.186 4.841 3.22 4.857c-.025.085-.502 1.722-1.66 3.413c-1 1.462-2.038 2.919-3.674 2.949c-1.607.029-2.123-.953-3.961-.953c-1.836 0-2.41.923-3.932.982c-1.578.06-2.78-1.581-3.79-3.037c-2.06-2.98-3.635-8.42-1.52-12.092C4.054 7.045 5.932 5.89 7.97 5.861c1.55-.03 3.013 1.043 3.96 1.043s2.726-1.29 4.595-1.101c.783.033 2.979.316 4.39 2.381c-.114.07-2.621 1.53-2.594 4.566"></svg:path>`,
})
export class GrommetIconsAppleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAppleAppStoreIcon],svg[grommet-icons-apple-app-store-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M3 17a1 1 0 0 1 0-2h10.5c1 0 2 2 1.5 2zm14 0a1 1 0 0 1 0-2h4a1 1 0 0 1 0 2zM12.633 3.501a1 1 0 0 1 1.734.998L7.46 16.495a1 1 0 0 1-1.734-.997zM4 18.5c.5-1 3.5-2 2.5-.28A853 853 0 0 1 4.867 21a1 1 0 0 1-1.734-.998zM9.133 4.499a1 1 0 1 1 1.734-.998L12.61 6.53a1 1 0 1 1-1.733.998zM13 11.5c-.898-1.5 0-4.5.716-3.004L20.366 20a1 1 0 0 1-1.733.998z"></svg:path>`,
})
export class GrommetIconsAppleAppStoreIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAppleMusicIcon],svg[grommet-icons-apple-music-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsAppleMusic1)"><svg:path fill="url(#grommetIconsAppleMusic0)" fill-rule="evenodd" d="M21.992 22.816c.768-.592 1.304-1.424 1.536-2.016c.464-1.192.472-2.544.472-3.4V6.6c0-.848 0-2.208-.472-3.4c-.232-.592-.776-1.432-1.536-2.016a5 5 0 0 0-1.688-.88C19.528.08 18.584 0 17.4 0H6.6C5.416 0 4.472.08 3.696.304a5.1 5.1 0 0 0-1.688.88C1.24 1.776.704 2.608.472 3.2C.008 4.392 0 5.744 0 6.6v10.8c0 .856.008 2.208.472 3.4c.232.592.776 1.432 1.536 2.016c.44.344.968.664 1.688.88c.776.224 1.72.304 2.904.304h10.8c1.184 0 2.136-.072 2.904-.304a5.1 5.1 0 0 0 1.688-.88m-5.706-19.05c.062-.013.576-.104.634-.11c.385-.033.6.22.6.63v11.289c0 .303-.003.577-.067.881a2.2 2.2 0 0 1-.348.821a2.1 2.1 0 0 1-.655.606a2.6 2.6 0 0 1-.831.303c-.552.11-.93.136-1.284.065a1.8 1.8 0 0 1-.864-.44a1.88 1.88 0 0 1-.604-1.184a1.9 1.9 0 0 1 .51-1.488c.197-.207.445-.37.776-.499c.347-.134.729-.215 1.317-.333l.464-.094c.205-.041.379-.093.52-.265s.144-.384.144-.593V8.084c0-.404-.182-.514-.566-.44c-.276.054-6.19 1.247-6.19 1.247c-.333.08-.45.19-.45.603v7.722c0 .303-.015.578-.08.882a2.2 2.2 0 0 1-.348.82a2.1 2.1 0 0 1-.654.606a2.6 2.6 0 0 1-.831.307c-.552.112-.93.136-1.284.065a1.8 1.8 0 0 1-.864-.443a1.86 1.86 0 0 1-.59-1.184a1.87 1.87 0 0 1 .496-1.488c.197-.208.445-.371.776-.5c.347-.134.729-.214 1.317-.333l.464-.093c.204-.042.379-.094.52-.266c.14-.172.157-.374.157-.582V6.104c0-.12.01-.201.016-.241a.72.72 0 0 1 .24-.462a1 1 0 0 1 .443-.2h.003z" clip-rule="evenodd"></svg:path></svg:g><svg:defs><svg:lineargradient id="grommetIconsAppleMusic0" x1="12" x2="12" y1="23.907" y2=".517" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:lineargradient><svg:clippath id="grommetIconsAppleMusic1"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsAppleMusicIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsApplePodcastsIcon],svg[grommet-icons-apple-podcasts-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsApplePodcasts1)"><svg:path fill="url(#grommetIconsApplePodcasts0)" fill-rule="evenodd" d="M21.992 22.816c.768-.592 1.304-1.424 1.536-2.016c.464-1.192.472-2.544.472-3.4V6.6c0-.848 0-2.208-.472-3.4c-.232-.592-.776-1.432-1.536-2.016a5 5 0 0 0-1.688-.88C19.528.08 18.584 0 17.4 0H6.6C5.416 0 4.472.08 3.696.304a5.1 5.1 0 0 0-1.688.88C1.24 1.776.704 2.608.472 3.2C.008 4.392 0 5.744 0 6.6v10.8c0 .856.008 2.208.472 3.4c.232.592.776 1.432 1.536 2.016c.44.344.968.664 1.688.88c.776.224 1.72.304 2.904.304h10.8c1.184 0 2.136-.072 2.904-.304a5.1 5.1 0 0 0 1.688-.88M3.56 10.992c.064-4.592 3.824-8.32 8.416-8.336c4.664-.008 8.464 3.784 8.48 8.44c0 3.68-2.36 6.816-5.648 7.968c-.08.032-.168-.032-.152-.12l.12-.864a.27.27 0 0 1 .168-.232c2.6-1.144 4.424-3.736 4.424-6.752c0-4.08-3.336-7.392-7.424-7.36c-3.992.032-7.256 3.288-7.296 7.288a7.37 7.37 0 0 0 4.424 6.824a.32.32 0 0 1 .168.232c.021.192.05.384.078.576l.042.288c.008.08-.072.152-.152.12c-3.32-1.168-5.696-4.352-5.648-8.072m10.408-.584c0 1.088-.88 1.968-1.968 1.968a1.967 1.967 0 0 1-1.968-1.968c0-1.088.88-1.968 1.968-1.968s1.968.888 1.968 1.968m.088 4.08a1.07 1.07 0 0 0-.32-.688c-.36-.376-.992-.624-1.736-.624s-1.376.24-1.736.624c-.184.2-.288.4-.32.688c-.064.558-.024 1.037.04 1.807v.009c.064.736.184 1.72.336 2.712c.112.712.2 1.096.28 1.368c.136.448.624.832 1.4.832s1.272-.392 1.4-.832c.08-.272.168-.656.28-1.368c.152-1 .272-1.976.336-2.712c.072-.776.104-1.256.04-1.816m-2.272-9.032c-2.992.112-5.392 2.584-5.432 5.576a5.65 5.65 0 0 0 2.472 4.744c.072.048.176-.008.176-.096a8 8 0 0 1-.008-.968a.33.33 0 0 0-.112-.272a4.57 4.57 0 0 1-1.448-3.456a4.585 4.585 0 0 1 4.392-4.448a4.574 4.574 0 0 1 4.752 4.568c0 1.312-.56 2.496-1.448 3.336a.38.38 0 0 0-.112.272c.016.312.008.616-.008.96c-.008.088.096.152.176.096a5.66 5.66 0 0 0 2.472-4.672c.008-3.184-2.656-5.768-5.872-5.64" clip-rule="evenodd"></svg:path></svg:g><svg:defs><svg:lineargradient id="grommetIconsApplePodcasts0" x1="12" x2="12" y1="0" y2="24" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:lineargradient><svg:clippath id="grommetIconsApplePodcasts1"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsApplePodcastsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAppsIcon],svg[grommet-icons-apps-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 5h2V3h-2zm-8 0h2V3h-2zM3 5h2V3H3zm16 8h2v-2h-2zm-8 0h2v-2h-2zm-8 0h2v-2H3zm16 8h2v-2h-2zm-8 0h2v-2h-2zm-8 0h2v-2H3z"></svg:path>`,
})
export class GrommetIconsAppsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAppsRoundedIcon],svg[grommet-icons-apps-rounded-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="2" d="M3 6.2c0-1.12 0-1.68.218-2.108a2 2 0 0 1 .874-.874C4.52 3 5.08 3 6.2 3h.6c1.12 0 1.68 0 2.108.218a2 2 0 0 1 .874.874C10 4.52 10 5.08 10 6.2v.6c0 1.12 0 1.68-.218 2.108a2 2 0 0 1-.874.874C8.48 10 7.92 10 6.8 10h-.6c-1.12 0-1.68 0-2.108-.218a2 2 0 0 1-.874-.874C3 8.48 3 7.92 3 6.8zm11 0c0-1.12 0-1.68.218-2.108a2 2 0 0 1 .874-.874C15.52 3 16.08 3 17.2 3h.6c1.12 0 1.68 0 2.108.218a2 2 0 0 1 .874.874C21 4.52 21 5.08 21 6.2v.6c0 1.12 0 1.68-.218 2.108a2 2 0 0 1-.874.874C19.48 10 18.92 10 17.8 10h-.6c-1.12 0-1.68 0-2.108-.218a2 2 0 0 1-.874-.874C14 8.48 14 7.92 14 6.8zm-11 11c0-1.12 0-1.68.218-2.108a2 2 0 0 1 .874-.874C4.52 14 5.08 14 6.2 14h.6c1.12 0 1.68 0 2.108.218a2 2 0 0 1 .874.874C10 15.52 10 16.08 10 17.2v.6c0 1.12 0 1.68-.218 2.108a2 2 0 0 1-.874.874C8.48 21 7.92 21 6.8 21h-.6c-1.12 0-1.68 0-2.108-.218a2 2 0 0 1-.874-.874C3 19.48 3 18.92 3 17.8zm11 0c0-1.12 0-1.68.218-2.108a2 2 0 0 1 .874-.874C15.52 14 16.08 14 17.2 14h.6c1.12 0 1.68 0 2.108.218a2 2 0 0 1 .874.874C21 15.52 21 16.08 21 17.2v.6c0 1.12 0 1.68-.218 2.108a2 2 0 0 1-.874.874C19.48 21 18.92 21 17.8 21h-.6c-1.12 0-1.68 0-2.108-.218a2 2 0 0 1-.874-.874C14 19.48 14 18.92 14 17.8z"></svg:path>`,
})
export class GrommetIconsAppsRoundedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsArchiveIcon],svg[grommet-icons-archive-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 23h18V6H3zm6-9h6v-4H9zM1 6h22V1H1z"></svg:path>`,
})
export class GrommetIconsArchiveIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsArchlinuxIcon],svg[grommet-icons-archlinux-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M11.995 0c-1.068 2.619-1.712 4.332-2.901 6.873c.729.773 1.624 1.673 3.077 2.69c-1.562-.643-2.628-1.289-3.425-1.959C7.224 10.78 4.84 15.304 0 24c3.804-2.196 6.752-3.55 9.5-4.066a7 7 0 0 1-.18-1.63l.004-.121c.06-2.437 1.328-4.311 2.83-4.184c1.501.127 2.668 2.207 2.608 4.644c-.011.459-.063.9-.153 1.309c2.717.532 5.634 1.882 9.387 4.048c-.74-1.362-1.4-2.59-2.031-3.76c-.994-.77-2.03-1.771-4.143-2.856c1.452.377 2.493.813 3.303 1.3C14.713 6.746 14.195 5.16 11.995 0"></svg:path>`,
})
export class GrommetIconsArchlinuxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsArticleIcon],svg[grommet-icons-article-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16 7h3v4h-3zm-7 8h11M9 11h4M9 7h4M6 18.5a2.5 2.5 0 1 1-5 0V7h5.025M6 18.5V3h17v15.5a2.5 2.5 0 0 1-2.5 2.5h-17"></svg:path>`,
})
export class GrommetIconsArticleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsArubaIcon],svg[grommet-icons-aruba-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.11 17.302c-3.074 0-5.602-2.46-5.602-5.465s2.528-5.465 5.602-5.465s5.601 2.46 5.601 5.465s-2.527 5.465-5.601 5.465M12.11 2C6.508 2 2 6.44 2 11.837c0 5.465 4.508 9.836 10.11 9.836c2.323 0 4.44-.751 6.148-2.049c1.025 1.708 3.962 2.05 3.962 2.05v-9.837C22.22 6.44 17.71 2 12.11 2"></svg:path>`,
})
export class GrommetIconsArubaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAscendIcon],svg[grommet-icons-ascend-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2 8l6-6l6 6m-3 13h11m-11-4h8m-8-4h5M8 2v20"></svg:path>`,
})
export class GrommetIconsAscendIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAscendingIcon],svg[grommet-icons-ascending-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m12.08 7.286l.354-.353l.354.353L17 11.498l-.707.708l-3.358-3.359V17h-1V8.847l-3.359 3.359l-.707-.708z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsAscendingIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAssistListeningIcon],svg[grommet-icons-assist-listening-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 21q1.136 1 2 1q3 0 3-3q0-2 2-4c1.267-1.267 2-3.067 2-5a7 7 0 0 0-14 0m11 0a4 4 0 1 0-8 0M3 20l5-6l1 4l5-6"></svg:path>`,
})
export class GrommetIconsAssistListeningIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAtmIcon],svg[grommet-icons-atm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4 10H1V6h22v4h-3M6 6h12v14.006A2.003 2.003 0 0 1 15.991 22H8.01A2 2 0 0 1 6 20.006zm7 12h3m-3-3h3m-3-3h3m-6 10V6M3 2h3m2 0h3m2 0h3m2 0h3"></svg:path>`,
})
export class GrommetIconsAtmIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAttachmentIcon],svg[grommet-icons-attachment-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m22 12l-9 9c-6 6-15-3-9-9l9-9c4-4 10 2 6 6l-9 9c-2 2-5-1-3-3l9-9"></svg:path>`,
})
export class GrommetIconsAttachmentIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsAttractionIcon],svg[grommet-icons-attraction-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m13 2l1.5.5L13 3zm1 16l1 5h-2zm0-1.5l2 6.5h-4zM12 6l9 5v2H3v-2zm-7.5 7h15c0 4.167 1.5 10 1.5 10H3s1.5-5.833 1.5-10Zm0 0h15c0 4.167 1.5 10 1.5 10H3s1.5-5.833 1.5-10Z"></svg:path>`,
})
export class GrommetIconsAttractionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBabyIcon],svg[grommet-icons-baby-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 15h4s-1 1.5-2 1.5s-2-1.5-2-1.5m2-9a1 1 0 1 0 0-2a1 1 0 0 0 0 2m6 6l-4-3m0 6l3 2.5l-2.5 2.5M6 12l4-3m0 6l-2.5 2.75L10 20m0-11h4v3h-4z"></svg:path>`,
})
export class GrommetIconsBabyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBackTenIcon],svg[grommet-icons-back-ten-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3.111 7.556C4.67 4.267 8.07 2 12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12m0-8v4h4m3 8V9l-2 .533M17 12c0-2-1-3.5-2.5-3.5S12 10 12 12s1 3.5 2.5 3.5S17 14 17 12Zm-2.5-3.5C16.925 8.5 17 11 17 12s0 3.5-2.5 3.5S12 13 12 12s.059-3.5 2.5-3.5Z"></svg:path>`,
})
export class GrommetIconsBackTenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBarIcon],svg[grommet-icons-bar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 5h16v18H7zm0 0h16V3c0-1.105-.895-2-1.994-2H8.994A1.99 1.99 0 0 0 7 3zM1 8.009C1 6.899 1.898 6 2.998 6H7v12H2.998A2.005 2.005 0 0 1 1 15.991zM11 8v10m4-10v10m4-10v10"></svg:path>`,
})
export class GrommetIconsBarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBarChartIcon],svg[grommet-icons-bar-chart-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M0 22h24M22 2h-4v16h4zM6 6H2v12h4zm8 12h-4v-8h4z"></svg:path>`,
})
export class GrommetIconsBarChartIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBasketIcon],svg[grommet-icons-basket-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 12h20l-2 11H4zm18-4l-6-7M4 8l6-7M1 8h22v4H1zm7 7v5m8-5v5m-4-5v5"></svg:path>`,
})
export class GrommetIconsBasketIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBeaconIcon],svg[grommet-icons-beacon-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 12a3 3 0 1 0 0-6a3 3 0 0 0 0 6zm0 0v11M7.05 4.05A6.98 6.98 0 0 0 5 9c0 1.933.784 3.683 2.05 4.95m9.9 0A6.98 6.98 0 0 0 19 9a6.98 6.98 0 0 0-2.05-4.95M4.222 1.222A10.97 10.97 0 0 0 1 9c0 3.037 1.231 5.787 3.222 7.778m15.556 0A10.97 10.97 0 0 0 23 9c0-3.038-1.231-5.788-3.222-7.778"></svg:path>`,
})
export class GrommetIconsBeaconIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBikeIcon],svg[grommet-icons-bike-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 19a4 4 0 1 0 0-8a4 4 0 0 0 0 8Zm14 0a4 4 0 1 0 0-8a4 4 0 0 0 0 8ZM5 6h5m9 9L16 5h-3M9 9l-4 6h7c0-3 2-6 5-6zm0 0L7 6"></svg:path>`,
})
export class GrommetIconsBikeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBitcoinIcon],svg[grommet-icons-bitcoin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M23.64 14.905c-1.602 6.429-8.114 10.342-14.544 8.738C2.669 22.041-1.244 15.528.359 9.1S8.473-1.244 14.902.36c6.43 1.603 10.342 8.116 8.739 14.546m-6.349-4.613c.24-1.597-.977-2.456-2.64-3.028l.54-2.164l-1.317-.328l-.525 2.106a55 55 0 0 0-1.055-.248l.53-2.12l-1.317-.328l-.54 2.162a44 44 0 0 1-.84-.197l.001-.007l-1.816-.453l-.35 1.406s.977.224.956.238c.534.133.63.486.614.766l-.615 2.464q.057.014.137.044l-.139-.035l-.86 3.453c-.066.162-.231.405-.604.312c.013.02-.957-.239-.957-.239l-.654 1.508l1.713.427q.479.122.94.242l-.546 2.188l1.316.328l.54-2.164q.537.145 1.049.272l-.538 2.155l1.317.328l.544-2.184c2.246.425 3.934.253 4.645-1.778c.573-1.635-.029-2.578-1.21-3.193c.86-.198 1.508-.764 1.681-1.933m-3.008 4.219c-.407 1.635-3.16.75-4.053.53l.723-2.9c.893.223 3.755.664 3.33 2.37m.407-4.243c-.371 1.487-2.663.732-3.406.547l.656-2.63c.743.186 3.137.532 2.75 2.083"></svg:path>`,
})
export class GrommetIconsBitcoinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBladesHorizontalIcon],svg[grommet-icons-blades-horizontal-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 17h4m3 0h4m3 0h4M4 21h2a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v16a1 1 0 0 0 1 1zm7 0h2a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1h-2a1 1 0 0 0-1 1v16a1 1 0 0 0 1 1zm7 0h2a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1h-2a1 1 0 0 0-1 1v16a1 1 0 0 0 1 1z"></svg:path>`,
})
export class GrommetIconsBladesHorizontalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBladesVerticalIcon],svg[grommet-icons-blades-vertical-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 3v4m0 3v4m0 3v4m4-17v2a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1zm0 7v2a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-2a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1zm0 7v2a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-2a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1z"></svg:path>`,
})
export class GrommetIconsBladesVerticalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBlockQuoteIcon],svg[grommet-icons-block-quote-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M.78 8.89c0-3.07 1.53-4.3 4.3-4.34L5.38 6C3.78 6.17 3 7 3.1 8.31h1.44V12H.78Zm5.9 0c0-3.07 1.53-4.3 4.3-4.34l.3 1.45C9.68 6.17 8.89 7 9 8.31h1.44V12H6.68Zm10.26 6.22c0 3.07-1.53 4.3-4.3 4.34L12.35 18c1.6-.16 2.39-1 2.28-2.3h-1.45V12h3.76Zm5.9 0c0 3.07-1.53 4.3-4.3 4.34l-.3-1.45c1.6-.16 2.39-1 2.28-2.3h-1.44V12h3.76Z"></svg:path>`,
})
export class GrommetIconsBlockQuoteIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBlogIcon],svg[grommet-icons-blog-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 16a3 3 0 1 0 0 6a3 3 0 0 0 0-6ZM5 1c9.925 0 18 8.075 18 18m-5 0c0-7.168-5.832-13-13-13m8 13c0-4.411-3.589-8-8-8m-3 0v8"></svg:path>`,
})
export class GrommetIconsBlogIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBluetoothIcon],svg[grommet-icons-bluetooth-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m7 7l10 9l-5 4V4l5 4l-10 8"></svg:path>`,
})
export class GrommetIconsBluetoothIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBoldIcon],svg[grommet-icons-bold-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M14 11.57a7.9 7.9 0 0 1 3.11 1.25a3.32 3.32 0 0 1 1.28 2.71A3.58 3.58 0 0 1 17 18.42a7.9 7.9 0 0 1-5 1.39H4.07v-.42a3.6 3.6 0 0 0 1.46-.2a1.17 1.17 0 0 0 .54-.52a4.8 4.8 0 0 0 .15-1.58V7a4.8 4.8 0 0 0-.15-1.6a1.13 1.13 0 0 0-.54-.52a3.7 3.7 0 0 0-1.46-.2v-.42h7.45a10.6 10.6 0 0 1 3.78.48a3.94 3.94 0 0 1 1.75 1.42a3.5 3.5 0 0 1 .64 2a2.86 2.86 0 0 1-.81 2A5.84 5.84 0 0 1 14 11.57m-4.17.58v5.56a1.27 1.27 0 0 0 .32.93a1.27 1.27 0 0 0 .93.31a3.6 3.6 0 0 0 1.69-.41A2.8 2.8 0 0 0 14 17.37a3.6 3.6 0 0 0 .41-1.73a3.8 3.8 0 0 0-.5-2a2.72 2.72 0 0 0-1.39-1.21a7.5 7.5 0 0 0-2.67-.28Zm0-.89a5.9 5.9 0 0 0 2.4-.37a2.73 2.73 0 0 0 1.19-1a3.17 3.17 0 0 0 .41-1.7a3.2 3.2 0 0 0-.41-1.69a2.6 2.6 0 0 0-1.16-1a6.2 6.2 0 0 0-2.43-.33Z"></svg:path>`,
})
export class GrommetIconsBoldIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBookIcon],svg[grommet-icons-book-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 1v10l3-2l3 2V1M5.5 18a2.5 2.5 0 1 0 0 5H22M3 20.5v-17A2.5 2.5 0 0 1 5.5 1H21v17.007H5.492M20.5 18a2.5 2.5 0 1 0 0 5"></svg:path>`,
})
export class GrommetIconsBookIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBookmarkIcon],svg[grommet-icons-bookmark-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 1v21l7-5l7 5V1z"></svg:path>`,
})
export class GrommetIconsBookmarkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBottomCornerIcon],svg[grommet-icons-bottom-corner-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 20h12V8"></svg:path>`,
})
export class GrommetIconsBottomCornerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBrailleIcon],svg[grommet-icons-braille-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 10a3 3 0 1 0 0-6a3 3 0 0 0 0 6zm10 10a3 3 0 1 0 0-6a3 3 0 0 0 0 6zM7 8a1 1 0 1 0 0-2a1 1 0 0 0 0 2zm10 10a1 1 0 1 0 0-2a1 1 0 0 0 0 2zM7 20a3 3 0 1 1 0-6a3 3 0 0 1 0 6zm10-10a3 3 0 1 1 0-6a3 3 0 0 1 0 6z"></svg:path>`,
})
export class GrommetIconsBrailleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBriefcaseIcon],svg[grommet-icons-briefcase-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 6h22v15H1zm5 0v15M18 6v15M8 6V3h8v3"></svg:path>`,
})
export class GrommetIconsBriefcaseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBrushIcon],svg[grommet-icons-brush-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9.312 11.73a5 5 0 0 0-5.362 1.12c-1.953 1.952-1.414 8.485-1.414 8.485s6.532.538 8.485-1.415a5 5 0 0 0 1.12-5.362L22.334 4.364a1.997 1.997 0 0 0 0-2.828a1.995 1.995 0 0 0-2.828 0zm1.002-1.617l1.838 1.838l1.697 1.697"></svg:path>`,
})
export class GrommetIconsBrushIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBucketIcon],svg[grommet-icons-bucket-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4 10H2c0-5.523 0-9 10-9s10 3.477 10 9h-2m-8 0c4.418 0 8-.895 8-2s-3.582-2-8-2s-8 .895-8 2s3.582 2 8 2ZM4 20c0 1.657 3.582 3 8 3s8-1.343 8-3m0-12v12zM4 20V8z"></svg:path>`,
})
export class GrommetIconsBucketIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBugIcon],svg[grommet-icons-bug-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M23 20c-1.38-2.09-3-3-4-3M5 17c-1 0-2.62.91-4 3M19 9c3 0 4-3 4-3M1 6s1 3 4 3m14 4h5zM5 13H0zm7 10V12zm0 0c-4 0-7-3-7-7V9s3-2.012 7-2s7 2 7 2v7c0 4-3 7-7 7ZM7 8V6c0-2.76 2.24-5 5-5s5 2.24 5 5v2"></svg:path>`,
})
export class GrommetIconsBugIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBundleIcon],svg[grommet-icons-bundle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 14H7zm0-11h6v10h-6M1 13v4h6m10 0h6v-4M1 17v4h6m16-4v4h-6M7 22h10V2H7zm0-9H1V3h6"></svg:path>`,
})
export class GrommetIconsBundleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBusIcon],svg[grommet-icons-bus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 12h18v8H3zm0-8c0-1.105.893-2 1.995-2h14.01C20.107 2 21 2.887 21 4v8H3zm0 16h3v2.001a.996.996 0 0 1-.999.999H3.999A.996.996 0 0 1 3 22.001zm15 0h3v2.001a.996.996 0 0 1-.999.999h-1.002a.996.996 0 0 1-.999-.999zM7 17a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm10 0a1 1 0 1 0 0-2a1 1 0 0 0 0 2ZM12 6v6M1 5v8m22-8v8m-13 3h4M3 6h18"></svg:path>`,
})
export class GrommetIconsBusIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsBusinessServiceIcon],svg[grommet-icons-business-service-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4 23H1V5h22v18h-7M8 5V1h8v4M9 16a4 4 0 1 0 0-8a4 4 0 0 0 0 8Zm5.008 1.876Q14.486 18 15 18a4 4 0 1 0-2.236-7.317m-5.062 5.043A4 4 0 0 0 10 23a4 4 0 0 0 1.401-7.748"></svg:path>`,
})
export class GrommetIconsBusinessServiceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCafeteriaIcon],svg[grommet-icons-cafeteria-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 1v7c0 1.657-1.347 3-2.997 3H5.997A3 3 0 0 1 3 8V1m3 6V1m3 6V1M6 11v10.504C6 22.33 6.666 23 7.5 23c.828 0 1.5-.68 1.5-1.496V11m6 7v3.5a1.5 1.5 0 1 0 3-.005V15s3 0 3-3V7c0-3-2-5-6-5z"></svg:path>`,
})
export class GrommetIconsCafeteriaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCalculatorIcon],svg[grommet-icons-calculator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 23H2V1h20zm-6-6h2v2h-2zm-5 0h2v2h-2zm5-5h2v2h-2zm-5 0h2v2h-2zm-5 5h2v2H6zm0-5h2v2H6zm12-3H6V5h12z"></svg:path>`,
})
export class GrommetIconsCalculatorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCalendarIcon],svg[grommet-icons-calendar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 5h20v17H2zm16 0V1M6 5V1m-4 9h20"></svg:path>`,
})
export class GrommetIconsCalendarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCameraIcon],svg[grommet-icons-camera-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 21V7h5l2-4h8l2 4h5v14zm11-3a5 5 0 1 0 0-10a5 5 0 0 0 0 10Z"></svg:path>`,
})
export class GrommetIconsCameraIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCapacityIcon],svg[grommet-icons-capacity-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M1 19h22V5H1zm3-3h3V8H4zm6 0h3V8h-3z"></svg:path>`,
})
export class GrommetIconsCapacityIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCarIcon],svg[grommet-icons-car-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 10.997c0-.55.44-.997 1.002-.997h19.996c.553 0 1.002.453 1.002.997v6.006c0 .55-.44.997-1.002.997H2.002A1.004 1.004 0 0 1 1 17.003zM6 2h12l4 8H2zm6 8.5L15 5M3 18h3v2.99c0 .558-.443 1.01-.999 1.01H3.999A1 1 0 0 1 3 20.99zm15 0h3v2.99c0 .558-.443 1.01-.999 1.01h-1.002A1 1 0 0 1 18 20.99zM5 15a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm14 0a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm-7-2v2m2-2v2m-4-2v2"></svg:path>`,
})
export class GrommetIconsCarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretDownIcon],svg[grommet-icons-caret-down-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 8L12 20L2 8z"></svg:path>`,
})
export class GrommetIconsCaretDownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretDownFillIcon],svg[grommet-icons-caret-down-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M18 9H6l6 6z"></svg:path>`,
})
export class GrommetIconsCaretDownFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretLeftFillIcon],svg[grommet-icons-caret-left-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M15 18V6l-6 6z"></svg:path>`,
})
export class GrommetIconsCaretLeftFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretNextIcon],svg[grommet-icons-caret-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6 2l12 10L6 22z"></svg:path>`,
})
export class GrommetIconsCaretNextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretPreviousIcon],svg[grommet-icons-caret-previous-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 2L6 12l12 10z"></svg:path>`,
})
export class GrommetIconsCaretPreviousIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretRightFillIcon],svg[grommet-icons-caret-right-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M9 6v12l6-6z"></svg:path>`,
})
export class GrommetIconsCaretRightFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretUpIcon],svg[grommet-icons-caret-up-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 16L12 4L2 16z"></svg:path>`,
})
export class GrommetIconsCaretUpIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCaretUpFillIcon],svg[grommet-icons-caret-up-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M6 15h12l-6-6z"></svg:path>`,
})
export class GrommetIconsCaretUpFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCartIcon],svg[grommet-icons-cart-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 5h17l-2 9H7L4 2H0m7 12l1 4h13m-2 5a1 1 0 1 1 0-2a1 1 0 0 1 0 2ZM9 23a1 1 0 1 1 0-2a1 1 0 0 1 0 2Z"></svg:path>`,
})
export class GrommetIconsCartIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCatalogIcon],svg[grommet-icons-catalog-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 6L1 4.5v13.943L12 23l11-4.557V4l-4 2M5 16V2l7 3l7-3v14l-7 3zm6.95-11v14"></svg:path>`,
})
export class GrommetIconsCatalogIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCatalogOptionIcon],svg[grommet-icons-catalog-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5.5 18a2.5 2.5 0 1 0 0 5H22M3 20.5v-17A2.5 2.5 0 0 1 5.5 1H21v17.007H5.492M20.5 18a2.5 2.5 0 1 0 0 5"></svg:path>`,
})
export class GrommetIconsCatalogOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCentosIcon],svg[grommet-icons-centos-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M5.161 6.25L3.249 8.164V3.227h4.935L6.24 5.172l-.147.146l.147.147l4.514 4.516v.738h-.83L5.457 6.251l-.15-.15zm5.594 3.142V3.227H8.772l-2.09 2.091zM5.307 6.693L3.25 8.753v1.966h6.083zm7.964 2.67l4.044-4.045l-2.119-2.12h-1.925zm4.636-4.045l-.15.147l-4.486 4.49v.764h.786l4.477-4.478l.146-.146l.15.146l1.96 1.96V3.2h-5.005l1.973 1.973zm-6.736-2.51v7l.828.828l.856-.856V2.782h1.924L12 0L9.192 2.807h1.98m3.478 7.911h6.141V8.794l-2.11-2.11zm-4.025 1.292l-.872-.872h-6.92V9.17L0 12.002l2.832 2.832V12.81h6.993zm2.647 2.68v6.056h1.964l2.045-2.046zm-7.955 2.63l4.092-4.093h-6.16v2.024zm13.364 0l2.11-2.11v-1.983h-6.2zm2.529-2.53l2.787-2.788l-2.787-2.788v1.925h-6.98l-.863.863l.808.808h7.035zm-2.53 3.118l-.146-.146L14 13.227h-.729v.875l4.452 4.452l.146.147l-.146.15l-1.897 1.896h4.964v-4.945l-1.96 1.96zm-5.825 3.256v-6.89l-.872-.872l-.799.799v6.963h-2.04L11.984 24l2.835-2.836zm-6.762-2.463l.147-.147l4.527-4.528v-.8h-.77l-4.534 4.536l-.146.146l-.147-.146l-1.92-1.922v4.907h4.887L6.24 18.85zm4.674-4.083L6.682 18.7l2.046 2.046h2.04z"></svg:path>`,
})
export class GrommetIconsCentosIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCertificateIcon],svg[grommet-icons-certificate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M15 19H2V1h16v4m0 0a5 5 0 1 1 0 10a5 5 0 0 1 0-10zm-3 9v8l3-2l3 2v-8M5 8h6m-6 3h5m-5 3h2M5 5h2"></svg:path>`,
})
export class GrommetIconsCertificateIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChannelIcon],svg[grommet-icons-channel-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 9h20v13H2zm19-7l-8 7h-2L3 2"></svg:path>`,
})
export class GrommetIconsChannelIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChapterAddIcon],svg[grommet-icons-chapter-add-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 14V5h14v15h-8m8-4h4V1H9v4M5 16v8m4-4H1"></svg:path>`,
})
export class GrommetIconsChapterAddIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChapterNextIcon],svg[grommet-icons-chapter-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 3.5V20l9-6v6l11-8l-11-8v6zM22 2v20z"></svg:path>`,
})
export class GrommetIconsChapterNextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChapterPreviousIcon],svg[grommet-icons-chapter-previous-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M23 3.5V20l-9-6v6L3 12l11-8v6zM2 2v20z"></svg:path>`,
})
export class GrommetIconsChapterPreviousIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChatIcon],svg[grommet-icons-chat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 1H5a4 4 0 0 0-4 4v8a4 4 0 0 0 4 4h12l6 5V5a4 4 0 0 0-4-4Zm0 6H6m13 4h-9"></svg:path>`,
})
export class GrommetIconsChatIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChatOptionIcon],svg[grommet-icons-chat-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 7V1h14v10h-3v5l-5-4M1 7h14v11H9l-5 4v-4H1z"></svg:path>`,
})
export class GrommetIconsChatOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCheckboxIcon],svg[grommet-icons-checkbox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 2h20v20H2z"></svg:path>`,
})
export class GrommetIconsCheckboxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCheckboxSelectedIcon],svg[grommet-icons-checkbox-selected-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 2h20v20H2zm3 11l5 4l9-11"></svg:path>`,
})
export class GrommetIconsCheckboxSelectedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCheckmarkIcon],svg[grommet-icons-checkmark-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2 14l7 6L22 4"></svg:path>`,
})
export class GrommetIconsCheckmarkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsChromeIcon],svg[grommet-icons-chrome-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:path d="M11.973 16.414A4.32 4.32 0 0 1 7.66 12.1a4.32 4.32 0 0 1 4.314-4.315a4.32 4.32 0 0 1 4.315 4.315a4.32 4.32 0 0 1-4.315 4.314"></svg:path><svg:path d="M13.791 17.181a5.51 5.51 0 0 1-5.38-.926a5.5 5.5 0 0 1-1.718-2.505l-.002-.006L1.936 5.51A11.9 11.9 0 0 0 .23 14.35a11.9 11.9 0 0 0 5.08 7.627a11.9 11.9 0 0 0 4.557 1.84z"></svg:path><svg:path d="M22.76 6.707a12.08 12.08 0 0 0-20.185-2.25l4.016 6.956a5.39 5.39 0 0 1 5.274-4.706z"></svg:path><svg:path d="M11.926 24c3.01 0 5.891-1.129 8.11-3.178a11.93 11.93 0 0 0 3.816-7.893a12.05 12.05 0 0 0-.744-5.144h-7.856a5.5 5.5 0 0 1 2.09 4.34a5.53 5.53 0 0 1-1.182 3.381l-5.008 8.47q.388.024.774.024"></svg:path></svg:g>`,
})
export class GrommetIconsChromeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCircleAlertIcon],svg[grommet-icons-circle-alert-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 14V6m0 12v-2m0-14C6.477 2 2 6.477 2 12s4.477 10 10 10s10-4.477 10-10S17.523 2 12 2z"></svg:path>`,
})
export class GrommetIconsCircleAlertIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCircleInformationIcon],svg[grommet-icons-circle-information-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm0-12v8m0-12v2"></svg:path>`,
})
export class GrommetIconsCircleInformationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCirclePlayIcon],svg[grommet-icons-circle-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm-2.5-6.5l6-3.5l-6-3.5zm1-2l2-1.5l-2-1.5z"></svg:path>`,
})
export class GrommetIconsCirclePlayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCircleQuestionIcon],svg[grommet-icons-circle-question-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm0-7v-1c0-1 0-1.5 1-2s2-1 2-2.5c0-1-1-2.5-3-2.5s-3 1.264-3 3m3 6v2"></svg:path>`,
})
export class GrommetIconsCircleQuestionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClearIcon],svg[grommet-icons-clear-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10ZM5 5l14 14"></svg:path>`,
})
export class GrommetIconsClearIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClearOptionIcon],svg[grommet-icons-clear-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 4a2 2 0 1 1 4 0v6h6v4H4v-4h6zM4 14h16v8H4zm12 8v-5.635M8 22v-5.635M12 22v-5.635"></svg:path>`,
})
export class GrommetIconsClearOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCliIcon],svg[grommet-icons-cli-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h22v22H1zm0 4h22M5 1v4m6 11h8M5 10l3 3l-3 3"></svg:path>`,
})
export class GrommetIconsCliIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClipboardIcon],svg[grommet-icons-clipboard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16 3h5v20H3V3h5m0-2h8v5H8z"></svg:path>`,
})
export class GrommetIconsClipboardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClockIcon],svg[grommet-icons-clock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M13 7a1 1 0 1 0-2 0v5a1 1 0 0 0 .4.8l4 3a1 1 0 0 0 1.2-1.6L13 11.5z"></svg:path><svg:path fill="currentColor" fill-rule="evenodd" d="M12 1C5.925 1 1 5.925 1 12s4.925 11 11 11s11-4.925 11-11S18.075 1 12 1M3 12a9 9 0 1 1 18 0a9 9 0 0 1-18 0" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsClockIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloneIcon],svg[grommet-icons-clone-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 23h16V7H7zM17 4V1h-3M1 14v3h3m-3-5V6zM4 1H1v3m5-3h6z"></svg:path>`,
})
export class GrommetIconsCloneIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloseIcon],svg[grommet-icons-close-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m3 3l18 18M3 21L21 3"></svg:path>`,
})
export class GrommetIconsCloseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClosedCaptionIcon],svg[grommet-icons-closed-caption-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 12c0-7 1.5-8 11-8s11 1 11 8s-1.5 8-11 8s-11-1-11-8Zm4.25 2c0 1.5.75 2 2.5 2s2.5-.5 2.5-2h-.271c0 1.25-1 2-2.229 2s-2.229-.75-2.229-2v-4C5.5 8.75 6.5 8 7.75 8s2.25.75 2.229 2h.271c0-1.25-1.021-2-2.5-2s-2.5.75-2.5 2zm8 0c0 1.5.75 2 2.5 2s2.5-.5 2.5-2h-.271c0 1.25-1 2-2.229 2s-2.229-.75-2.229-2v-4c-.021-1.25.979-2 2.229-2s2.25.75 2.229 2h.271c0-1.25-1.021-2-2.5-2s-2.5.75-2.5 2z"></svg:path>`,
})
export class GrommetIconsClosedCaptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudIcon],svg[grommet-icons-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 17v1c0 3-2 4-5 4h-2c-3 0-5-1-5-4v-1A5 5 0 0 1 6 7h6M6 7V6c0-3 2-4 5-4h2c3 0 5 1 5 4v1a5 5 0 0 1 0 10h-6"></svg:path>`,
})
export class GrommetIconsCloudIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudComputerIcon],svg[grommet-icons-cloud-computer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 6V5c0-3 1.5-4 4-4h4c2.5 0 4 1.5 4 4v1c3 0 5 2 5 5s-2 5-5 5M14 6H6c-3 0-5 1.5-5 5s2 5 5 5m2 3h8v-7H8zm4 0v4zm-3 4h6z"></svg:path>`,
})
export class GrommetIconsCloudComputerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudDownloadIcon],svg[grommet-icons-cloud-download-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 17A5 5 0 0 1 6 7h6M6 7V6c0-3 2-4 5-4h2c3 0 5 1 5 4v1a5 5 0 0 1 0 10m-6-5v9m-4-4l4 4l4-4"></svg:path>`,
})
export class GrommetIconsCloudDownloadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudSoftwareIcon],svg[grommet-icons-cloud-software-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 23h8V12H8zm0-7h8m-4-4v4M6 6V5c0-3 1.5-4 4-4h4c2.5 0 4 1.5 4 4v1c3 0 5 2 5 5s-2 5-5 5M14 6H6c-3 0-5 1.5-5 5s2 5 5 5"></svg:path>`,
})
export class GrommetIconsCloudSoftwareIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudUploadIcon],svg[grommet-icons-cloud-upload-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 17A5 5 0 0 1 6 7h6M6 7V6c0-3 2-4 5-4h2c3 0 5 1 5 4v1a5 5 0 0 1 0 10m-6-4v9m-4-6l3.965-4.035L16 16"></svg:path>`,
})
export class GrommetIconsCloudUploadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCloudlinuxIcon],svg[grommet-icons-cloudlinux-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M14.068 5.79q.073.069.142.139q.472.484.881 1.055l.001.001c.266.381.51.792.744 1.234c.046-1.496-.243-2.81-.914-4.036c-.307-.563-.618-1.141-1.14-1.532c-.749-.562-1.596-.944-2.502-1.185a8 8 0 0 0-1.242-.229C6.917.809 3.77 2.365 2.093 5.477c.188.05.268-.111.376-.198c3.53-2.871 8.406-2.579 11.539.451zm-3.528 10.9c-.575-.335-1.099-.74-1.638-1.185c.545 1.559 2.637 3.432 4.421 3.92c-.073-.06-.14-.127-.22-.178c-.908-.585-1.654-1.338-2.29-2.206c-.087-.12-.153-.282-.273-.351M23.593 9.6c.015-.065.018-.123-.068-.137l-.013.138l-.028.327a8.633 8.633 0 0 1-10.866 7.614l-.519-.145c1.151 1.294 2.568 2.07 4.244 2.387c.563.106 1.108.09 1.662-.06c2.074-.567 3.706-1.75 4.812-3.583c1.235-2.048 1.52-4.243.776-6.542M14.47 20.53c-2.821-.515-4.856-2.149-6.25-4.638a7.35 7.35 0 0 0 1.16 4.367c.256.403.498.832.908 1.1c2.679 1.743 5.467 1.921 8.32.483c1.49-.75 2.59-1.927 3.324-3.44c-2.166 1.853-4.644 2.641-7.462 2.128M9.766.5c5.73.51 9.29 5.722 7.69 11.262c-.018.063-.062.127.047.168c1.308-1.283 2.121-2.818 2.357-4.65a2.6 2.6 0 0 0-.04-.935C19.26 3.91 17.88 2.08 15.678.922c-1.922-1.01-3.952-1.2-6.038-.523c-.065-.012-.127-.017-.137.074l.141.013q.06.01.122.016m6.969 11.226l-.002-.038l-.002-.016l-.006.016zm3.816-2.163c-.515 2.819-2.154 4.848-4.747 6.253c1.576.038 2.942-.281 4.202-1.015c.508-.295 1.037-.583 1.382-1.076c2.771-3.958 1.356-9.512-2.99-11.658c1.876 2.169 2.671 4.661 2.153 7.496m-1.094 1.196a.25.25 0 0 0-.155.105a8.4 8.4 0 0 1-2.433 2.46a.5.5 0 0 0-.174.169c-.315.577-.731 1.082-1.121 1.608c1.866-.936 3.163-2.38 3.883-4.342M16.36 5.667c-.117.098-.041.179-.024.247c.235.94.335 1.904.153 2.844c-.196 1.014.162 1.945.242 2.915c.247-.55.329-1.137.388-1.727c.15-1.492-.115-2.912-.76-4.279m-1.836 10.702a.7.7 0 0 0-.323.01c-.58.156-1.176.228-1.772.293c-.036-.017-.077-.03-.086.021c-.003.014.052.037.08.056c1.993.643 3.93.468 5.854-.37c-.053-.036-.073-.061-.096-.063a.4.4 0 0 0-.121.01a8.3 8.3 0 0 1-3.536.043m-5.65-.855l-.034-.02q.013.014.028.026zm-.486-6.56c.037-.014.076-.034.046-.077c-.008-.01-.063.011-.097.018a.4.4 0 0 0 .05.058m-2.89 12.895c.034.056.073.1.143.048l-.087-.106zM10.65 4.51c.967.61 1.825 1.352 2.447 2.307c.347.534.73.595 1.3.4c-1.003-1.293-2.218-2.219-3.747-2.707M8.337 8.895c-1.863.954-3.11 2.448-3.877 4.4c.063-.011.095-.008.112-.023a.4.4 0 0 0 .079-.093a8.3 8.3 0 0 1 2.47-2.53a.7.7 0 0 0 .221-.235c.3-.521.67-.993 1.046-1.46a.4.4 0 0 1-.05-.059M7.6 18.103a.25.25 0 0 0 .036-.184a8.4 8.4 0 0 1-.02-3.46a.5.5 0 0 0 .004-.242c-.186-.63-.248-1.282-.345-1.93c-.658 1.982-.554 3.92.325 5.816M6.09 7.61c1.056-.229 2.117-.224 3.18-.06c.147.024.308.091.441.056c.644-.17 1.3-.255 1.997-.32c-1.488-.718-4.292-.563-5.898.354c.094-.01.19-.01.28-.03m-.538 14.18l-.212-.25a8.633 8.633 0 0 1 2.3-13.068q.233-.133.47-.264c-1.73-.101-3.281.352-4.69 1.313c-.472.323-.847.72-1.132 1.218c-1.066 1.867-1.384 3.857-.87 5.936c.576 2.321 1.926 4.074 4.078 5.174l.056-.058m2.119-2.068c-1.63-2.357-1.906-4.951-1.065-7.779c-1.141 1.09-1.881 2.28-2.254 3.69c-.15.568-.321 1.145-.216 1.738c.839 4.759 5.767 7.686 10.357 6.13c-2.86-.208-5.184-1.408-6.822-3.779M.445 14.447C-.105 12.86-.16 11.25.363 9.664c1-3.033 3.098-4.879 6.225-5.54c.479-.101.954.032 1.419.136a7.36 7.36 0 0 1 3.908 2.267c-2.745-.774-5.34-.49-7.697 1.14c-2.356 1.63-3.551 3.94-3.773 6.78"></svg:path>`,
})
export class GrommetIconsCloudlinuxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsClusterIcon],svg[grommet-icons-cluster-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 9h8V1H8zM1 23h8v-8H1zm14 0h8v-8h-8zM5 15l3-6zm5 4h4zm6-10l3 6z"></svg:path>`,
})
export class GrommetIconsClusterIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCoatCheckIcon],svg[grommet-icons-coat-check-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m12 11l10.155 6.462c.467.297.845.981.845 1.547v1.982A1 1 0 0 1 21.998 22H2.002A1 1 0 0 1 1 20.99v-1.98c0-.558.379-1.251.845-1.548zm3-6a3 3 0 0 0-6 0c0 .932.411 1.802 1.09 2.314C11 8 12 8 12 9.5V11"></svg:path>`,
})
export class GrommetIconsCoatCheckIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCodeIcon],svg[grommet-icons-code-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m9 22l6-20m2 15l5-5l-5-5M7 17l-5-5l5-5"></svg:path>`,
})
export class GrommetIconsCodeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCodeSandboxIcon],svg[grommet-icons-code-sandbox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m12 1.5l-9 5v11l9 5l9-5v-11zm0 21v-11m9-5l-9 5m0 0l-9-5m18 11V12l-4.5 2.5V20zm-18 0V12l4.5 2.5V20zm9-16L7.5 4L12 6.5L16.5 4z"></svg:path>`,
})
export class GrommetIconsCodeSandboxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCodepenIcon],svg[grommet-icons-codepen-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12 22.03c-5.53 0-10.03-4.5-10.03-10.03S6.47 1.97 12 1.97S22.03 6.47 22.03 12S17.53 22.03 12 22.03M12 0C5.372 0 0 5.373 0 12c0 6.628 5.372 12 12 12s12-5.372 12-12c0-6.627-5.372-12-12-12m6.144 13.067L16.55 12l1.595-1.067zm-5.506 4.524v-2.975l2.764-1.849l2.232 1.493zM12 13.509L9.745 12L12 10.492L14.255 12zm-.638 4.082L6.366 14.26l2.232-1.493l2.764 1.85zm-5.506-6.658L7.45 12l-1.595 1.067zm5.506-4.523v2.974l-2.764 1.85L6.366 9.74zm1.276 0l4.996 3.33l-2.232 1.493l-2.764-1.85zm6.776 3.246l-.005-.027l-.011-.054l-.01-.03q-.007-.024-.017-.047l-.035-.074l-.019-.03a.6.6 0 0 0-.08-.1l-.026-.025l-.036-.03l-.029-.022l-.01-.008l-6.782-4.521a.64.64 0 0 0-.708 0l-6.782 4.52l-.01.009l-.03.022l-.035.03q-.014.012-.026.025a.6.6 0 0 0-.099.13l-.021.043l-.014.03q-.01.024-.017.047l-.01.031l-.01.054l-.006.027l-.006.083v4.522q0 .042.006.083l.005.028l.011.053l.01.031q.007.023.017.047l.014.03a1 1 0 0 0 .067.111a.4.4 0 0 0 .053.062l.026.025l.065.052l.01.008l6.782 4.522a.64.64 0 0 0 .708 0l6.782-4.522l.01-.008l.065-.052q.014-.012.026-.025l.032-.034l.021-.028l.027-.039l.019-.029l.021-.042l.014-.031q.01-.023.017-.047l.01-.03l.01-.054l.006-.028l.006-.083V9.739z"></svg:path>`,
})
export class GrommetIconsCodepenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCoffeeIcon],svg[grommet-icons-coffee-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:path stroke="currentColor" stroke-width="2" d="M6.264 20.192c4.096 2.868 8.602-.081 11.47-4.177s4.097-9.338.002-12.206s-8.602.08-11.47 4.176s-4.098 9.339-.002 12.207Z"></svg:path><svg:path fill="currentColor" d="M16.589 5.447c-1.393.246-5.326 2.375-5.408 5.98c-.083 3.604-2.787 6.594-3.77 7.126c1.803.042 5.326-2.375 5.408-5.98c.083-3.604 2.786-6.594 3.77-7.126"></svg:path></svg:g>`,
})
export class GrommetIconsCoffeeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsColumnsIcon],svg[grommet-icons-columns-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 2v20zm-5 0v20zM7 2v20zM2 22h20V2H2z"></svg:path>`,
})
export class GrommetIconsColumnsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCommandIcon],svg[grommet-icons-command-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 9H5.5C3.5 9 2 7.5 2 5.5S3.5 2 5.5 2S9 3.5 9 5.5v13c0 2-1.5 3.5-3.5 3.5S2 20.5 2 18.5S3.5 15 5.5 15h13c2 0 3.5 1.5 3.5 3.5S20.5 22 18.5 22S15 20.5 15 18.5v-13c0-2 1.5-3.5 3.5-3.5S22 3.5 22 5.5S20.5 9 18.5 9z"></svg:path>`,
})
export class GrommetIconsCommandIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCompareIcon],svg[grommet-icons-compare-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M11 7H1v10h6V8m4-3v4l2-2zm0 12l2 2v-4zm2 0h10V7h-6v9"></svg:path>`,
})
export class GrommetIconsCompareIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCompassIcon],svg[grommet-icons-compass-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm-2-12l5-1l-1 5l-5 1zm2 4a2 2 0 1 0 0-4a2 2 0 0 0 0 4Z"></svg:path>`,
})
export class GrommetIconsCompassIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsComplianceIcon],svg[grommet-icons-compliance-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 6h8V1H8zm8-3h5v20H3V3h5m0 11l3 3l6-6"></svg:path>`,
})
export class GrommetIconsComplianceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsConfigureIcon],svg[grommet-icons-configure-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16 15c4.009-.065 7-3.033 7-7c0-3.012-.997-2.015-2-1c-.991.98-3 3-3 3l-4-4s2.02-2.009 3-3c1.015-1.003 1.015-2-1-2c-3.967 0-6.947 2.991-7 7c.042.976 0 3 0 3c-1.885 1.897-4.34 4.353-6 6c-2.932 2.944 1.056 6.932 4 4c1.65-1.662 4.113-4.125 6-6c0 0 2.024-.042 3 0Z"></svg:path>`,
})
export class GrommetIconsConfigureIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsConnectIcon],svg[grommet-icons-connect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 21c-2.5 2.5-5 2-7 0s-2.5-4.5 0-7l3-3l7 7zm4-18c2.5-2.5 5-2 7.001 0s2.499 4.5 0 7l-3 3L11 6zm-3 7l-2.5 2.5zm3 3l-2.5 2.5z"></svg:path>`,
})
export class GrommetIconsConnectIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsConnectivityIcon],svg[grommet-icons-connectivity-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 8v4l3 3l6-6l-4-4H5L2.5 3M17 7l3 3v7m-7-6l3 3m-5-1l3 3m-7 1l3 3h10"></svg:path>`,
})
export class GrommetIconsConnectivityIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsConsoleIcon],svg[grommet-icons-console-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M16 18a1 1 0 1 0 0-2zm-8-2a1 1 0 1 0 0 2zm.707-8.707a1 1 0 1 0-1.414 1.414zM11 11l.707.707a1 1 0 0 0 0-1.414zm-3.707 2.293a1 1 0 1 0 1.414 1.414zM7 4h10V2H7zm13 3v10h2V7zm-3 13H7v2h10zM4 17V7H2v10zm3 3a3 3 0 0 1-3-3H2a5 5 0 0 0 5 5zm13-3a3 3 0 0 1-3 3v2a5 5 0 0 0 5-5zM17 4a3 3 0 0 1 3 3h2a5 5 0 0 0-5-5zM7 2a5 5 0 0 0-5 5h2a3 3 0 0 1 3-3zm9 14H8v2h8zM7.293 8.707l3 3l1.414-1.414l-3-3zm3 1.586l-3 3l1.414 1.414l3-3z"></svg:path>`,
})
export class GrommetIconsConsoleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsContactIcon],svg[grommet-icons-contact-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 2h21v16h-8l-8 4v-4H1zm5 8h1v1H6zm5 0h1v1h-1zm5 0h1v1h-1z"></svg:path>`,
})
export class GrommetIconsContactIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsContactInfoIcon],svg[grommet-icons-contact-info-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 12a3 3 0 1 1 0-6a3 3 0 0 1 0 6Zm4 6v-2c0-2.25-1.787-4-4.036-4h.054C2.768 12 1 13.75 1 16v2M12 7h12M12 17h10m-10-5h7"></svg:path>`,
})
export class GrommetIconsContactInfoIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsContractIcon],svg[grommet-icons-contract-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 14h8v8m-9 1l9-9M23 1l-9 9m8 0h-8V2"></svg:path>`,
})
export class GrommetIconsContractIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCopyIcon],svg[grommet-icons-copy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 15h8zm0-4h10zm0-4h4zm7-6v6h6M6 5H2v18h16v-4m4 0H6V1h11l5 5z"></svg:path>`,
})
export class GrommetIconsCopyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCpuIcon],svg[grommet-icons-cpu-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 18h3m-3-4h3m-3-4h3M1 6h3m16 12h3m-3-4h3m-3-4h3m-3-4h3M6 1v3m4-3v3m4-3v3m4-3v3M6 20v3m4-3v3m4-3v3m4-3v3M5 20h14a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1zm8-13h4v4h-4z"></svg:path>`,
})
export class GrommetIconsCpuIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCreativeCommonsIcon],svg[grommet-icons-creative-commons-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M11.984 0c-3.292 0-6.19 1.218-8.428 3.49C1.25 5.827 0 8.856 0 12.016c0 3.194 1.218 6.157 3.523 8.461c2.304 2.305 5.3 3.556 8.46 3.556s6.223-1.251 8.593-3.588C22.815 18.239 24 15.309 24 12.016c0-3.259-1.185-6.222-3.457-8.493C18.24 1.218 15.276 0 11.983 0m.032 2.173c2.7 0 5.104 1.02 6.98 2.897c1.843 1.843 2.83 4.28 2.83 6.946c0 2.7-.954 5.07-2.797 6.881c-1.943 1.91-4.445 2.93-7.013 2.93c-2.6 0-5.037-1.02-6.913-2.897s-2.93-4.346-2.93-6.914c0-2.6 1.053-5.07 2.93-6.98c1.843-1.875 4.214-2.863 6.913-2.863m-.154 7.846c-.68-1.237-1.837-1.73-3.181-1.73c-1.957 0-3.514 1.384-3.514 3.727c0 2.382 1.464 3.727 3.58 3.727c1.358 0 2.516-.745 3.154-1.877l-1.49-.758c-.333.798-.839 1.038-1.478 1.038c-1.105 0-1.61-.919-1.61-2.13s.426-2.13 1.61-2.13c.32 0 .959.173 1.331.972zm6.932 0c-.68-1.237-1.837-1.73-3.181-1.73c-1.957 0-3.514 1.384-3.514 3.727c0 2.382 1.464 3.727 3.58 3.727c1.358 0 2.516-.745 3.154-1.877l-1.49-.758c-.333.798-.839 1.038-1.477 1.038c-1.105 0-1.611-.919-1.611-2.13s.426-2.13 1.61-2.13c.32 0 .959.173 1.331.972z"></svg:path>`,
})
export class GrommetIconsCreativeCommonsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCreditCardIcon],svg[grommet-icons-credit-card-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 5c0-.552.44-1 1.002-1h19.996A1 1 0 0 1 23 5v14c0 .552-.44 1-1.002 1H2.002A1 1 0 0 1 1 19zm0 3h22v2H1zm4 7h2v.5H5zm5 0h6v.5h-6z"></svg:path>`,
})
export class GrommetIconsCreditCardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCss3Icon],svg[grommet-icons-css3-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 3h14l-3 15l-8 4l-7-4l1-4m1-5h14"></svg:path>`,
})
export class GrommetIconsCss3Icon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCubeIcon],svg[grommet-icons-cube-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m12 2l10 5v10l-10 5l-10-5V7zM2 7l10 5l10-5m-10 5v10z"></svg:path>`,
})
export class GrommetIconsCubeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCubesIcon],svg[grommet-icons-cubes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6.5 10.5l5.5 3l5.5-3v-6l-5.5-3l-5.5 3zm0-6l5.5 3l5.5-3m-5.5 3v6zm-11 12l5.5 3l5.5-3v-6l-5.5-3l-5.5 3zm0-6l5.5 3l5.5-3m-5.5 3v6zm5.5 3l5.5 3l5.5-3v-6l-5.5-3l-5.5 3zm0-6l5.5 3l5.5-3m-5.5 3v6z"></svg:path>`,
})
export class GrommetIconsCubesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCurrencyIcon],svg[grommet-icons-currency-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 5h22v14H1zm1 4a3 3 0 0 0 3-3m-3 9a3 3 0 0 1 3 3m17-9a3 3 0 0 1-3-3m3 9a3 3 0 0 0-3 3m-7-2c1.657 0 3-1.79 3-4s-1.343-4-3-4s-3 1.79-3 4s1.343 4 3 4Z"></svg:path>`,
})
export class GrommetIconsCurrencyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCursorIcon],svg[grommet-icons-cursor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6 3l12 11l-5 1l3 5.5l-3 1.5l-3-6l-4 3z"></svg:path>`,
})
export class GrommetIconsCursorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCutIcon],svg[grommet-icons-cut-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M23 4L8 16zm0 16L8 8zM5 9a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0 12a3 3 0 1 0 0-6a3 3 0 0 0 0 6Z"></svg:path>`,
})
export class GrommetIconsCutIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsCycleIcon],svg[grommet-icons-cycle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M13 20c6-1 8-6 8-10m-7 6l-2 4l4 3M0 9l4-3l3 4m2 10c-6-3-7-8-5-14m16 1C16 1 10 1 6 4.006M20 2v5h-5"></svg:path>`,
})
export class GrommetIconsCycleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDashboardIcon],svg[grommet-icons-dashboard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm3-6a3 3 0 0 0-6 0M5 5l2 2m5 0v6m0-10v2m7 7h2M3 12h2m12-5l2-2M3 17h18"></svg:path>`,
})
export class GrommetIconsDashboardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDatabaseIcon],svg[grommet-icons-database-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 2h22v7H1zm3 10h1v1H4zm0-7h1v1H4zm0 14h1v1H4zm-3-3h22v7H1zm0-7h22v7H1z"></svg:path>`,
})
export class GrommetIconsDatabaseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDebianIcon],svg[grommet-icons-debian-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M2.656 5.167c.03.167.383-.233.107.39c-.532.371-.064.162-.107-.39m.003.032l-.005-.06q0 .035.005.06m-.446 1.76c-.344.44-.159.535-.195.834c.125-.382.147-.612.195-.833M13.04 0c-.372.031-.742.05-1.108.097l.161.022c.27-.099.661-.054.947-.119m-.611.467l.01-.04l-.15.066zM12.015.31c.212.038.459.067.424.118c.232-.051.285-.098-.424-.118M9.8 13.202c-.138-.153-.22-.338-.312-.521c.088.323.267.6.434.881zm4.317-.237q.216-.17.39-.336a3 3 0 0 1-.984.05c-.398.007.075.206.594.286m1.54-.817c.237-.327.41-.685.47-1.055c-.053.264-.196.492-.33.732c-.744.468-.07-.278-.001-.561c-.8 1.006-.11.603-.139.884m.788-2.05c.048-.716-.141-.49-.205-.217c.075.039.133.505.205.217m-4.558 5.147a4.7 4.7 0 0 1-1.527-1.168c.228.335.475.66.794.916c-.54-.183-1.26-1.308-1.47-1.354c.93 1.666 3.773 2.92 5.262 2.298c-.69.026-1.565.014-2.339-.272c-.294-.151-.685-.45-.697-.553c-.015.038-.038.076-.023.133m.04-.157a.06.06 0 0 0-.016.05zm-2.8-1.583c.26.355.468.74.802 1.017c-.24-.469-.419-.663-.747-1.296zM20.46 8.576c.006-.365.101-.191.138-.281c-.071-.041-.26-.321-.374-.858c.083-.126.222.327.335.345c-.073-.426-.198-.752-.203-1.08c-.33-.689-.117.092-.384-.295c-.351-1.095.29-.254.334-.752c.532.771.836 1.965.975 2.46a10 10 0 0 0-.488-1.753c.162.068-.26-1.241.21-.374c-.502-1.848-2.15-3.575-3.665-4.386c.185.17.42.383.335.416c-.753-.448-.621-.483-.729-.673c-.614-.25-.654.02-1.06 0c-1.159-.613-1.382-.548-2.447-.933l.049.227c-.767-.256-.894.097-1.722 0c-.05-.039.265-.142.525-.18c-.741.098-.706-.146-1.432.027c.179-.125.368-.208.559-.315c-.605.037-1.444.352-1.185.065c-.986.44-2.737 1.058-3.72 1.98l-.03-.207c-.451.54-1.965 1.615-2.085 2.315l-.12.028c-.235.397-.387.846-.573 1.255c-.306.522-.449.2-.405.282c-.603 1.223-.902 2.25-1.161 3.092c.184.276.004 1.659.074 2.766c-.303 5.467 3.837 10.775 8.362 12c.663.238 1.65.229 2.488.253c-.99-.283-1.118-.15-2.082-.486c-.695-.328-.848-.702-1.34-1.13l.195.345c-.966-.342-.562-.423-1.348-.672l.208-.272c-.313-.023-.83-.528-.97-.807l-.343.014c-.412-.508-.631-.874-.615-1.158l-.11.197c-.126-.215-1.515-1.904-.795-1.511c-.134-.122-.312-.2-.505-.55l.147-.167c-.346-.447-.638-1.018-.616-1.209c.185.25.313.297.44.34c-.875-2.173-.924-.12-1.588-2.212l.14-.012c-.107-.162-.172-.337-.259-.51l.061-.609c-.63-.729-.176-3.1-.085-4.4c.063-.528.526-1.09.879-1.973L4.26 7.18c.41-.716 2.343-2.875 3.238-2.764c.433-.544-.086-.002-.171-.139c.952-.985 1.251-.696 1.894-.873c.693-.412-.595.16-.266-.157c1.198-.306.849-.696 2.412-.851c.165.094-.383.145-.52.266c.998-.488 3.16-.377 4.562.271c1.63.762 3.459 3.011 3.53 5.128l.083.022c-.042.842.129 1.815-.166 2.709l.2-.424c.024.644-.188.955-.378 1.508l-.344.172c-.281.546.027.346-.174.78c-.44.391-1.333 1.222-1.619 1.298c-.209-.004.142-.246.188-.34c-.588.403-.472.605-1.371.85l-.026-.058c-2.218 1.043-5.297-1.024-5.256-3.844c-.024.179-.068.134-.117.206c-.114-1.45.67-2.908 1.993-3.503c1.294-.64 2.812-.378 3.739.486c-.51-.667-1.523-1.374-2.724-1.308c-1.177.019-2.278.767-2.645 1.578c-.603.38-.673 1.463-.935 1.662c-.354 2.597.664 3.72 2.387 5.04c.137.092.155.145.144.192l.001-.002c2.032.76 4.131.575 5.89-.834c.447-.348.935-.94 1.077-.95c-.213.32.036.155-.128.437c.446-.719-.193-.293.461-1.241l.242.332c-.09-.596.741-1.32.657-2.264c.19-.289.212.31.01.974c.28-.736.074-.855.146-1.462c.078.204.18.42.233.636c-.183-.711.187-1.198.279-1.612c-.09-.04-.282.315-.326-.526"></svg:path>`,
})
export class GrommetIconsDebianIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDeliverIcon],svg[grommet-icons-deliver-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 18H1V3h13v14m0 1H9m-3 3a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm11 0a3 3 0 1 0 0-6a3 3 0 0 0 0 6ZM14 8h5l4 5v5h-3"></svg:path>`,
})
export class GrommetIconsDeliverIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDeployIcon],svg[grommet-icons-deploy-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M23 1s-6.528-.458-9 2c-.023.037-4 4-4 4L5 8l-3 2l8 4l4 8l2-3l1-5s3.963-3.977 4-4c2.458-2.472 2-9 2-9Zm-6 7a1 1 0 1 1 0-2a1 1 0 0 1 0 2ZM7 17c-1-1-3-1-4 0s-1 5-1 5s4 0 5-1s1-3 0-4Z"></svg:path>`,
})
export class GrommetIconsDeployIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDescendIcon],svg[grommet-icons-descend-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2 16l6 6l6-6M11 3h11M11 7h8m-8 4h5M8 22V2"></svg:path>`,
})
export class GrommetIconsDescendIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDescendingIcon],svg[grommet-icons-descending-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m11.92 16.714l-.354.353l-.354-.353L7 12.502l.707-.708l3.359 3.359V7h1v8.153l3.358-3.359l.707.708z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsDescendingIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDesktopIcon],svg[grommet-icons-desktop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h22v18H1zm4 22h14zm5-4v4zm4 0v4z"></svg:path>`,
})
export class GrommetIconsDesktopIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDetachIcon],svg[grommet-icons-detach-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m4 4l16 16m2-8l-5.28 5.28M15 19l-2 2c-6 6-15-3-9-9l2-2m2-2l5-5c4-4 10 2 6 6l-5 5m-2 2l-2 2c-2 2-5-1-3-3l2-2m2-2l5-5"></svg:path>`,
})
export class GrommetIconsDetachIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDeviceIcon],svg[grommet-icons-device-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2 7h20v14h-6l-4-4l-4 4H2zm4-5l5 5h2l5-5"></svg:path>`,
})
export class GrommetIconsDeviceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDiamondIcon],svg[grommet-icons-diamond-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 3h12l4 6l-10 12L2 9zM2 9h20M11 3L7 9l5 11m1-17l4 6l-5 11"></svg:path>`,
})
export class GrommetIconsDiamondIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDirectionsIcon],svg[grommet-icons-directions-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m17 11l5-5l-5-5m5 5h-4a6 6 0 0 0-6 6v12M7 6l-5 5l5 5m-5-5h4a6 6 0 0 1 6 6v7"></svg:path>`,
})
export class GrommetIconsDirectionsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDisabledOutlineIcon],svg[grommet-icons-disabled-outline-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 12H6M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2z"></svg:path>`,
})
export class GrommetIconsDisabledOutlineIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDiscIcon],svg[grommet-icons-disc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 21a9 9 0 1 0 0-18a9 9 0 0 0 0 18Zm0-7a2 2 0 1 0 0-4a2 2 0 0 0 0 4Z"></svg:path>`,
})
export class GrommetIconsDiscIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDislikeIcon],svg[grommet-icons-dislike-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M23 1H4Q1 1 1 4v10h7v6q0 3 3 3h2s.016-6 .016-7.326S14 13 16 13h7zm-5 0v12"></svg:path>`,
})
export class GrommetIconsDislikeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDislikeFillIcon],svg[grommet-icons-dislike-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M13 24h.997l.003-.997L13 23l1 .003v-.261l.002-.698l.006-2.207c.004-1.665.008-3.498.008-4.163c0-.42.155-.83.454-1.132c.287-.291.767-.542 1.53-.542h6.999L23 13v1h1V0h-1v1v-1H4C2.846 0 1.797.29 1.043 1.043C.289 1.797 0 2.846 0 4v9.999L1 14H0v1h7v5c0 1.154.29 2.203 1.043 2.957C8.797 23.711 9.846 24 11 24zm6-12V2h3v10z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsDislikeFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDockerIcon],svg[grommet-icons-docker-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M6.942 14.9q.085 0 .158.03a.179.179 0 1 0 .246.24a.438.438 0 1 1-.404-.27m0 1.185a.749.749 0 1 1 .002-1.497a.749.749 0 0 1-.002 1.497m13.444-4.901c-2.124 5.628-6.92 8.135-12.576 8.135c-2.672 0-4.803-.92-6.167-2.452l.01-.006c.393.02.745.026 1.101.026c.327 0 .646-.003.941-.02l.084-.006s.091-.006.046-.007a8.6 8.6 0 0 0 1.877-.306q.149-.042.286-.09a.197.197 0 0 0-.128-.371c-.69.239-1.6.37-2.715.395a22 22 0 0 1-1.86-.045a6 6 0 0 1-.386-.58l-.187-.34C.15 14.411-.096 13.12.034 11.716h16.363c1.344 0 2.656-.502 3.28-1.055c-1.117-.908-1.006-3.064-.295-3.886c.618.496 1.613 1.54 1.442 2.871c.777-.39 2.127-.583 3.176.022c-.659 1.286-2.107 1.67-3.614 1.516m-18.13.135h2.212V9.106H2.255zm2.552 0h2.213V9.106H4.808zm0-2.553h2.213V6.553H4.808zm2.553 2.553h2.213V9.106H7.361zm0-2.553h2.213V6.553H7.361zm2.553 2.553h2.213V9.106H9.914zm0-2.553h2.213V6.553H9.914zm0-2.553h2.213V4H9.914zm2.553 5.106h2.213V9.106h-2.213z"></svg:path>`,
})
export class GrommetIconsDockerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentIcon],svg[grommet-icons-document-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 1v7h7m0 15H3V1h12l3 3l3 3z"></svg:path>`,
})
export class GrommetIconsDocumentIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentCloudIcon],svg[grommet-icons-document-cloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-4M18 1v5h5m-12 7H6.002A3.003 3.003 0 0 0 3 16c0 1.657 1.343 3 2.99 3H7v1.01A2.993 2.993 0 0 0 10.002 23h1.996A3 3 0 0 0 15 20.01V19m-4 0h4.998A3.003 3.003 0 0 0 19 16c0-1.657-1.343-3-2.99-3H15v-1.01A2.993 2.993 0 0 0 11.998 9h-1.996A3 3 0 0 0 7 11.99V13"></svg:path>`,
})
export class GrommetIconsDocumentCloudIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentConfigIcon],svg[grommet-icons-document-config-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23h-7m2-22v5h5M9 14v-3m0 9a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0 3v-3m3-3h3M3 17h3m-1-4l2 2m4 4l2 2m0-8l-2 2m-4 4l-2 2"></svg:path>`,
})
export class GrommetIconsDocumentConfigIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentCsvIcon],svg[grommet-icons-document-csv-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M7 13H5c-1 0-2 .5-2 1.5v3c0 1 1 1.5 2 1.5h2m6.25-6h-2.5c-1.5 0-2 .5-2 1.5s.5 1.5 2 1.5s2 .5 2 1.5s-.5 1.5-2 1.5h-2.5m12.25-7v.5C20.5 13 18 19 18 19h-.5S15 13 15 12.5V12"></svg:path>`,
})
export class GrommetIconsDocumentCsvIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentDownloadIcon],svg[grommet-icons-document-download-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 1H17.5L21 4.5V23H3zM16 1v5h5m-9 3v9m-4-3l4 4l4-4"></svg:path>`,
})
export class GrommetIconsDocumentDownloadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentExcelIcon],svg[grommet-icons-document-excel-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M9.25 12l-2 3.25l-2-3.25H5l2.25 3.5l-2.5 3.5H5l2.25-3.25L9.5 19h.25l-2.5-3.5L9.5 12z"></svg:path>`,
})
export class GrommetIconsDocumentExcelIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentImageIcon],svg[grommet-icons-document-image-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-3M18 1v5h5M3 11h13v12H3zm4 5a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm-2 7l2-3l2 2l4-6l3 4"></svg:path>`,
})
export class GrommetIconsDocumentImageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentLockedIcon],svg[grommet-icons-document-locked-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-6m1-22v5h5M4 15h10v8H4zm2 0v-2a3 3 0 0 1 6 0v2m-4 4h2"></svg:path>`,
})
export class GrommetIconsDocumentLockedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentMissingIcon],svg[grommet-icons-document-missing-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 1H17.5L21 4.5V23H3zM16 1v5h5M9 12l6 6m0-6l-6 6"></svg:path>`,
})
export class GrommetIconsDocumentMissingIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentNotesIcon],svg[grommet-icons-document-notes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M9.75 12v6.5H9.5l-5-6.5H4v7h.25v-6.5h.25l5 6.5h.5v-7z"></svg:path>`,
})
export class GrommetIconsDocumentNotesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentOutlookIcon],svg[grommet-icons-document-outlook-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 9V1H17.5L21 4.5V23H2M16 1v5h5M7.75 15.75C7.75 13.5 6.5 12 5 12s-2.75 1.5-2.75 3.75S3.5 19.5 5 19.5s2.75-1.5 2.75-3.75ZM5 12c2.425 0 3 2.5 3 3.75s-.5 3.75-3 3.75s-3-2.5-3-3.75S2.559 12 5 12Z"></svg:path>`,
})
export class GrommetIconsDocumentOutlookIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentPdfIcon],svg[grommet-icons-document-pdf-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M3 12h1.5c2 0 2.25 1.25 2.25 2s-.25 2-2.25 2H3.25v2H3zm6.5 6v-6h1.705c1.137 0 2.295.5 2.295 3s-1.158 3-2.295 3zm7 1v-7h4m-4 3.5h3"></svg:path>`,
})
export class GrommetIconsDocumentPdfIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentPerformanceIcon],svg[grommet-icons-document-performance-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M3 19l5-5l4 4l6.5-6.5M19 17v-6h-6"></svg:path>`,
})
export class GrommetIconsDocumentPerformanceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentPptIcon],svg[grommet-icons-document-ppt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M4 12h1.5c2 0 3.5.5 3.5 2.25S7.5 16.5 5.5 16.5H4.25V19H4z"></svg:path>`,
})
export class GrommetIconsDocumentPptIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentRtfIcon],svg[grommet-icons-document-rtf-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5m-2.5 6h-4v7m3-3.5h-3m-8-3.5h6m-3 0v7M3 19v-7h1.5C5 12 7 12 7 14s-2 2-2.5 2H3m2.25 0l2.25 3"></svg:path>`,
})
export class GrommetIconsDocumentRtfIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentSoundIcon],svg[grommet-icons-document-sound-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 9V1H17.5L21 4.5V23h-3M16 1v5h5M1 14.01v4h3l4 3V11l-4 3.01zM11 18a2 2 0 1 0 0-4m0 8a6 6 0 1 0 0-12"></svg:path>`,
})
export class GrommetIconsDocumentSoundIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentStoreIcon],svg[grommet-icons-document-store-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-6m1-22v5h5M3 12s1-2 6-2s6 2 6 2v9s-1 2-6 2s-6-2-6-2zm0 5s2 2 6 2s6-2 6-2M3 13s2 2 6 2s6-2 6-2"></svg:path>`,
})
export class GrommetIconsDocumentStoreIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentTestIcon],svg[grommet-icons-document-test-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 6V1H19.5L23 4.5V23h-3M18 1v5h5M6 9h8M8 9v4.5l-5 8V23h14v-1.581L12 13.5V9m-6.5 8.5s2 1.5 4.5 0s4.5 0 4.5 0"></svg:path>`,
})
export class GrommetIconsDocumentTestIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentTextIcon],svg[grommet-icons-document-text-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 16h10zm0-4h12zm0-4h5zm8-7v7h7M3 23V1h12l6 6v16z"></svg:path>`,
})
export class GrommetIconsDocumentTextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentThreatIcon],svg[grommet-icons-document-threat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-6m1-22v5h5M9 23a5 5 0 1 0 0-10a5 5 0 0 0 0 10Zm0-12V9c0-1 0-2 2-2s2 1 2 2s0 2 2 2h2m-9 0h2v2H8z"></svg:path>`,
})
export class GrommetIconsDocumentThreatIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentTimeIcon],svg[grommet-icons-document-time-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23h-6m1-22v5h5M10 23a7 7 0 1 0 0-14a7 7 0 0 0 0 14Zm0-11v4l3 3"></svg:path>`,
})
export class GrommetIconsDocumentTimeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentTransferIcon],svg[grommet-icons-document-transfer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M8 12l-4 4l4 4m-4-4h11"></svg:path>`,
})
export class GrommetIconsDocumentTransferIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentTxtIcon],svg[grommet-icons-document-txt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M2 12h5m-2.5 0v7M16 12h5m-2.5 0v7m-4-7.5l-6 7m0-7l6 7"></svg:path>`,
})
export class GrommetIconsDocumentTxtIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentUpdateIcon],svg[grommet-icons-document-update-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 7V1H17.5L21 4.5V23h-6m1-22v5h5M8 23A7 7 0 1 0 8 9a7 7 0 0 0 0 14Zm-3.5-6.5L8 13l3.5 3.5m-3.5-3V20"></svg:path>`,
})
export class GrommetIconsDocumentUpdateIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentUploadIcon],svg[grommet-icons-document-upload-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 1H17.5L21 4.5V23H3zM16 1v5h5m-9 14v-9m-4 3l4-4l4 4"></svg:path>`,
})
export class GrommetIconsDocumentUploadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentUserIcon],svg[grommet-icons-document-user-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23h-7m2-22v5h5M8 11a3 3 0 1 0 0 6a3 3 0 0 0 0-6ZM3 23v-1c0-4 3-5 5-5s5 1 5 5v1z"></svg:path>`,
})
export class GrommetIconsDocumentUserIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentVerifiedIcon],svg[grommet-icons-document-verified-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2.998 1H17.5L21 4.5V23H3zM16 1v5h5M7.5 15l3 3l6-6"></svg:path>`,
})
export class GrommetIconsDocumentVerifiedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentVideoIcon],svg[grommet-icons-document-video-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 7V1H19.5L23 4.5V23H4M18 1v5h5M3 10h9v9H3zm9 3l5-2.5v8L12 16z"></svg:path>`,
})
export class GrommetIconsDocumentVideoIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentWindowsIcon],svg[grommet-icons-document-windows-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5m-8.75 5.5l-6 7m0-7l6 7M20.5 12h-4v6h4m-1-3h-3M7 12H3v6h4m-1-3H3"></svg:path>`,
})
export class GrommetIconsDocumentWindowsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentWordIcon],svg[grommet-icons-document-word-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5m-9 6l-1.5 6.75h-.25L9.5 12H9l-2.75 6.75H6L4.5 12H4l2 7h.5L9 12.5h.5L12 19h.5l2-7z"></svg:path>`,
})
export class GrommetIconsDocumentWordIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDocumentZipIcon],svg[grommet-icons-document-zip-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.998 9V1H19.5L23 4.5V23H4M18 1v5h5M2 13h5v1l-4 4v1h5m3-7v8zm4 1v7zm5 2a2 2 0 0 0-2-2h-3v4h3a2 2 0 0 0 2-2Z"></svg:path>`,
})
export class GrommetIconsDocumentZipIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDomainIcon],svg[grommet-icons-domain-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M13 3v4zM9 3v4zM5 3v4zM1 7h22zm0 14h22V3H1z"></svg:path>`,
})
export class GrommetIconsDomainIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDosIcon],svg[grommet-icons-dos-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m0 4.546l4.934-.124c.958-.02 1.844.082 2.74.392v.175C6.9 5.648 6.345 6.43 5.84 7.306l-2.997.021l.02 8.498l1.772.123l1.813-.02c1.082-.011 2.08-1.741 2.174-2.679l.267-2.955c.114-1.298.865-2.297 2.03-2.833c.73 1.226 1.05 2.585 1.05 4.017c0 4.501-2.791 7.344-7.19 7.18L0 18.471zm20.704 4.172c-.237-1.37-1.38-2.05-2.75-2.05c-.938 0-2.473.411-2.524 1.586c-.01.062 0 .113.02.165l.557 1.596c.113.33.134.69.134 1.04c0 .402-.052.804-.103 1.206c-1.978-.556-3.369-1.514-3.369-3.74c0-2.997 2.421-4.398 5.181-4.398c3.07 0 5.47 1.432 5.666 4.595zm-8.24 5.366l2.75.02c.288 1.762 1.452 2.246 3.182 2.246c1.082 0 2.699-.257 2.699-1.638c0-.72-.567-1.092-1.143-1.41l.144-2.75c2.194.596 3.904 1.42 3.904 3.995c0 3.05-2.905 4.337-5.583 4.337c-1.916 0-4.398-.556-5.418-2.369c-.32-.556-.618-1.277-.618-1.926a1 1 0 0 1 .02-.155zm7.24-2.627c0 1.576-.422 3.049-1.143 4.43c-.185.01-.37.03-.556.03c-.597 0-2.05-.401-2.05-1.133c0-.123.02-.247.062-.36l.546-1.823c.113-.36.072-.794.072-1.154c0-1.411-.639-2.297-.69-3.265c-.03-.68 1.257-1 1.71-1c.309 0 .618 0 .927.031a8.5 8.5 0 0 1 1.123 4.244M10.61 17.03l.648-.927l1.082.02c.515 1.123 1.36 1.885 2.39 2.545a7.2 7.2 0 0 1-2.277.37c-1.143 0-2.215-.257-3.275-.659zm1.926-10.321a8 8 0 0 0-.495-.02c-1.74 0-3.327 1.523-3.533 3.233l-.453 3.719c-.062.525-.515 1.06-.855 1.442c-.288.309-.628.37-1.05.37h-.052a9.6 9.6 0 0 1-.814-3.862C5.284 7.358 7.654 4 12.062 4c.865 0 1.689.165 2.502.391c-.864.639-1.544 1.35-2.028 2.318"></svg:path>`,
})
export class GrommetIconsDosIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDownIcon],svg[grommet-icons-down-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2 8.35l10.173 9.823L21.997 8"></svg:path>`,
})
export class GrommetIconsDownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDownloadIcon],svg[grommet-icons-download-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 17v6h22v-6M12 2v17m-7-7l7 7l7-7"></svg:path>`,
})
export class GrommetIconsDownloadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDownloadOptionIcon],svg[grommet-icons-download-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12 24C5.373 24 0 18.627 0 12S5.373 0 12 0s12 5.373 12 12s-5.373 12-12 12M2 12c0 5.523 4.477 10 10 10s10-4.477 10-10S17.523 2 12 2S2 6.477 2 12m14.293-.707L13 14.586V6h-2v8.586l-3.293-3.293l-1.414 1.414l5 5l.707.707l.707-.707l5-5z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsDownloadOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDragIcon],svg[grommet-icons-drag-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M15 5h2V3h-2zM7 5h2V3H7zm8 8h2v-2h-2zm-8 0h2v-2H7zm8 8h2v-2h-2zm-8 0h2v-2H7z"></svg:path>`,
})
export class GrommetIconsDragIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDrawerIcon],svg[grommet-icons-drawer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 14L6 2h12l5 12l-2 8H3zm0 0h22"></svg:path>`,
})
export class GrommetIconsDrawerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDribbbleIcon],svg[grommet-icons-dribbble-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M11.432 8.635c-1.77-3.15-3.666-5.716-3.803-5.904A10.26 10.26 0 0 0 1.97 9.887c.27.004 4.54.056 9.46-1.252m1.278 3.443l.404-.122a34 34 0 0 0-.832-1.741c-5.278 1.58-10.342 1.464-10.521 1.46q-.006.16-.008.325a10.2 10.2 0 0 0 2.63 6.852l-.007-.01s2.804-4.976 8.334-6.764M5.701 20.08l.003-.005c-.076-.058-.157-.115-.233-.176c.137.11.23.181.23.181M9.62 2.076c-.036.01-.063.02-.1.03a.5.5 0 0 1 .1-.03m9.15 2.234A10.2 10.2 0 0 0 12 1.751c-.833 0-1.64.103-2.415.289c.157.206 2.08 2.762 3.83 5.978c3.865-1.447 5.327-3.666 5.354-3.708M12 24C5.373 24 0 18.627 0 12S5.373 0 12 0s12 5.373 12 12s-5.373 12-12 12m1.744-10.322c-6.015 2.096-8.001 6.31-8.04 6.396A10.2 10.2 0 0 0 12 22.247c1.42 0 2.772-.29 4.002-.811c-.152-.899-.747-4.038-2.19-7.783c-.024.01-.046.015-.068.025m.46-4.132a30 30 0 0 1 .901 2.016c3.54-.446 7.024.31 7.14.335a10.2 10.2 0 0 0-2.332-6.406c-.02.029-1.663 2.405-5.709 4.055m1.528 3.634c1.347 3.698 1.89 6.708 1.994 7.32a10.24 10.24 0 0 0 4.39-6.874c-.203-.066-3.07-.977-6.384-.446"></svg:path>`,
})
export class GrommetIconsDribbbleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDriveCageIcon],svg[grommet-icons-drive-cage-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 23V2h22v21M1 8h22zm0 6h22zm0 6h22zM4 5h12zm14 0h2zm0 6h2zm0 6h2zM4 11h12zm0 6h12z"></svg:path>`,
})
export class GrommetIconsDriveCageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDropboxIcon],svg[grommet-icons-dropbox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M7.06 1L0 5.61l4.882 3.908L12 5.124zM0 13.428l7.06 4.61L12 13.914L4.882 9.52zm12 .486l4.94 4.124l7.06-4.61l-4.882-3.91zM24 5.61L16.94 1L12 5.124l7.118 4.395zM12.014 14.8L7.06 18.913l-2.12-1.385v1.552l7.074 4.243l7.075-4.243v-1.552l-2.12 1.385z"></svg:path>`,
})
export class GrommetIconsDropboxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDuplicateIcon],svg[grommet-icons-duplicate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4.5 17H1V1h16v3.5M7 7h16v16H7zm8 4v8zm-4 4h8z"></svg:path>`,
})
export class GrommetIconsDuplicateIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsDxcIcon],svg[grommet-icons-dxc-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m12 14l4 7H8zm0-4L8 3h8zM2 18H0V6h2a6 6 0 1 1 0 12m20 0a6 6 0 1 1 0-12h2v12z"></svg:path>`,
})
export class GrommetIconsDxcIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEdgeIcon],svg[grommet-icons-edge-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsEdge3)"><svg:path fill="url(#grommetIconsEdge0)" d="M21.666 17.873c-.32.165-.65.308-.991.44a9.6 9.6 0 0 1-3.369.606c-4.437 0-8.3-3.05-8.3-6.97c0-1.102.638-2.06 1.54-2.556c-4.018.166-5.042 4.35-5.042 6.795c0 6.927 6.386 7.632 7.762 7.632c.737 0 1.86-.22 2.532-.43a.5.5 0 0 0 .12-.044a12.07 12.07 0 0 0 6.243-4.956c.21-.319-.154-.704-.495-.517"></svg:path><svg:path fill="url(#grommetIconsEdge1)" d="M9.908 22.641a7.4 7.4 0 0 1-2.136-2.004a7.62 7.62 0 0 1-1.42-4.449a7.62 7.62 0 0 1 2.896-6.002a7.7 7.7 0 0 1 1.288-.804c.297-.143.792-.385 1.453-.374c1.1.011 1.937.573 2.41 1.211a3.02 3.02 0 0 1 .595 1.751c0-.022 2.29-7.466-7.508-7.466C3.391 4.504 0 8.414 0 11.849c0 2.17.506 3.91 1.134 5.253a12.07 12.07 0 0 0 7.21 6.343a11.9 11.9 0 0 0 3.667.573c1.32 0 2.598-.22 3.798-.617a7.1 7.1 0 0 1-2.136.33a7.2 7.2 0 0 1-3.765-1.09"></svg:path><svg:path fill="url(#grommetIconsEdge2)" d="M14.279 13.964c-.077.099-.309.23-.309.528c0 .242.154.485.44.683c1.355.936 3.887.815 3.898.815c1.046 0 1.992-.297 2.84-.782c.386-.22.738-.496 1.057-.793c1.101-1.046 1.773-2.522 1.795-4.163c.022-2.103-.75-3.502-1.068-4.118C20.95 2.235 16.668 0 12 0C5.427 0 .088 5.286 0 11.838c.044-3.425 3.446-6.2 7.497-6.2c.33 0 2.202.033 3.941.947c1.53.804 2.334 1.773 2.896 2.742c.583 1.002.682 2.269.682 2.764c0 .496-.264 1.256-.737 1.873"></svg:path></svg:g><svg:defs><svg:lineargradient id="grommetIconsEdge0" x1="5.501" x2="22.225" y1="16.605" y2="16.605" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:lineargradient><svg:lineargradient id="grommetIconsEdge1" x1="14.318" x2="3.868" y1="9.347" y2="20.726" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset=".162" stop-color="currentColor"></svg:stop><svg:stop offset=".667" stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:lineargradient><svg:radialgradient id="grommetIconsEdge2" cx="0" cy="0" r="1" gradientTransform="rotate(92.128 -.93 3.333)scale(18.9898 40.4341)" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset=".111" stop-color="currentColor"></svg:stop><svg:stop offset=".232" stop-color="currentColor"></svg:stop><svg:stop offset=".315" stop-color="currentColor"></svg:stop><svg:stop offset=".673" stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:radialgradient><svg:clippath id="grommetIconsEdge3"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsEdgeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEditIcon],svg[grommet-icons-edit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m14 4l6 6zm8.294 1.294c.39.39.387 1.025-.008 1.42L9 20l-7 2l2-7L17.286 1.714a1 1 0 0 1 1.42-.008zM3 19l2 2m2-4l8-8"></svg:path>`,
})
export class GrommetIconsEditIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEjectIcon],svg[grommet-icons-eject-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M21 14L12 2L3 14zM2 22h20v-4H2z"></svg:path>`,
})
export class GrommetIconsEjectIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsElevatorIcon],svg[grommet-icons-elevator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M1 2.991C1 1.891 1.89 1 2.991 1H21.01C22.109 1 23 1.89 23 2.991V21.01c0 1.1-.89 1.991-1.991 1.991H2.99A1.99 1.99 0 0 1 1 21.009zM16.5 8l1.5 2h-3zm0 8l1.5-2h-3zM5 13l1.556-3.112C6.801 9.398 7.444 9 8 9h0c.552 0 1.2.398 1.444.888L11 13m-4.5 5l1.25-8h.5l1.25 8M8 7a1 1 0 1 0 0-2a1 1 0 0 0 0 2Z"></svg:path>`,
})
export class GrommetIconsElevatorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEmergencyIcon],svg[grommet-icons-emergency-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M10 7.172V2h4v5.172l3.657-3.657l2.828 2.828L16.828 10H22v4h-5.172l3.657 3.657l-2.828 2.828L14 16.828V22h-4v-5.172l-3.657 3.657l-2.828-2.828L7.172 14H2v-4h5.172L3.515 6.343l2.828-2.828z"></svg:path>`,
})
export class GrommetIconsEmergencyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEmojiIcon],svg[grommet-icons-emoji-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M12 1.73A10.27 10.27 0 1 0 22.24 12A10.25 10.25 0 0 0 12 1.73M21 12a9 9 0 1 1-9-9a9 9 0 0 1 9 9"></svg:path><svg:path fill="currentColor" d="M8.8 11.05a1.55 1.55 0 1 0-1.51-1.5a1.56 1.56 0 0 0 1.51 1.5m6.64 0a1.55 1.55 0 1 0 0-3.09a1.53 1.53 0 0 0-1.51 1.59a1.51 1.51 0 0 0 1.51 1.5m-3.25 5.3A6.58 6.58 0 0 1 6.9 13.5a5.71 5.71 0 0 0 5.3 4a5.54 5.54 0 0 0 5.31-4a6.27 6.27 0 0 1-5.32 2.85"></svg:path>`,
})
export class GrommetIconsEmojiIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEmptyCircleIcon],svg[grommet-icons-empty-circle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm0-6a4 4 0 1 0 0-8a4 4 0 0 0 0 8Z"></svg:path>`,
})
export class GrommetIconsEmptyCircleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEraseIcon],svg[grommet-icons-erase-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 21L22 6l-4-4L2 18l3 3h14M6 14l4 4"></svg:path>`,
})
export class GrommetIconsEraseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEscalatorIcon],svg[grommet-icons-escalator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M22 9a2 2 0 0 0-1.998-2H16L6 17H4c-1.105 0-2 .888-2 2h0a2 2 0 0 0 1.998 2H8l10-10h2c1.105 0 2-.888 2-2zM7 9a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 6v-2.505c0-.273.232-.495.5-.495h0c.276 0 .5.214.5.505V14zm5-11a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 6V7.495c0-.273.232-.495.5-.495h0c.276 0 .5.214.5.505V9z"></svg:path>`,
})
export class GrommetIconsEscalatorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsExpandIcon],svg[grommet-icons-expand-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m10 14l-8 8m-1-7v8h8M22 2l-8 8m1-9h8v8"></svg:path>`,
})
export class GrommetIconsExpandIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsEzmeralIcon],svg[grommet-icons-ezmeral-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M7 8h34v8H7zM1 8h6v8H1zm40 0h6v8h-6zM7 16h34v6H7zM7 2h34v6H7zM1 8l6-6v6zm0 8l6 6v-6zm46-8l-6-6v6zm0 8l-6 6v-6z"></svg:path>`,
})
export class GrommetIconsEzmeralIcon {
  readonly viewBox = input("0 0 48 24")
  readonly width = input("2em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFacebookIcon],svg[grommet-icons-facebook-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsFacebook0)"><svg:path fill="currentColor" d="M12 0C5.373 0 0 5.373 0 12c0 6.016 4.432 10.984 10.207 11.852V15.18h-2.97v-3.155h2.97V9.927c0-3.475 1.693-5 4.58-5c1.384 0 2.115.102 2.462.149v2.753h-1.97c-1.226 0-1.655 1.163-1.655 2.473v1.724h3.594l-.488 3.155h-3.106v8.696C19.481 23.083 24 18.075 24 12c0-6.627-5.373-12-12-12"></svg:path></svg:g><svg:defs><svg:clippath id="grommetIconsFacebook0"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsFacebookIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFacebookOptionIcon],svg[grommet-icons-facebook-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M9.945 22v-8.834H7V9.485h2.945V6.54c0-3.043 1.926-4.54 4.64-4.54c1.3 0 2.418.097 2.744.14v3.18h-1.883c-1.476 0-1.82.703-1.82 1.732v2.433h3.68l-.736 3.68h-2.944L13.685 22"></svg:path>`,
})
export class GrommetIconsFacebookOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFanIcon],svg[grommet-icons-fan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 15a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm3-3c4 3 5 7 5 7m-8 4c6.075 0 11-4.925 11-11S18.075 1 12 1S1 5.925 1 12s4.925 11 11 11Zm0-14c3-5 7-6 7-6m-7 12c-3 4-7 5-7 5m4-8C5 9 4 5 4 5"></svg:path>`,
})
export class GrommetIconsFanIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFanOptionIcon],svg[grommet-icons-fan-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" stroke="currentColor" stroke-width="2"><svg:rect width="20" height="20" x="2" y="2" rx="1"></svg:rect><svg:rect width="20" height="20" x="2" y="2" rx="10"></svg:rect><svg:path d="M15 9.5c.5-.333.9-1.7.5-2.5S13.333 5.667 13 5.5m1 5c1.5-2 0-3.5-2.5-5c-1.546-.927 2-1.5 4.5.5c1.875 1.5 1 2.5-2 5.5zm-5.015 3.902c-.5.333-.9 1.7-.5 2.5s2.167 1.333 2.5 1.5m-1-5c-1.5 2 0 3.5 2.5 5c1.546.927-2 1.5-4.5-.5c-1.875-1.5-1-2.5 2-5.5zm-.443-4.458c-.334-.5-1.7-.9-2.5-.5s-1.334 2.166-1.5 2.5m5-1c-2-1.5-3.5 0-5 2.5c-.928 1.546-1.5-2 .5-4.5c1.5-1.875 2.5-1 5.5 2zm3.902 5.014c.333.5 1.7.9 2.5.5s1.333-2.166 1.5-2.5m-5 1c2 1.5 3.5 0 5-2.5c.927-1.546 1.5 2-.5 4.5c-1.5 1.876-2.5 1-5.5-2z"></svg:path><svg:path d="M3.5 5a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm0 17a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm17 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm0-17a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM12 14a2 2 0 1 0 0-4a2 2 0 0 0 0 4Z" clip-rule="evenodd"></svg:path></svg:g>`,
})
export class GrommetIconsFanOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFastForwardIcon],svg[grommet-icons-fast-forward-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 2.059V8L1 2.059v20L9 16v6.059l13-10z"></svg:path>`,
})
export class GrommetIconsFastForwardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFavoriteIcon],svg[grommet-icons-favorite-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 8.4C1 4 4.5 3 6.5 3C9 3 11 5 12 6.5C13 5 15 3 17.5 3c2 0 5.5 1 5.5 5.4C23 15 12 21 12 21S1 15 1 8.4Z"></svg:path>`,
})
export class GrommetIconsFavoriteIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFedoraIcon],svg[grommet-icons-fedora-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12 0C5.375 0 .005 5.368 0 11.992v9.286A2.73 2.73 0 0 0 2.728 24h9.277C18.63 23.997 24 18.626 24 12S18.627 0 12 0m4.595 5.577c-.379 0-.517-.073-1.072-.073a2.973 2.973 0 0 0-2.973 2.968v2.583a.42.42 0 0 0 .42.419h1.953c.728 0 1.316.58 1.316 1.31c0 .734-.594 1.312-1.33 1.312H12.55v2.985a5.63 5.63 0 0 1-5.631 5.632c-.472 0-.808-.053-1.245-.167c-.637-.167-1.157-.689-1.157-1.296c0-.734.533-1.269 1.33-1.269c.378 0 .516.073 1.072.073c1.64 0 2.97-1.328 2.972-2.968v-2.583a.42.42 0 0 0-.42-.419H7.518c-.727 0-1.315-.58-1.315-1.31c0-.735.594-1.312 1.33-1.312H9.89V8.476a5.63 5.63 0 0 1 5.632-5.632c.472 0 .807.054 1.244.168c.637.167 1.158.689 1.158 1.296c0 .734-.533 1.269-1.33 1.269"></svg:path>`,
})
export class GrommetIconsFedoraIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFigmaIcon],svg[grommet-icons-figma-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd" transform="translate(4)"><svg:circle cx="12" cy="12" r="4"></svg:circle><svg:path d="M4 24a4 4 0 0 0 4-4v-4H4a4 4 0 1 0 0 8m0-8h4V8H4a4 4 0 1 0 0 8m0-8h4V0H4a4 4 0 1 0 0 8m8 0H8V0h4a4 4 0 1 1 0 8"></svg:path></svg:g>`,
})
export class GrommetIconsFigmaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFilterIcon],svg[grommet-icons-filter-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m3 6l7 7v8h4v-8l7-7V3H3z"></svg:path>`,
})
export class GrommetIconsFilterIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFingerPrintIcon],svg[grommet-icons-finger-print-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M14 15a2 2 0 1 0-2 2h0m0 3a5 5 0 1 1 5-5a1.5 1.5 0 0 0 3 0a8 8 0 1 0-8 8h2M1 15c0 2.672.953 5.122 2.537 7.027M20.52 8.042A10.98 10.98 0 0 0 12 4a10.98 10.98 0 0 0-8.464 3.974m14.99-5.363A13.94 13.94 0 0 0 12 1a13.94 13.94 0 0 0-6.481 1.587"></svg:path>`,
})
export class GrommetIconsFingerPrintIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFireballIcon],svg[grommet-icons-fireball-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10Zm0-9l-1.75 1l.75-2l-1.25-1h1.5L12 9.25l.75 1.75h1.5L13 12l.75 2z"></svg:path>`,
})
export class GrommetIconsFireballIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFirefoxIcon],svg[grommet-icons-firefox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="m4.25 1l-.145 3.684c.624-.154 1.235-.246 1.83.01c1.291-1.768 3.148-2.376 5.194-2.6l.038.06q-.06.068-.122.131a9.5 9.5 0 0 0-1.948 2.847c-.134.306-.166.658-.238.99c-.029.129.037.198.168.216c.607.083 1.212.18 1.822.25c.382.043.769.051 1.154.067c.18.008.258.097.262.267c.015.684-.213 1.27-.759 1.69a4.4 4.4 0 0 1-1.781.804q-.1.024-.208.054l.198 2.515l-1.854-.888c-.186.392-.2.78-.082 1.176c.316 1.073 1.431 1.695 2.586 1.41c.404-.1.795-.27 1.176-.444c.376-.17.729-.39 1.096-.583c.622-.324 1.249-.296 1.874-.006c.09.041.18.095.249.165c.234.234.417.503.323.857c-.091.342-.34.54-.677.611a3 3 0 0 1-.594.059c-.132-.001-.202.042-.274.148c-.68 1.009-1.562 1.752-2.79 1.964c-.517.09-1.05.075-1.576.104c-.07.004-.14-.006-.255-.012c.07.064.105.104.147.135c1.39 1.027 2.927 1.378 4.614.974c1.381-.33 2.61-.949 3.623-1.965c.966-.968 1.408-2.15 1.422-3.496c.014-1.353-.288-2.634-.98-3.807c-.121-.205-.279-.39-.454-.63c1.19.54 2.24 1.153 2.727 2.42c.143-1.582-.148-3.08-.758-4.525c-.61-1.444-1.485-2.695-2.682-3.731c.05.01.1.018.149.034c2.694.844 4.763 2.45 6.056 4.991c.626 1.23.946 2.552 1.111 3.913c.177 1.457.156 2.906-.216 4.338c-.622 2.398-1.935 4.359-3.753 6.013c-1.73 1.574-3.745 2.595-6.052 2.887c-5.14.651-9.19-1.198-12.063-5.526C.665 16.849.126 14.92.023 12.859C-.144 9.51.61 6.385 2.272 3.478A9.9 9.9 0 0 1 4.049 1.18c.05-.048.102-.093.2-.181"></svg:path>`,
})
export class GrommetIconsFirefoxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFirewallIcon],svg[grommet-icons-firewall-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M6.006 2.02a1 1 0 1 0 0 2h6a1 1 0 1 0 0-2zm5.916 2.976a1.1 1.1 0 0 0-1.1 1.1v1.8a1.1 1.1 0 0 0 1.1 1.1h5.8a1.1 1.1 0 0 0 1.1-1.1v-1.8a1.1 1.1 0 0 0-1.1-1.1zm-8.822 0a1.1 1.1 0 0 0-1.1 1.1v1.8a1.1 1.1 0 0 0 1.1 1.1h5.8a1.1 1.1 0 0 0 1.1-1.1v-1.8a1.1 1.1 0 0 0-1.1-1.1zm0 9.992a1.1 1.1 0 0 0-1.1 1.1v1.8a1.1 1.1 0 0 0 1.1 1.1h5.8a1.1 1.1 0 0 0 1.1-1.1v-1.8a1.1 1.1 0 0 0-1.1-1.1zm7.723 1.1a1.1 1.1 0 0 1 1.1-1.1h5.8a1.1 1.1 0 0 1 1.1 1.1v1.8a1.1 1.1 0 0 1-1.1 1.1h-5.8a1.1 1.1 0 0 1-1.1-1.1zm-5.975-5.015a1.1 1.1 0 0 1 1.1-1.1h5.8a1.1 1.1 0 0 1 1.1 1.1v1.8a1.1 1.1 0 0 1-1.1 1.1h-5.8a1.1 1.1 0 0 1-1.1-1.1zm9.866-1.061a1.1 1.1 0 0 0-1.1 1.1v1.8a1.1 1.1 0 0 0 1.1 1.1h5.8a1.1 1.1 0 0 0 1.1-1.1v-1.8a1.1 1.1 0 0 0-1.1-1.1zM14 3.019a1 1 0 0 1 1-1h6a1 1 0 1 1 0 2h-6a1 1 0 0 1-1-1m1 16.962a1 1 0 0 0 0 2h6a1 1 0 1 0 0-2zm-9.988 1a1 1 0 0 1 1-1h6a1 1 0 0 1 0 2h-6a1 1 0 0 1-1-1M3 9.973a1 1 0 0 0-1 1v2a1 1 0 1 0 2 0v-2a1 1 0 0 0-1-1m16.63-3.977a1 1 0 1 1 2 0v2a1 1 0 0 1-2 0zm1.015 8.992a1 1 0 0 0-1 1v2a1 1 0 1 0 2 0v-2a1 1 0 0 0-1-1M2 20.981a1 1 0 1 1 2 0a1 1 0 0 1-2 0M3.011 2.019a1 1 0 1 0 0 2a1 1 0 0 0 0-2" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsFirewallIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFlagIcon],svg[grommet-icons-flag-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 24V2c8-3.524 11 4.644 20 0v12c-8 4.895-13-4.103-20 0"></svg:path>`,
})
export class GrommetIconsFlagIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFlagFillIcon],svg[grommet-icons-flag-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" stroke="currentColor" stroke-width="2" d="M2 24V2c8-3.524 11 4.644 20 0v12c-8 4.895-13-4.103-20 0"></svg:path>`,
})
export class GrommetIconsFlagFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFlowsIcon],svg[grommet-icons-flows-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2zm0 11a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></svg:path>`,
})
export class GrommetIconsFlowsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFolderIcon],svg[grommet-icons-folder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 10V2h8l3 4h9v4zm0 0h20v12H2z"></svg:path>`,
})
export class GrommetIconsFolderIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFolderCycleIcon],svg[grommet-icons-folder-cycle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 18a5 5 0 0 1 5-5c1.985 0 3.7 1.156 4.5 3m.5 2a5 5 0 0 1-5 5c-1.985 0-3.699-1.156-4.5-3m5.5-4h4v-4m-6 8H7v4m-3-1H1V1h8l3 4h11v18h-4M1 9h22M4 23H1V1h8l3 4h11v18h-4M1 9h22"></svg:path>`,
})
export class GrommetIconsFolderCycleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFolderOpenIcon],svg[grommet-icons-folder-open-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 2h6l2 4h6v4H5zm-3 8h20l-3 12H5z"></svg:path>`,
})
export class GrommetIconsFolderOpenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormAddIcon],svg[grommet-icons-form-add-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 18V6m-6 6h12"></svg:path>`,
})
export class GrommetIconsFormAddIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormAttachmentIcon],svg[grommet-icons-form-attachment-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6 13.293l6.36-6.36c2.828-2.828 7.069 1.413 4.242 4.24l-6.361 6.36c-1.414 1.414-3.534-.706-2.12-2.12l6.36-6.36"></svg:path>`,
})
export class GrommetIconsFormAttachmentIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormCalendarIcon],svg[grommet-icons-form-calendar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 19h12V8H6zm9-11V5zM9 8V5zm-3 3.5h12z"></svg:path>`,
})
export class GrommetIconsFormCalendarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormCheckmarkIcon],svg[grommet-icons-form-checkmark-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6 13l4.2 3.6L18 7"></svg:path>`,
})
export class GrommetIconsFormCheckmarkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormClockIcon],svg[grommet-icons-form-clock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 18a6 6 0 1 0 0-12a6 6 0 0 0 0 12Zm0-10v4l3 1"></svg:path>`,
})
export class GrommetIconsFormClockIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormCloseIcon],svg[grommet-icons-form-close-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m7 7l10 10M7 17L17 7"></svg:path>`,
})
export class GrommetIconsFormCloseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormCutIcon],svg[grommet-icons-form-cut-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m18 7.524l-7.857 6.286zm0 8.38L10.143 9.62zm-9.429-5.761a1.571 1.571 0 1 0 0-3.143a1.571 1.571 0 0 0 0 3.143Zm0 6.286a1.571 1.571 0 1 0 0-3.143a1.571 1.571 0 0 0 0 3.143Z"></svg:path>`,
})
export class GrommetIconsFormCutIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormDownIcon],svg[grommet-icons-form-down-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m18 9l-6 6l-6-6"></svg:path>`,
})
export class GrommetIconsFormDownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormEditIcon],svg[grommet-icons-form-edit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m11.996 8.336l3.497 3.498zm5.54-.54a.994.994 0 0 1-.004 1.416l-7.451 7.451L6 17.83l1.166-4.08l7.451-7.452a.997.997 0 0 1 1.416-.005z"></svg:path>`,
})
export class GrommetIconsFormEditIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormFilterIcon],svg[grommet-icons-form-filter-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m6 8l5.667 4.667V18h.666v-5.333L18 8V6H6z"></svg:path>`,
})
export class GrommetIconsFormFilterIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormFolderIcon],svg[grommet-icons-form-folder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 18V6h4.8l1.8 2.4H18V18z"></svg:path>`,
})
export class GrommetIconsFormFolderIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormLocationIcon],svg[grommet-icons-form-location-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 17s-4-3-4-6c0-2.5 2-4 4-4s4 1.5 4 4c0 3-4 6-4 6Zm0-5a1 1 0 1 0 0-2a1 1 0 0 0 0 2Z"></svg:path>`,
})
export class GrommetIconsFormLocationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormLockIcon],svg[grommet-icons-form-lock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 11V8a3 3 0 0 1 6 0v3m-3 2v3m5 2v-7H7v7z"></svg:path>`,
})
export class GrommetIconsFormLockIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormNextIcon],svg[grommet-icons-form-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m9 6l6 6l-6 6"></svg:path>`,
})
export class GrommetIconsFormNextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormNextLinkIcon],svg[grommet-icons-form-next-link-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 12.4h12M12.6 7l5.4 5.4l-5.4 5.4"></svg:path>`,
})
export class GrommetIconsFormNextLinkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormPinIcon],svg[grommet-icons-form-pin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m4 19l4.455-4.454M12.273 5L18 10.727m-4.454-4.454L9.727 10.09s-2.545-.636-4.454 1.273l6.363 6.363c1.91-1.909 1.273-4.454 1.273-4.454l3.818-3.818l-3.181-3.182Z"></svg:path>`,
})
export class GrommetIconsFormPinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormPreviousIcon],svg[grommet-icons-form-previous-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m15 6l-6 6l6 6"></svg:path>`,
})
export class GrommetIconsFormPreviousIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormPreviousLinkIcon],svg[grommet-icons-form-previous-link-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 12.4H6M11.4 7L6 12.4l5.4 5.4"></svg:path>`,
})
export class GrommetIconsFormPreviousLinkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormRefreshIcon],svg[grommet-icons-form-refresh-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17.333 9.333C16.398 7.36 14.358 6 12 6a6 6 0 1 0 6 6m.5-6v4h-4"></svg:path>`,
})
export class GrommetIconsFormRefreshIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormScheduleIcon],svg[grommet-icons-form-schedule-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 19h12V8H6zm2-4h2zm3 0h5zm4-7V5zM9 8V5zm-3 3.5h12z"></svg:path>`,
})
export class GrommetIconsFormScheduleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormSearchIcon],svg[grommet-icons-form-search-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M13.8 13.8L18 18zM10.5 15a4.5 4.5 0 1 0 0-9a4.5 4.5 0 0 0 0 9Z"></svg:path>`,
})
export class GrommetIconsFormSearchIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormSubtractIcon],svg[grommet-icons-form-subtract-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 12h12"></svg:path>`,
})
export class GrommetIconsFormSubtractIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormTrashIcon],svg[grommet-icons-form-trash-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7.5 9h9v10h-9zM5 9h14M9.364 6h5v3h-5zm1.181 5v6m3-6v6"></svg:path>`,
})
export class GrommetIconsFormTrashIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormUpIcon],svg[grommet-icons-form-up-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m18 15l-6-6l-6 6"></svg:path>`,
})
export class GrommetIconsFormUpIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormUploadIcon],svg[grommet-icons-form-upload-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 14.182v3.273h12v-3.273M12 6v8M8.182 9.818L12 6l3.818 3.818"></svg:path>`,
})
export class GrommetIconsFormUploadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormViewIcon],svg[grommet-icons-form-view-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 17c-2.727 0-6-2.778-6-5s3.273-5 6-5s6 2.778 6 5s-3.273 5-6 5Zm-1-5a1 1 0 1 0 2 0a1 1 0 0 0-2 0Z"></svg:path>`,
})
export class GrommetIconsFormViewIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFormViewHideIcon],svg[grommet-icons-form-view-hide-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 12h3c.5 2.5 3.273 5 6 5s5.5-2.5 6-5h3m-9 5v3m-4.5-4.5l-2 2m11-2l2 2"></svg:path>`,
})
export class GrommetIconsFormViewHideIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsForwardTenIcon],svg[grommet-icons-forward-ten-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20.889 7.556C19.33 4.267 15.93 2 12 2C6.477 2 2 6.477 2 12s4.477 10 10 10s10-4.477 10-10m0-8v4h-4m-9 8V9l-2 .533M17 12c0-2-1-3.5-2.5-3.5S12 10 12 12s1 3.5 2.5 3.5S17 14 17 12Zm-2.5-3.5C16.925 8.5 17 11 17 12s0 3.5-2.5 3.5S12 13 12 12s.059-3.5 2.5-3.5Z"></svg:path>`,
})
export class GrommetIconsForwardTenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsFreebsdIcon],svg[grommet-icons-freebsd-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M23.725.403c1.273 1.272-2.254 6.862-2.85 7.458s-2.111.048-3.383-1.224s-1.82-2.787-1.224-3.383s6.185-4.123 7.457-2.85M5.885 1.75C3.943.647 1.179-.58.3.3c-.891.89.38 3.717 1.493 5.662A11.95 11.95 0 0 1 5.885 1.75m15.9 5.674c.179.606.147 1.108-.143 1.397c-.678.678-2.508-.044-4.158-1.614a8 8 0 0 1-.341-.323c-.596-.597-1.06-1.232-1.357-1.817c-.578-1.036-.723-1.952-.286-2.388c.238-.238.619-.303 1.083-.22c.303-.19.66-.404 1.053-.623a11.5 11.5 0 0 0-5.33-1.301C5.928.534.757 5.704.757 12.082S5.927 23.63 12.306 23.63s11.548-5.17 11.548-11.548c0-2.06-.54-3.991-1.485-5.666c-.204.373-.403.714-.584 1.007"></svg:path>`,
})
export class GrommetIconsFreebsdIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGalleryIcon],svg[grommet-icons-gallery-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h18v18H1zm4 18v4h18V5.97h-4M6 8a1 1 0 1 0 0-2a1 1 0 0 0 0 2ZM2 18l5-6l3 3l4-5l5 6"></svg:path>`,
})
export class GrommetIconsGalleryIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGamepadIcon],svg[grommet-icons-gamepad-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 6V2m0 4c2.498.044 4.006 0 5 0c2 0 4 .5 5 4s1 5.5 1 8s-2 3-4 3s-3.054-4-7-4s-5 4-7 4s-4-.5-4-3s0-4.5 1-8s3-4 5-4c.994 0 2.502.044 5 0Zm6 9a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm-4-3a1 1 0 1 0 0-2a1 1 0 0 0 0 2ZM4 12h6M7 9v6"></svg:path>`,
})
export class GrommetIconsGamepadIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGatewayIcon],svg[grommet-icons-gateway-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 20v-5m0-6V4m-7 8h5m9 0h-5m-.969-4.031L12 9.344l-1.031-1.375zm-2.062 8.07L12 14.664l1.031 1.375H10.97zM6 13.031L4.625 12L6 10.969zm12-2.062L19.375 12L18 13.031V10.97zM12 1c6.075 0 11 4.925 11 11s-4.925 11-11 11S1 18.075 1 12S5.925 1 12 1z"></svg:path>`,
})
export class GrommetIconsGatewayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGatsbyjsIcon],svg[grommet-icons-gatsbyjs-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M12 24C5.373 24 0 18.627 0 12S5.373 0 12 0s12 5.373 12 12s-5.373 12-12 12m10-11.908h-6.452v1.834h4.424c-.645 2.753-2.674 5.046-5.346 5.964L4.027 9.34c1.106-3.211 4.24-5.505 7.835-5.505c2.765 0 5.254 1.377 6.82 3.487l1.383-1.193C18.22 3.651 15.272 2 11.862 2C7.069 2 3.014 5.395 2 9.89L14.165 22C18.59 20.899 22 16.862 22 12.091zM2 12c0 2.593 1.018 5.092 2.963 7.037S9.5 22 12 22z"></svg:path>`,
})
export class GrommetIconsGatsbyjsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGemIcon],svg[grommet-icons-gem-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M7 1h-.414l-.293.293l-3 3L3 4.586v14.828l.293.293l3 3l.293.293h10.828l.293-.293l3-3l.293-.293V4.586l-.293-.293l-3-3L17.414 1zM5 6v12h1V6zm3 15h8v-1H8zm11-3V6h-1v12zM16 3H8v1h8zm0 3v12H8V6z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsGemIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGiftIcon],svg[grommet-icons-gift-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 11h18v12H3zm-1 0V7h20v4zm10 12V7zM7 7h5s-2-5-5-5C3.5 2 3 7 7 7Zm10.184 0h-5s1.816-5 5-5c3.316 0 4 5 0 5Z"></svg:path>`,
})
export class GrommetIconsGiftIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGithubIcon],svg[grommet-icons-github-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M11.999 1C5.926 1 1 5.925 1 12c0 4.86 3.152 8.983 7.523 10.437c.55.102.75-.238.75-.53c0-.26-.009-.952-.014-1.87c-3.06.664-3.706-1.475-3.706-1.475c-.5-1.27-1.221-1.61-1.221-1.61c-.999-.681.075-.668.075-.668c1.105.078 1.685 1.134 1.685 1.134c.981 1.68 2.575 1.195 3.202.914c.1-.71.384-1.195.698-1.47c-2.442-.278-5.01-1.222-5.01-5.437c0-1.2.428-2.183 1.132-2.952c-.114-.278-.491-1.397.108-2.91c0 0 .923-.297 3.025 1.127A10.5 10.5 0 0 1 12 6.32a10.5 10.5 0 0 1 2.754.37c2.1-1.424 3.022-1.128 3.022-1.128c.6 1.514.223 2.633.11 2.911c.705.769 1.13 1.751 1.13 2.952c0 4.226-2.572 5.156-5.022 5.428c.395.34.747 1.01.747 2.037c0 1.47-.014 2.657-.014 3.017c0 .295.199.637.756.53C19.851 20.979 23 16.859 23 12c0-6.075-4.926-11-11.001-11"></svg:path>`,
})
export class GrommetIconsGithubIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGlobeIcon],svg[grommet-icons-globe-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 8s3.5 1 5 2s.564 2.42 1 3s2-1 2 2s3 1 3 4s2.5 2.5 3 1s2.233-3.134 2-5s-1-3-3-3s-3.5-.5-4-2s3-2 2-5s0-4 0-4m10 11c0 6.075-4.925 11-11 11S1 18.075 1 12S5.925 1 12 1s11 4.925 11 11Z"></svg:path>`,
})
export class GrommetIconsGlobeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGolangIcon],svg[grommet-icons-golang-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor"><svg:path d="M21.468 3.206c.713 1.474-1.455 1.631-1.344 2.289c.204 1.186.065 2.947-.092 4.68c-.27 2.937 1.927 9.97-2.65 12.796c-.862.538-2.91.834-4.93.88h-.019c-2.02-.046-4.346-.342-5.208-.88c-4.568-2.826-2.372-9.859-2.631-12.796c-.167-1.733-.306-3.494-.093-4.68c.111-.658-2.057-.806-1.343-2.289c.611-1.27 2.122-.148 2.501-.547C7.596.65 9.95.111 11.803.065h1.02c1.843.093 4.197.593 6.143 2.594c.38.399 1.881-.722 2.502.547m-10.036 7.7c-.083.038-.222.946.204.992c.288.028 1.075.12 1.26 0c.362-.232.325-.788.121-.927c-.343-.222-1.492-.11-1.585-.064M9.08 4.106c-.955-.111-2.558.778-2.799 2.669c-.26 1.964 2.057 3.91 4.281 1.964c1.196-1.047 1.613-4.272-1.482-4.633m6.45 0c-3.096.361-2.679 3.586-1.483 4.633c2.223 1.946 4.54 0 4.28-1.964c-.231-1.89-1.834-2.78-2.798-2.669"></svg:path><svg:path d="M12.313 8.988c.584-.083 1.983.584 1.89 1.372c-.111.917-3.605 1.038-3.79-.056c-.111-.667.417-1.093 1.9-1.316m7.885 7.386c-.287-.01-.472-.39-.472-.621c0-.417.055-.908.361-1.121c.63-.436 1.13 1.751.111 1.742m-15.78 0c-1.02.009-.52-2.178.111-1.742c.306.213.361.704.361 1.12c0 .233-.185.612-.472.622m13.844 6.718c.185.269.194.454-.102.593c-1.13.519-2.14-.176-1.76-.37c.805-.408 1.306-1.048 1.862-.223m-11.908.093c.556-.825 1.057-.186 1.863.222c.38.195-.63.89-1.76.37c-.297-.138-.288-.324-.103-.592"></svg:path><svg:path d="M19.735 3.429c.046-.195.528-.288.797.046c.324.398-.473.945-.51.723c-.092-.603-.342-.575-.287-.77m-14.854 0c.056.194-.195.166-.287.769c-.037.222-.834-.325-.51-.723c.26-.334.741-.241.797-.046m10.86 4.512a1.056 1.056 0 1 1 0-2.112a1.056 1.056 0 0 1 0 2.112m-.278-1.177a.334.334 0 1 0 0-.667a.334.334 0 0 0 0 .667m-4.105 2.67c.111-.427.5-.566.816-.575c.806-.019 1.112.417 1.14.76c.055.583-2.206.722-1.956-.186M8.875 7.94a1.056 1.056 0 1 1 0-2.112a1.056 1.056 0 0 1 0 2.112m.278-1.177a.334.334 0 1 0 0-.667a.334.334 0 0 0 0 .667"></svg:path></svg:g>`,
})
export class GrommetIconsGolangIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGoogleIcon],svg[grommet-icons-google-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M5.266 9.765A7.08 7.08 0 0 1 12 4.909c1.69 0 3.218.6 4.418 1.582L19.91 3C17.782 1.145 15.055 0 12 0C7.27 0 3.198 2.698 1.24 6.65zm10.774 8.248c-1.09.703-2.474 1.078-4.04 1.078a7.08 7.08 0 0 1-6.723-4.823l-4.04 3.067A11.97 11.97 0 0 0 12 24c2.933 0 5.735-1.043 7.834-3zM19.834 21c2.195-2.048 3.62-5.096 3.62-9c0-.71-.109-1.473-.272-2.182H12v4.637h6.436c-.317 1.559-1.17 2.766-2.395 3.558z"></svg:path><svg:path fill="currentColor" d="M5.277 14.268A7.1 7.1 0 0 1 4.909 12c0-.782.125-1.533.357-2.235L1.24 6.65A11.9 11.9 0 0 0 0 12c0 1.92.445 3.73 1.237 5.335z"></svg:path>`,
})
export class GrommetIconsGoogleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGooglePayIcon],svg[grommet-icons-google-pay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g fill="currentColor" clip-path="url(#grommetIconsGooglePay0)"><svg:path d="m12.645 14.34l5.937-10.285l3.234 1.867a4.366 4.366 0 0 1 1.598 5.964l-3.344 5.791a2.73 2.73 0 0 1-3.728 1l-3.002-1.732a1.91 1.91 0 0 1-.695-2.606Z"></svg:path><svg:path d="M12.012 7.2L4.625 19.991l3.235 1.867a4.367 4.367 0 0 0 5.964-1.598l4.791-8.298a2.73 2.73 0 0 0-1-3.729l-3.002-1.732a1.903 1.903 0 0 0-2.601.697Z"></svg:path><svg:path d="m18.582 4.053l-2.29-1.32A5.456 5.456 0 0 0 8.839 4.73l-4.246 7.352a2.73 2.73 0 0 0 1 3.728l2.289 1.32a2.73 2.73 0 0 0 3.728-1l5.069-8.779a3.81 3.81 0 0 1 5.206-1.394z"></svg:path><svg:path d="M9.496 6.885L6.971 5.43a2.357 2.357 0 0 0-3.215.86L.725 11.524a5.383 5.383 0 0 0 1.977 7.363l1.923 1.108l2.332 1.343l1.012.582a4.14 4.14 0 0 1-1.27-5.51l.786-1.355l2.872-4.964a2.343 2.343 0 0 0-.86-3.207Z"></svg:path></svg:g><svg:defs><svg:clippath id="grommetIconsGooglePay0"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsGooglePayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGooglePlayIcon],svg[grommet-icons-google-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor"><svg:path d="M11.128 12.003L1.09 22.011a2.73 2.73 0 0 0 3.999 1.62l11.29-6.377l-5.251-5.25Z"></svg:path><svg:path d="m21.338 9.67l-4.899-2.806l-5.311 5.14l5.251 5.25l4.944-2.865a2.663 2.663 0 0 0 0-4.719zM1.09 1.995q-.09.339-.09.69V21.32q0 .351.09.69l10.038-10.007zm10.038 10.008l5.311-5.139L5.126.383A2.8 2.8 0 0 0 3.723 0A2.73 2.73 0 0 0 1.09 1.988z"></svg:path></svg:g>`,
})
export class GrommetIconsGooglePlayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGooglePlusIcon],svg[grommet-icons-google-plus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M.02 11.203c.066-3.906 3.676-7.327 7.603-7.197c1.882-.087 3.65.728 5.091 1.872a27 27 0 0 1-1.932 1.991C9.05 6.68 6.586 6.338 4.853 7.713c-2.478 1.705-2.59 5.731-.207 7.567c2.318 2.092 6.7 1.053 7.34-2.15c-1.451-.022-2.907 0-4.36-.048c-.003-.861-.007-1.723-.003-2.585a708 708 0 0 1 7.286.008c.145 2.027-.124 4.185-1.376 5.86c-1.896 2.655-5.702 3.43-8.672 2.292c-2.98-1.13-5.092-4.26-4.84-7.454m19.623-2.882h2.165q.005 1.083.015 2.165c.724.008 1.452.008 2.176.015v2.154l-2.176.018q-.01 1.086-.015 2.165H19.64c-.007-.72-.007-1.44-.014-2.161l-2.177-.022v-2.154q1.086-.009 2.177-.015q.005-1.085.018-2.165"></svg:path>`,
})
export class GrommetIconsGooglePlusIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGoogleWalletIcon],svg[grommet-icons-google-wallet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.3 20.206a2.365 2.365 0 0 1-2.722 1.046a2.386 2.386 0 0 1-1.687-2.377c-.34-3.8-2.458-7.06-5.693-8.798a2.26 2.26 0 0 1-.93-3.058c.4-.748 1.162-1.2 2-1.2c.375 0 .749.09 1.071.271c2.145 1.143 4.035 2.785 5.4 4.658c-.185-1.543-.673-3.047-1.49-4.36a2.4 2.4 0 0 1-.193-2.04a2.34 2.34 0 0 1 1.393-1.432a2.05 2.05 0 0 1 .852-.155c.464 0 .916.142 1.29.387q.052.032.101.066c-.036-.04-.1-.079-.1-.079c2.782 1.701 5.153 4.045 6.791 6.66a19.7 19.7 0 0 0-1.773-6.22a2.497 2.497 0 0 1 1.188-3.33A2.4 2.4 0 0 1 18.868 0a2.53 2.53 0 0 1 2.259 1.432a24.5 24.5 0 0 1 1.767 5.084c.4 1.78.62 3.652.633 5.51c0 1.884-.22 3.716-.62 5.51c-.103.477-.22.929-.361 1.406c-.439 1.587-.942 2.813-1.445 3.755A2.5 2.5 0 0 1 18.907 24c-.374 0-.735-.09-1.07-.245c-.75-.349-1.2-.994-1.368-1.703a2.5 2.5 0 0 1-.065-.581c0-.542-.026-.903-.026-.903c0-2.695-.644-5.261-1.83-7.518a15.7 15.7 0 0 1-2.247 7.156"></svg:path>`,
})
export class GrommetIconsGoogleWalletIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGraphQlIcon],svg[grommet-icons-graph-ql-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="m14.334 2.852l4.71 2.72q.214-.225.495-.39A2.234 2.234 0 0 1 22.585 6a2.234 2.234 0 0 1-1.546 3.31v5.38a2.228 2.228 0 1 1-2.166 3.544l-4.626 2.67A2.23 2.23 0 0 1 12.192 24a2.227 2.227 0 0 1-2.12-2.925l-4.678-2.701q-.23.257-.544.442a2.23 2.23 0 1 1-1.725-4.076V9.26A2.232 2.232 0 0 1 1.798 6a2.232 2.232 0 0 1 3.458-.51l4.765-2.752A2.227 2.227 0 0 1 12.192 0a2.227 2.227 0 0 1 2.142 2.852m-.493.88q-.075.081-.157.155l6.278 10.875l.069-.02V9.255a2.226 2.226 0 0 1-1.501-2.816zm-3.28.019a2 2 0 0 1-.093-.107L5.816 6.33a2.227 2.227 0 0 1-1.683 2.976v5.386l.1.02zm2.24.624a2.24 2.24 0 0 1-1.401-.06L5.14 15.154a2.24 2.24 0 0 1 .74 1.12h12.621a2.24 2.24 0 0 1 .595-.996zm.949 15.8l4.739-2.736a2 2 0 0 1-.035-.157H5.93q-.02.105-.049.208l4.716 2.723a2.22 2.22 0 0 1 1.596-.672c.607 0 1.156.242 1.558.634"></svg:path>`,
})
export class GrommetIconsGraphQlIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGremlinIcon],svg[grommet-icons-gremlin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 14a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm10 0a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm-5 2c.5 0 1-.321 1-1c0-.113-2-.075-2 0c0 .679.5 1 1 1Zm-9 8s.003-8 0-12c.003-4-.447-8 9-8s8.997 4 9 8c-.003 4 0 12 0 12m-.932-17c3.26 0 3.58-3.58 2.046-5.114S17 .673 17 3.932M7 19c0-.203 2.5 1.152 5 1c2.5.152 5-1.499 5-1c0 .802-1.5 2-5 2s-5-1.493-5-2ZM3.945 7C.605 7 .38 3.42 1.908 1.886C3.435.352 7 .673 7 3.932"></svg:path>`,
})
export class GrommetIconsGremlinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGridIcon],svg[grommet-icons-grid-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 1v22m8-22v22M1 8h22M1 16h22M1 1h22v22H1z"></svg:path>`,
})
export class GrommetIconsGridIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGrommetIcon],svg[grommet-icons-grommet-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="4" d="M12 2C6.485 2 2 6.485 2 12s4.485 10 10 10s10-4.485 10-10S17.515 2 12 2Z"></svg:path>`,
})
export class GrommetIconsGrommetIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGroupIcon],svg[grommet-icons-group-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 13a4 4 0 1 0 0-8a4 4 0 0 0 0 8Zm-6 9v-3a6 6 0 1 1 12 0v3M13 5c.404-1.664 2.015-3 4-3c2.172 0 3.98 1.79 4 4c-.02 2.21-1.828 4-4 4h-1h1c3.288 0 6 2.686 6 6v2M11 5c-.404-1.664-2.015-3-4-3c-2.172 0-3.98 1.79-4 4c.02 2.21 1.828 4 4 4h1h-1c-3.288 0-6 2.686-6 6v2"></svg:path>`,
})
export class GrommetIconsGroupIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsGrowIcon],svg[grommet-icons-grow-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 16V7m0 4c0-3 0-6-7-6c0 3 0 6 7 6Zm-8 5h16m-2 0l-2 7H8l-2-7m6-9c0-3 0-6 7-6c0 3 0 6-7 6Z"></svg:path>`,
})
export class GrommetIconsGrowIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHadoopIcon],svg[grommet-icons-hadoop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M22.615 8.515c-.162.64-.49 1.186-1.168 1.428c-.332.118-.547-.004-.886-.102c.333.032.524.083.822-.064a2.72 2.72 0 0 0 1.232-1.262m1.355 4.215c-.08.777-.287 1.532-.656 2.172a3.55 3.55 0 0 1-1.407 1.385c-.417.222-.902.35-1.403.403a5.8 5.8 0 0 1-1.56-.06l-.07.234l-.088.29c-.103.337-.347.64-.652.865c-.314.232-.698.383-1.055.4a1.44 1.44 0 0 1-.84-.203c-.226-.133-.42-.318-.631-.522l-.277-.267l-.187-.18l-.02.123c.098.197.167.382.237.572c.045.12.09.241.145.373c.082.195.144.358.187.527s.066.344.072.557l.015.542q.005.255.014.512c.075.136.12.23.142.334a1.3 1.3 0 0 1 .01.393c-.028.394-.158.614-.377.739c-.206.116-.478.133-.814.132l-.235-.001c-.327 0-.832 0-.986-.01a3.4 3.4 0 0 1-.525-.066a.8.8 0 0 1-.28-.121c-.112.09-.21.171-.309.252c-.29.24-.584.483-.702.561q-.069.106-.125.206c-.121.214-.211.371-.498.527c-.564.304-1 .278-1.382.043c-.365-.224-.666-.635-.98-1.12c-.15-.23-.39-.61-.506-.983c-.129-.412-.11-.812.3-1.023l.058-.03l.123-.064a7 7 0 0 1-1.393-.05l.012.086c.024.208.04.344.02.48c-.02.14-.075.265-.193.442l-.058.082c-.11.163-.175.26-.326.594c.015.165.02.298.009.43a2.7 2.7 0 0 1-.093.477c-.13.478-.61.588-1.142.554c-.488-.03-1.023-.184-1.332-.268l-.174-.046a5 5 0 0 1-.778-.248c-.473-.216-.777-.555-.481-1.204q.07-.155.127-.314q.02-.055.036-.11a1.5 1.5 0 0 1-.746-.103c-.437-.166-.853-.483-1.108-.738a2.7 2.7 0 0 1-.482-.65a2.5 2.5 0 0 1-.252-.748c-.069-.408-.02-.455.22-.687l.087-.083l.357-.354l.382-.38l.175-.472l.111-.305a6 6 0 0 1-.083-.596a9 9 0 0 1-.037-1.14c.01-.256.034-.492.073-.718c-.2.044-.402.041-.586-.017a.9.9 0 0 1-.55-.489a2 2 0 0 1-.116-.299a5 5 0 0 1-.088-.322a1 1 0 0 1-.286-.525a1 1 0 0 1 .098-.612a2 2 0 0 1 .384-.536c.425-.442 1.03-.774 1.31-.823l.212-.037l-.07.203c-.037.102-.08.21-.12.315l-.037.095a.7.7 0 0 1 .243.406c.057.25.02.537-.028.753l-.058.258l-.163-.207q-.059-.074-.104-.149a1 1 0 0 0-.086-.122c-.022.336-.063.672-.302.902q.009.042.015.047c.004.002.044-.02.142-.058c.169-.065.318-.172.46-.298c.144-.13.276-.28.41-.426c.152-.294.325-.573.528-.837q.306-.405.713-.766a6.6 6.6 0 0 1 2.047-1.266c.72-.278 1.521-.453 2.506-.611q.186-.193.379-.382q.206-.2.43-.398a2 2 0 0 1 .562-.368c.158-.065.313-.09.488-.103c.301-.4.575-.744.898-1.042a4.1 4.1 0 0 1 1.247-.792c1.082-.456 1.999-.694 2.866-.64c.873.055 1.69.405 2.566 1.128c.165.136.334.284.502.43c.367.324.727.64 1.15.904c.16.099.294.185.427.286c.132.1.253.212.38.362c.153.183.292.366.415.562q.154.243.272.527c.18-.072.369-.152.521-.28c.131-.113.24-.273.35-.432c.097-.143.195-.286.312-.403a.5.5 0 0 1 .128-.095c.178-.094.404-.079.618-.005c.202.07.397.192.53.323c.06.06.11.123.141.185c.266.509.458 1.216.573 1.943c.13.818.164 1.665.102 2.288m-3.725-3.348l-.113.003a.6.6 0 0 1 .13.19a.4.4 0 0 1 .052-.043zM1.499 11.654c.098-.011.16.04.236.116c-.014-.181-.048-.316-.176-.39l-.025.088q-.023.088-.035.186m.391 1.98l.03-.097q.091-.292.198-.568a1.7 1.7 0 0 1-.518.33c-.524.21-.513.028-.739-.427c.48-.383.258-.853.41-1.382c.036-.122.087-.24.16-.397C.93 11.395.006 12.24.623 12.88c.067.241.118.43.21.62c.189.395.682.294 1.057.135m1.389 6.399c-.537-.811-1.021-1.778-1.305-2.76l-.158.421l-.72.73c-.178.18-.213.187-.177.442c.048.35.255.723.564 1.038c.282.288 1.258 1.04 1.641.528c.082-.11.12-.245.155-.384zm11.783-1.536c-.085-.202-.131-.39-.19-.582c-.128.457-.3.886-.456 1.38c-.148.467-.909 1.822-1.356 2.151c.087.065.247.09.532.115c.201.017.995.021 1.195.024c.439.008.606-.025.65-.525c.022-.245.002-.29-.12-.505l-.032-1.148c-.01-.362-.08-.572-.223-.91m7.89-9.624c-.103-.247-.51-.618-.77-.366c-.262.255-.459.614-.744.84c-.389.305-1.057.2-.936.85c.088.472.114 1.001.012 1.442c-.096.423-.196 1.024-.417 1.308c.065-.24.175-.867.206-1.29a3.7 3.7 0 0 0-.053-.791l-.147.003c-.178.004-.469.26-.547.419c-.218.444-.233.856-.477 1.279c.197-.473.086-.899.266-1.405c.063-.179.223-.333.409-.427c-.104-.015-.207.03-.36.065c-.651.156-.622.668-1.023 1.157c.387-.709.19-1.179 1.028-1.37c.275-.063.477-.088.658.057h.011l.143-.012q-.035-.196-.074-.379a4 4 0 0 0-1.13.02q.128-.056.248-.1q.187-.07.367-.103a.2.2 0 0 0 .023-.079a.243.243 0 0 0-.218-.263a.24.24 0 0 0-.259.218a.3.3 0 0 0 .025.13a.6.6 0 0 1 .015-.569l-.039.012a.7.7 0 0 1-.382-.005a2.6 2.6 0 0 0 .622-.212a.59.59 0 0 1 .553-.033c.079.023.167.054.273.093a6 6 0 0 0-.562-.948c-.208-.289-.382-.388-.689-.57a6 6 0 0 1-.196-.12c-.227-.042-.48-.083-.698-.041c.18-.07.315-.1.473-.112c-.434-.311-.809-.66-1.237-1.02c-1.653-1.39-2.99-1.292-5.095-.378c-.854.372-1.16.746-1.686 1.418h.017a23 23 0 0 0 1.615-.084c-.568.133-1.112.239-1.67.323c-.615.094-.804.078-1.269.53c-.855.833-1.572 1.774-2.355 2.618c-.453.49-.679.98-.945 1.537c-.264.554-.227.764.127 1.243c.362.491.58.713.734 1.148c.4-.617.882-1.13 1.39-1.743a25 25 0 0 0-1.136 1.97c-.214.42-.319.635-.312 1.1c.401.462.653.709 1.025.81c.401.111.78.093 1.142-.095c.902-.467 1.759-1.076 2.76-1.153c.517-.92.372-2.079.186-3.127c-.154-.87-.115-1.638.093-2.511c.052.848.12 1.635.288 2.467c.245 1.218.297 2.27-.216 3.528c-1.106.027-2.014.675-2.995 1.177c-.441.225-.854.248-1.348.13c-.503-.119-.889-.507-1.407-1.074c.05-.429.107-.695.26-1.01c-.185-.557-.414-.812-.838-1.381c-.49-.66-.499-.946-.128-1.697c.268-.544.519-1.047.942-1.543c.417-.492.8-.95 1.188-1.384c-1.61.245-2.638.633-3.738 1.598c-.815.715-1.324 1.604-1.687 2.63c-.23.648-.294 1.3-.185 2.36c.139 1.364.864 2.904 1.644 4.027c-.095.7-.219 1.226-.414 1.726c-.221.566.66.74 1.085.856c.345.095 1.656.455 1.786-.01c.077-.278.077-.473.058-.767c.234-.466.305-.528.472-.804c.178-.295.193-.413.195-.76c0-.447-.011-1.245.008-1.534c.06.258.132.618.192.963a7.3 7.3 0 0 0 2.789-.036c.012-.065.033-.136.057-.223c.063-.218.188-.436.25-.654l-.05.644c-.018.221-.019.4.009.62l.044.364c-.067-.11-.172-.218-.233-.33c-.415.274-.643.428-1.05.643c-.431.228.062 1.043.24 1.326c.39.62.857 1.355 1.583.971c.238-.125.378-.417.537-.626c.213-.126 1.343-1.08 1.523-1.183c.254-.146 1.167-1.642 1.283-2.015c.255-.827.555-1.456.64-2.318c-.582-.245-.864-.517-1.281-1.007a5.4 5.4 0 0 0 1.723.867q.344.302.704.623c.35.312.672.63 1.18.639c.55.01 1.17-.41 1.33-.931l.184-.602a6 6 0 0 1-.318-.087c-.114.274-.105.38-.158.582c-.093.344-.567.583-1.063.463c-.176-.043-.28-.067-.343-.113c.05.136.147.251.328.284c.2.037.347.053.638-.013c-.36.187-.475.195-.73.138c-.67-.149-.434-.956-.288-1.45c.092-.31.06-.64-.009-.951c.254.16.455.302.744.4c1.354.455 2.96.92 4.322.159c1.068-.596 1.654-1.919 1.785-3.13c.118-1.08-.085-2.765-.527-3.836m-6.909 6.503c-.026.168-.062.376-.089.544c.072-.19.157-.41.24-.591c.087-.192.136-.209.32-.31c.133-.072.372-.17.503-.242c-.135.023-.378.072-.512.095c-.362.061-.405.148-.462.504M9.865 8.908c-.419.415-.826 1.84-.956 2.409c.204-.474.718-1.797 1.112-2.134a1.4 1.4 0 0 1 .266-.192c-.282.473-.261.59-.162 1.224c.084-.644.307-.893.672-1.373c.4-.101.776-.22 1.187-.381q-.695.078-1.39.144c-.39.036-.452.03-.729.303m6.964 1.58a.77.77 0 0 0-.432-.392c.167-.09.33-.183.457-.298c-.373.173-.807.13-1.127.347c-.283.19-.672.794-.959 1.05c.207-.08.407-.22.591-.367q.002.166.075.325a.8.8 0 0 0 .212.264a.3.3 0 0 1-.062-.085a.323.323 0 1 1 .6-.231a2 2 0 0 0-.411.403a1.7 1.7 0 0 0-.25.457a4.4 4.4 0 0 1 2.081-1.345a2.3 2.3 0 0 0-.705.118a.8.8 0 0 0-.07-.247m-2.666-.584c.126-.556.349-1.092 1.212-1.506c-1.144.286-1.359.767-1.212 1.506"></svg:path>`,
})
export class GrommetIconsHadoopIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHaltIcon],svg[grommet-icons-halt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 23h7c3 0 4-2 4-5V6c0-2-1-2-1.5-2S16 4 16 6v7v-9c0-1 0-2-1.5-2S13 3 13 4v9V3c0-1 0-2-1.5-2S10 2 10 3v10v-9c0-1 .03-2-1.5-2C7 2 7 3 7 4v14v-4c0-1-.55-2-2-2H4v6c0 3.962 2 5.024 4 5Z"></svg:path>`,
})
export class GrommetIconsHaltIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHelpIcon],svg[grommet-icons-help-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 23a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0-5v-3c0-2 1-2 3-3s3-2.842 3-5A6 6 0 1 0 6 7"></svg:path>`,
})
export class GrommetIconsHelpIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHelpBookIcon],svg[grommet-icons-help-book-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M11.937 12.136v-.864c0-.864 0-1.296.797-1.728c.798-.432 1.595-.864 1.595-2.16c0-.865-.797-2.16-2.392-2.16S9.546 6.316 9.546 7.816"></svg:path><svg:path fill="currentColor" stroke="currentColor" stroke-width="2" d="M11.938 13v1.728"></svg:path><svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5.5 18a2.5 2.5 0 1 0 0 5H22M3 20.5v-17A2.5 2.5 0 0 1 5.5 1H21v17.007H5.492M20.5 18a2.5 2.5 0 1 0 0 5"></svg:path>`,
})
export class GrommetIconsHelpBookIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHelpOptionIcon],svg[grommet-icons-help-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M2 7a5 5 0 0 1 5-5h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5zm5-3a3 3 0 0 0-3 3v10a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V7a3 3 0 0 0-3-3zm5 3.5c-1.448 0-2 1.052-2 1.5a1 1 0 1 1-2 0c0-1.552 1.448-3.5 4-3.5c1.156 0 2.17.289 2.909.935C15.664 7.096 16 8.017 16 9c0 1.188-.306 2.028-.882 2.668a4.3 4.3 0 0 1-.828.693a15 15 0 0 1-.368.235l-.018.011c-.124.078-.236.15-.347.223c-.35.236-.493.412-.55.526c-.046.088-.077.205-.028.438a1 1 0 0 1-1.958.412c-.126-.6-.083-1.197.204-1.759c.273-.535.718-.942 1.218-1.277c.14-.094.277-.18.399-.257l.009-.006c.122-.076.228-.142.328-.21c.205-.136.346-.25.453-.368c.174-.193.368-.516.368-1.329c0-.517-.164-.846-.409-1.06c-.261-.229-.747-.44-1.591-.44m0 8.281a1 1 0 1 0 0 2a1 1 0 0 0 0-2" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsHelpOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHerokuIcon],svg[grommet-icons-heroku-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M20.443 0H3.162A2.16 2.16 0 0 0 1 2.162V21.84C1 23.034 1.97 24 3.162 24h17.28a2.16 2.16 0 0 0 2.16-2.16V2.162A2.16 2.16 0 0 0 20.442 0m.958 21.84a.96.96 0 0 1-.958.96H3.162a.96.96 0 0 1-.962-.96V2.162c0-.532.432-.962.962-.962h17.28c.53 0 .96.43.96.962zm-15-1.439l2.701-2.4L6.4 15.6zm9.757-9.729c-.486-.488-1.373-1.071-2.856-1.071c-1.627 0-3.303.424-4.5.812V3.6H6.4v10.41l1.697-.769c.028-.013 2.763-1.239 5.205-1.239c1.218 0 1.488.67 1.501 1.231v7.17h2.398v-7.2c.003-.155-.012-1.486-1.043-2.53M13 7.5h2.401c1.085-1.228 1.637-2.536 1.8-3.899h-2.399c-.267 1.36-.858 2.66-1.802 3.9"></svg:path>`,
})
export class GrommetIconsHerokuIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHideIcon],svg[grommet-icons-hide-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 17c-2.727 0-6-2.778-6-5s3.273-5 6-5s6 2.778 6 5s-3.273 5-6 5Zm-1-5a1 1 0 1 0 2 0a1 1 0 0 0-2 0Zm9-7L4 19"></svg:path>`,
})
export class GrommetIconsHideIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHistoryIcon],svg[grommet-icons-history-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 12c0 6.075 4.925 11 11 11s11-4.925 11-11S18.075 1 12 1C7.563 1 4 4 2 7.5M1 1v7h7m8 9l-4-4V6"></svg:path>`,
})
export class GrommetIconsHistoryIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHomeIcon],svg[grommet-icons-home-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m1 11l11-9l11 9m-8 12v-8H9v8m-5 0V9m16 14V9"></svg:path>`,
})
export class GrommetIconsHomeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHomeOptionIcon],svg[grommet-icons-home-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:rect width="18" height="18" x="3" y="3" fill="none" stroke="currentColor" stroke-width="2" rx="4"></svg:rect>`,
})
export class GrommetIconsHomeOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHomeRoundedIcon],svg[grommet-icons-home-rounded-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 22V9.76a2 2 0 0 1 .851-1.636l9.575-6.72a1 1 0 0 1 1.149 0l9.574 6.72A2 2 0 0 1 23 9.76V22a1 1 0 0 1-1 1h-5.333a1 1 0 0 1-1-1v-5.674a1 1 0 0 0-1-1H9.333a1 1 0 0 0-1 1V22a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1z"></svg:path>`,
})
export class GrommetIconsHomeRoundedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHortonIcon],svg[grommet-icons-horton-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M2.3 12.977h-.157c-.319.989-.755 1.957-.932 2.97c-.222 1.292-.238 2.624-.34 3.936c-.027.339.126.517.465.52c1.526.02 1.352.081 1.602-1.198c.017-.089.029-.178.057-.267c.52-1.658.545-3.285-.27-4.875c-.178-.344-.283-.723-.424-1.086m16.606-5.922a.5.5 0 1 0-1 0a.5.5 0 0 0 1 0m2.176 2.737c.206.44.153 1.021.15 1.538c-.005.924-.057 1.849-.098 2.773a609 609 0 0 1-.125 2.922c-.577-.355-1.037-.605-1.457-.908c-.141-.1-.242-.335-.254-.517c-.097-1.097-.194-2.2-.226-3.301c-.017-.695-.202-1.207-.925-1.405c-.383-.1-.58-.367-.726-.735c-.614-1.525-1.251-3.047-1.93-4.686c1.163.214 2.237.388 3.298.618c.19.04.375.278.488.472c.622 1.066 1.276 2.115 1.805 3.23m.569 7.225c0 .016.016.028.04.08c.771.057 1.615.352 2.172-.718c-.364-.105-.686-.201-1.01-.286c-1.202-.315-1.202-.315-1.202.924m-6.3-3.205c-.098-.347-.271-.496-.63-.549c-.436-.068-.933-.117-1.272-.359c-1.497-1.086-2.942-2.244-4.424-3.354c-.4-.303-.496-.618-.33-1.082c.245-.694.455-1.4.718-2.208c-.226.025-.331.029-.432.049c-1.308.29-2.604.646-3.928.86c-.86.14-1.38.577-1.9 1.25C1.888 10.055.92 11.79.564 13.85c-.13.754-.371 1.489-.565 2.232c.06.04.125.08.186.12c.125-.144.302-.27.367-.435c.404-1.09.779-2.188 1.175-3.278c.205-.565.387-.61.762-.129c.243.307.465.658.594 1.021c.69 1.95 1.352 3.908 2.026 5.861c.408 1.19.412 1.17 1.683 1.175c.48 0 .654-.17.614-.638c-.085-1.013-.138-2.026-.23-3.035c-.045-.517.185-.755.662-.884c1.174-.315 2.34-.654 3.507-.99c.735-.205.807-.18 1.17.473c.764 1.36 1.563 2.704 2.261 4.101c.38.767.884 1.062 1.715.973c.61-.065.658-.073.493-.682c-.545-1.974-1.102-3.948-1.635-5.922m-.118-7.54c-.38-1.04-.928-1.61-2.054-1.719c-.719-.069-1.417-.36-2.128-.553h-.217c-.097.214-.214.42-.287.638c-.408 1.223-.787 2.454-1.207 3.669c-.157.452-.056.742.327 1.021c1.062.787 2.127 1.578 3.148 2.422c1.635.989 2.24.989 2.975.985s1.32 0 2.059 0c-.093-.263-.15-.448-.222-.626c-.808-1.941-1.68-3.859-2.394-5.837m-3.786 9.18c-.287.035-.658.052-.682.536c-.053 1.082-.15 2.163-.234 3.354c.02.25-.073.78.113.912c.274.19.759.154 1.138.101c.153-.02.34-.307.392-.508c.14-.521.153-1.082.327-1.59c.318-.945.12-1.777-.453-2.531c-.12-.162-.411-.3-.601-.275M24 15.725c-.093-.566-.145-1.082-.274-1.579c-.033-.125-.291-.238-.46-.258c-.11-.016-.32.113-.36.222c-.286.75.19 1.506 1.094 1.615"></svg:path>`,
})
export class GrommetIconsHortonIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHostIcon],svg[grommet-icons-host-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 4h1v1h-1zM3 1h18v22H3zm0 12h18zm0 5h18zM3 8h18z"></svg:path>`,
})
export class GrommetIconsHostIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHostMaintenanceIcon],svg[grommet-icons-host-maintenance-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m14 23l6-6m1-3a2 2 0 1 0 2 2M17 4h1v1h-1zm-7 19H3V1h18v10M3 13h14M3 18h10M3 8h18"></svg:path>`,
})
export class GrommetIconsHostMaintenanceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHpIcon],svg[grommet-icons-hp-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M8.421 0L5 15.127h2.138L10.56 0zm8.993 8.873l-1.496 6.225h2.138l1.496-6.225zm-3.635 0L10.36 24h2.138l3.42-15.127zm-3.647 0l-1.497 6.225h2.138l1.496-6.225z"></svg:path>`,
})
export class GrommetIconsHpIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHpeIcon],svg[grommet-icons-hpe-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M2 6h44v12H2zm3 3h38v6H5z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsHpeIcon {
  readonly viewBox = input("0 0 48 24")
  readonly width = input("2em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHpeLabsIcon],svg[grommet-icons-hpe-labs-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 23H5V2h12v13h-4V6H9v13h11"></svg:path>`,
})
export class GrommetIconsHpeLabsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHpiIcon],svg[grommet-icons-hpi-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M15.793 15.333a.5.5 0 0 0 .442-.315l2.196-6.37c.06-.173-.04-.315-.225-.315h-1a.5.5 0 0 0-.443.315l-2.206 6.37c-.06.174.041.315.225.315zM24 12c0 6.627-5.373 12-12 12c-.183 0-.36-.02-.542-.027l2.42-6.991a.5.5 0 0 1 .443-.315H16c2.607 0 2.631-.865 2.906-1.66c.642-1.862 1.75-5.073 1.964-5.701C21.178 8.41 21.28 7 19.001 7H15a.5.5 0 0 0-.443.315L8.925 23.587C3.79 22.227 0 17.562 0 12C0 6.694 3.448 2.2 8.223.615L2.776 16.351c-.06.174.04.316.224.316h2a.5.5 0 0 0 .443-.316l2.666-7.703a.5.5 0 0 1 .442-.315h.989c.183 0 .284.142.224.315l-2.656 7.703c-.059.174.042.316.225.316h2a.5.5 0 0 0 .442-.316l2.427-7.036C12.765 7.682 12.312 7 10.346 7H9.013c-.183 0-.284-.142-.224-.315L11.086.046C11.39.023 11.691 0 12 0c6.627 0 12 5.373 12 12"></svg:path>`,
})
export class GrommetIconsHpiIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsHtml5Icon],svg[grommet-icons-html5-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 2h18v16l-9 4l-9-4zm14 4H8v5h8v5l-4 1.5L8 16v-2"></svg:path>`,
})
export class GrommetIconsHtml5Icon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsIceCreamIcon],svg[grommet-icons-ice-cream-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 8c2 0 2-1.5.5-1.5c0-3-2-5.5-5.5-5.5S6.5 3.5 6.5 6.5C5 6.5 5 8 7 8m0 0h10l-5 13z"></svg:path>`,
})
export class GrommetIconsIceCreamIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsImageIcon],svg[grommet-icons-image-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 3h22v18H1zm5 6a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm17 6l-5-6l-6 7l-3-3l-8 8"></svg:path>`,
})
export class GrommetIconsImageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsImpactIcon],svg[grommet-icons-impact-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m13 2l9 4v11l-9 5zm9 4l-9 5zM9 22V2zm0-10L3 5zm0 0H1zm0 0l-6 7z"></svg:path>`,
})
export class GrommetIconsImpactIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInProgressIcon],svg[grommet-icons-in-progress-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h22M10 4.5h4V6c0 1-2 2-2 2s-2-1-2-2zM5 1v5c0 3 5 3.235 5 6s-5 3-5 6v5M19 1v5c0 3-5 3.235-5 6s5 3 5 6v5M1 23h22M8 21c0-2 4-4 4-4s4 2 4 4v2H8z"></svg:path>`,
})
export class GrommetIconsInProgressIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInboxIcon],svg[grommet-icons-inbox-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 13L6 2h12l5 11v9H1zm0 0h7v3h8v-3h7"></svg:path>`,
})
export class GrommetIconsInboxIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsIndicatorIcon],svg[grommet-icons-indicator-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 11a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm7-3A7 7 0 0 0 5 8c0 1.933.5 3 2 5s3 3.5 3 6v4h4v-4c0-2.5 1.5-4 3-6s2-3.067 2-5Z"></svg:path>`,
})
export class GrommetIconsIndicatorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInfoIcon],svg[grommet-icons-info-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M15 17c0-3 4-5 4-9s-3-7-7-7s-7 3-7 7s4 6 4 9v3q0 3 3 3t3-3zm-6 1h6"></svg:path>`,
})
export class GrommetIconsInfoIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInheritIcon],svg[grommet-icons-inherit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m17 18l-5-3zM7 18l5-3v-4m5 9a3 3 0 1 0 6 0a3 3 0 0 0-6 0ZM4 17a3 3 0 1 0 0 6a3 3 0 0 0 0-6ZM17 6a5 5 0 1 1-10.001-.001A5 5 0 0 1 17 6Z"></svg:path>`,
})
export class GrommetIconsInheritIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInsecureIcon],svg[grommet-icons-insecure-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 6.919V6a4.724 4.724 0 0 1 5-5a4.724 4.724 0 0 1 5 5v5.052M12 23a7 7 0 1 0-7-7a7 7 0 0 0 7 7zm2.985-7h-5.97"></svg:path>`,
})
export class GrommetIconsInsecureIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInspectIcon],svg[grommet-icons-inspect-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5.5 21a4.5 4.5 0 1 0 0-9a4.5 4.5 0 0 0 0 9ZM1 16V6.5A4.5 4.5 0 0 1 5.5 2H6m17 14V6.5A4.5 4.5 0 0 0 18.5 2H18m.5 19a4.5 4.5 0 1 0 0-9a4.5 4.5 0 0 0 0 9ZM10 17s0-2 2-2s2 2 2 2"></svg:path>`,
})
export class GrommetIconsInspectIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInstagramIcon],svg[grommet-icons-instagram-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M17.318.077c1.218.056 2.06.235 2.838.537a5.4 5.4 0 0 1 1.956 1.274a5.4 5.4 0 0 1 1.274 1.956c.302.779.481 1.62.537 2.838C23.992 8.192 24 8.724 24 12s-.008 3.808-.077 5.318c-.056 1.218-.235 2.06-.537 2.839a5.4 5.4 0 0 1-1.274 1.955a5.4 5.4 0 0 1-1.956 1.274c-.779.302-1.62.481-2.838.537c-1.51.069-2.041.077-5.318.077s-3.809-.008-5.318-.077c-1.218-.056-2.06-.235-2.839-.537a5.4 5.4 0 0 1-1.955-1.274a5.4 5.4 0 0 1-1.274-1.956c-.302-.779-.481-1.62-.537-2.838C.008 15.81 0 15.278 0 12s.008-3.81.077-5.318c.056-1.218.235-2.06.537-2.838a5.4 5.4 0 0 1 1.274-1.956A5.4 5.4 0 0 1 3.843.614C4.623.312 5.464.133 6.682.077C8.19.008 8.722 0 12 0s3.81.008 5.318.077M12 2.667c-3.24 0-3.736.007-5.197.074c-.927.042-1.483.16-1.994.359c-.435.17-.712.35-1.036.673S3.27 4.374 3.1 4.81c-.198.51-.317 1.067-.359 1.994c-.067 1.46-.074 1.956-.074 5.196s.007 3.736.074 5.197c.042.927.16 1.483.359 1.993c.17.436.35.713.673 1.037s.601.504 1.036.673c.51.198 1.067.317 1.994.359c1.462.067 1.958.074 5.197.074c3.24 0 3.735-.007 5.197-.074c.927-.042 1.483-.16 1.994-.359c.435-.17.712-.35 1.036-.673s.504-.601.673-1.036c.198-.51.317-1.067.359-1.994c.067-1.462.074-1.958.074-5.197s-.007-3.735-.074-5.197c-.042-.927-.16-1.483-.359-1.993a2.7 2.7 0 0 0-.673-1.037A2.7 2.7 0 0 0 19.19 3.1c-.51-.198-1.067-.317-1.994-.359c-1.461-.067-1.957-.074-5.197-.074m0 15.555a6.222 6.222 0 1 1 0-12.444a6.222 6.222 0 0 1 0 12.444m0-2.666a3.556 3.556 0 1 0 0-7.112a3.556 3.556 0 0 0 0 7.112m6.222-8.445a1.333 1.333 0 1 1 0-2.667a1.333 1.333 0 0 1 0 2.667"></svg:path>`,
})
export class GrommetIconsInstagramIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInstallIcon],svg[grommet-icons-install-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 13.5v4L12 22l-7-4.5v-4m7 8.5v-8.5m6.5-5l-6.5-4L15.5 2L22 6zm-13 0l6.5-4L8.5 2L2 6zm13 .5L12 13l3.5 2.5l6.5-4zm-13 0l6.5 4l-3.5 2.5l-6.5-4z"></svg:path>`,
})
export class GrommetIconsInstallIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInstallOptionIcon],svg[grommet-icons-install-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 6v10zm0-5c6.075 0 11 4.925 11 11s-4.925 11-11 11S1 18.075 1 12S5.925 1 12 1Zm5 11l-5 5l-5-5"></svg:path>`,
})
export class GrommetIconsInstallOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsIntegrationIcon],svg[grommet-icons-integration-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 21h18V9H5m14 6H1V3h18"></svg:path>`,
})
export class GrommetIconsIntegrationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsInternetExplorerIcon],svg[grommet-icons-internet-explorer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M24 12.337c0-1.898-.491-3.68-1.351-5.229c3.666-8.298-3.929-7.083-4.352-7c-1.609.315-3.097.82-4.47 1.461a11 11 0 0 0-.612-.017C8.09 1.552 3.8 5.126 2.702 9.918c2.702-3.03 4.592-4.253 5.724-4.742q-.27.242-.532.489l-.17.165q-.173.164-.342.332l-.196.2q-.15.151-.295.304l-.204.216a27 27 0 0 0-1.15 1.31a31 31 0 0 0-.41.505l-.2.253l-.183.24l-.206.272l-.14.193a34 34 0 0 0-1.168 1.7l-.002.003q-.139.217-.27.428l-.014.023q-.132.211-.255.418l-.009.014q-.333.557-.613 1.063c-.971 1.76-1.444 2.99-1.464 3.063c-3.068 10.966 6.505 6.335 7.841 5.644a10.74 10.74 0 0 0 4.77 1.11c4.69 0 8.68-2.993 10.165-7.173h-5.666c-.839 1.416-2.453 2.376-4.308 2.376c-2.717 0-4.92-2.059-4.92-4.598h15.426q.089-.683.089-1.39M21.985 1.724c.929.627 1.674 1.61.394 4.926a10.82 10.82 0 0 0-5.267-4.372c.998-.482 3.47-1.501 4.873-.554M2.248 21.989c-.757-.776-.89-2.665.779-6.108a10.82 10.82 0 0 0 4.696 5.739c-1.08.595-3.95 1.934-5.475.369M8.46 10.776c.086-2.468 2.235-4.444 4.874-4.444c2.64 0 4.787 1.976 4.874 4.444z"></svg:path>`,
})
export class GrommetIconsInternetExplorerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsItalicIcon],svg[grommet-icons-italic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="m13 19.56l-.13.43H6.3l.16-.43a4 4 0 0 0 1.3-.17a1.6 1.6 0 0 0 .76-.55a7.2 7.2 0 0 0 .8-2l2.77-9.61a7 7 0 0 0 .35-1.81a.86.86 0 0 0-.15-.52a.94.94 0 0 0-.46-.32a4.3 4.3 0 0 0-1.22-.11l.14-.43h6.16l-.13.43a2.6 2.6 0 0 0-1.12.17a1.8 1.8 0 0 0-.81.67a9 9 0 0 0-.71 1.93l-2.74 9.63a9 9 0 0 0-.4 1.69a.83.83 0 0 0 .15.5a.92.92 0 0 0 .47.32a6.4 6.4 0 0 0 1.38.18"></svg:path>`,
})
export class GrommetIconsItalicIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsIterationIcon],svg[grommet-icons-iteration-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 9v14h14M5 5v14h14M9 15h14V1H9z"></svg:path>`,
})
export class GrommetIconsIterationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsJavaIcon],svg[grommet-icons-java-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 18V9h16v9c0 4-2 5-8 5s-8-1-8-5Zm16-9v3a3 3 0 1 0 3-3zm-4-3V2M5 6V4m4 2V0"></svg:path>`,
})
export class GrommetIconsJavaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsJsIcon],svg[grommet-icons-js-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" fill-rule="evenodd"><svg:path fill="currentColor" d="M0 0h24v24H0z"></svg:path><svg:path stroke="currentColor" stroke-width="2" d="M12 11v8c0 .876-.523 2-2 2c-2.385 0-2.5-2-2.5-2m13.29-5.484q-.9-1.515-2.386-1.516C16.856 12 16 13 16 14s.5 2 2.508 2.5c1.278.318 2.492 1 2.492 2.5s-1.315 2-2.5 2q-2.271 0-3-2"></svg:path></svg:g>`,
})
export class GrommetIconsJsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsKeyIcon],svg[grommet-icons-key-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 13v3h3v3h3v2l2 2h5v-4L12.74 8.74C12.91 8.19 13 7.6 13 7c0-3.31-2.69-6-6-6S1 3.69 1 7a6.005 6.005 0 0 0 8.47 5.47zM6 7a1 1 0 1 1 0-2a1 1 0 0 1 0 2Z"></svg:path>`,
})
export class GrommetIconsKeyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsKeyboardIcon],svg[grommet-icons-keyboard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 9h3m-3 3h2m-2 3h1m3 0h10m1 0h1m1 0h1m-3-3h3m-2-3h2m-5 0h2M7 9h2m1 0h2m1 0h2M5 15h1m0-3h2m1 0h2m1 0h2m1 0h2M1 7v10a1 1 0 0 0 1 1h20a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1z"></svg:path>`,
})
export class GrommetIconsKeyboardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsKubernetesIcon],svg[grommet-icons-kubernetes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsKubernetes0)"><svg:path fill="currentColor" d="M14.27 13.385h-.268c-.21.129-.302.373-.21.582l1.002 2.457a5.15 5.15 0 0 0 2.084-2.62l-2.596-.419zm-4.016.338a.48.48 0 0 0-.443-.338h-.093l-2.584.431a5.14 5.14 0 0 0 2.084 2.596l.99-2.41v-.035c.046-.058.046-.162.046-.244m2.13.943a.44.44 0 0 0-.594-.175c-.081.059-.14.094-.174.175h-.012l-1.269 2.293a5.2 5.2 0 0 0 2.83.14c.162-.035.337-.081.5-.14l-1.27-2.293h-.01Zm4.017-5.32l-1.956 1.711l.011.035a.43.43 0 0 0-.046.617q.087.104.21.14l.011.011l2.526.722c.082-1.13-.163-2.27-.756-3.236m-3.621.186c.012.244.21.43.454.419a.54.54 0 0 0 .245-.058h.011l2.13-1.525a5.2 5.2 0 0 0-2.991-1.444zm-2.258.36a.44.44 0 0 0 .605-.104a.45.45 0 0 0 .093-.244h.012l.14-2.62c-.175.023-.35.058-.536.093a5.06 5.06 0 0 0-2.468 1.35l2.142 1.526h.011ZM9.368 11.86a.426.426 0 0 0 .303-.535c0-.093-.059-.163-.128-.221v-.012L7.588 9.31a5.2 5.2 0 0 0-.734 3.283l2.515-.722zm1.91.769l.721.349l.722-.35l.174-.78l-.5-.616h-.804l-.5.617l.186.78Zm12.677 1.536l-2.06-8.929a1.59 1.59 0 0 0-.862-1.094L12.687.148a1.7 1.7 0 0 0-1.385 0L2.966 4.142c-.431.21-.757.605-.862 1.094l-2.06 8.93a1.4 1.4 0 0 0 0 .686c.011.07.035.14.058.21c.035.105.093.221.151.314c.035.047.058.093.105.128L6.121 22.7c.023 0 .058.047.058.07c.116.105.221.186.326.256c.14.093.303.163.466.198c.128.058.267.058.372.058h9.454c.081 0 .162-.035.232-.058c.059-.012.117-.035.163-.047c.047-.023.082-.035.128-.058c.059-.023.117-.058.175-.093c.14-.093.268-.21.384-.326l.175-.233l5.588-6.962c.116-.14.198-.29.256-.442a1.4 1.4 0 0 0 .058-.21a1.4 1.4 0 0 0 0-.687Zm-8.65 3.481c.024.07.047.14.082.198a.4.4 0 0 0-.035.303c.14.28.268.536.442.792c.093.128.187.267.28.395c0 .035.035.094.046.14a.5.5 0 0 1-.174.687a.495.495 0 0 1-.687-.175c-.012-.035-.024-.058-.035-.093c-.023-.035-.047-.105-.07-.105a5 5 0 0 1-.14-.477c-.104-.29-.198-.57-.349-.838a.44.44 0 0 0-.245-.163l-.093-.186a6.52 6.52 0 0 1-4.621-.012l-.117.21a.4.4 0 0 0-.221.104c-.163.28-.28.57-.384.897a4 4 0 0 1-.14.465c-.023 0-.047.082-.07.117a.501.501 0 0 1-.943-.338c.012-.035.035-.058.047-.093c.046-.035.046-.093.046-.128c.105-.14.187-.268.28-.407c.186-.245.337-.524.454-.804a.6.6 0 0 0-.035-.29l.081-.21a6.53 6.53 0 0 1-2.875-3.598l-.233.035a.45.45 0 0 0-.268-.104a6 6 0 0 0-.896.256c-.128.07-.28.128-.431.174a1 1 0 0 1-.151.035a.51.51 0 0 1-.63-.314a.504.504 0 0 1 .327-.64c.023 0 .058-.012.093-.012v-.012h.012l.128-.023c.163-.047.326-.047.477-.047c.303 0 .605-.07.896-.14a.7.7 0 0 0 .222-.22l.22-.059a6.35 6.35 0 0 1 1.002-4.505l-.163-.14a.5.5 0 0 0-.093-.29a5 5 0 0 0-.745-.525q-.21-.105-.42-.244c-.022-.024-.07-.059-.092-.082l-.012-.011c-.233-.187-.291-.49-.128-.734a.53.53 0 0 1 .407-.175a.66.66 0 0 1 .35.14l.104.082c.117.104.222.233.326.349c.21.221.431.43.676.605a.45.45 0 0 0 .302.035l.175.128a6.4 6.4 0 0 1 3.26-1.862c.29-.07.605-.117.908-.14l.011-.21a.52.52 0 0 0 .163-.267a4.6 4.6 0 0 0-.058-.897a3 3 0 0 1-.07-.477v-.14a.517.517 0 0 1 .454-.559a.497.497 0 0 1 .548.443v.256a3 3 0 0 1-.07.477c-.047.291-.07.594-.058.897a.4.4 0 0 0 .163.256l.011.22A6.5 6.5 0 0 1 16.692 7.6l.186-.14a.45.45 0 0 0 .303-.034a4.2 4.2 0 0 0 .675-.606c.105-.116.21-.233.326-.349c.035-.023.081-.07.116-.07c.198-.21.513-.21.687 0c.221.186.21.5 0 .699c0 .023-.035.046-.07.07a3 3 0 0 1-.512.326a4.3 4.3 0 0 0-.745.523a.36.36 0 0 0-.093.28l-.186.163a6.33 6.33 0 0 1 1.024 4.494l.221.058a.5.5 0 0 0 .222.21c.29.08.593.127.896.162h.477c.035.035.093.047.14.059a.5.5 0 0 1 .43.57c-.058.268-.279.466-.558.43c-.035-.01-.082-.01-.082-.023v-.011a.6.6 0 0 1-.163-.023a2.3 2.3 0 0 1-.419-.175a4 4 0 0 0-.896-.245c-.105 0-.198 0-.268.094l-.221-.035c-.477 1.525-1.525 2.805-2.876 3.62Z"></svg:path></svg:g><svg:defs><svg:clippath id="grommetIconsKubernetes0"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsKubernetesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLanguageIcon],svg[grommet-icons-language-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 23c6.075 0 11-4.925 11-11S18.075 1 12 1S1 5.925 1 12s4.925 11 11 11Zm0 0c3 0 4-5 4-11S15 1 12 1S8 6 8 12s1 11 4 11ZM2 16h20M2 8h20"></svg:path>`,
})
export class GrommetIconsLanguageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLastfmIcon],svg[grommet-icons-lastfm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M20.297 10.924q-.315-.102-.609-.194c-1.494-.47-2.394-.753-2.394-1.916c0-.943.727-1.627 1.73-1.627c.769 0 1.341.319 1.855 1.038c.048.067.138.09.213.05l1.507-.768a.16.16 0 0 0 .084-.101a.17.17 0 0 0-.014-.131c-.807-1.435-1.972-2.133-3.56-2.133c-2.417 0-3.98 1.462-3.98 3.725c0 2.314 1.511 3.25 4.298 4.168c1.616.538 2.33.824 2.33 1.973c0 1.291-1.165 2.22-2.755 2.166c-1.666-.056-2.17-.94-2.806-2.386a720 720 0 0 1-2.307-5.337C12.662 6.621 10.232 5 7.22 5C3.239 5 0 8.239 0 12.22s3.238 7.219 7.219 7.219c2.17 0 4.206-.962 5.582-2.641a.17.17 0 0 0 .025-.173l-.91-2.1a.17.17 0 0 0-.149-.1a.17.17 0 0 0-.154.09a4.95 4.95 0 0 1-4.395 2.66a4.96 4.96 0 0 1-4.954-4.955a4.96 4.96 0 0 1 4.954-4.955c1.989 0 3.81 1.18 4.534 2.941l2.251 5.134l.26.577c1.017 2.37 2.512 3.432 4.854 3.44c2.784 0 4.883-1.844 4.883-4.29c0-2.457-1.358-3.378-3.703-4.143"></svg:path>`,
})
export class GrommetIconsLastfmIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLaunchIcon],svg[grommet-icons-launch-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 9v5s-3 2-3 5v1h4l2 3h4l2-3h4v-1c0-3-3-5-3-5V9c0-4-3-8-5-8S7 5 7 9Zm1 11h8M12 9a1 1 0 1 0 0-2a1 1 0 0 0 0 2Z"></svg:path>`,
})
export class GrommetIconsLaunchIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLayerIcon],svg[grommet-icons-layer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h16v16H1zm19 6h3v16H7v-3"></svg:path>`,
})
export class GrommetIconsLayerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLicenseIcon],svg[grommet-icons-license-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 13v3h3v3h3v2l2 2h5v-4L12.74 8.74C12.91 8.19 13 7.6 13 7c0-3.31-2.69-6-6-6S1 3.69 1 7a6.005 6.005 0 0 0 8.47 5.47zM6 7a1 1 0 1 1 0-2a1 1 0 0 1 0 2Z"></svg:path>`,
})
export class GrommetIconsLicenseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLikeIcon],svg[grommet-icons-like-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 23h19q3 0 3-3V10h-7V4q0-3-3-3h-2s-.016 6-.016 7.326S10 11 8 11H1zm5 0V11"></svg:path>`,
})
export class GrommetIconsLikeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLikeFillIcon],svg[grommet-icons-like-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M11 0h-.997L10 .997L11 1l-1-.003v.261l-.002.698l-.006 2.207l-.008 4.163c0 .42-.155.83-.454 1.132C9.243 9.749 8.763 10 8 10H1.001L1 11v-1H0v14h1v-1v1h19c1.154 0 2.203-.29 2.957-1.043C23.711 22.203 24 21.154 24 20v-9.999L23 10h1V9h-7V4c0-1.154-.29-2.203-1.043-2.957C15.203.289 14.154 0 13 0zM5 12v10H2V12z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsLikeFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLineChartIcon],svg[grommet-icons-line-chart-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m1 16l7-7l5 5L23 4M0 22h23.999M16 4h7v7"></svg:path>`,
})
export class GrommetIconsLineChartIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkIcon],svg[grommet-icons-link-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16.125 2.42a2.01 2.01 0 0 1 2.84-.001l2.616 2.617a2 2 0 0 1-.001 2.839l-3.705 3.705a2.01 2.01 0 0 1-2.84.001L12.42 8.964a2 2 0 0 1 .001-2.839zm-10 10a2.01 2.01 0 0 1 2.84-.001l2.616 2.617a2 2 0 0 1-.001 2.839L7.875 21.58a2.01 2.01 0 0 1-2.84.001L2.42 18.964a2 2 0 0 1 .001-2.839zM7 17L17 7"></svg:path>`,
})
export class GrommetIconsLinkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkBottomIcon],svg[grommet-icons-link-bottom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 19V1M4 11l8 8l8-8M2 22h20"></svg:path>`,
})
export class GrommetIconsLinkBottomIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkDownIcon],svg[grommet-icons-link-down-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22V2M3 13l9 9l9-9"></svg:path>`,
})
export class GrommetIconsLinkDownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkNextIcon],svg[grommet-icons-link-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 12h20m-9-9l9 9l-9 9"></svg:path>`,
})
export class GrommetIconsLinkNextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkPreviousIcon],svg[grommet-icons-link-previous-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 12H2m9-9l-9 9l9 9"></svg:path>`,
})
export class GrommetIconsLinkPreviousIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkTopIcon],svg[grommet-icons-link-top-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 5v18M4 13l8-8l8 8M2 2h20"></svg:path>`,
})
export class GrommetIconsLinkTopIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkUpIcon],svg[grommet-icons-link-up-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 2v20M3 11l9-9l9 9"></svg:path>`,
})
export class GrommetIconsLinkUpIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkedinIcon],svg[grommet-icons-linkedin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M20.452 20.45h-3.56v-5.57c0-1.328-.022-3.036-1.85-3.036c-1.851 0-2.134 1.447-2.134 2.942v5.664H9.352V8.997h3.413v1.566h.049c.475-.9 1.636-1.85 3.367-1.85c3.605 0 4.27 2.371 4.27 5.456zM5.339 7.433a2.063 2.063 0 1 1 0-4.13a2.065 2.065 0 0 1 0 4.13M7.12 20.45H3.558V8.997H7.12zM23 0H1a1 1 0 0 0-1 1v22a1 1 0 0 0 1 1h22a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1"></svg:path>`,
})
export class GrommetIconsLinkedinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLinkedinOptionIcon],svg[grommet-icons-linkedin-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M22.037 22h-4.152v-6.496c0-1.55-.026-3.542-2.157-3.542c-2.16 0-2.49 1.688-2.49 3.43V22H9.09V8.64h3.98v1.827h.058c.553-1.05 1.908-2.158 3.928-2.158c4.204 0 4.98 2.766 4.98 6.364zM4.409 6.816A2.407 2.407 0 0 1 2 4.407a2.408 2.408 0 1 1 2.41 2.408M6.486 22H2.33V8.64h4.156z"></svg:path>`,
})
export class GrommetIconsLinkedinOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsListIcon],svg[grommet-icons-list-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M9 6h12M9 12h12M9 18h8M4 7a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 6a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 6a1 1 0 1 0 0-2a1 1 0 0 0 0 2Z"></svg:path>`,
})
export class GrommetIconsListIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLocalIcon],svg[grommet-icons-local-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 4h22v16H1zm10 4h12M1 16h22M1 12h22M11 4v8"></svg:path>`,
})
export class GrommetIconsLocalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLocationIcon],svg[grommet-icons-location-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22s-8-6-8-12c0-5 4-8 8-8s8 3 8 8c0 6-8 12-8 12Zm0-9a3 3 0 1 0 0-6a3 3 0 0 0 0 6Z"></svg:path>`,
})
export class GrommetIconsLocationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLocationPinIcon],svg[grommet-icons-location-pin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 10a4 4 0 1 0 0-8a4 4 0 0 0 0 8Zm0 0v12"></svg:path>`,
})
export class GrommetIconsLocationPinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLockIcon],svg[grommet-icons-lock-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 23V11H5v12zm-7-8v4m5-8V7c0-3 0-6-5-6S7 4 7 7v4"></svg:path>`,
})
export class GrommetIconsLockIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLoginIcon],svg[grommet-icons-login-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 15v7h13V2H9v7m9 3H0m13-5l5 5l-5 5"></svg:path>`,
})
export class GrommetIconsLoginIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLogoutIcon],svg[grommet-icons-logout-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M13 9V2H1v20h12v-7m9-3H5m12-5l5 5l-5 5"></svg:path>`,
})
export class GrommetIconsLogoutIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsLoungeIcon],svg[grommet-icons-lounge-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 5.997C5 5.447 5.45 5 6.007 5h11.986C18.55 5 19 5.453 19 5.997V13H5zM22 8v7.003c0 .55-.455.997-.992.997H2.992A1 1 0 0 1 2 15.003V8m3 8v2zm14 0v2z"></svg:path>`,
})
export class GrommetIconsLoungeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMagicIcon],svg[grommet-icons-magic-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2.5 19.5l17-17l2 2l-17 17zm.5.5L15 8l1 1L4 21zM5.5 3l-.5.5l.5.5l.5-.5zm6 0l-.5.5l.5.5l.5-.5zm-3 3l-.5.5l.5.5l.5-.5zm12 6l-.5.5l.5.5l.5-.5zm0 5l-.5.5l.5.5l.5-.5z"></svg:path>`,
})
export class GrommetIconsMagicIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMailIcon],svg[grommet-icons-mail-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M23 20V6l-11 9L1 6v14zm-11-8l10-8H2z"></svg:path>`,
})
export class GrommetIconsMailIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMailOptionIcon],svg[grommet-icons-mail-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 4h22v16H1zm0 1l11 8.5L23 5"></svg:path>`,
})
export class GrommetIconsMailOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMandrivaIcon],svg[grommet-icons-mandriva-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M14.557 17.2c-3.021 2.72-6.354 4.146-9.075 4.148h-.008c-1.938-.001-3.565-.726-4.544-2.222C-1.43 15.519.858 8.797 6.042 4.11Q6.703 3.514 7.38 3C3.038 7.582 1.269 13.546 3.443 16.868c1.952 2.983 6.474 2.899 10.912.13l-1.323-1.332c-.157-.157-.428-.258-.692-.258a.8.8 0 0 0-.292.05l-4.279 1.656a.8.8 0 0 1-.275.06c-.176 0-.259-.093-.292-.148c-.052-.085-.087-.24.042-.495l2.094-4.082c.148-.29.1-.748-.105-1L6.336 7.892c-.195-.239-.18-.406-.134-.504c.033-.07.126-.19.372-.19q.072 0 .155.015l4.528.729a1 1 0 0 0 .11.008c.307 0 .655-.18.809-.417l2.487-3.853c.152-.237.302-.287.4-.287c.09 0 .306.044.368.449l.706 4.533c.05.322.358.664.672.747l4.434 1.175c.332.088.399.259.408.352c.01.094-.019.274-.325.43l-4.094 2.07c-.29.147-.52.545-.502.87l.253 4.582q.016.29-.113.428a.3.3 0 0 1-.217.093q-.185 0-.387-.203zM24 11.403c-1.245-.672-1.423-.64-2.355.424c.671-1.244.639-1.423-.425-2.356c1.245.672 1.424.64 2.356-.424c-.672 1.245-.64 1.424.424 2.356"></svg:path>`,
})
export class GrommetIconsMandrivaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsManualIcon],svg[grommet-icons-manual-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 9v8zm-4 0v8zM8 5a4 4 0 1 0 8 0a4 4 0 0 0-8 0ZM4 23h16v-3H4zm3-3h10v-3H7z"></svg:path>`,
})
export class GrommetIconsManualIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMapIcon],svg[grommet-icons-map-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M15 15h4l3 7H2l3-7h4m4-7a1 1 0 1 1-2 0a1 1 0 0 1 2 0M6 8c0 5 6 10 6 10s6-5 6-10c0-3.417-2.686-6-6-6S6 4.583 6 8Z"></svg:path>`,
})
export class GrommetIconsMapIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMapLocationIcon],svg[grommet-icons-map-location-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17.5 6.5L23 9v13l-7-3l-8 3l-7-3V6l5 2m10 11v-7M8 22V12m4 4.273S6 11.5 6 7c0-3.75 3-6 6-6s6 2.25 6 6c0 4.5-6 9.273-6 9.273ZM13 7a1 1 0 1 0-2 0a1 1 0 0 0 2 0Z"></svg:path>`,
})
export class GrommetIconsMapLocationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMastercardIcon],svg[grommet-icons-mastercard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:circle cx="7" cy="12" r="7"></svg:circle><svg:circle cx="17" cy="12" r="7" fill-opacity=".8"></svg:circle></svg:g>`,
})
export class GrommetIconsMastercardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMediumIcon],svg[grommet-icons-medium-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M6.783 5a6.783 6.783 0 1 0 0 13.565A6.783 6.783 0 0 0 6.783 5m10.695.522c-1.873 0-3.391 2.803-3.391 6.26c0 .65.053 1.274.152 1.862c.132.784.346 1.502.622 2.121q.207.466.46.849c.335.51.724.904 1.149 1.148c.319.183.657.282 1.008.282s.69-.1 1.009-.282c.425-.244.814-.638 1.149-1.148q.251-.384.46-.849c.276-.618.49-1.337.621-2.12c.1-.589.153-1.214.153-1.862c0-3.458-1.519-6.261-3.392-6.261m5.218.521c-.136 0-.266.09-.388.259q-.123.167-.234.434a4 4 0 0 0-.107.288q-.105.31-.193.7c-.118.52-.214 1.138-.28 1.825a23 23 0 0 0-.103 2.234a25 25 0 0 0 .103 2.234c.066.686.162 1.305.28 1.824q.088.391.193.7q.051.156.107.288q.112.266.234.435c.122.168.253.258.388.258c.72 0 1.304-2.57 1.304-5.74c0-3.169-.584-5.739-1.304-5.739"></svg:path>`,
})
export class GrommetIconsMediumIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMemoryIcon],svg[grommet-icons-memory-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 18h4m-4-4h4m-4-4h4m-4-4h4m6 12h3m-3-4h3m-3-4h3m-3-4h3M1 18h3m-3-4h3m-3-4h3M1 6h3m11 14h4a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1zM5 20h4a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1z"></svg:path>`,
})
export class GrommetIconsMemoryIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMenuIcon],svg[grommet-icons-menu-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 19h20M2 5h20M2 12h20"></svg:path>`,
})
export class GrommetIconsMenuIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMetaIcon],svg[grommet-icons-meta-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M2.562 14.676c0-3.929 1.965-8.028 4.27-8.028c1.282 0 2.307.768 3.93 3.074a1703 1703 0 0 1-2.477 3.844c-2.05 3.245-2.733 3.929-3.844 3.929c-1.11.085-1.879-.94-1.879-2.819m13.41-1.452l-1.452-2.391c-.342-.598-.77-1.196-1.11-1.708c1.28-1.965 2.305-2.99 3.586-2.99c2.563 0 4.613 3.844 4.613 8.627c0 1.793-.598 2.818-1.794 2.818s-1.623-.769-3.844-4.356Zm-3.673-5.808C10.419 4.94 8.797 4 6.919 4C2.988 4 0 9.21 0 14.676c0 3.417 1.623 5.552 4.356 5.552c1.964 0 3.33-.94 5.893-5.381c0 0 1.025-1.879 1.794-3.16c.256.427.512.854.768 1.366l1.196 2.05c2.306 3.929 3.587 5.21 5.893 5.21c2.648 0 4.1-2.22 4.1-5.722C23.915 8.783 20.84 4 17.167 4c-1.964 0-3.502 1.537-4.868 3.416"></svg:path>`,
})
export class GrommetIconsMetaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMicrofocusIcon],svg[grommet-icons-microfocus-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M1 5h4v14h14v4H1zm4-4h18v18h-4V5H5z"></svg:path>`,
})
export class GrommetIconsMicrofocusIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMicrophoneIcon],svg[grommet-icons-microphone-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 11c0 3 1.79 4 4 4s4-1 4-4V5c0-3-1.79-4-4-4S8 2 8 5zM4 9v2c0 5 3.582 8 8 8s8-3 8-8V9m-8 15v-5"></svg:path>`,
})
export class GrommetIconsMicrophoneIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMoneyIcon],svg[grommet-icons-money-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16 16c0-1.105-3.134-2-7-2s-7 .895-7 2s3.134 2 7 2s7-.895 7-2ZM2 16v4.937C2 22.077 5.134 23 9 23s7-.924 7-2.063V16M9 5c-4.418 0-8 .895-8 2s3.582 2 8 2M1 7v5c0 1.013 3.582 2 8 2M23 4c0-1.105-3.1-2-6.923-2s-6.923.895-6.923 2s3.1 2 6.923 2S23 5.105 23 4Zm-7 12c3.824 0 7-.987 7-2V4M9.154 4v10.166M9 9c0 1.013 3.253 2 7.077 2S23 10.013 23 9"></svg:path>`,
})
export class GrommetIconsMoneyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMonitorIcon],svg[grommet-icons-monitor-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 16h22V2H1zm4 6h14zm4 0h6v-6H9z"></svg:path>`,
})
export class GrommetIconsMonitorIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMonospaceIcon],svg[grommet-icons-monospace-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M11.88 17h-2.3l-.93-2.9H4.44L3.57 17H1.32L5.41 4.17h2.25ZM8 11.93L6.52 7.17l-1.43 4.76ZM14.13 17L12.22 4.17h1.66L15.07 13l1.46-8.82h1.92l1.4 9l1.23-9h1.62L20.78 17h-1.72l-1.6-9.6l-1.58 9.6Zm-2.77.95v1.39H1.89v-1.39h-.51v1.91h10.49v-1.91zm10.81.05v1.39h-9.48V18h-.5v1.91h10.49V18z"></svg:path>`,
})
export class GrommetIconsMonospaceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMoonIcon],svg[grommet-icons-moon-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="2" d="M9.874 5.008c2.728-1.68 6.604-1.014 8.25.197c-2.955.84-5.11 3.267-5.242 6.415c-.18 4.28 3.006 6.588 5.24 7.152c-1.964 1.343-4.36 1.293-5.235 1.172c-3.568-.492-6.902-3.433-7.007-7.711c-.106-4.278 2.573-6.35 3.994-7.225z"></svg:path>`,
})
export class GrommetIconsMoonIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMoreIcon],svg[grommet-icons-more-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 13v-2h2v2zm8 0v-2h2v2zm8 0v-2h2v2z"></svg:path>`,
})
export class GrommetIconsMoreIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMoreVerticalIcon],svg[grommet-icons-more-vertical-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M14 14h-4v-4h4zm0-7h-4V3h4zm0 14h-4v-4h4z"></svg:path>`,
})
export class GrommetIconsMoreVerticalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMouseIcon],svg[grommet-icons-mouse-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 4a5 5 0 0 1 5 5v6a5 5 0 0 1-10 0V9a5 5 0 0 1 5-5zm0 0v6m-6 0h12"></svg:path>`,
})
export class GrommetIconsMouseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMultimediaIcon],svg[grommet-icons-multimedia-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2 8v14h20V9M11 4L7 8M2 4v4h15l4-4zm14 0l-4 4"></svg:path>`,
})
export class GrommetIconsMultimediaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMultipleIcon],svg[grommet-icons-multiple-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 15h4V1H9v4m6 14h4V5H5v4M1 23h14V9H1z"></svg:path>`,
})
export class GrommetIconsMultipleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMusicIcon],svg[grommet-icons-music-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 17.998C1 16.894 1.887 16 2.998 16H9v4.002A1.993 1.993 0 0 1 7.002 22H2.998A2 2 0 0 1 1 20.002zm14 0c0-1.104.887-1.998 1.998-1.998H23v4.002A1.993 1.993 0 0 1 21.002 22h-4.004A2 2 0 0 1 15 20.002zM9 16V2h14v13.5M9 6h14"></svg:path>`,
})
export class GrommetIconsMusicIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsMysqlIcon],svg[grommet-icons-mysql-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M5.462 4.04a2.7 2.7 0 0 0-.67.074v.038h.037c.13.267.36.44.521.67l.372.781l.038-.037c.23-.162.336-.422.335-.819c-.093-.097-.107-.219-.187-.335c-.106-.154-.312-.242-.446-.372m18.017 19.097c.175.129.293.329.521.41v-.038c-.12-.152-.15-.362-.26-.521q-.243-.24-.484-.484a8 8 0 0 0-1.713-1.638c-.508-.365-1.649-.859-1.861-1.451l-.038-.038c.361-.04.784-.171 1.117-.26c.56-.15 1.06-.112 1.638-.26l.782-.224v-.15c-.292-.3-.5-.696-.819-.967c-.834-.71-1.743-1.42-2.68-2.01c-.52-.329-1.162-.541-1.713-.82c-.185-.093-.51-.142-.632-.297c-.29-.37-.447-.837-.67-1.266c-.467-.9-.927-1.883-1.34-2.83c-.283-.645-.467-1.281-.82-1.86c-1.69-2.78-3.51-4.457-6.328-6.106c-.6-.35-1.322-.489-2.084-.67l-1.229-.074c-.25-.105-.51-.41-.744-.559C3.188.434.792-.849.102.838c-.437 1.065.652 2.104 1.042 2.643c.273.379.623.803.819 1.229c.128.28.15.56.26.856c.271.73.506 1.522.856 2.196c.178.341.373.7.596 1.005c.138.187.372.27.409.559c-.23.321-.242.82-.371 1.228c-.582 1.835-.363 4.115.484 5.473c.259.416.87 1.31 1.711.967c.736-.3.572-1.228.782-2.047c.047-.186.019-.323.112-.447v.037l.67 1.34c.496.799 1.376 1.634 2.122 2.197c.386.292.69.797 1.191.968v-.038h-.037c-.098-.15-.25-.213-.372-.335a8.6 8.6 0 0 1-.857-.968c-.678-.92-1.277-1.928-1.823-2.977c-.261-.502-.488-1.054-.708-1.564c-.085-.197-.084-.494-.26-.596c-.241.374-.596.676-.782 1.117c-.298.705-.337 1.565-.447 2.457c-.065.023-.036.007-.075.037c-.518-.125-.7-.659-.893-1.117c-.487-1.157-.578-3.022-.149-4.355c.111-.345.613-1.431.41-1.75c-.098-.318-.417-.501-.596-.744A6 6 0 0 1 3.6 7.166c-.398-.902-.585-1.916-1.005-2.829c-.2-.436-.54-.877-.819-1.265c-.308-.43-.654-.746-.893-1.266c-.085-.185-.201-.48-.075-.67c.04-.128.097-.182.224-.223c.216-.167.817.055 1.042.148c.597.248 1.095.484 1.6.82c.243.16.489.472.782.558h.335c.525.12 1.112.037 1.601.186c.865.263 1.64.672 2.345 1.117c2.146 1.355 3.9 3.283 5.1 5.584c.193.37.277.724.447 1.117c.343.792.775 1.607 1.116 2.382c.34.773.673 1.553 1.154 2.196c.253.338 1.231.52 1.676.708c.311.131.821.269 1.116.446c.564.34 1.11.745 1.638 1.117c.264.187 1.077.595 1.117.93c-1.31-.034-2.31.087-3.164.448c-.243.102-.63.105-.67.409c.133.14.154.35.26.521c.204.33.549.773.856 1.005c.337.254.683.525 1.043.745c.64.39 1.356.614 1.972 1.005c.365.231.726.521 1.08.782"></svg:path>`,
})
export class GrommetIconsMysqlIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNavigateIcon],svg[grommet-icons-navigate-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m20 11l2-3l-2-3h-8v6zm-8 13V0M4 2L2 5l2 3h8V2z"></svg:path>`,
})
export class GrommetIconsNavigateIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNetworkIcon],svg[grommet-icons-network-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7.5 7v8.514c0 4.243 5.5 2.829 5.5 6.6V24m-2-14L7.5 7L4 10m12.5-8v8.44c0 4.068-3.5 2.712-3.5 6.328V24m0-19l3.5-3L20 5"></svg:path>`,
})
export class GrommetIconsNetworkIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNetworkDriveIcon],svg[grommet-icons-network-drive-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 14v4M22 6v6a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h18a1 1 0 0 1 1 1zM12 21a2 2 0 1 0 0-4a2 2 0 0 0 0 4zM6 10a1 1 0 1 0 0-2a1 1 0 0 0 0 2z"></svg:path>`,
})
export class GrommetIconsNetworkDriveIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNewIcon],svg[grommet-icons-new-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 1v22M2 6l20 12m0-12L2 18"></svg:path>`,
})
export class GrommetIconsNewIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNewWindowIcon],svg[grommet-icons-new-window-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M11 9h8m-4 4V5m2 12v6H1V7h6m0-6h16v16H7z"></svg:path>`,
})
export class GrommetIconsNewWindowIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNextIcon],svg[grommet-icons-next-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m7 2l10 10L7 22"></svg:path>`,
})
export class GrommetIconsNextIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNodeIcon],svg[grommet-icons-node-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M11.899 24c-.322 0-.64-.084-.923-.247l-2.935-1.738c-.439-.245-.225-.332-.08-.382c.584-.204.703-.25 1.327-.605c.066-.037.152-.024.219.015l2.255 1.339a.3.3 0 0 0 .273 0l8.794-5.077a.28.28 0 0 0 .134-.237V6.919a.28.28 0 0 0-.136-.242l-8.79-5.072a.27.27 0 0 0-.271 0l-8.79 5.072a.28.28 0 0 0-.139.24v10.148c0 .097.053.19.137.236l2.408 1.391c1.308.654 2.107-.116 2.107-.891V7.785a.25.25 0 0 1 .255-.254h1.114c.139 0 .253.11.253.254v10.02c0 1.744-.95 2.745-2.604 2.745c-.509 0-.91 0-2.028-.55l-2.307-1.33a1.86 1.86 0 0 1-.922-1.605V6.917c0-.66.352-1.277.922-1.602L10.976.236a1.93 1.93 0 0 1 1.849 0l8.792 5.08c.568.329.922.943.922 1.603v10.149a1.86 1.86 0 0 1-.922 1.602l-8.792 5.079a1.85 1.85 0 0 1-.927.246zm2.716-6.993c-3.848 0-4.654-1.766-4.654-3.248c0-.14.113-.253.254-.253h1.136c.126 0 .231.091.251.215c.172 1.158.683 1.742 3.01 1.742c1.853 0 2.641-.419 2.641-1.402c0-.566-.225-.986-3.104-1.268c-2.408-.238-3.896-.768-3.896-2.694c0-1.775 1.497-2.831 4.004-2.831c2.815 0 4.211.977 4.387 3.077a.256.256 0 0 1-.255.278h-1.143a.25.25 0 0 1-.246-.199c-.275-1.217-.94-1.607-2.747-1.607c-2.023 0-2.259.705-2.259 1.233c0 .64.277.828 3.007 1.189c2.703.359 3.987.865 3.987 2.765c0 1.915-1.599 3.014-4.385 3.014z"></svg:path>`,
})
export class GrommetIconsNodeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNodesIcon],svg[grommet-icons-nodes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 4a2 2 0 1 1-4 0a2 2 0 0 1 4 0Zm0 16a2 2 0 1 1-4 0a2 2 0 0 1 4 0ZM7 8a2 2 0 1 1-4 0a2 2 0 0 1 4 0Zm0 8a2 2 0 1 1-4 0a2 2 0 0 1 4 0Zm14-8a2 2 0 1 1-4 0a2 2 0 0 1 4 0Zm0 8a2 2 0 1 1-4 0a2 2 0 0 1 4 0Z"></svg:path>`,
})
export class GrommetIconsNodesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNortonIcon],svg[grommet-icons-norton-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M21.91.816h.797v.797h-.797zm0 .797h-1.477v.797h1.477zm-1.477.797v1.554h-.738V2.41zm0 1.554h.758v.758h-.758zm.758 0v-.748h.836v.748zm-.758.758v1.613h-.65v.632h-.69v.923H18.5v.787h-.758V7.9h-.758l-.01.777h.768v.826h-.758l.01.903h-.651l.01.758h-.603l-.01.855c-1.136 1.635-1.923 3.351-2.633 5.063c-.09.217-.123.395-.233.524c-.239.28-.595.368-.885.36c-.542-.015-1.02-.268-1.272-.855c-.85-1.97-1.27-3.35-3.605-5.937c-.376-.417-.289-.949.01-1.117c.286-.163.687-.046 1.136.204c1.258.7 1.958 1.415 3.45 3.4c.543-1.311 1.482-3.084 2.652-5.004h.544V7.9h.515v-.75h.758V5.956h.817l-.01.758h.758v-.758h-.748v-.68h.68v-.554h1.253V5.5h.768v-.778zm-1.506 0v-.758h.768v.758zm0-.758h-.767v-.728h.767zm1.506-1.55h.757v.798h-.757zm2.27-.799h.819v.799h-.82zm-6.156 2.929v.742h-.933V6.53h-.933v.734a7.1 7.1 0 0 0-3.313-.812c-3.947 0-7.152 3.195-7.152 7.142c0 3.946 3.205 7.141 7.152 7.141a7.14 7.14 0 0 0 7.141-7.141a7.1 7.1 0 0 0-.823-3.333h.726v-.933h.622v-.933h.622v-.777h.27a10.36 10.36 0 0 1 1.887 5.976C21.813 19.34 17.153 24 11.406 24S1 19.34 1 13.594S5.66 3.187 11.406 3.187c1.87 0 3.624.493 5.14 1.357M22.703 0h.819v.817h-.82z"></svg:path>`,
})
export class GrommetIconsNortonIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNoteIcon],svg[grommet-icons-note-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 23h15l7-7V1H1zm14 0v-8h8"></svg:path>`,
})
export class GrommetIconsNoteIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNotesIcon],svg[grommet-icons-notes-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 1v22h13l5-5V1zm3 16h5m-5-4h12M6 9h10M3 5h18m0 12h-6v6"></svg:path>`,
})
export class GrommetIconsNotesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNotificationIcon],svg[grommet-icons-notification-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4 19V9a8 8 0 0 1 16 0v10M1 19h22m-8 0v1a3 3 0 1 1-6 0v-1"></svg:path>`,
})
export class GrommetIconsNotificationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsNpmIcon],svg[grommet-icons-npm-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:path d="M0 0h24v24H0z"></svg:path><svg:path d="M16.718 7.928h-4.513V20.25H4V3h16v17.249h-3.282z"></svg:path></svg:g>`,
})
export class GrommetIconsNpmIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsObjectGroupIcon],svg[grommet-icons-object-group-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h3v3H1zm19 0h3v3h-3zM4 2h16M4 22h16M1 20h3v3H1zm19 0h3v3h-3zM2 4v16M22 4v16M7 7h7v6H7zm10 3v7h-7v-2"></svg:path>`,
})
export class GrommetIconsObjectGroupIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsObjectUngroupIcon],svg[grommet-icons-object-ungroup-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 1h3v3H1zm12 0h3v3h-3zM4 2h9m2 7h5M4 15h9M1 13h3v3H1zm12 0h3v3h-3zM2 4v9m13-9v9m5-5h3v3h-3zm-9 14h9M8 20h3v3H8zm12 0h3v3h-3zM9 16v4m13-9v9"></svg:path>`,
})
export class GrommetIconsObjectUngroupIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOfflineStorageIcon],svg[grommet-icons-offline-storage-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 21a9 9 0 1 0 0-18a9 9 0 1 0 0 18Zm8-12h-8a3 3 0 0 0 0 6h8"></svg:path>`,
})
export class GrommetIconsOfflineStorageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOnedriveIcon],svg[grommet-icons-onedrive-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M21.692 13.987a2.733 2.733 0 0 1 .602 5.232h-13.7a3.166 3.166 0 1 1 .281-6.321a3.963 3.963 0 0 1 7.482-2.05a3.454 3.454 0 0 1 5.336 3.138M8.187 12.209a3.873 3.873 0 0 0-3.44 3.843c0 .81.252 1.563.681 2.186h-2.15a3.279 3.279 0 0 1-.237-6.549a3.692 3.692 0 0 1 5.668-3.86a5.103 5.103 0 0 1 9.648 1.757l-.109-.003c-.568 0-1.125.115-1.64.337a4.64 4.64 0 0 0-3.778-1.929a4.67 4.67 0 0 0-4.643 4.218"></svg:path>`,
})
export class GrommetIconsOnedriveIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOperaIcon],svg[grommet-icons-opera-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.125 0C5.568 0 1 4.756 1 11.889C1 18.236 5.438 24 12.125 24c6.752 0 11.226-5.763 11.226-12.111C23.35 4.699 18.62 0 12.125 0m0 21.32a3.3 3.3 0 0 1-1.425-.298c-1.141-.575-1.828-1.85-2.23-3.41c-.435-1.809-.484-4.004-.484-5.926c0-3.431.25-6.532 1.65-8.08c.612-.65 1.418-1.034 2.476-1.037h.013c1.379 0 2.345.675 3.016 1.734c1 1.688 1.227 4.424 1.227 7.368c0 4.18-.278 9.65-4.243 9.65"></svg:path>`,
})
export class GrommetIconsOperaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOptimizeIcon],svg[grommet-icons-optimize-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 22h4v-4H2zM22 2L12 12m10-2V2h-8m8 11h-4v9h4zm-12 9h4v-6h-4z"></svg:path>`,
})
export class GrommetIconsOptimizeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOracleIcon],svg[grommet-icons-oracle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M7.957 18.912A6.953 6.953 0 0 1 1 11.962A6.963 6.963 0 0 1 7.957 5h8.087A6.96 6.96 0 0 1 23 11.962a6.95 6.95 0 0 1-6.956 6.95zm7.907-2.453a4.497 4.497 0 0 0 4.503-4.497a4.507 4.507 0 0 0-4.503-4.508H8.136a4.507 4.507 0 0 0-4.503 4.508a4.5 4.5 0 0 0 4.503 4.497z"></svg:path>`,
})
export class GrommetIconsOracleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOrderedListIcon],svg[grommet-icons-ordered-list-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M5.77 6.42h18.06v1.75H5.77zm0 5.29h18.06v1.75H5.77zm0 5.28h18.06v1.75H5.77zM3.13 4.87V8a2 2 0 0 0 0 .45a.3.3 0 0 0 .13.16a.6.6 0 0 0 .32.06h.12v.11h-2v-.07h.1a.8.8 0 0 0 .35-.06a.3.3 0 0 0 .14-.16A1.8 1.8 0 0 0 2.3 8V6a1.3 1.3 0 0 0 0-.33a.24.24 0 0 0-.1-.11a.3.3 0 0 0-.16 0a.9.9 0 0 0-.35.09l-.05-.1L3 4.87Zm.52 9.43H1.37v-.06a12.5 12.5 0 0 0 1.27-1.67a1.8 1.8 0 0 0 .22-.84a.7.7 0 0 0-.18-.5a.6.6 0 0 0-.45-.2a.75.75 0 0 0-.68.44h-.11a1.6 1.6 0 0 1 .47-.81a1.1 1.1 0 0 1 .72-.26a1.06 1.06 0 0 1 .54.14a1 1 0 0 1 .38.37a.9.9 0 0 1 .14.45a1.6 1.6 0 0 1-.21.77a7.3 7.3 0 0 1-1.25 1.47h.83a2 2 0 0 0 .4 0a.33.33 0 0 0 .15-.09a1.2 1.2 0 0 0 .16-.26h.1ZM2.09 18v-.1a2 2 0 0 0 .45-.17a.7.7 0 0 0 .22-.25a.7.7 0 0 0 .09-.34a.55.55 0 0 0-.17-.41a.58.58 0 0 0-.43-.17a.8.8 0 0 0-.68.42h-.11a1.7 1.7 0 0 1 .54-.79a1.16 1.16 0 0 1 .71-.24a.93.93 0 0 1 .66.24a.76.76 0 0 1 .26.57a.8.8 0 0 1-.12.41a1 1 0 0 1-.38.35a1.34 1.34 0 0 1 .51.4a1 1 0 0 1 .17.6a1.38 1.38 0 0 1-.44 1A1.62 1.62 0 0 1 2.2 20a1.24 1.24 0 0 1-.71-.16a.34.34 0 0 1-.16-.29a.32.32 0 0 1 .31-.32a.4.4 0 0 1 .18 0l.32.25a.84.84 0 0 0 .52.23a.47.47 0 0 0 .34-.16a.6.6 0 0 0 .15-.42a1.1 1.1 0 0 0-.27-.72a1.42 1.42 0 0 0-.79-.41"></svg:path>`,
})
export class GrommetIconsOrderedListIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOrganizationIcon],svg[grommet-icons-organization-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 3v20H4V3zM8.042 9h2V7h-2zM14 9h2V7h-2zm-5.958 6h2v-2h-2zm2 8h4v-4h-4zM14 15h2v-2h-2zM2 3h20V1H2z"></svg:path>`,
})
export class GrommetIconsOrganizationIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsOverviewIcon],svg[grommet-icons-overview-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18.5 21a4.5 4.5 0 1 0 0-9a4.5 4.5 0 0 0 0 9ZM10 7h4M1.5 14.5S5.5 5 6 4s1.5-1 2-1s2 0 2 2v11m-4.5 5a4.5 4.5 0 1 1 0-9a4.5 4.5 0 0 1 0 9Zm17-6.5S18.5 5 18 4s-1.5-1-2-1s-2 0-2 2v11m-4 0h4"></svg:path>`,
})
export class GrommetIconsOverviewIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPackageIcon],svg[grommet-icons-package-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.371.571L12 .423l-.371.148l-10 4L1 4.823v14.354l.629.251l10 4l.371.149l.371-.149l10-4l.629-.251V4.823l-.629-.252zM3 6.977v10.846l8 3.2V10.177zm10 3.2v10.846l8-3.2V6.977zM19.307 5.5L12 2.577L9.943 3.4l7.307 2.923zm-14.614 0L7.25 4.477L14.557 7.4L12 8.423z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsPackageIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPaintIcon],svg[grommet-icons-paint-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M4 10H2c0-5.523 0-9 10-9s10 3.477 10 9h-2m-8 0c4.418 0 8-.895 8-2s-3.582-2-8-2s-8 .895-8 2s3.582 2 8 2ZM4 20c0 1.657 3.582 3 8 3s8-1.343 8-3m0-12v12zM4 20V8zm4-7v4m5-4v4m-6-5c-1.5 0-3-1-3-4m14 5.5V12c0-2 2-1 2-4M8 13a1 1 0 0 0-1-1m6 1a1 1 0 0 1 2 0v.5m0 0a1.5 1.5 0 0 0 3 0M8 17a2.5 2.5 0 1 0 5 0"></svg:path>`,
})
export class GrommetIconsPaintIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPanIcon],svg[grommet-icons-pan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8.5 5.5L12 2l3.5 3.5M22 12H2m3.5-3.5L2 12l3.5 3.5m13 0L22 12l-3.5-3.5M12 22V2M8.5 18.5L12 22l3.5-3.5"></svg:path>`,
})
export class GrommetIconsPanIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPauseIcon],svg[grommet-icons-pause-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 21h6V3H3zm12 0h6V3h-6z"></svg:path>`,
})
export class GrommetIconsPauseIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPauseFillIcon],svg[grommet-icons-pause-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 21h6V3H3zm1-2h4V5H4zm1-2h2V7H5zm10 4h6V3h-6zm1-2h4V5h-4zm1-2h2V7h-2z"></svg:path>`,
})
export class GrommetIconsPauseFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPaypalIcon],svg[grommet-icons-paypal-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M21.495 7.054c-1.07 4.973-4.48 7.604-9.89 7.604H9.643L8.18 24h3.182c.46 0 .85-.334.923-.788l.037-.198l.732-4.636l.047-.256a.93.93 0 0 1 .922-.788h.581c3.76 0 6.705-1.528 7.565-5.946c.345-1.773.179-3.26-.674-4.334M19.317 1.81C18.206.543 16.197 0 13.627 0H6.169c-.526 0-.973.383-1.055.9L2.008 20.598a.64.64 0 0 0 .633.74h4.604l1.157-7.334l-.036.229c.082-.518.526-.9 1.051-.9h2.188c4.299 0 7.664-1.747 8.648-6.797c.029-.15.076-.437.076-.437c.279-1.87-.002-3.138-1.012-4.288"></svg:path>`,
})
export class GrommetIconsPaypalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPerformanceIcon],svg[grommet-icons-performance-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m12 19l-2 3l-3-1l-.5-3.5L3 17l-1-3l3-2l-3-2l1-3l3.5-.5L7 3l3-1l2 3l2-3l3 1l.5 3.5L21 7l1 3l-3 2l3 2l-1 3l-3.5.5L17 21l-3 1zm0-3a4 4 0 1 0 0-8a4 4 0 0 0 0 8"></svg:path>`,
})
export class GrommetIconsPerformanceIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPersonalComputerIcon],svg[grommet-icons-personal-computer-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 18h18V5a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1zm-1 2h20c1 0 1-1 1-1H1s0 1 1 1Z"></svg:path>`,
})
export class GrommetIconsPersonalComputerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPhoneIcon],svg[grommet-icons-phone-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6.375 2C5 2 3 3.5 2.5 4.5c-.715 1.43-.597 1.99-.125 3.5c.625 2 2.457 5.545 5 8c3.625 3.5 7 5 8.5 5.5s3.125 0 4.125-1s2-2 .875-3.5c-.797-1.063-1.959-2.292-3.375-3c-1.288-.644-2.056-.41-2.5.5c-.246.503-.322 1.466-.5 2c-.225.674-1.125.5-2.125 0C11.418 16.021 9 14 7 11c-1.24-1.859.742-1.87 2-2.5c1-.5 1.31-1.65.5-3C8 3 7.5 2 6.375 2Z"></svg:path>`,
})
export class GrommetIconsPhoneIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPhoneFlipIcon],svg[grommet-icons-phone-flip-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M8 1a1 1 0 0 0-.928.629l-4 10a1 1 0 0 0 0 .742l4 10A1 1 0 0 0 8 23h10a1 1 0 0 0 .928-1.371L15.078 12l3.851-9.629A1 1 0 0 0 18 1zM5.477 11l3.2-8h2.794a1 1 0 0 0 1 1h.057a1 1 0 0 0 1.001-1h2.994l-3.2 8zm0 2l3.2 8h7.846l-3.2-8zm5.558 6.773a1 1 0 0 1 .937-1.351h.056a1 1 0 1 1 0 2h-.056a1 1 0 0 1-.937-.649" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsPhoneFlipIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPhoneHorizontalIcon],svg[grommet-icons-phone-horizontal-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M23 7a2 2 0 0 0-2-2H3a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h18a2 2 0 0 0 2-2zm-2 5.996V17H3V7h18v4.004a1 1 0 0 0-.094-.004h-1a1 1 0 0 0 0 2h1q.048 0 .094-.004" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsPhoneHorizontalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPhoneVerticalIcon],svg[grommet-icons-phone-vertical-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M17 23a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v18a2 2 0 0 0 2 2zm-6-2H7V3h10v18h-4v-1.031a1 1 0 0 0-2 0z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsPhoneVerticalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPieChartIcon],svg[grommet-icons-pie-chart-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M10 23a9 9 0 0 1 0-18v9l1.162 1.162l5.202 5.202A8.97 8.97 0 0 1 10 23Zm4-13V1a9 9 0 0 1 9 9zm0 3h8a8.96 8.96 0 0 1-2.107 5.787z"></svg:path>`,
})
export class GrommetIconsPieChartIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPiedPiperIcon],svg[grommet-icons-pied-piper-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M0 19.421c2.274 0 4.042-.758 4.042-.758s3.032-7.579 7.326-7.579c3.285 0 3.79 2.527 3.79 2.527S19.958 4.263 24 3c-3.79 3.032-3.284 6.316-5.053 7.832c-1.768 1.515-1.768.006-3.79 3.543c-4.546.505-6.032 2.014-9.094 3.783c5.305-2.526 6.316-2.78 11.116-2.526c.504.026.758.252.505.757c-.733 1.466-1.28 3.673-2.273 3.537c-5.558-.758-8.843.506-11.622.506S0 19.926 0 19.422"></svg:path>`,
})
export class GrommetIconsPiedPiperIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPinIcon],svg[grommet-icons-pin-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m16 3l-6 6s-4-1-7 2l10 10c3-3 2-7 2-7l6-6zM1 23l7-7m6-15l9 9"></svg:path>`,
})
export class GrommetIconsPinIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPinterestIcon],svg[grommet-icons-pinterest-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12 0C5.372 0 0 5.372 0 12c0 5.084 3.163 9.426 7.627 11.174c-.105-.95-.2-2.406.042-3.442c.217-.936 1.407-5.965 1.407-5.965s-.36-.718-.36-1.781c0-1.669.968-2.915 2.172-2.915c1.024 0 1.518.769 1.518 1.69c0 1.03-.655 2.57-.993 3.996c-.283 1.195.598 2.169 1.777 2.169c2.133 0 3.772-2.25 3.772-5.495c0-2.873-2.065-4.883-5.013-4.883c-3.414 0-5.418 2.562-5.418 5.208c0 1.031.397 2.138.893 2.739a.36.36 0 0 1 .083.344c-.091.38-.293 1.194-.333 1.361c-.053.219-.174.266-.402.16c-1.499-.698-2.435-2.888-2.435-4.649c0-3.785 2.75-7.261 7.929-7.261c4.162 0 7.397 2.966 7.397 6.93c0 4.136-2.607 7.464-6.227 7.464c-1.216 0-2.358-.632-2.75-1.378l-.748 2.853c-.27 1.042-1.002 2.348-1.492 3.146A12 12 0 0 0 12 24c6.628 0 12-5.373 12-12c0-6.628-5.372-12-12-12"></svg:path>`,
})
export class GrommetIconsPinterestIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPlanIcon],svg[grommet-icons-plan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 4V0zM7 18H5zm12 0H9zM7 14H5zm12 0H9zM6 4V0zM1 9h22zm0 14h22V4H1z"></svg:path>`,
})
export class GrommetIconsPlanIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPlayIcon],svg[grommet-icons-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m3 22l18-10L3 2z"></svg:path>`,
})
export class GrommetIconsPlayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPlayFillIcon],svg[grommet-icons-play-fill-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m3 22l18-10L3 2zm2-3l12.6-7L5 5zm2-3l7.2-4L7 8zm2-3l1.8-1L9 11z"></svg:path>`,
})
export class GrommetIconsPlayFillIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPlugIcon],svg[grommet-icons-plug-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M15 6V1m-3 23v-9M9 6V1M6 6h12v7a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2z"></svg:path>`,
})
export class GrommetIconsPlugIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPocketIcon],svg[grommet-icons-pocket-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" fill-rule="evenodd"><svg:path fill="currentColor" d="M12 2H2a2 2 0 0 0-2 2v8c0 5.982 6 11 12 11s12-5.018 12-11V4a2 2 0 0 0-2-2z"></svg:path><svg:path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="m6 9l6.404 6L18 9"></svg:path></svg:g>`,
})
export class GrommetIconsPocketIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPowerIcon],svg[grommet-icons-power-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M16 4c3.364 1.43 6 4.99 6 9c0 5.6-4.473 10-10 10S2 18.6 2 13c0-4.01 2.636-7.57 6-9m4-3v10"></svg:path>`,
})
export class GrommetIconsPowerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPowerCycleIcon],svg[grommet-icons-power-cycle-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 8a8.96 8.96 0 0 0-8.036-5C7.014 3 3 7.03 3 12m1 4a8.96 8.96 0 0 0 8.036 5C16.986 21 21 16.97 21 12M9 16H3v6M21 2v6h-6"></svg:path>`,
})
export class GrommetIconsPowerCycleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPowerForceShutdownIcon],svg[grommet-icons-power-force-shutdown-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 21a9 9 0 1 0 0-18a9 9 0 0 0 0 18Zm0-13v8"></svg:path>`,
})
export class GrommetIconsPowerForceShutdownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPowerResetIcon],svg[grommet-icons-power-reset-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 8c-1.403-2.96-4.463-5-8-5a9 9 0 1 0 0 18a9 9 0 0 0 9-9m0-9v6h-6"></svg:path>`,
})
export class GrommetIconsPowerResetIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPowerShutdownIcon],svg[grommet-icons-power-shutdown-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 1v8M6.994 4.52a9 9 0 0 0-1.358 1.116a9 9 0 1 0 11.37-1.117"></svg:path>`,
})
export class GrommetIconsPowerShutdownIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPreviousIcon],svg[grommet-icons-previous-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 2L7 12l10 10"></svg:path>`,
})
export class GrommetIconsPreviousIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsPrintIcon],svg[grommet-icons-print-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 19H1V7h22v12h-5M3 16h18M6 16v7h12v-7m0-9V1H6v6m11 5h2v-1h-2z"></svg:path>`,
})
export class GrommetIconsPrintIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsProductHuntIcon],svg[grommet-icons-product-hunt-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M13.6 8.4h-3.4V12h3.4a1.8 1.8 0 1 0 0-3.6m0 6h-3.4V18H7.8V6h5.8a4.2 4.2 0 1 1 0 8.4M12 0C5.372 0 0 5.372 0 12s5.372 12 12 12c6.627 0 12-5.372 12-12S18.627 0 12 0"></svg:path>`,
})
export class GrommetIconsProductHuntIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsProjectsIcon],svg[grommet-icons-projects-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 15v8H1v-8zm14 0v8h-8v-8zM9 1v8H1V1zm14 0v8h-8V1z"></svg:path>`,
})
export class GrommetIconsProjectsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsQrIcon],svg[grommet-icons-qr-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none" fill-rule="evenodd"><svg:path fill="currentColor" d="M13 14h1v1h-1zm1 1h1v1h-1zm0 1h1v1h-1zm2 0h1v1h-1zm0 1h1v1h-1zm-3-1h1v1h-1zm2 0h1v1h-1zm0 1h1v1h-1zm3-1h1v1h-1zm0-1h1v1h-1zm1-1h1v1h-1zm-2 2h1v1h-1zm0 1h1v1h-1zm-1 1h1v1h-1zm-1 0h1v1h-1zm2 0h1v1h-1zm1 0h1v1h-1zm-2 1h1v1h-1zm-2 0h1v1h-1zm1 0h1v1h-1zm-2 0h1v1h-1zm0 1h1v1h-1zm1 1h1v1h-1zm1 0h1v1h-1zm2 0h1v1h-1zm1 0h1v1h-1zm-1-2h1v1h-1zm1 0h1v1h-1zm1-1h1v1h-1zm0-1h1v1h-1zm0 3h1v1h-1zm0-1h1v1h-1zm1-1h1v1h-1zm0-1h1v1h-1zm1 3h1v1h-1zm0-2h1v1h-1zm0 1h1v1h-1zm-2-3h1v1h-1zm-6 1h1v1h-1zm-1 0h1v1h-1zm0 1h1v1h-1zm2 0h1v1h-1zm-3 0h1v1h-1zm2 0h1v1h-1zm-2 1h1v1h-1zm0 1h1v1h-1zm0-19h1v1h-1zm1 1h1v1h-1zm-1 2h1v1h-1zm1 1h1v1h-1zm-1 1h1v1h-1zm1 0h1v1h-1zm0 1h1v1h-1zm0 1h1v1h-1zm-1 1h1v1h-1zm1 0h1v1h-1zm-1 1h1v1h-1zM1 11h1v1H1zm1 1h1v1H2zm2-1h1v1H4zm0 1h1v1H4zm1-1h1v1H5zm1 1h1v1H6zm1-1h1v1H7zm1 1h1v1H8zm0-1h1v1H8zm1 0h1v1H9zm1 0h1v1h-1zm1 1h1v1h-1zm2 0h1v1h-1zm1-1h1v1h-1zm1 0h1v1h-1zm1 0h1v1h-1zm-1 2h1v1h-1zm-2 9h1v1h-1zm-1 0h1v1h-1zm0-9h1v1h-1zm-1 0h1v1h-1zm0 1h1v1h-1zm0 1h1v1h-1zm11-1h1v1h-1zm-1 1h1v1h-1zm1 2h1v1h-1zm-5-4h1v1h-1zm1-1h1v1h-1zm4 0h1v1h-1zm0 1h1v1h-1zm-1 0h1v1h-1zm1 8h1v1h-1zm-1 1h1v1h-1zm-2 0h1v1h-1zm3 0h1v1h-1z"></svg:path><svg:path stroke="currentColor" stroke-width="2" d="M15 2h7v7h-7zM2 2h7v7H2zm0 13h7v7H2zM18 5h1v1h-1zM5 5h1v1H5zm0 13h1v1H5z"></svg:path></svg:g>`,
})
export class GrommetIconsQrIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRadialIcon],svg[grommet-icons-radial-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:circle cx="12" cy="12" r="11" fill="none" stroke="currentColor" stroke-width="2"></svg:circle>`,
})
export class GrommetIconsRadialIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRadialSelectedIcon],svg[grommet-icons-radial-selected-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 23c6.075 0 11-4.925 11-11S18.075 1 12 1S1 5.925 1 12s4.925 11 11 11Zm0-10a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 2a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0 2a5 5 0 1 0 0-10a5 5 0 0 0 0 10Z"></svg:path>`,
})
export class GrommetIconsRadialSelectedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRaspberryIcon],svg[grommet-icons-raspberry-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M8.087 0a.67.67 0 0 0-.399.165c-.363-.14-.717-.19-1.032.096c-.486-.063-.643.067-.762.22c-.107-.003-.8-.11-1.117.364c-.798-.095-1.049.47-.763.994c-.163.253-.332.5.048.981c-.134.268-.05.558.267.91c-.084.376.081.641.376.848c-.055.515.472.815.629.922c.06.3.187.584.79.74c.099.447.46.525.811.619c-1.159.673-2.153 1.558-2.146 3.733l-.17.303c-1.328.808-2.524 3.406-.654 5.518c.122.66.326 1.134.508 1.66c.273 2.117 2.054 3.11 2.523 3.227c.688.524 1.422 1.02 2.413 1.369c.935.964 1.946 1.331 3.005 1.331s2.07-.367 3.005-1.331c.992-.348 1.725-.845 2.413-1.37c.47-.117 2.25-1.11 2.523-3.226c.182-.526.386-1 .508-1.66c1.87-2.112.674-4.71-.655-5.518l-.169-.303c.007-2.175-.987-3.06-2.146-3.733c.35-.094.712-.172.812-.619c.602-.156.729-.44.79-.74c.157-.107.683-.407.628-.922c.295-.207.46-.472.376-.848c.318-.352.402-.642.267-.91c.38-.48.212-.728.049-.98c.285-.526.034-1.09-.763-.995c-.318-.474-1.01-.367-1.117-.365c-.12-.152-.277-.282-.763-.22c-.315-.284-.669-.235-1.032-.095A.67.67 0 0 0 16.742 0c-.232-.007-.43.134-.643.202c-.523-.17-.641.061-.898.156c-.569-.12-.742.143-1.015.42l-.32-.007c-.858.506-1.282 1.535-1.452 1.535S11.82 1.277 10.961.771l-.319.006C10.37.501 10.196.238 9.627.358C9.37.263 9.252.031 8.73.202C8.516.134 8.318-.007 8.087 0m.03.609c.436.161.663.37.9.575c.08-.108.202-.188.052-.45c.31.18.544.39.717.625c.191-.122.112-.287.113-.441c.322.262.528.54.778.812c.05-.037.093-.162.132-.358c.768.744 1.852 2.62.279 3.365C9.749 3.633 8.15 2.83 6.38 2.228c2.277 1.174 3.602 2.123 4.327 2.932c-.371 1.49-2.31 1.56-3.019 1.518c.145-.068.266-.15.31-.275c-.179-.127-.81-.013-1.25-.26c.17-.036.249-.069.328-.194c-.416-.132-.864-.248-1.128-.468c.143.002.275.033.46-.095c-.372-.201-.77-.36-1.078-.668c.193-.004.4-.002.46-.073a4.2 4.2 0 0 1-.867-.704c.27.033.382.006.448-.04c-.258-.265-.583-.487-.739-.812c.2.069.383.093.515-.008c-.088-.198-.464-.314-.68-.776c.211.02.436.046.48 0c-.097-.398-.264-.622-.429-.854c.451-.007 1.134.001 1.103-.037l-.279-.285c.44-.118.892.02 1.219.122c.147-.115-.004-.263-.183-.413q.56.076 1.019.255C7.56.945 7.289.799 7.159.65c.578.11.825.263 1.068.417c.177-.169.01-.312-.11-.46m8.594 0c-.12.147-.287.29-.11.46c.244-.155.49-.308 1.068-.418c-.13.148-.401.294-.238.442a4.4 4.4 0 0 1 1.02-.255c-.18.15-.33.298-.184.413c.328-.102.779-.24 1.22-.122l-.28.285c-.03.038.652.03 1.103.037c-.165.232-.331.456-.43.854c.045.046.27.02.48 0c-.215.462-.591.578-.679.776c.132.101.315.077.515.008c-.155.325-.48.547-.738.811c.065.047.178.074.447.041a4.2 4.2 0 0 1-.866.704c.06.07.267.069.46.073c-.31.307-.706.467-1.079.668c.186.128.318.097.46.095c-.264.22-.711.336-1.127.468c.079.125.158.158.327.193c-.44.248-1.071.134-1.249.26c.043.125.164.208.31.276c-.71.041-2.648-.029-3.02-1.518c.726-.81 2.05-1.758 4.328-2.932c-1.772.602-3.37 1.405-4.708 2.509c-1.574-.744-.49-2.62.278-3.365c.04.196.082.32.133.358c.25-.272.455-.55.777-.812c0 .154-.078.32.114.441c.172-.235.406-.446.716-.624c-.15.26-.027.341.053.45c.236-.207.463-.415.899-.576M12.414 6.77c1.359 0 2.492.923 2.494 1.466c.004.68-.994 1.38-2.476 1.398h-.036c-1.482-.018-2.48-.717-2.476-1.398c.003-.543 1.136-1.466 2.494-1.466m-3.813.447h.084q.331 0 .677.059c.777.13-3.72 4.058-3.79 3.182c-.062-2.003 1.278-3.213 3.03-3.241m7.542 0h.084c1.751.028 3.091 1.238 3.03 3.24c-.07.877-4.567-3.05-3.791-3.181q.346-.059.677-.06m-6.825 2.5c.259-.002.521.036.777.122c1.363.458 2.052 2.053 1.54 3.56c-.513 1.509-2.034 2.36-3.398 1.903c-1.363-.458-2.052-2.053-1.54-3.56c.417-1.226 1.498-2.017 2.62-2.025m6.193 0c1.122.008 2.204.8 2.62 2.024c.513 1.508-.176 3.103-1.54 3.56c-1.364.459-2.885-.393-3.398-1.901c-.512-1.508.177-3.103 1.54-3.56c.256-.087.519-.125.778-.123M5.33 11.4c1.062.015.273 5.052-.69 4.624c-1.092-.879-1.444-3.451.582-4.608a.4.4 0 0 1 .082-.014zm14.168 0l.027.002a.4.4 0 0 1 .081.014c2.026 1.157 1.675 3.73.582 4.608c-.963.428-1.752-4.609-.69-4.624m-7.084 3.482a2.8 2.8 0 0 1 1.874.7c.52.47.822 1.14.818 1.811c0 .66-.295 1.313-.8 1.778a2.88 2.88 0 0 1-1.892.739a2.88 2.88 0 0 1-1.892-.739a2.45 2.45 0 0 1-.8-1.778a2.46 2.46 0 0 1 .818-1.81a2.8 2.8 0 0 1 1.874-.7M6.148 16.2c.703-.014 1.568.541 2.268 1.353c.78.94 1.136 2.593.485 3.08c-.617.372-2.115.218-3.178-1.31c-.718-1.282-.624-2.587-.12-2.97q.248-.149.545-.153m12.465 0h.067c.197.004.38.052.546.153c.503.383.597 1.688-.12 2.97c-1.064 1.528-2.562 1.682-3.178 1.31c-.652-.487-.297-2.14.484-3.08c.678-.787 1.51-1.333 2.201-1.353m-6.199 4.504c1.09-.012 2.71.447 2.69 1.032c.017.405-1.312 1.574-2.667 1.516h-.045c-1.356.058-2.685-1.11-2.667-1.516c-.02-.585 1.6-1.044 2.69-1.032"></svg:path>`,
})
export class GrommetIconsRaspberryIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsReactjsIcon],svg[grommet-icons-reactjs-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:circle cx="11.996" cy="11.653" r="2.142"></svg:circle><svg:path fill-rule="nonzero" d="M11.996 7.81c2.768 0 5.397.39 7.396 1.078c2.22.764 3.575 1.894 3.575 2.765c0 .927-1.463 2.131-3.838 2.918c-1.89.626-4.42.968-7.133.968c-2.85 0-5.409-.335-7.277-.974c-1.17-.4-2.136-.92-2.799-1.486c-.595-.51-.895-1.016-.895-1.426c0-.874 1.302-1.988 3.475-2.746c2-.697 4.693-1.098 7.496-1.098m0-1.025c-2.912 0-5.718.418-7.834 1.156C1.622 8.825 0 10.213 0 11.653c0 1.486 1.741 2.978 4.387 3.882c1.989.68 4.654 1.029 7.609 1.029c2.816 0 5.451-.356 7.456-1.02c2.75-.912 4.54-2.385 4.54-3.891c0-1.445-1.675-2.842-4.266-3.735c-2.113-.727-4.854-1.134-7.73-1.134m-3.348 2.96c1.383-2.399 3.034-4.481 4.63-5.87c1.77-1.54 3.426-2.15 4.18-1.714c.803.463 1.116 2.331.611 4.782c-.402 1.95-1.37 4.312-2.725 6.663c-1.424 2.469-2.992 4.518-4.48 5.817c-.93.814-1.863 1.391-2.685 1.682c-.738.26-1.327.268-1.682.063c-.757-.436-1.072-2.121-.643-4.381c.395-2.081 1.394-4.615 2.794-7.043m-.888-.512c-1.455 2.522-2.495 5.162-2.913 7.363c-.501 2.643-.11 4.742 1.137 5.46c1.289.744 3.45-.02 5.555-1.86c1.583-1.383 3.217-3.518 4.693-6.077c1.406-2.44 2.415-4.9 2.84-6.969c.585-2.837.203-5.124-1.102-5.876c-1.252-.722-3.298.03-5.366 1.83C10.92 4.568 9.197 6.74 7.76 9.231m.891 4.378C7.264 11.215 6.285 8.744 5.879 6.67c-.451-2.305-.152-4.043.602-4.479c.802-.465 2.577.198 4.448 1.859c1.49 1.322 3.052 3.34 4.411 5.689c1.429 2.467 2.42 4.848 2.804 6.785c.24 1.213.274 2.31.115 3.167c-.143.77-.43 1.284-.785 1.49c-.756.437-2.373-.132-4.117-1.633c-1.605-1.38-3.302-3.511-4.706-5.937m-.887.514c1.459 2.52 3.226 4.74 4.925 6.2c2.039 1.755 4.052 2.463 5.298 1.742c1.287-.745 1.705-2.999 1.162-5.742c-.408-2.061-1.441-4.543-2.922-7.1c-1.41-2.437-3.039-4.54-4.618-5.942C9.443 1.36 7.271.548 5.967 1.303c-1.25.724-1.62 2.873-1.094 5.563c.429 2.193 1.45 4.769 2.891 7.258"></svg:path></svg:g>`,
})
export class GrommetIconsReactjsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRedditIcon],svg[grommet-icons-reddit-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M15.57 15.284c-.897 0-1.65-.728-1.65-1.625s.753-1.648 1.65-1.648s1.625.752 1.625 1.649c0 .896-.728 1.624-1.625 1.624m.381 3.153c-.835.835-2.124 1.24-3.939 1.24h-.026c-1.815 0-3.102-.405-3.937-1.24a.655.655 0 1 1 .927-.927c.576.576 1.56.856 3.01.856l.013.001h.013c1.45 0 2.435-.281 3.012-.857a.655.655 0 1 1 .927.927m-9.145-4.778c0-.896.752-1.648 1.648-1.648c.897 0 1.624.752 1.624 1.649c0 .896-.727 1.624-1.624 1.624c-.896 0-1.649-.728-1.649-1.625M19.999 3.311c.607 0 1.102.494 1.102 1.101s-.495 1.102-1.102 1.102a1.103 1.103 0 0 1-1.102-1.102c0-.607.494-1.101 1.102-1.101M24 11.875a2.887 2.887 0 0 0-2.884-2.884c-.689 0-1.321.243-1.818.647c-1.758-1.105-3.99-1.771-6.383-1.912l1.248-3.946l3.43.808a2.415 2.415 0 0 0 2.405 2.237a2.415 2.415 0 0 0 2.412-2.413A2.415 2.415 0 0 0 19.998 2c-.93 0-1.739.53-2.141 1.303l-3.986-.938a.655.655 0 0 0-.774.44l-1.55 4.897c-2.578.063-5.001.732-6.889 1.902a2.87 2.87 0 0 0-1.774-.613A2.887 2.887 0 0 0 0 11.875a2.88 2.88 0 0 0 1.249 2.373a5 5 0 0 0-.048.693c0 1.988 1.156 3.837 3.254 5.207c2.011 1.313 4.674 2.036 7.496 2.036s5.485-.723 7.497-2.036c2.098-1.37 3.254-3.22 3.254-5.207q-.001-.32-.04-.633A2.88 2.88 0 0 0 24 11.875"></svg:path>`,
})
export class GrommetIconsRedditIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRedhatIcon],svg[grommet-icons-redhat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:path d="M5.832 6.125Q6.8 2.52 8.492 3.24c1.691.722 2.416.481 3.141 0q.726-.48 2.175 0l3.384 1.443q1.45.48 1.934 3.366q.483 2.884.725 3.846q3.625 1.443 4.109 4.328c.483 2.885-3.384 5.77-11.36 4.327S-.452 14.54.032 11.895q.483-2.644 4.592-2.644z"></svg:path><svg:path d="M4.887 8.538q2.193 3.76 8.04 4.7t6.82-1.88q.417 2.126 0 2.82c-.914 1.527-3.411 1.969-6.09 1.646q-5.846-.706-8.526-3.76q-.73-.94-.731-1.646q0-.705.487-1.88"></svg:path></svg:g>`,
})
export class GrommetIconsRedhatIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRedoIcon],svg[grommet-icons-redo-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M16.82 4L15.4 5.44L17.94 8H8.23a6 6 0 0 0 0 12h2v-2h-2a4 4 0 0 1 0-8h9.71l-2.54 2.51l1.41 1.41L21.77 9Z"></svg:path>`,
})
export class GrommetIconsRedoIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRefreshIcon],svg[grommet-icons-refresh-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 8c-1.403-2.96-4.463-5-8-5a9 9 0 1 0 0 18a9 9 0 0 0 9-9m0-9v6h-6"></svg:path>`,
})
export class GrommetIconsRefreshIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsResourcesIcon],svg[grommet-icons-resources-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m12 3l9 4.5l-9 4.5l-9-4.5zm4.5 7.25L21 12.5L12 17l-9-4.5l4.5-2.25m9 5L21 17.5L12 22l-9-4.5l4.5-2.25"></svg:path>`,
})
export class GrommetIconsResourcesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRestaurantIcon],svg[grommet-icons-restaurant-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 18H5zm-7 0v-6zm3 0v-4zm-6 0v-4zm10 4V11.33a3.001 3.001 0 1 0-2.08-5.63C16.55 3.874 14.46 2 12 2S7.45 3.874 7.08 5.7A3 3 0 1 0 5 11.33V22z"></svg:path>`,
})
export class GrommetIconsRestaurantIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRestroomIcon],svg[grommet-icons-restroom-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M6 11h12M6 3h12m-6 13a5 5 0 0 0 5-5H7a5 5 0 0 0 5 5ZM7 3h10v8H7zm0 3h2.5m5 9.5l1.5 6H8l1.5-6"></svg:path>`,
})
export class GrommetIconsRestroomIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRestroomMenIcon],svg[grommet-icons-restroom-men-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 13.5L11 8l-1 13m7-7.5L13 8l1 13M12 5a1 1 0 1 0 0-2a1 1 0 0 0 0 2m-1 3h2v5.5h-2z"></svg:path>`,
})
export class GrommetIconsRestroomMenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRestroomWomenIcon],svg[grommet-icons-restroom-women-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 13.5L11 8l1 13m5-7.5L13 8l-1 13m0-16a1 1 0 1 0 0-2a1 1 0 0 0 0 2m-1 3h2l1.5 8.5h-5z"></svg:path>`,
})
export class GrommetIconsRestroomWomenIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsResumeIcon],svg[grommet-icons-resume-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 20h5V4H1zm10-1l11-7l-11-7z"></svg:path>`,
})
export class GrommetIconsResumeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsReturnIcon],svg[grommet-icons-return-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m9 19l-5-5l5-5m9-5v10H5"></svg:path>`,
})
export class GrommetIconsReturnIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRevertIcon],svg[grommet-icons-revert-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 3L3 8l5 5m4 7h3a6 6 0 1 0 0-12H4"></svg:path>`,
})
export class GrommetIconsRevertIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRewindIcon],svg[grommet-icons-rewind-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 3.5V20l-9-6v6L2 12l11-8v6z"></svg:path>`,
})
export class GrommetIconsRewindIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRiskIcon],svg[grommet-icons-risk-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 10h1V9h-1zm4 0h1V9h-1zm0-4h1V5h-1zm-4 0h1V5h-1zM9 19h1v-1H9zm-4-4h1v-1H5zm5-5H1v13h13v-9m-4 0h13V1H10z"></svg:path>`,
})
export class GrommetIconsRiskIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRobotIcon],svg[grommet-icons-robot-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18.348 15.954a7 7 0 1 0-12.622.156M12 3a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0 20a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm0-17V3M9 14a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm6 0a1 1 0 1 0 0-2a1 1 0 0 0 0 2Zm-9 4.988L5 16s.072-.772.5-.5c.93.591 3.074 1.5 6.5 1.5c3.554 0 5.618-.916 6.5-1.5c.359-.238.5.5.5.5l-1 2.988S17.005 21 12 21s-6-2.012-6-2.012Z"></svg:path>`,
})
export class GrommetIconsRobotIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRotateLeftIcon],svg[grommet-icons-rotate-left-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M11.52 3.43A9.1 9.1 0 0 0 5.7 5.55v-3.2H4.07v6.5h6.5V7.21H6.3a7.46 7.46 0 1 1-1.47 8.65l-1.46.73a9.11 9.11 0 1 0 8.15-13.16"></svg:path>`,
})
export class GrommetIconsRotateLeftIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRotateRightIcon],svg[grommet-icons-rotate-right-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M12.48 3.43a9.1 9.1 0 0 1 5.82 2.12v-3.2h1.64v6.5h-6.5V7.21h4.26a7.46 7.46 0 1 0 1.47 8.65l1.46.73a9.11 9.11 0 1 1-8.15-13.16"></svg:path>`,
})
export class GrommetIconsRotateRightIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRssIcon],svg[grommet-icons-rss-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 21C22 10.507 13.493 2 3 2m14 19c0-7.732-6.268-14-14-14m9 14a9 9 0 0 0-9-9m1 11a3 3 0 1 0 0-6a3 3 0 0 0 0 6Z"></svg:path>`,
})
export class GrommetIconsRssIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsRunIcon],svg[grommet-icons-run-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m15 11l3 2m0-8a1 1 0 1 0 0-2a1 1 0 0 0 0 2M9.5 9.5L9.525 6H15L8 17H4m11-9l-3 5l.5 1L17 7.5L15 6m-4 7l5 3.5v5"></svg:path>`,
})
export class GrommetIconsRunIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSafariOptionIcon],svg[grommet-icons-safari-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.541 11.844c.022-.414-.33-.772-.775-.79c-.47-.02-.827.325-.835.802c-.007.423.337.758.79.772c.45.014.795-.317.82-.784m4.673-6.722l-.053-.04c-.053.044-.109.087-.16.135c-1.914 1.787-3.831 3.571-5.741 5.364a3.5 3.5 0 0 0-.588.696c-1.401 2.307-2.789 4.623-4.179 6.936c-.057.094-.102.194-.153.292a.24.24 0 0 0 .172-.069c1.94-1.812 3.883-3.62 5.814-5.443a4 4 0 0 0 .65-.824c1.371-2.256 2.73-4.521 4.092-6.783c.052-.086.098-.176.146-.264m-.07-1.094c1.999 1.394 3.251 3.257 3.89 5.607c-.233.057-.45.093-.652.167c-.087.032-.145.144-.216.219c.094.041.193.124.283.117c.206-.02.408-.083.655-.138c.45 2.407.068 4.629-1.183 6.729c-.195-.118-.357-.242-.537-.317c-.12-.05-.266-.033-.4-.045c.065.109.11.242.202.323c.148.129.327.224.533.36c-1.375 2.01-3.215 3.324-5.6 3.961c-.07-.266-.121-.526-.212-.773c-.033-.09-.157-.147-.241-.22c-.026.1-.087.206-.074.3c.035.242.103.48.17.774c-2.431.453-4.689.095-6.835-1.19c.164-.256.33-.483.458-.73c.053-.101.025-.244.034-.368c-.113.06-.259.095-.332.187c-.167.208-.298.446-.47.713c-2.039-1.396-3.342-3.275-3.983-5.68c.31-.072.574-.12.826-.203c.085-.028.14-.146.21-.224c-.1-.033-.204-.104-.299-.092c-.253.033-.502.1-.797.164c-.448-2.44-.062-4.696 1.251-6.829c.19.12.354.25.54.334c.144.066.314.074.473.108c-.096-.14-.172-.3-.294-.412c-.145-.131-.33-.219-.525-.343c1.425-1.992 3.285-3.301 5.708-3.895c.056.241.087.471.165.683c.045.119.165.208.251.31c.03-.12.092-.245.08-.362c-.022-.22-.087-.434-.145-.698c2.415-.423 4.647-.028 6.751 1.255c-.15.242-.302.46-.42.695c-.042.082.003.208.008.313c.09-.04.208-.056.262-.123c.16-.201.293-.422.465-.677M11.845 22.42c5.876-.014 10.586-4.76 10.577-10.659c-.008-5.772-4.783-10.507-10.589-10.499C5.936 1.27 1.25 6.01 1.264 11.95c.015 5.752 4.797 10.484 10.581 10.47M11.836 0c6.506-.007 11.845 5.307 11.85 11.794A11.86 11.86 0 0 1 11.86 23.683C5.328 23.715 0 18.378 0 11.8C0 5.303 5.312.007 11.836.001"></svg:path>`,
})
export class GrommetIconsSafariOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSamsungPayIcon],svg[grommet-icons-samsung-pay-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:g clip-path="url(#grommetIconsSamsungPay0)"><svg:path fill="currentColor" fill-rule="evenodd" d="M21.33 2.688c1.395 1.48 2.192 3.502 2.477 5.723c.284 2.135.17 3.587.17 3.587s.086 1.452-.199 3.587c-.284 2.22-1.082 4.243-2.477 5.723c-1.48 1.395-3.502 2.192-5.722 2.477c-2.136.256-3.588.2-3.588.2s-1.451.084-3.587-.2c-2.22-.285-4.242-1.082-5.722-2.477c-1.395-1.48-2.193-3.502-2.477-5.723c-.257-2.135-.2-3.587-.2-3.587s-.057-1.452.228-3.587C.518 6.19 1.315 4.169 2.71 2.688C4.19 1.293 6.212.496 8.433.211C10.568-.045 12.02.012 12.02.012s1.452-.085 3.587.2c2.22.284 4.242 1.081 5.723 2.476M6.582 8.496H4.447v6.292h1.167v-1.793h.968c.342 0 .655-.058.911-.172s.513-.284.712-.483a2.1 2.1 0 0 0 .484-.712c.114-.285.17-.57.17-.883a2.3 2.3 0 0 0-.17-.882a2.1 2.1 0 0 0-.484-.712a2.1 2.1 0 0 0-.712-.484a2.4 2.4 0 0 0-.91-.17Zm-.996 3.388V9.55h.91c.172 0 .342.028.485.085a1.1 1.1 0 0 1 .37.256c.085.114.17.228.228.37c.057.143.085.285.085.456c0 .17-.028.313-.085.456a1.08 1.08 0 0 1-.598.626a1.3 1.3 0 0 1-.484.085zm5.067 2.733c.314.114.598.171.912.171c.341 0 .626-.057.91-.171c.285-.142.542-.313.712-.541v.712h1.168V9.72h-1.168v.655c-.199-.2-.427-.37-.711-.484a2.3 2.3 0 0 0-.912-.17c-.341 0-.626.056-.939.17c-.285.114-.57.256-.797.484a2.7 2.7 0 0 0-.57.797c-.142.313-.199.684-.199 1.082c0 .399.086.769.228 1.082s.342.57.57.797c.227.2.512.37.796.484m1.709-.996c-.171.085-.37.114-.598.114a1.9 1.9 0 0 1-.598-.114l-.004-.002c-.17-.085-.339-.17-.48-.311a3 3 0 0 1-.313-.484a1.3 1.3 0 0 1-.114-.57c0-.199.028-.398.114-.569c.057-.17.17-.313.313-.456a1.4 1.4 0 0 1 .484-.313c.2-.085.399-.114.598-.114c.2 0 .398.029.598.143c.199.085.341.17.484.313c.142.114.227.285.313.456c.085.17.114.37.114.569s-.029.398-.114.57a1.6 1.6 0 0 1-.313.455c-.143.142-.313.228-.484.313m5.039-.427l-1.424-3.445h-1.224l2.05 4.812l-1.053 2.533h1.195l2.99-7.345H18.71z" clip-rule="evenodd"></svg:path></svg:g><svg:defs><svg:clippath id="grommetIconsSamsungPay0"><svg:path fill="#fff" d="M0 0h24v24H0z"></svg:path></svg:clippath></svg:defs></svg:g>`,
})
export class GrommetIconsSamsungPayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSansIcon],svg[grommet-icons-sans-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 12h22M2 22h20a1 1 0 0 0 1-1V3a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1zM5 8a1 1 0 1 0 0-2a1 1 0 0 0 0 2zm0 10a1 1 0 1 0 0-2a1 1 0 0 0 0 2z"></svg:path>`,
})
export class GrommetIconsSansIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSatelliteIcon],svg[grommet-icons-satellite-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 17C2.97 12.794 2.97 6.118 7 2l15 15c-4.118 4.03-10.794 4.03-15 0Zm0 0c-3.295 0-6 2.95-6 6h12c0-1.139-.37-2.034-1-3m3-11l4-4zm5.5-8a2.5 2.5 0 1 0 0 5a2.5 2.5 0 0 0 0-5Z"></svg:path>`,
})
export class GrommetIconsSatelliteIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSaveIcon],svg[grommet-icons-save-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M3 2v19h18V3h-9v11m-4-3l4 4l4-4"></svg:path>`,
})
export class GrommetIconsSaveIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScanIcon],svg[grommet-icons-scan-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M20 10V3H4v7m-3 2h22zm3 1v3zm16 3v-3zM7 21H4v-3m16 0v3h-3m-8 0h6z"></svg:path>`,
})
export class GrommetIconsScanIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScheduleIcon],svg[grommet-icons-schedule-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 23h22V4H1zM18 4V0zM6 4V0zM1 8.5h22zM6 14q.834-2 2.5-2c1.3 0 2 1 2 2s-1 2-2 3l-2 2v.5h5.405m5.08 1L17 12h-.5c-.5 1.5-2 2-2.743 2"></svg:path>`,
})
export class GrommetIconsScheduleIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScheduleNewIcon],svg[grommet-icons-schedule-new-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 0v3M1 7h18M6 0v3m-2 8h2m2 0h8M4 15h2m2 0h6m-2 4H1V3h18v10m0 2v9m-4-7l8 5m0-5l-8 5"></svg:path>`,
})
export class GrommetIconsScheduleNewIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSchedulePlayIcon],svg[grommet-icons-schedule-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 0v3M1 7h18M6 0v3m-2 8h2m2 0h8M4 15h2m2 0h6m-1 4H1V3h18v10m-1 10a5 5 0 1 0 0-10a5 5 0 0 0 0 10Zm-.5-6l1.5 1l-1.5 1z"></svg:path>`,
})
export class GrommetIconsSchedulePlayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSchedulesIcon],svg[grommet-icons-schedules-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M17 7h6v16H7v-4m16-8h-6M13 0v3M1 7h16M1 3h16v16H1zm4-3v3m-1 8h2m2 0h6M4 15h2m2 0h6"></svg:path>`,
})
export class GrommetIconsSchedulesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScoIcon],svg[grommet-icons-sco-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M16.16 16.105H24c-.247.746-.99 1.367-1.794 1.367H1.273c-.804 0-1.33-.62-1.268-1.367h14.702a3.8 3.8 0 0 1-.2-1.957h.959c0 .187 0 .343.03.497c.31-.093.62-.28.897-.497h.619c-.526.56-1.02 1.025-1.268 1.212c.114.277.261.528.416.745m5.583-8.39C20.692 7.28 19.362 7 17.847 7c-1.546 0-3.03.28-4.206.715l.032.062h8.009zm-6.617 4.941c.031-.062.062-.124.093-.218l-10.792.032l-.03-.125c2.659-.528 5.967-.839 9.523-.839c.557 0 1.144 0 1.67.031c.03-.124.062-.217.093-.342H7.026l-.031-.093c2.319-.497 5.225-.808 8.348-.808h.34c-.03-.062-.061-.186-.092-.28H9.715l-.03-.093a29 29 0 0 1 5.38-.684a1.6 1.6 0 0 0-.34-.31h-2.876l-.062-.094c1.484-.466 3.278-.745 5.226-.745c1.887 0 3.556.31 4.885.745l-.03.063l-6.555.03q.138.094.278.28c.37-.031.741-.031 1.113-.031c2.35 0 4.514.28 6.245.653l-.03.186h-6.896c.031.094.031.218.062.311c2.505.032 4.793.31 6.71.683l-.031.187h-6.617c0 .125 0 .249-.031.342c2.412.124 4.638.373 6.494.745l-.031.156h-4.144a.55.55 0 0 1-.155.218h-.494c.03-.062.093-.125.124-.218h-1.948c-.031.093-.031.156-.063.218zm-.62 1.492c.032-.156.063-.31.125-.467H6.839l-.062-.124c2.32-.466 5.195-.777 8.287-.777c0-.031.031-.093.062-.124h.773c0 .031-.031.093-.031.124c.587.031 1.206.031 1.762.093a1 1 0 0 0 .154-.217h.495c-.03.093-.093.155-.186.249a33 33 0 0 1 4.36.559l-.03.217h-4.98c-.154.156-.278.31-.432.467h-.618c.217-.125.402-.31.557-.467h-1.423a3 3 0 0 0-.062.467z"></svg:path>`,
})
export class GrommetIconsScoIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScorecardIcon],svg[grommet-icons-scorecard-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M9 18v-6m3 6v-5m3 5v-8m2-7h4v20H3V3h4m0-2h10v4H7z"></svg:path>`,
})
export class GrommetIconsScorecardIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsScriptIcon],svg[grommet-icons-script-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M4 1a4 4 0 0 0-4 4v1h5.847L4.01 18.859l-.01.07V19a4 4 0 0 0 4 4h12a4 4 0 0 0 4-4v-1h-5.847L19.99 5.141l.01-.07V5a4 4 0 0 0-4-4zm0 2a2 2 0 0 0-1.732 1h3.464A2 2 0 0 0 4 3m3.465 0H16a2 2 0 0 1 1.999 1.936L16.133 18H10v1a2 2 0 0 1-3.999.064L7.99 5.141l.01-.07V5c0-.729-.195-1.412-.535-2M20 21h-8.535a4 4 0 0 0 .409-1h9.858A2 2 0 0 1 20 21" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsScriptIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSdIcon],svg[grommet-icons-sd-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M2 2a2 2 0 0 1 2-2h13a1 1 0 0 1 .707.293l4 4A1 1 0 0 1 22 5v17a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2zm14.586 0H4v20h16V5.414zM5 7V3h2v4zm3-4v4h2V3zm3 4V3h2v4zm3-2v2h2V5z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsSdIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSearchIcon],svg[grommet-icons-search-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m15 15l7 7zm-5.5 2a7.5 7.5 0 1 0 0-15a7.5 7.5 0 0 0 0 15Z"></svg:path>`,
})
export class GrommetIconsSearchIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSearchAdvancedIcon],svg[grommet-icons-search-advanced-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m15 16l6 6zm-5 2a7 7 0 1 0 0-14a7 7 0 0 0 0 14ZM20 1v6m-3-3h6"></svg:path>`,
})
export class GrommetIconsSearchAdvancedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSecureIcon],svg[grommet-icons-secure-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 11V6c0-3 2-5 5-5s5 2 5 5v5m-5 12a7 7 0 1 0 0-14a7 7 0 0 0 0 14Zm0-8v4m0-3a1 1 0 1 0 0-2a1 1 0 0 0 0 2Z"></svg:path>`,
})
export class GrommetIconsSecureIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSelectIcon],svg[grommet-icons-select-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 1h6zm11.188 18.472L16 22l-3.5-4.5l-3 3.5L7 7l13 6.5l-4.5 1.5zM19 4V1h-3M6 1H3v3m0 10v3h3M19 6v4zM3 12V6z"></svg:path>`,
})
export class GrommetIconsSelectIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSelectionIcon],svg[grommet-icons-selection-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M5 18h3V6H5zm7-16v20zM1 22h22V2H1z"></svg:path>`,
})
export class GrommetIconsSelectionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSemanticsIcon],svg[grommet-icons-semantics-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m2 17l10-5l10 5v4l-10-5l-10 5zm0-9l10-5l10 5v4L12 7L2 12z"></svg:path>`,
})
export class GrommetIconsSemanticsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSendIcon],svg[grommet-icons-send-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M22 3L2 11l18.5 8zM10 20.5l3-4.5m2.5-6.5L9 14l.859 6.012c.078.546.216.537.306-.003L11 15z"></svg:path>`,
})
export class GrommetIconsSendIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsServerIcon],svg[grommet-icons-server-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M14 19h1v-1h-1zm-9 4h14V1H5zM8 5h8zm0 4h8zm0 4h8z"></svg:path>`,
})
export class GrommetIconsServerIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsServerClusterIcon],svg[grommet-icons-server-cluster-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 8h22V1H1zm10-3h1V4h-1zm8 0h1V4h-1zm-4 0h1V4h-1zm-4 7h1v-1h-1zm8 0h1v-1h-1zm-4 0h1v-1h-1zm-4 7h1v-1h-1zm8 0h1v-1h-1zm-4 0h1v-1h-1zM1 15h22V8H1zm0 7h22v-7H1zm20 1H3"></svg:path>`,
})
export class GrommetIconsServerClusterIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsServersIcon],svg[grommet-icons-servers-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 19h1v-1H7zm11 0h1v-1h-1zM1 23h11V1H1zm11 0h11V1H12zM4 5h5zm11 0h5zM4 9h5zm11 0h5zM4 13h5zm11 0h5z"></svg:path>`,
})
export class GrommetIconsServersIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsServicePlayIcon],svg[grommet-icons-service-play-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 13A6 6 0 1 0 7 1a6 6 0 0 0 0 12Zm7.995 3.657a6 6 0 1 0-1.89-10.22m-8.281 6.255A6 6 0 0 0 9 23a6 6 0 0 0 2.127-11.612M6.5 6L8 7L6.5 8z"></svg:path>`,
})
export class GrommetIconsServicePlayIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsServicesIcon],svg[grommet-icons-services-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M6 9a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0-6V0m0 12V9M0 6h3m6 0h3M2 2l2 2m4 4l2 2m0-8L8 4M4 8l-2 2m16 2a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0-6V3m0 12v-3m-6-3h3m6 0h3M14 5l2 2m4 4l2 2m0-8l-2 2m-4 4l-2 2m-5 8a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0-6v-3m0 12v-3m-6-3h3m6 0h3M5 14l2 2m4 4l2 2m0-8l-2 2m-4 4l-2 2"></svg:path>`,
})
export class GrommetIconsServicesIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSettingsOptionIcon],svg[grommet-icons-settings-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 9V0m3 12h9M0 12h9m3 12v-9m0 6a9 9 0 1 0 0-18a9 9 0 0 0 0 18ZM3.5 8.5L1 7.5m19.5 8l2.5 1M3 3l2.5 2.5M3 3l2.5 2.5M18 18l2.5 2.5m0-17.5L18 5.5M5.5 18L3 20.5m9-5.5a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm8.5-6.5l2.5-1m-7.5-4l1-2.5m-1 19.5l1 2.5m-8-2.5l-1 2.5m-4-7.5l-2.5 1m7.5-13L7.5 1"></svg:path>`,
})
export class GrommetIconsSettingsOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShareIcon],svg[grommet-icons-share-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M19 13v10H1V5h10m3-4h9v9m-13 4L23 1z"></svg:path>`,
})
export class GrommetIconsShareIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShareOptionIcon],svg[grommet-icons-share-option-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M18 8a3 3 0 1 0 0-6a3 3 0 0 0 0 6ZM6 15a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm12 7a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm-2-4l-8-5m8-7l-8 5"></svg:path>`,
})
export class GrommetIconsShareOptionIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShareRoundedIcon],svg[grommet-icons-share-rounded-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M20.924 3.617a1 1 0 0 0-.215-.322l-.004-.004A1 1 0 0 0 20 3h-6a1 1 0 1 0 0 2h3.586l-7.293 7.293a1 1 0 1 0 1.414 1.414L19 6.414V10a1 1 0 1 0 2 0V3.997q0-.196-.076-.38M3 8a5 5 0 0 1 5-5h1a1 1 0 0 1 0 2H8a3 3 0 0 0-3 3v8a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3v-1a1 1 0 1 1 2 0v1a5 5 0 0 1-5 5H8a5 5 0 0 1-5-5z" clip-rule="evenodd"></svg:path>`,
})
export class GrommetIconsShareRoundedIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShieldIcon],svg[grommet-icons-shield-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22S3 18 3 5l9-3l9 3c0 13-9 17-9 17ZM4 11h16m-8-9v20"></svg:path>`,
})
export class GrommetIconsShieldIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShieldSecurityIcon],svg[grommet-icons-shield-security-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22s-9-4-9-11V5l9-3l9 3v6c0 7-9 11-9 11Zm0-8a3 3 0 1 0 0-6a3 3 0 0 0 0 6Zm0-6V5m0 12v-3m-6-3h3m6 0h3M8 7l2 2m4 4l2 2m0-8l-2 2m-4 4l-2 2"></svg:path>`,
})
export class GrommetIconsShieldSecurityIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShiftIcon],svg[grommet-icons-shift-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 0v24M2 12h10m10 0H12M6 8l-4 4l4 4m12-8l4 4l-4 4"></svg:path>`,
})
export class GrommetIconsShiftIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsShopIcon],svg[grommet-icons-shop-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M4 7h16v16H4zm4 2V5c0-2.21 1.795-4 4-4h0c2.21 0 4 1.795 4 4v4"></svg:path>`,
})
export class GrommetIconsShopIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSidebarIcon],svg[grommet-icons-sidebar-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 22h22V2H1zM16 2v20z"></svg:path>`,
})
export class GrommetIconsSidebarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSignIcon],svg[grommet-icons-sign-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M8 23h7c3 0 4-2 4-5V6c0-2-1-2-1.5-2S16 4 16 6v7c0-1 0-2-1.5-2S13 13 13 13c0-1 0-2-1.5-2S10 12 10 13V4c0-1 .03-2-1.5-2C7 2 7 3 7 4v14v-4c0-1-.55-2-2-2H4v6c0 3.962 2 5.024 4 5Z"></svg:path>`,
})
export class GrommetIconsSignIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSkypeIcon],svg[grommet-icons-skype-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.052 18.856c-4.027 0-5.828-1.98-5.828-3.463c0-.761.562-1.295 1.336-1.295c1.724 0 1.277 2.475 4.492 2.475c1.645 0 2.554-.894 2.554-1.809c0-.55-.271-1.159-1.355-1.426l-3.581-.894c-2.884-.723-3.407-2.282-3.407-3.748c0-3.043 2.864-4.185 5.556-4.185c2.477 0 5.4 1.369 5.4 3.194c0 .783-.678 1.238-1.452 1.238c-1.47 0-1.2-2.035-4.161-2.035c-1.47 0-2.284.665-2.284 1.618c0 .95 1.16 1.254 2.168 1.483l2.651.59c2.903.646 3.64 2.34 3.64 3.938c0 2.472-1.898 4.319-5.73 4.319m11.1-4.887l-.022.127l-.04-.241zc.124-.674.19-1.364.19-2.054a11.3 11.3 0 0 0-3.32-8.014A11.3 11.3 0 0 0 12.006.583c-.722 0-1.444.067-2.147.202l-.005.001q.06.03.118.062L9.736.81l.12-.024A6.7 6.7 0 0 0 6.709 0a6.66 6.66 0 0 0-4.744 1.965A6.67 6.67 0 0 0 0 6.71c0 1.14.293 2.26.844 3.252l.02-.123l.041.237l-.061-.114a11.4 11.4 0 0 0-.171 1.953c0 1.53.3 3.015.892 4.412a11.3 11.3 0 0 0 2.427 3.602a11.3 11.3 0 0 0 3.603 2.428c1.397.592 2.882.892 4.412.892c.665 0 1.332-.061 1.984-.177q-.057-.03-.115-.063l.242.043l-.127.02c1.004.569 2.14.87 3.3.87a6.66 6.66 0 0 0 4.744-1.965A6.66 6.66 0 0 0 24 17.233a6.7 6.7 0 0 0-.85-3.264"></svg:path>`,
})
export class GrommetIconsSkypeIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSlackIcon],svg[grommet-icons-slack-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M5.048 15.124a2.51 2.51 0 0 1-2.515 2.514A2.51 2.51 0 0 1 .02 15.124a2.51 2.51 0 0 1 2.514-2.514h2.515zm1.257 0a2.51 2.51 0 0 1 2.514-2.514a2.51 2.51 0 0 1 2.514 2.514v6.286a2.51 2.51 0 0 1-2.514 2.514a2.51 2.51 0 0 1-2.514-2.514zM8.819 5.029a2.51 2.51 0 0 1-2.514-2.515A2.51 2.51 0 0 1 8.819 0a2.51 2.51 0 0 1 2.514 2.514V5.03zm0 1.276a2.51 2.51 0 0 1 2.514 2.514a2.51 2.51 0 0 1-2.514 2.514H2.514A2.51 2.51 0 0 1 0 8.82a2.51 2.51 0 0 1 2.514-2.514zm10.076 2.514a2.51 2.51 0 0 1 2.515-2.514a2.51 2.51 0 0 1 2.514 2.514a2.51 2.51 0 0 1-2.514 2.514h-2.515zm-1.257 0a2.51 2.51 0 0 1-2.514 2.514a2.51 2.51 0 0 1-2.515-2.514V2.514A2.51 2.51 0 0 1 15.124 0a2.51 2.51 0 0 1 2.514 2.514zm-2.514 10.076a2.51 2.51 0 0 1 2.514 2.514a2.51 2.51 0 0 1-2.514 2.515a2.51 2.51 0 0 1-2.514-2.515v-2.514zm0-1.257a2.51 2.51 0 0 1-2.514-2.514a2.51 2.51 0 0 1 2.514-2.514h6.305a2.51 2.51 0 0 1 2.514 2.514a2.51 2.51 0 0 1-2.514 2.514z"></svg:path>`,
})
export class GrommetIconsSlackIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSnapchatIcon],svg[grommet-icons-snapchat-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12.151 22.532c-.068 0-.133-.003-.182-.005q-.06.005-.12.005c-1.43 0-2.385-.676-3.228-1.272c-.604-.427-1.174-.83-1.842-.941a6 6 0 0 0-.97-.082c-.567 0-1.016.088-1.344.152c-.201.039-.375.073-.51.073c-.14 0-.311-.031-.383-.275a8 8 0 0 1-.136-.557c-.098-.447-.169-.72-.336-.746C1.308 18.607.249 18.2.038 17.706A.5.5 0 0 1 0 17.551a.29.29 0 0 1 .241-.3c1.423-.235 2.689-.987 3.762-2.237a8.4 8.4 0 0 0 1.29-2.008q.31-.632.122-1.078c-.231-.544-.996-.787-1.502-.948a5 5 0 0 1-.34-.115c-.448-.177-1.186-.551-1.088-1.068c.072-.377.57-.64.973-.64q.169 0 .294.06c.456.213.865.321 1.217.321c.438 0 .65-.167.7-.214l-.042-.717c-.103-1.636-.231-3.67.29-4.838C7.473.276 10.777.005 11.752.005L12.236 0c.978 0 4.289.272 5.848 3.767c.52 1.168.392 3.205.289 4.842l-.005.078l-.038.637c.048.044.242.197.635.212c.336-.013.722-.12 1.147-.32a.9.9 0 0 1 .375-.074c.15 0 .301.03.428.082l.007.003c.36.128.596.384.601.652c.005.25-.181.625-1.097.986a6 6 0 0 1-.34.116c-.506.16-1.27.403-1.501.947c-.126.295-.084.658.123 1.077l.006.014c.064.15 1.605 3.665 5.045 4.231a.29.29 0 0 1 .24.3a.5.5 0 0 1-.038.157c-.209.491-1.268.898-3.06 1.175c-.169.026-.24.298-.337.743c-.04.184-.08.364-.136.553q-.078.267-.355.266h-.028q-.183 0-.51-.065a6.7 6.7 0 0 0-1.345-.142q-.474 0-.97.082c-.666.11-1.236.513-1.84.94c-.844.597-1.8 1.273-3.229 1.273"></svg:path>`,
})
export class GrommetIconsSnapchatIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSolarisIcon],svg[grommet-icons-solaris-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M10.18 4.764c-.872-.309-1.27-1.127-1.27-1.127c-.068-.094-.353-.779-.353-.78c-.3-.792-.845-1.035-.845-1.035c-.671-.394-1.5-.404-1.5-.404c.035-.005.493.207.493.207c1.08.515.756 1.744.756 1.744c-.07.331-.086.97-.086.97a1.55 1.55 0 0 0 .659 1.312a7.6 7.6 0 0 1 2.147-.887M5.597 8.17c-.836.399-1.696.102-1.696.102c-.115-.018-.8-.3-.8-.3c-.773-.35-1.33-.136-1.33-.136c-.754.196-1.347.775-1.347.775c.021-.028.495-.202.495-.202c1.128-.4 1.768.698 1.768.698c.184.284.625.747.625.747a1.55 1.55 0 0 0 1.393.462a7.5 7.5 0 0 1 .892-2.146m-.832 5.65c-.31.872-1.127 1.272-1.127 1.272c-.095.067-.78.352-.78.352c-.793.3-1.035.846-1.035.846c-.395.67-.404 1.5-.404 1.5c-.005-.036.206-.494.206-.494c.515-1.08 1.745-.756 1.745-.756c.33.07.969.086.969.086a1.55 1.55 0 0 0 1.312-.658a7.6 7.6 0 0 1-.886-2.148m3.406 4.583c.398.836.102 1.696.102 1.696c-.019.115-.301.801-.302.801c-.349.772-.134 1.33-.134 1.33c.195.754.775 1.347.775 1.347c-.029-.022-.203-.496-.203-.496c-.4-1.127.699-1.767.699-1.767c.283-.185.747-.626.747-.626a1.55 1.55 0 0 0 .461-1.393a7.6 7.6 0 0 1-2.145-.892m5.649.833c.873.309 1.272 1.127 1.272 1.127c.068.094.353.78.352.78c.3.792.846 1.035.846 1.035c.671.395 1.5.404 1.5.404c-.035.005-.493-.207-.493-.207c-1.08-.515-.757-1.744-.757-1.744c.07-.33.086-.97.086-.97a1.55 1.55 0 0 0-.658-1.312a7.6 7.6 0 0 1-2.148.887m4.584-3.406c.835-.399 1.696-.102 1.696-.102c.115.018.8.301.8.301c.773.35 1.33.135 1.33.135c.754-.196 1.347-.775 1.347-.775c-.022.029-.495.202-.495.202c-1.128.4-1.768-.698-1.768-.698c-.185-.284-.625-.747-.625-.747a1.55 1.55 0 0 0-1.394-.462a7.6 7.6 0 0 1-.891 2.146m.832-5.65c.31-.872 1.127-1.272 1.127-1.272c.094-.067.78-.353.78-.352c.792-.3 1.035-.845 1.035-.845c.395-.672.405-1.5.405-1.5c.004.035-.208.493-.208.493c-.514 1.08-1.744.756-1.744.756c-.33-.07-.97-.086-.97-.086a1.55 1.55 0 0 0-1.312.658q.307.504.536 1.066q.22.538.351 1.082M15.83 5.597c-.398-.836-.103-1.696-.103-1.696c.02-.115.302-.801.303-.8c.349-.773.134-1.33.134-1.33c-.196-.755-.775-1.348-.775-1.348c.029.022.203.496.203.496c.399 1.127-.7 1.768-.7 1.768c-.282.185-.746.625-.746.625a1.55 1.55 0 0 0-.462 1.393a7.6 7.6 0 0 1 2.146.892m-8.243.387c-.925.042-1.602-.565-1.602-.565c-.099-.061-.62-.588-.62-.589c-.577-.621-1.174-.64-1.174-.64c-.77-.113-1.542.19-1.542.19c.031-.017.535.007.535.007c1.194.07 1.358 1.33 1.358 1.33c.06.333.286.93.286.93a1.55 1.55 0 0 0 1.104.967a7.6 7.6 0 0 1 1.655-1.63m-2.961 4.882c-.624.684-1.533.734-1.533.734c-.113.026-.855.023-.855.022C1.391 11.59.956 12 .956 12C.33 12.464 0 13.225 0 13.225c.01-.035.383-.375.383-.375c.893-.795 1.9-.02 1.9-.02c.278.194.86.457.86.457a1.55 1.55 0 0 0 1.465-.097a7.6 7.6 0 0 1 .018-2.324m1.358 5.547c.042.925-.564 1.603-.564 1.603c-.062.098-.59.62-.59.62c-.621.576-.64 1.173-.64 1.173c-.113.77.19 1.542.19 1.542c-.017-.03.007-.535.007-.535c.07-1.194 1.33-1.358 1.33-1.358c.333-.06.93-.285.93-.285a1.55 1.55 0 0 0 .968-1.105a7.6 7.6 0 0 1-1.631-1.655m4.882 2.962c.684.624.734 1.532.734 1.532c.026.113.023.855.023.855c-.033.847.376 1.283.376 1.283c.465.624 1.226.955 1.226.955c-.035-.01-.375-.382-.375-.382c-.795-.894-.019-1.901-.019-1.901c.193-.278.456-.86.456-.86a1.55 1.55 0 0 0-.097-1.465a7.5 7.5 0 0 1-2.324-.017m5.547-1.358c.925-.043 1.603.564 1.603.564c.098.062.62.589.62.589c.576.622 1.173.64 1.173.64c.77.113 1.542-.19 1.542-.19c-.03.017-.535-.006-.535-.006c-1.194-.07-1.358-1.33-1.358-1.33c-.06-.333-.285-.931-.285-.931a1.55 1.55 0 0 0-1.105-.967a7.6 7.6 0 0 1-1.655 1.63m2.962-4.882c.624-.684 1.532-.734 1.532-.734c.114-.026.856-.023.856-.022c.846.032 1.282-.377 1.282-.377c.624-.465.955-1.225.955-1.225c-.01.034-.382.374-.382.374c-.894.795-1.901.02-1.901.02c-.278-.194-.86-.457-.86-.457a1.55 1.55 0 0 0-1.465.097a7.6 7.6 0 0 1-.017 2.324m-1.358-5.547c-.043-.925.564-1.602.564-1.602c.062-.099.589-.621.59-.62c.62-.577.64-1.174.64-1.174c.112-.77-.191-1.542-.191-1.542c.018.031-.006.535-.006.535c-.07 1.194-1.33 1.358-1.33 1.358c-.333.06-.931.286-.931.286a1.55 1.55 0 0 0-.968 1.104a7.6 7.6 0 0 1 1.632 1.655m-4.883-2.961c-.684-.624-.734-1.533-.734-1.533c-.026-.113-.023-.855-.022-.855C12.41 1.39 12 .956 12 .956C11.536.33 10.776 0 10.776 0c.034.01.374.383.374.383c.795.894.02 1.9.02 1.9c-.193.278-.457.86-.457.86c-.345.78.053 1.399.097 1.466a7.5 7.5 0 0 1 2.324.017"></svg:path>`,
})
export class GrommetIconsSolarisIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSortIcon],svg[grommet-icons-sort-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M7 3h15M7 9h9m-9 6h15M2 2h2v2H2zm0 6h2v2H2zm0 6h2v2H2zm0 6h2v2H2zm5 1h9"></svg:path>`,
})
export class GrommetIconsSortIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSoundcloudIcon],svg[grommet-icons-soundcloud-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="none"><svg:path fill="url(#grommetIconsSoundcloud0)" d="M0 15.922q0 .447.324.676a.9.9 0 0 0 .692.162q.347-.067.486-.246q.14-.178.14-.592v-3.238a.8.8 0 0 0-.24-.587a.8.8 0 0 0-.587-.24a.8.8 0 0 0-.575.24a.8.8 0 0 0-.24.587zm2.569 1.385q0 .324.229.486q.228.162.586.162q.369 0 .597-.162q.23-.162.23-.486v-7.55a.8.8 0 0 0-.24-.575a.8.8 0 0 0-.587-.24a.8.8 0 0 0-.575.24a.8.8 0 0 0-.24.576v7.55Zm2.557.358q0 .323.235.485t.603.162q.357 0 .586-.162q.23-.162.229-.485v-6.891q0-.346-.24-.592a.78.78 0 0 0-.575-.246a.8.8 0 0 0-.592.246a.8.8 0 0 0-.246.592zm2.569.033q0 .614.826.614q.827 0 .827-.614V6.53q0-.938-.57-1.06q-.369-.09-.726.211t-.357.849zm2.613.324V5.872q0-.582.346-.693a6.54 6.54 0 0 1 4.668.625A6.56 6.56 0 0 1 17.707 8a6.54 6.54 0 0 1 1.055 3.065q.681-.29 1.452-.29q1.563 0 2.675 1.106q1.11 1.106 1.111 2.657q0 1.564-1.111 2.67t-2.664 1.105l-9.716-.01a.26.26 0 0 1-.15-.124a.33.33 0 0 1-.05-.156Z"></svg:path><svg:defs><svg:lineargradient id="grommetIconsSoundcloud0" x1="30064.2" x2="30064.2" y1="507.299" y2="18096.6" gradientUnits="userSpaceOnUse"><svg:stop stop-color="currentColor"></svg:stop><svg:stop offset="1" stop-color="currentColor"></svg:stop></svg:lineargradient></svg:defs></svg:g>`,
})
export class GrommetIconsSoundcloudIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSpaIcon],svg[grommet-icons-spa-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12 22c1.5 0 4-1 4-5.5S12 6 12 6s-4 6-4 10.5s2.5 5.5 4 5.5Zm0 0c-1.5 0-2.953-.22-5-1.5C3 18 2.5 10 2.5 10s4.5.5 6.5 2m3 10c1.5 0 2.953-.22 5-1.5C21 18 21.5 10 21.5 10s-4.5.5-6.5 2"></svg:path>`,
})
export class GrommetIconsSpaIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSpectrumIcon],svg[grommet-icons-spectrum-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M22.131 23.399h-9.006c-.795 0-1.506-.62-1.503-1.417c.018-4.958-.878-6.584-2.753-8.224c-1.969-1.72-5.414-2.055-7.375-2.094a1.51 1.51 0 0 1-1.479-1.507L0 1.533A1.505 1.505 0 0 1 1.431.023c2.95-.133 9.632.183 15.09 4.956c4.434 3.875 6.824 9.541 7.118 16.859a1.507 1.507 0 0 1-1.508 1.56"></svg:path>`,
})
export class GrommetIconsSpectrumIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSplitIcon],svg[grommet-icons-split-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 22h22V2H1zM12 2v20z"></svg:path>`,
})
export class GrommetIconsSplitIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSplitsIcon],svg[grommet-icons-splits-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M1 22h22V2H1zM8 2v20zm8 0v20z"></svg:path>`,
})
export class GrommetIconsSplitsIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSpotifyIcon],svg[grommet-icons-spotify-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M19.098 10.638C15.23 8.341 8.85 8.13 5.158 9.251a1.122 1.122 0 1 1-.652-2.148C8.745 5.816 15.79 6.064 20.244 8.708a1.122 1.122 0 1 1-1.146 1.93m-.126 3.403a.936.936 0 0 1-1.287.308c-3.225-1.983-8.142-2.557-11.958-1.399a.937.937 0 0 1-1.167-.623a.937.937 0 0 1 .624-1.167c4.358-1.322 9.776-.682 13.48 1.594c.44.271.578.847.308 1.287m-1.469 3.267a.75.75 0 0 1-1.028.25c-2.818-1.723-6.365-2.112-10.542-1.158a.748.748 0 1 1-.333-1.458c4.571-1.045 8.492-.595 11.655 1.338a.75.75 0 0 1 .248 1.028M12 0C5.373 0 0 5.372 0 12s5.373 12 12 12c6.628 0 12-5.372 12-12S18.628 0 12 0"></svg:path>`,
})
export class GrommetIconsSpotifyIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsSquareIcon],svg[grommet-icons-square-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M14.444 8.333H9.556c-.675 0-1.223.548-1.223 1.223v4.888c0 .675.548 1.223 1.223 1.223h4.888c.675 0 1.223-.548 1.223-1.223V9.556c0-.675-.548-1.223-1.223-1.223M18.111 22H5.89A3.89 3.89 0 0 1 2 18.111V5.89A3.89 3.89 0 0 1 5.889 2H18.11A3.89 3.89 0 0 1 22 5.889V18.11A3.89 3.89 0 0 1 18.111 22m0 2A5.89 5.89 0 0 0 24 18.111V5.89A5.89 5.89 0 0 0 18.111 0H5.89A5.89 5.89 0 0 0 0 5.889V18.11A5.89 5.89 0 0 0 5.889 24z"></svg:path>`,
})
export class GrommetIconsSquareIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStackOverflowIcon],svg[grommet-icons-stack-overflow-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor"><svg:path d="M19.127 21.866v-6.43h2.134V24H2v-8.563h2.134v6.43z"></svg:path><svg:path d="m6.49 14.827l10.475 2.19l.444-2.107l-10.476-2.19zm1.385-4.989l9.7 4.518l.887-1.94l-9.7-4.545zm2.688-4.766l8.231 6.845l1.358-1.635l-8.23-6.846zM15.884 0l-1.718 1.275l6.374 8.591l1.719-1.275zM6.268 19.704h10.697V17.57H6.268z"></svg:path></svg:g>`,
})
export class GrommetIconsStackOverflowIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStakeholderIcon],svg[grommet-icons-stakeholder-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="m14 9l4.5 2L23 9V4l-4.5-2L14 4zM7 7a4 4 0 1 0 0 8a4 4 0 0 0 0-8ZM1 23v-2c0-4 2.5-6 6-6s6 2 6 6v2zM14 4l4.5 2L23 4m-4.5 2v5z"></svg:path>`,
})
export class GrommetIconsStakeholderIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStarIcon],svg[grommet-icons-star-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" d="M12 16.667L5 22l3-8l-6-4.5h7.5L12 2l2.5 7.5H22L16 14l3 8z"></svg:path>`,
})
export class GrommetIconsStarIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStarHalfIcon],svg[grommet-icons-star-half-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:g fill="currentColor" fill-rule="evenodd"><svg:path fill-opacity=".2" d="M12 16.667V2l2.5 7.5H22L16 14l3 8z"></svg:path><svg:path d="M12 16.667L5 22l3-8l-6-4.5h7.5L12 2z"></svg:path></svg:g>`,
})
export class GrommetIconsStarHalfIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStarOutlineIcon],svg[grommet-icons-star-outline-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12.606 16.805L12 16.343l-.606.462L5.755 21.1l2.381-6.35l.263-.701l-.599-.45L3 10h6.72l.229-.684L12 3.162l2.051 6.154l.228.684H21l-4.8 3.6l-.6.45l.264.701l2.38 6.35z"></svg:path>`,
})
export class GrommetIconsStarOutlineIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStatusCriticalIcon],svg[grommet-icons-status-critical-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M12.703 2.703a.99.99 0 0 0-1.406 0l-8.594 8.594a.99.99 0 0 0 0 1.406l8.594 8.594a.99.99 0 0 0 1.406 0l8.594-8.594a.99.99 0 0 0 0-1.406zM8.983 14.7L14.7 8.983m-5.717 0L14.7 14.7"></svg:path>`,
})
export class GrommetIconsStatusCriticalIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStatusCriticalSmallIcon],svg[grommet-icons-status-critical-small-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" fill-rule="evenodd" stroke="currentColor" d="M6.712 1.263a1.005 1.005 0 0 0-1.424 0L1.263 5.288a1.005 1.005 0 0 0 0 1.424l4.025 4.025a1.005 1.005 0 0 0 1.424 0l4.025-4.025a1.005 1.005 0 0 0 0-1.424z"></svg:path>`,
})
export class GrommetIconsStatusCriticalSmallIcon {
  readonly viewBox = input("0 0 12 12")
  readonly width = input("1em")
  readonly height = input("1em")
}

@Component({
  standalone: true,
  selector: 'svg[grommetIconsStatusDisabledIcon],svg[grommet-icons-status-disabled-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="none" stroke="currentColor" stroke-width="2" d="M2 3.99C2 2.892 2.898 2 3.99 2h16.02C21.108 2 22 2.898 22 3.99v16.02c0 1.099-.898 1.99-1.99 1.99H3.99A1.995 1.995 0 0 1 2 20.01zM18 12H6"></svg:path>`,
})
export class GrommetIconsStatusDisabledIcon {
  readonly viewBox = input("0 0 24 24")
  readonly width = input("1em")
  readonly height = input("1em")
}
