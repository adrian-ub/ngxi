import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[streamlineEmojisChildrenCrossingIcon],svg[streamline-emojis-children-crossing-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="#ffe500" stroke="#45413c" stroke-linecap="round" stroke-linejoin="round" d="M24 3.523L44.478 24L24 44.479L3.522 24Z"></svg:path><svg:path fill="#45413c" d="M11.74 45.4a12.26 1.6 0 1 0 24.52 0a12.26 1.6 0 1 0-24.52 0" opacity=".15"></svg:path><svg:path fill="#45413c" d="M17.91 16.94a2.32 2.32 0 1 0 4.64 0a2.32 2.32 0 1 0-4.64 0m8.95-2.87a2.32 2.32 0 1 0 4.64 0a2.32 2.32 0 1 0-4.64 0m-5.38 19.12a.69.69 0 0 0 .65-.7L22 28.77a.94.94 0 0 0-.21-.52l-1.56-1.68a2.4 2.4 0 0 0 .16-.43l.18-.85l1.18.84A.73.73 0 1 0 22.6 25l-1.68-1.22l.41-1.89a1.73 1.73 0 0 0-1.33-2.1l-.29-.06a1.75 1.75 0 0 0-1.11.12l-2.85 1.56a.8.8 0 0 0-.33.42l-.87 2.47a.69.69 0 0 0 1.3.45l.76-2.18l.81-.44l-.71 3.21a1.8 1.8 0 0 0 0 .83l-.31 2.46c-.08.18-1.38 3.38-1.45 3.56a.72.72 0 1 0 1.34.55l1.44-3.56l.07-.24l.2-1.55a2.05 2.05 0 0 0 .92.08c.15.17 1.56 1.69 1.72 1.85l.11 3.22a.67.67 0 0 0 .73.65m11.4-10.33L30.23 21l.37-1.71A1.89 1.89 0 0 0 29.15 17c-1-.22-1.48.29-4.28 1.83a.93.93 0 0 0-.44.59l-1 2.78a.87.87 0 0 0 1.64.58l.85-2.45l.6-.33l-.52 2.34L23.81 27a.68.68 0 0 0 .7 1l.69-.05l-.11.81v.2l-1.28 3.16a.82.82 0 1 0 1.53.61l1.27-3.15l.15-.61l.15-1.15l2.41-.18l1.48 1.59l.11 3.27a.77.77 0 0 0 .8.75a.79.79 0 0 0 .75-.81l-.12-3.62a1.1 1.1 0 0 0-.26-.58l-.7-.76a.68.68 0 0 0 .62-.94l-1.37-3.05l1.24.89a.9.9 0 0 0 1.25-.2a.91.91 0 0 0-.24-1.32"></svg:path>`,
})
export class StreamlineEmojisChildrenCrossingIcon {
  readonly viewBox = input("0 0 48 48")
  readonly width = input("1em")
  readonly height = input("1em")
}
