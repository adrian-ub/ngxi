import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[deviconPlainMeteorWordmarkIcon],svg[devicon-plain-meteor-wordmark-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="currentColor" d="M10.2 67.8C8.5 65.2 6.9 62 5.3 60H1v19h4V66c1 .8 1.1 1.8 1.7 2.6c.6.9 1.2 1.8 1.8 2.6s.9 1.7 1.6 2.6c1.7-2.7 3-5.2 5-8C15 70 15 75 15 79h5V60h-5c-1.6 2-3.2 5.2-4.8 7.8m112.6 4.1c.6-.4 1.2-.9 1.7-1.3c1.4-.9 2-2.4 2.2-4c.1-.7 0-1.5 0-2.2l-.1-.7c-.2-.8-.6-1.5-1.1-2.2c-.5-.8-1.3-1-1.9-1.5c-1.1-.3-2.2-.7-3.4-.7c-2.7.1-5.5 0-8.3 0h-.8v19.6h5v-6h2.5c1.3 2 2.7 4 3.9 6h4.7c-1.4-1.9-3-4.8-4.4-7m-.6-5c-.4.7-.9 1.2-1.8 1.3c-.6.1-.8.2-1.4.2h-3.1v-5.5h3.8c.5 0 .8.1 1.3.3c.7.3 1.1.9 1.4 1.6c.5.8.4 1.2-.2 2.1M25 79h14v-4H29v-4h9v-3h-9v-4h10v-4H25zm40 0h14v-4.7H68.8l.2-.1V71h9v-3h-9v-4h10v-4H65zM44 63.1h5.7s.3 0 .3.1v14.6h4V63.9h5v-4H44zm61.6 1.2c.3 0 .5-.2.5-.5c0-.1 0-.2-.1-.3l-6.6-6.1l5.8 6.6c0 .2.2.3.4.3m1.3 3.4L92.6 54.6l13.1 14.3c.2.1.4.2.6.2c.5 0 .8-.4.8-.8c0-.3-.1-.5-.2-.6m-27-10.3L99 78.2c.2.2.5.4.9.4c.6 0 1.2-.5 1.2-1.2c0-.4-.2-.7-.4-.9zm-2.4-8l25.3 26.9c.2.2.5.3.8.3c.6 0 1.2-.5 1.2-1.2c0-.2-.1-.5-.2-.7c-.1.1-27.1-25.3-27.1-25.3m8.1 2.7l19.1 20.8c.2.2.5.4.9.4c.6 0 1.2-.5 1.2-1.2c0-.4-.2-.7-.4-.9zm10.7 25L82 64l13.1 14.3c.2.1.4.2.6.2c.5 0 .8-.4.8-.8c0-.3-.1-.5-.2-.6m-11-6.6l5.8 6.6c.1.1.2.3.4.3c.3 0 .5-.2.5-.5c0-.1 0-.2-.1-.3s-6.6-6.1-6.6-6.1"></svg:path>`,
})
export class DeviconPlainMeteorWordmarkIcon {
  readonly viewBox = input("0 0 128 128")
  readonly width = input("1em")
  readonly height = input("1em")
}
