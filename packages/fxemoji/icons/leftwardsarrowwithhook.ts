import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[fxemojiLeftwardsarrowwithhookIcon],svg[fxemoji-leftwardsarrowwithhook-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="#597B91" d="M508.333 32.666C508.333 16.35 494.984 3 478.668 3H29.032C14.348 3 2.333 15.015 2.333 29.699v452.602C2.333 496.985 14.348 509 29.032 509h449.635c16.316 0 29.666-13.35 29.666-29.666z"></svg:path><svg:path fill="#2B5770" d="M478.668 488.915H29.032c-14.684 0-26.699-12.015-26.699-26.699v20.085C2.333 496.985 14.348 509 29.032 509h449.635c16.316 0 29.666-13.35 29.666-29.666v-20.085c0 16.316-13.349 29.666-29.665 29.666"></svg:path><svg:path fill="#F9F9F7" d="M289.993 77.789H143c-17.434 0-31.566 14.133-31.566 31.566s14.133 31.566 31.566 31.566h146.993c46.557 0 84.434 37.877 84.434 84.434s-37.877 84.434-84.434 84.434h-92.568v-50.04c0-8.166-9.092-13.045-15.897-8.531L58.515 332.824c-6.104 4.049-6.104 13.012 0 17.062l123.013 81.606c6.805 4.514 15.897-.365 15.897-8.531v-50.039h92.568c81.368 0 147.566-66.198 147.566-147.567c.001-81.368-66.198-147.566-147.566-147.566"></svg:path>`,
})
export class FxemojiLeftwardsarrowwithhookIcon {
  readonly viewBox = input("0 0 512 512")
  readonly width = input("1em")
  readonly height = input("1em")
}
