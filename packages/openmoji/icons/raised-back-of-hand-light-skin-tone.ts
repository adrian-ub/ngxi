import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[openmojiRaisedBackOfHandLightSkinToneIcon],svg[openmoji-raised-back-of-hand-light-skin-tone-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="#fadcbc" stroke="#fadcbc" d="M41.766 13.738a3.531 3.531 0 1 1 7-.095l.256 18.998l-.021 4.325l-.176-12.999a3.531 3.531 0 1 1 6.999-.095l.176 13l-.053 10.084c-.424 8.31-7.752 13.63-16.06 13.205a19.62 19.62 0 0 1-12.727-5.657c-2.918-3.09-10.086-11.535-10.086-11.535c-1.463-2.158-.328-5.494.8-4.673l9.876 4.651l.04-5.4l-.04 5.4l.04-5.4l-.04-21.85a3.531 3.531 0 1 1 6.999-.095l.095 7l.186 10.636l-.367-22.192a3.531 3.531 0 1 1 6.999-.095l.206 24.016"></svg:path><svg:path fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M41.766 13.738a3.531 3.531 0 1 1 7-.095l.256 18.998l-.004.951l-.193-9.625a3.531 3.531 0 1 1 6.999-.095l.176 13l-.053 10.084c-.424 8.31-7.752 13.63-16.06 13.205a19.62 19.62 0 0 1-12.727-5.657c-2.918-3.09-10.086-11.535-10.086-11.535c-1.463-2.158-.328-5.494.8-4.673l9.876 4.651l.04-5.4l-.04 5.4l.04-5.4l-.04-21.85a3.531 3.531 0 1 1 6.999-.095l.095 7l.186 10.636l-.367-22.192a3.531 3.531 0 1 1 6.999-.095l.176 20.454"></svg:path>`,
})
export class OpenmojiRaisedBackOfHandLightSkinToneIcon {
  readonly viewBox = input("0 0 72 72")
  readonly width = input("1em")
  readonly height = input("1em")
}
