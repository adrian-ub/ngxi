import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[vscodeIconsFileTypeStylishHaskellIcon],svg[vscode-icons-file-type-stylish-haskell-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="#95538e" d="m14.188 19.163l-2.967-5.218l-6.388 11.248A1.68 1.68 0 0 0 6.292 27.7h3.952a1.68 1.68 0 0 0 1.46-.85l2.482-4.371a3.37 3.37 0 0 0 .002-3.316m-1.223-5.106a3.36 3.36 0 0 0 2.925 1.7h5.86L14.418 2.85A1.68 1.68 0 0 0 12.965 2h-3.96a1.68 1.68 0 0 0-1.679 1.678a1.66 1.66 0 0 0 .22.831Zm14.206 11.134l-3.528-6.262a3.36 3.36 0 0 0-2.927-1.712h-5.851l5.42 9.623a1.68 1.68 0 0 0 1.462.854h3.96a1.68 1.68 0 0 0 1.464-2.5Zm-11.156-2.932l-2.234 3.87A2.58 2.58 0 0 0 16.015 30a2.58 2.58 0 0 0 2.235-3.871Z"></svg:path>`,
})
export class VscodeIconsFileTypeStylishHaskellIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
