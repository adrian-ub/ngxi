import { Component, input } from '@angular/core'

@Component({
  standalone: true,
  selector: 'svg[vscodeIconsFolderTypeJsOpenedIcon],svg[vscode-icons-folder-type-js-opened-icon]',
  host: {
    role: 'img',
    '[attr.viewBox]': 'viewBox()',
    '[attr.width]': 'width()',
    '[attr.height]': 'height()',
  },
  template: `<svg:path fill="#dbd27b" d="M27.4 5.5h-9.2l-2.1 4.2H4.3v16.8h25.2v-21Zm0 18.7H6.6V11.8h20.8Zm0-14.5h-8.2l1-2.1h7.1v2.1Z"></svg:path><svg:path fill="#dbd27b" d="M25.7 13.7H.5l3.8 12.8h25.2z"></svg:path><svg:path fill="#f5de19" d="M23.013 26.479a3.27 3.27 0 0 0 2.961 1.821c1.244 0 2.033-.622 2.033-1.481c0-1.025-.815-1.393-2.181-1.989l-.753-.324c-2.155-.92-3.61-2.068-3.61-4.512c0-2.252 1.752-3.96 4.38-3.96a4.43 4.43 0 0 1 4.267 2.4l-2.33 1.5a2.04 2.04 0 0 0-1.936-1.288a1.305 1.305 0 0 0-1.43 1.167q-.006.06-.007.121c0 .876.561 1.262 1.84 1.822l.753.324c2.549 1.1 3.986 2.208 3.986 4.713c0 2.7-2.12 4.179-4.967 4.179a5.76 5.76 0 0 1-5.433-3.072Zm-10.583.263c.473.876.876 1.542 1.927 1.542s1.6-.385 1.6-1.884V16.229h3v10.215c0 3.1-1.822 4.521-4.477 4.521a4.65 4.65 0 0 1-4.494-2.742Z"></svg:path>`,
})
export class VscodeIconsFolderTypeJsOpenedIcon {
  readonly viewBox = input("0 0 32 32")
  readonly width = input("1em")
  readonly height = input("1em")
}
