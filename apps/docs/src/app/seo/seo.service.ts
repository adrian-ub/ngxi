...
import { ManifestService, type SetMeta } from '../services/manifest.service';
 ...
 * cards, JSON-LD). Driven from `IconBrowser` on every route/manifest change
...
export class SeoService {
  ...
  /** Home SEO: title, a manifest-derived description, and a WebSite JSON-LD. */
  applyHomePage(): void {
    const sets = this.manifest.manifestValue().sets;
    const totalIcons = sets.reduce((sum, set) => sum + set.iconCount, 0);
    const description =
      sets.length === 0
  ...
  /**
   * Collection SEO. Unknown collections (set not in the manifest, only
   * reachable at runtime through the server-render fallback) keep the
   * home-page tags but point the canonical at the raw request path.
   */
  applyCollectionPage(set: SetMeta | undefined): void {
    if (set === undefined) {
      this.applyHomePage();
    ...
    const description = `${set.name}: ${set.iconCount} icons (${set.aliasCount} aliases), ${set.license} license, by ${set.author}`;
    const canonical = `${SITE_URL}/collection/${set.collection}`;
    this.applyBase({
      title,
      description,
      canonical,
