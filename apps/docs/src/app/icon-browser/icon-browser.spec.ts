...
import { SetMeta, IconDocsManifest, IconIndexEntry, EMPTY_MANIFEST } from '../services/manifest.service';
import { SampleSet } from '../services/icon-set.service';
import { trustIconBody, type TrustedIconBody } from '../services/trusted-icon-body';
...
describe('IconBrowser', () => {
  ...
  const createMockSamples = (collection: string): SampleSet => ({
    ...
    viewBox: '0 0 24 24',
    samples: new Map([
      ['home', trustIconBody('<path d="M12 2L2 7l10 5 10-5-10-5z" />') as TrustedIconBody],
      ['user', trustIconBody('<path d="M12 12c2.21 0 4-1.79 4-4z" />') as TrustedIconBody],
    ]),
  });
  ...
  const createMockIconSetService = () => ({
    ...
    icons: () =>
      signal<ReadonlyMap<string, string> | undefined>(
  ...
  it('opens detail dialog on icon select', () => {
    ...
    fixture.componentInstance.onIconSelect(entry, 'test-set-1');
    fixture.detectChanges();
