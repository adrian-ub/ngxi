...
import { debouncedSignal } from '../util/debounce';
...
export class IconBrowser {
  ...
  readonly debouncedQuery = debouncedSignal(this.query, 250);
  ...
  /**
   * The active icon bundle loaded reactively via Signals.
   */
  readonly currentBundle = computed<IconSetBundle | undefined>(() => {
    const set = this.selectedSet();
    if (!set) return undefined;
    return this.iconSetService.bundle(set.collection)();
  });

  /** Whether the browsed set ships per-icon categories (not just set-level). */
  private readonly browseHasPerIconCategories = computed(() => {
    const index = this.currentBundle()?.index ?? [];
    return index.some(
  ...
  /**
   * The bundle backing the detail dialog: the browse bundle when browsing,
   * otherwise the bundle of the group the selected icon belongs to (global
   * search results render their own lazy bundle already).
   */
  readonly dialogBundle = computed<IconSetBundle | undefined>(() => {
    const browse = this.currentBundle();
    if (browse !== undefined) return browse;
    const entry = this.selectedEntry();
    if (entry === null) return undefined;
    const group = this.globalGroups().find((g) =>
      g.entries.some((e) => e.name === entry.name),
    );
    if (group === undefined) return undefined;
    return this.iconSetService.bundle(group.set.collection)();
  });
  ...
  /** Lazy bundle read for a search-result group's grid. */
  protected bundleOf(set: SetMeta): IconSetBundle | undefined {
    return this.iconSetService.bundle(set.collection)();
  }
  ...
  onIconSelect(entry: IconIndexEntry): void {
