...
describe('setJsonContent', () => {
  it('emits a minified, byte-stable copy of the parsed reference', () => {
    const content = setJsonContent(lucideFixture);
    expect(content).toBe(JSON.stringify(lucideFixture));
...
describe('buildSetIndex', () => {
  ...
  it('assigns split-set icons to their entry with entry-scoped imports and names', () => {
    ...
    // Entry components are named with `<collection>-<entry>` as the set name.
    expect(acc?.className).toBe('Fluent20FilledAccessibility20Filled');
    expect(acc?.selectorAttr).toBe('fluent20FilledAccessibility20Filled');
    ...
    expect(alias?.className).toBe('Fluent20FilledAccessibility20Filled');
  });
});
...
describe('emitDocsManifest', () => {
  it('scans packages and emits manifest + per-set verbatim and index assets', () => {
    ...
    try {
      ...
      const setFile = readFileSync(
        join(root, 'public/icons/lucide.json'),
        'utf-8',
      );
      expect(setFile).toBe(JSON.stringify(lucideFixture));
