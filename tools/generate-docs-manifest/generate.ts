...
/**
 * The minified, byte-identical copy of the source `icon-set.json`
 * (zero-drift verbatim set copy, design D2).
 */
export function setJsonContent(data: IconifyJSON): string {
 ...
 * (aliases point at their importable parent component). Split sets name entry
 * components with `<collection>-<entry>` as the set name, mirroring
...
export function buildSetIndex(
  ...
  const entryScope = (name: string): { entry: string; setName: string } => {
    ...
    return entry
      ? { entry: entry.name, setName: `${collection}-${entry.name}` }
  ...
  // disambiguation must resolve against exactly that scope so docs class
  // names match the generated components.
  const scopeNamesBySet = new Map<string, string[]>();
  for (const iconName of Object.keys(data.icons)) {
    const setName = entryScope(iconName).setName;
    const names = scopeNamesBySet.get(setName);
    if (names) {
      names.push(iconName);
    } else {
      scopeNamesBySet.set(setName, [iconName]);
    }
  }
  const collidingBySet = new Map<string, Set<string>>();
  for (const [setName, names] of scopeNamesBySet) {
    collidingBySet.set(setName, iconCollidingNames(setName, names));
  }
  const collidingNames = (setName: string): Set<string> =>
    collidingBySet.get(setName) ?? new Set();

  for (const iconName of Object.keys(data.icons)) {
    const { entry, setName } = entryScope(iconName);
    const colliding = collidingNames(setName);
 ...
 * `icon-set.json`, or resolvable through `@iconify/json`) and emits
 * `manifest.json`, `<collection>.json` (verbatim minified reference) and
 * `<collection>.index.json` (naming index) into `outDir`. Throws when
 * `packages/` is missing or a reference is unparseable; the CLI maps both to
 * a non-zero exit.
 */
export function emitDocsManifest(
  ...
  for (const set of sets) {
    ...
    const files = [
      [join(outDir, `${set.collection}.json`), setJsonContent(set.data)],
      [
