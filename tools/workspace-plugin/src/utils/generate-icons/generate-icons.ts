...
export async function generateIcons(
  tree: Tree,
  options: GenerateIconsComponent
) {
  removeOldIcons(options);
  generateIcon(tree, options, options.iconsSourcePath);
  const indexPath = path.join('packages', options.iconLibraryName, 'index.ts');
...
function generateIcon(
  ...
  tree.children(parentDir).forEach((dir) => {
    ...
    const newIconDir = path.relative(
      options.iconsSourcePath,
      path.join(path.dirname(dirPath), iconName)
    );

    const { className, name, propertyName } = names(
      options.iconPrefix + '-' + iconName
    );

    generateFiles(
      tree,
      path.join(__dirname, './files'),
      path.join('libs', options.iconLibraryName, newIconDir),
      {
...
function removeOldIcons(options: GenerateIconsComponent) {
  ...
  if (!fs.existsSync(pathLibrary)) {
    logger.info(
      `Path ${pathLibrary} does not exist. Skipping removal of old icons.`
    );
    return;
  }
