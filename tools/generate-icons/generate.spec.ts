...
describe('generate-icons script', () => {
  ...
  it('generates a self-contained secondary entry with only icon components', async () => {
    ...
    // Component is named after the entry and is fully self-contained: it no
    // longer imports the primary base.
    expect(arrowDown).toContain('Fluent20FilledArrowDown');
    expect(arrowDown).toContain('svg[fluent20FilledArrowDown]');
...
describe('generateReadme', () => {
  ...
  it('uses the matching secondary entry for split collections', () => {
    ...
    // generated from the filtered reference), so the class is the full name.
    expect(readme).toContain(
      "import { Fluent20FilledAccessibility20Filled } from '@ngxi/fluent/20-filled'",
    );
    expect(readme).toContain(
      'template: `<svg fluent20FilledAccessibility20Filled></svg>`',
    );
  });
